 ...
 */
export function judgeBypass(injection, wrappedContext) {
    ...
    if (UNTRUSTED_PREAMBLE.length < MIN_PREAMBLE_LEN ||
...
const CATEGORIES = ["basic-override", "role-jailbreak", "fence-escape", "known-bypasses"];
/**
...
function printComparison(baseline, current) {
    ...
    for (const cat of cats) {
        ...
        console.error(`${cat.padEnd(18)} ${String(b.bypassed).padStart(3)}/${String(b.total).padStart(3)}     ${String(c.bypassed).padStart(3)}/${String(c.total).padStart(3)}`);
    }
    console.error(`${"TOTAL".padEnd(18)} ${String(baseline.bypassed).padStart(3)}/${String(baseline.total).padStart(3)}     ${String(current.bypassed).padStart(3)}/${String(current.total).padStart(3)}`);
}
function printHelp() {
