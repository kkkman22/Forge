...
const subcommandArb = fc.constantFrom("review", "decide", "learn");
describe("R4.5 audit-writer prefix property", () => {
    ...
    it("after any sequence of writes to the same file, new content always starts with old content", async () => {
        ...
        }), subcommandArb, fc.array(fc.dictionary(fc.string({ minLength: 1, maxLength: 10 }), fc.jsonValue()), {
            minLength: 2,
            maxLength: 10,
    ...
    it("single write produces non-empty content with runId", async () => {
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 20 }).filter((s) => s.trim().length > 0), fc.string({ minLength: 1, maxLength: 20 }).filter((s) => /^[a-z0-9_-]+$/.test(s)), fc.dictionary(fc.string({ minLength: 1, maxLength: 10 }), fc.jsonValue()), subcommandArb, async (runId, topic, payload, subcommand) => {
            const forgeRoot = makeTmp();
            const writer = new WorkflowAuditWriter(forgeRoot, () => false);
...
function resolveExpectedPath(forgeRoot, subcommand, topic, runId) {
    switch (subcommand) {
        case "review":
...
//# sourceMappingURL=audit-writer-prefix.property.test.js.map