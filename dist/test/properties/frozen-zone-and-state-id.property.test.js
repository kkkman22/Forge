...
const openPathArb = fc.oneof(fc.constant("knowledge/sessions/run-1.md"), fc.constant("knowledge/evolved-rules.md"), fc.string({ minLength: 1, maxLength: 20 }).map((s) => `other/${s}.md`));
const forgePathArb = fc.oneof(frozenPathArb.map((p) => `.forge/${p}`), frozenPathArb, guardedPathArb.map((p) => `.forge/${p}`), openPathArb.map((p) => `.forge/${p}`), fc.string({ minLength: 1, maxLength: 40 }).map((s) => `/abs/project/.forge/${s}`));
describe("R11.1 frozen-zone property", () => {
    ...
    it("paths with .. traversal under frozen prefixes still resolve to frozen zone", () => {
        fc.assert(fc.property(frozenPathArb, fc.array(fc.constantFrom("..", "."), { minLength: 0, maxLength: 5 }), (path, segments) => {
...
describe("R11.1 hard-frozen source files", () => {
    ...
    it("random non-matching paths are never hard-frozen", () => {
        fc.assert(fc.property(fc
            ...
            .filter((s) => !s.includes("prompt-defense-patterns")), (path) => {
...
describe("R12.4 state-id uniqueness", () => {
    ...
    it("built env does not leak between sequential calls", () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 20 }), fc.integer({ min: 1, max: 10 }), fc.integer({ min: 1, max: 5 }), (maxParallel, reviewConcurrency, runtimeConcurrency) => {
            ...
            const leakedKeys = Object.keys(env).filter((k) => k.startsWith("FORGE_MAX_PARALLEL_AGENTS_RUNTIME") && !snapshotBefore.has(k));
    ...
    it("buildEnv always includes CLAUDE_CODE_WORKFLOWS=1 when env var is set", () => {
        ...
        try {
            fc.assert(fc.property(fc.integer({ min: 1, max: 20 }), fc.integer({ min: 1, max: 10 }), (maxParallel, reviewConcurrency) => {
    ...
    it("buildEnv sets runtime concurrency only when provided", () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 20 }), fc.integer({ min: 1, max: 10 }), fc.option(fc.integer({ min: 1, max: 5 })), (maxParallel, reviewConcurrency, runtimeConcurrency) => {
            const env = buildEnv({
                maxParallelAgents: maxParallel,
                reviewConcurrency,
...
//# sourceMappingURL=frozen-zone-and-state-id.property.test.js.map