...
import { detectPlanFormat, extractHeadingAnchors, FORBIDDEN_PLACEHOLDERS, validateAtomicTask, validateDesignReferences, validateLightweightPlan, validateLightweightTask, validatePlan, validatePlanTasks, } from "../src/plan.js";
// ---------------------------------------------------------------------------
...
const designRefArb = fc
    .tuple(safeStringArb, safeStringArb)
    .map(([section, anchor]) => `design.md#${section}-${anchor}`);
// ---------------------------------------------------------------------------
// Generators — valid LightweightTask
// ---------------------------------------------------------------------------
const validLightweightTaskArb = fc
    .tuple(fc.integer({ min: 1, max: 100 }), safeStringArb, filePathArb, safeStringArb, designRefArb, fc.option(fc.integer({ min: 1, max: 10 })), runCommandArb, commitMessageArb)
    .map(([taskNumber, title, filePath, goal, designReference, propertyRef, verifyCommand, commitMessage,]) => ({
    taskNumber,
    title,
    filePath,
    goal,
...
// ---------------------------------------------------------------------------
const lightweightTaskMissingFieldArb = fc.oneof(validLightweightTaskArb.map((t) => ({ ...t, title: "" })), validLightweightTaskArb.map((t) => ({ ...t, filePath: "" })), validLightweightTaskArb.map((t) => ({ ...t, goal: "" })), validLightweightTaskArb.map((t) => ({ ...t, designReference: "" })), validLightweightTaskArb.map((t) => ({ ...t, verifyCommand: "" })), validLightweightTaskArb.map((t) => ({ ...t, commitMessage: "" })));
// ---------------------------------------------------------------------------
// Generators — task with placeholder
// ---------------------------------------------------------------------------
const forbiddenPlaceholderArb = fc.constantFrom(...FORBIDDEN_PLACEHOLDERS);
const lightweightTaskWithPlaceholderArb = fc
...
const headingTextArb = fc
    ...
    .filter((s) => s.length > 0);
const markdownWithHeadingsArb = fc
    .tuple(fc.array(fc.tuple(fc.integer({ min: 1, max: 6 }), headingTextArb), {
    minLength: 1,
    maxLength: 10,
}), safeStringArb)
    .map(([headings, body]) => {
...
// ---------------------------------------------------------------------------
const frontmatterWithFormatArb = fc.oneof(fc.constant('format: "lightweight"'), fc.constant("format: lightweight"), fc.constant('format: "full"'), fc.constant("format: full"), fc.constant(""), safeStringArb.map((s) => `format: ${s}`));
// ---------------------------------------------------------------------------
...
describe("Property 1: LightweightTask validation — valid tasks pass, invalid tasks fail", () => {
    it("valid LightweightTasks pass validation", () => {
        fc.assert(fc.property(validLightweightTaskArb, (task) => {
...
describe("Property 3: Design Reference validation", () => {
    ...
    it("references NOT matching any heading fail", () => {
        fc.assert(fc.property(markdownWithHeadingsArb, safeStringArb, (content, fakeAnchor) => {
            ...
            if (anchors.includes(fakeAnchor))
                return;
            const result = validateDesignReferences(refs, content);
            expect(result.valid).toBe(false);
            expect(result.errors.length).toBeGreaterThan(0);
...
describe("Property 4: Format detection defaults to full", () => {
    it('returns "lightweight" only when frontmatter has format: "lightweight"', () => {
        fc.assert(fc.property(frontmatterWithFormatArb, (frontmatter) => {
...
describe("Property 5: Lightweight plan validation", () => {
    it("non-empty array of valid tasks with valid deps passes", () => {
        fc.assert(fc.property(fc.array(validLightweightTaskArb, { minLength: 1, maxLength: 10 }), (tasks) => {
    ...
    it("array with invalid task fails", () => {
        fc.assert(fc.property(fc.array(validLightweightTaskArb, { minLength: 0, maxLength: 3 }), lightweightTaskMissingFieldArb, fc.array(validLightweightTaskArb, { minLength: 0, maxLength: 3 }), (before, bad, after) => {
            const tasks = [
...
describe("Property 6: Placeholder scanning covers all text fields", () => {
    it("injecting placeholder into any text field causes validation failure", () => {
        fc.assert(fc.property(lightweightTaskWithPlaceholderArb, (task) => {
            const result = validateLightweightTask(task);
            expect(result.valid).toBe(false);
...
describe("validatePlan with designContent integration", () => {
    ...
    const makeTask = (num, ref = `design.md#section-${num}`) => ({
        taskNumber: num,
        title: `Task ${num}`,
        filePath: `src/file${num}.ts`,
        goal: `Goal ${num}`,
...
//# sourceMappingURL=plan-lightweight.test.js.map