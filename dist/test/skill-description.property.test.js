...
describe("validateDescription — property-based", () => {
    ...
    it("contains Use when + ≤1024 chars + no forbidden → valid=true", () => {
        fc.assert(fc.property(safeFillerArb, safeFillerArb, (prefix, suffix) => {
...
describe("validateDescription — unit", () => {
    ...
    it("flags missing 'Use when' trigger", () => {
        const description = "Processes incoming requests with a disciplined pipeline when the agent is idle.";
        const doc = buildSkillDoc(description);
        const result = validateDescription("skills/forge-x/SKILL.md", doc);
        expect(result.valid).toBe(false);
...
//# sourceMappingURL=skill-description.property.test.js.map