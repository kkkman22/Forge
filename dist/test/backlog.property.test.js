...
import { appendToBacklog, findOverlappingEntries, parseBacklog, resolveEntry, serializeBacklog, } from "../src/backlog.js";
// ---------------------------------------------------------------------------
// Arbitraries
// ---------------------------------------------------------------------------
...
describe("Feature: forge-review-fix-optimization, Property 12: Backlog append with deduplication and tagging", () => {
    it("new findings are added, duplicates skipped, originals preserved", () => {
        fc.assert(fc.property(fc.array(backlogEntryArb, { maxLength: 10 }), fc.array(backlogEntryArb, { maxLength: 10 }), (existing, newFindings) => {
...
describe("Feature: forge-review-fix-optimization, Property 13: Backlog overlap detection", () => {
    it("returns exactly entries whose filePath matches affected files", () => {
        fc.assert(fc.property(fc.array(backlogEntryArb, { maxLength: 20 }), fc.array(fc.lorem({ mode: "words", maxCount: 3 }).filter((s) => s.length > 0), { maxLength: 5 }), (entries, affectedFiles) => {
...
describe("Feature: forge-review-fix-optimization, Property 14: Backlog resolve marks entry", () => {
    it("resolveEntry sets resolved, resolvedBy, resolvedDate while preserving other fields", () => {
        fc.assert(fc.property(fc.array(backlogEntryArb, { minLength: 1, maxLength: 20 }), fc.string({ minLength: 1, maxLength: 20 }), safeDateStr(), (entries, resolvedBy, resolvedDate) => {
            ...
            if (!original)
                throw new Error("Expected to find entry in before array");
    ...
    it("returns null for non-existent ID", () => {
        fc.assert(fc.property(fc.array(backlogEntryArb, { maxLength: 10 }), fc.string({ minLength: 1, maxLength: 20 }), safeDateStr(), (entries, resolvedBy, resolvedDate) => {
...
describe("Feature: forge-review-fix-optimization, Property 15: Backlog round-trip", () => {
    it("serializeBacklog -> parseBacklog produces equivalent entries", () => {
        fc.assert(fc.property(fc.array(backlogEntryArb, { maxLength: 15 }), (entries) => {
            ...
            for (const orig of uniqueEntries) {
                ...
                if (orig.resolvedBy)
                    expect(reparsed.resolvedBy).toBe(orig.resolvedBy);
                if (orig.resolvedDate)
                    expect(reparsed.resolvedDate).toBe(orig.resolvedDate);
            }
        }), { numRuns: 100 });
    });
});
//# sourceMappingURL=backlog.property.test.js.map