...
import { buildCaffeinateCommand, buildPowerShellCommand, buildSleepPreventionCommand, buildSystemdInhibitCommand, isSupportedPlatform, } from "../src/sleep-preventer.js";
// ---------------------------------------------------------------------------
// Generators
// ---------------------------------------------------------------------------
...
const supportedPlatformArb = fc.constantFrom("darwin", "linux", "win32");
...
describe("Feature: gnhf-inspired-enhancements, Property 19: 平台休眠防护命令生成", () => {
    ...
    it("darwin generates a caffeinate command with -i and -w <PID>", () => {
        fc.assert(fc.property(pidArb, (pid) => {
    ...
    it("buildSystemdInhibitCommand includes --what=idle:sleep and --mode=block", () => {
        fc.assert(fc.property(fc.string({ minLength: 1 }), fc.array(fc.string(), { maxLength: 3 }), fc.string({ minLength: 1 }), fc.array(fc.string(), { maxLength: 5 }), (execPath, execArgv, scriptPath, argv) => {
    ...
    it("buildSleepPreventionCommand returns non-null for all supported platforms", () => {
        fc.assert(fc.property(supportedPlatformArb, pidArb, (platform, pid) => {
