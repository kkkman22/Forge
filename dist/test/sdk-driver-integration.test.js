...
describe("circuit breaker: review blocked × 3 → aborted", () => {
    ...
    it("tracks reviewFixAttempts across blocked review iterations", async () => {
        ...
        const hasSummary = logCalls.some((msg) => typeof msg === "string" && msg.includes("driver.summary"));
    ...
    it("outputs completion summary with circuit breaker info on abort", async () => {
        ...
        const logCalls = console.log.mock.calls.map((c) => c[0]);
        const hasSummary = logCalls.some((msg) => typeof msg === "string" &&
...
describe("StatusFile lifecycle across full flow", () => {
    ...
    it("updates loop_iteration after each iteration", async () => {
        ...
        const iterationUpdates = writtenContents.filter((content) => content.includes("loop_iteration:") && !content.includes("loop_iteration: 0"));
...
//# sourceMappingURL=sdk-driver-integration.test.js.map