...
import { existsSync, mkdirSync, openSync, readdirSync, readFileSync, rmSync, unlinkSync, writeFileSync, } from "node:fs";
...
beforeEach(() => {
    vi.clearAllMocks();
    randomUUID.mockReturnValue(FAKE_UUID);
    // Default: branchExists returns false (branch not found), rev-parse HEAD returns SHA
    execFileSync.mockImplementation((_exec, args, _opts) => {
...
describe("setupNewRun", () => {
    ...
    it("creates run directory and initializes notes file", () => {
        ...
        expect(writeFileSync).toHaveBeenCalledWith(expectedNotesPath, expect.stringContaining(`# Run: ${FAKE_UUID}`), "utf-8");
    });
});
// ---------------------------------------------------------------------------
...
describe("resumeRun", () => {
    it("reads existing notes and returns correct lastIteration", () => {
        ...
        existsSync.mockImplementation((p) => {
            if (p === `${CWD}/.forge/runs/`)
                return true;
            if (p === `${CWD}/.forge/runs/existing-run/notes.md`)
                return true;
            return false;
        });
        // Mock readdirSync to return a directory entry
        readdirSync.mockReturnValue([{ name: "existing-run", isDirectory: () => true }]);
        ...
        readFileSync.mockReturnValue(notesContent);
...
describe("setupWorktree", () => {
    it("removes stale worktree directory before creating new one", () => {
        ...
        existsSync.mockImplementation((p) => p === stalePath);
    ...
    it("creates worktree when under limit", () => {
        ...
        const worktreeAddCall = mock.mock.calls.find((call) => Array.isArray(call[1]) && call[1][0] === "worktree" && call[1][1] === "add");
...
describe("setupWorktree orphan branch cleanup", () => {
    it("deletes orphan branch when run directory initialization fails", () => {
        ...
        mkdirSync.mockImplementation((_p, _opts) => {
            mkdirCallCount++;
        ...
        expect(() => RunManager.setupWorktree("my task", "/main/repo")).toThrow(/Run directory initialization failed/);
...
//# sourceMappingURL=run-manager.test.js.map