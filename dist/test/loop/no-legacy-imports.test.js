...
function grepImports(pattern, paths) {
    try {
        // Match actual import/from statements only (not comments or strings)
        const out = execSync(`grep -rn "from.*['\\"].*${pattern}" ${paths.join(" ")} --include="*.ts" 2>/dev/null || true`, { encoding: "utf-8", cwd: process.cwd() }).trim();
        if (!out)
            return [];
        return out
            .split("\n")
...
//# sourceMappingURL=no-legacy-imports.test.js.map