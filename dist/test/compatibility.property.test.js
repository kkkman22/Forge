...
describe("compatibility property tests", () => {
    ...
    const versionOutputArb = fc
        .oneof(fc.constantFrom("claude", "Claude Code", "Claude Code CLI", "anthropic/claude"), fc.stringMatching(/^[a-zA-Z ]{0,20}/))
        .chain((prefix) => fc
        .tuple(fc.constant(prefix), semverArb, fc.oneof(fc.constant(""), fc.stringMatching(/^[a-z ]{0,30}/)))
    ...
    describe("compareSemver", () => {
        it("satisfies anti-symmetry: compare(a,b) === -compare(b,a)", async () => {
            ...
            fc.assert(fc.property(semverArb, semverArb, (a, b) => {
                expect(compareSemver(a, b)).toBe(-compareSemver(b, a));
            }));
        });
        it("satisfies transitivity: a<b && b<c → a<c", async () => {
            const { compareSemver } = await import("../src/compatibility.js");
            fc.assert(fc.property(semverArb, semverArb, semverArb, (a, b, c) => {
        ...
        it("returns 0 for identical versions", async () => {
            const { compareSemver } = await import("../src/compatibility.js");
            fc.assert(fc.property(semverArb, (v) => {
    ...
    describe("parseClaudeVersion", () => {
        it("extracts version from arbitrary output formats", async () => {
            ...
            fc.assert(fc.property(versionOutputArb, (output) => {
        ...
        it("returns null for input with no version pattern", async () => {
            const { parseClaudeVersion } = await import("../src/compatibility.js");
            fc.assert(fc.property(fc.stringMatching(/^[a-zA-Z _\-!@#$%^&*()]{1,50}$/), (noVersion) => {
    ...
    describe("checkClaudeVersion", () => {
        ...
        it("returns non-empty verdict for any input", async () => {
            ...
            fc.assert(fc.property(fc.oneof(semverArb, fc.constant(null)), (current) => {
...
//# sourceMappingURL=compatibility.property.test.js.map