 ...
 *   - Property 3: Only .forge/ changes → always fresh
...
/** File path that starts with `.forge/`. */
const forgeFileArb = fc.string({ minLength: 1, maxLength: 20 }).map((s) => `.forge/${s}`);
/** File path that does NOT start with `.forge/`. */
const projectFileArb = fc
    .tuple(fc.string({ minLength: 1, maxLength: 10 }), fc.string({ minLength: 1, maxLength: 10 }))
    .filter(([prefix]) => prefix !== ".forge")
    .map(([prefix, name]) => `${prefix}/${name}`);
/** Any file path. */
const anyFileArb = fc.oneof(forgeFileArb, projectFileArb);
/** Array of only .forge/ files. */
const forgeOnlyFilesArb = fc.array(forgeFileArb, { minLength: 0, maxLength: 10 });
/** Array with at least one project file. */
const arrayWithProjectFileArb = fc
    .tuple(fc.array(anyFileArb, { minLength: 0, maxLength: 5 }), projectFileArb, fc.array(anyFileArb, { minLength: 0, maxLength: 5 }))
...
describe("Property 1: undefined reviewedCommit → always fresh (backward compat)", () => {
    it("for any HEAD and any file list, returns fresh: true", () => {
        fc.assert(fc.property(commitHashArb, fc.array(anyFileArb), (currentHead, changedFiles) => {
...
describe("Property 2: same commit → always fresh", () => {
    it("for any commit hash and any file list, returns fresh: true", () => {
        fc.assert(fc.property(commitHashArb, fc.array(anyFileArb), (commit, changedFiles) => {
...
// ---------------------------------------------------------------------------
// Property 3: Only .forge/ changes → fresh
// ---------------------------------------------------------------------------
describe("Property 3: diff only .forge/ files → always fresh", () => {
    it("for any different commits and .forge/-only file list, returns fresh: true", () => {
        fc.assert(fc.property(commitHashArb, commitHashArb, forgeOnlyFilesArb, (reviewedCommit, currentHead, changedFiles) => {
...
describe("Property 4: project code changed → not fresh", () => {
    it("for any different commits with at least one non-.forge/ file, returns fresh: false", () => {
        fc.assert(fc.property(commitHashArb, commitHashArb, arrayWithProjectFileArb, (reviewedCommit, currentHead, changedFiles) => {
            fc.pre(reviewedCommit !== currentHead);
            const result = checkReviewFreshness(reviewedCommit, currentHead, changedFiles);
            expect(result.fresh).toBe(false);
            expect(result.reason).toContain("project code changed");
            expect(result.changedFiles).toBeDefined();
            // changedFiles should only contain non-.forge/ files
            const nonForge = result.changedFiles;
