...
const alreadyTriggeredArb = fc.record({
...
describe("shouldTriggerInlineGrill properties", () => {
    it("autonomous mode always returns trigger=false", () => {
        fc.assert(fc.property(fc.constantFrom(...REASONS), alreadyTriggeredArb, (reason, triggered) => {
    ...
    it("interactive + already triggered reason always returns trigger=false", () => {
        fc.assert(fc.property(fc.constantFrom(...REASONS), (reason) => {
            const triggered = {
    ...
    it("is idempotent: same input always yields same output", () => {
        fc.assert(fc.property(fc.constantFrom("interactive", "autonomous"), fc.constantFrom(...REASONS), alreadyTriggeredArb, (mode, reason, triggered) => {
...
describe("formatInlineGrillInjection properties", () => {
    const completedResultArb = fc.record({
    ...
    it("completed result always contains alignment summary and mode", () => {
        fc.assert(fc.property(completedResultArb, fc.constantFrom("spec", "decide"), (result, mode) => {
...
//# sourceMappingURL=grill-inline.property.test.js.map