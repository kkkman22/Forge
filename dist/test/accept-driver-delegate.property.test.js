...
import { checkContractFresh, RUNNERS, resolveTestCommand, } from "../src/accept-driver.js";
describe("RUNNERS — three delegates added, mixed removed (Req3 AC1, AC6)", () => {
    ...
    it("each delegate supports() its own scenario type", () => {
        for (const t of ["unit", "component", "contract"]) {
            const runner = RUNNERS.find((r) => r.type === t);
            expect(runner).toBeDefined();
            expect(runner.supports({ type: t })).toBe(true);
            expect(runner.supports({ type: "api" })).toBe(false);
        }
    });
});
describe("resolveTestCommand — pure command resolver (Req3 AC3)", () => {
    ...
    it("falls back to convention <pkg manager> run test:<layer> when mapping absent", () => {
        const cfgNoMap = { packageManager: "npm" };
...
//# sourceMappingURL=accept-driver-delegate.property.test.js.map