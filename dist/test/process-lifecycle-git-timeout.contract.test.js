            ...
            lines.forEach((line, i) => {
                if (line.includes('execFileSync("git"') || line.includes("execFileSync('git'")) {
                    ...
                    if (/killSignal\s*:\s*["']SIGKILL["']/.test(window)) {
                        violations.push(`${rel}:${i + 1} — execFileSync('git') uses killSignal SIGKILL (must be SIGTERM to allow cleanup)`);
                    }
                }
            });
        }
...
//# sourceMappingURL=process-lifecycle-git-timeout.contract.test.js.map