...
import { applyCrossValidation, CONFIDENCE_THRESHOLD, CROSS_VALIDATION_BOOST, deduplicateFindings, filterByConfidence, LINE_TOLERANCE, LOW_CONFIDENCE_MIN, MAX_CONFIDENCE, runReportQualityGate, } from "../src/review.js";
// ---------------------------------------------------------------------------
// Generators
// ---------------------------------------------------------------------------
const severityArb = fc.constantFrom("P0", "P1", "P2", "P3");
const fixRouteArb = fc.constantFrom("safe_auto", "gated_auto", "manual", "advisory");
const reviewerArb = fc.constantFrom("spec-check", "quality-check", "security-check");
...
const findingsArrayArb = fc.array(findingArb, {
    minLength: 0,
    maxLength: 20,
});
// ---------------------------------------------------------------------------
...
describe("Property 18: 去重合并正确性", () => {
    it("duplicate findings (same file, close line, same description) are merged", () => {
        fc.assert(fc.property(findingArb, fc.integer({ min: 0, max: LINE_TOLERANCE }), reviewerArb, (base, lineOffset, otherReviewer) => {
            const dup = {
                ...base,
    ...
    it("merged finding keeps highest severity", () => {
        ...
        fc.assert(fc.property(findingArb, severityArb, (base, otherSeverity) => {
            const dup = {
                ...base,
            ...
            const result = deduplicateFindings([base, dup]);
            expect(result.length).toBe(1);
            const expectedSeverity = severityRank[base.severity] <= severityRank[otherSeverity]
                ? base.severity
                : otherSeverity;
...
describe("Property 20: 报告质量门", () => {
    ...
    it("gate passes iff all 6 items pass", () => {
        fc.assert(fc.property(fc.array(fc.boolean(), { minLength: 6, maxLength: 6 }), (_itemResults) => {
...
//# sourceMappingURL=review.property.test.js.map