 ...
 *     into a `DecisionTree` so `/forge resume` can pick up where a
...
import { applyAnswer, generateDecisionTree, parseGrillFindings, renderGrillFindings, resumeGrillFromFindings, selectNextQuestion, } from "../src/grill.js";
// ---------------------------------------------------------------------------
// Fixtures
// ---------------------------------------------------------------------------
...
describe("parseGrillFindings round-trip", () => {
    ...
    it("preserves the resolved/pending mix and userAnswer after a full round trip", () => {
        ...
        const withFunc = applyAnswer(base, "functionality-1", "Export batches of orders on demand.", FIXED_NOW);
        const withBoundary = applyAnswer(withFunc, "boundary-1", "Out of scope: streaming and PDF rendering.", FIXED_NOW);
...
describe("resumeGrillFromFindings", () => {
    ...
    it("identifies the first pending node as the next question", () => {
        ...
        if (result === null)
            throw new Error("expected a parsed tree");
    ...
    it("returns nextNode=null when every node in the recovered tree is terminal", () => {
        ...
        const fully = pendingIds.reduce((acc, id) => applyAnswer(acc, id, `answered ${id}`, FIXED_NOW), base);
...
//# sourceMappingURL=grill-resume.test.js.map