...
import { deserializeCheckpointMarker, deserializeClassification, deserializeRecoveryReport, serializeCheckpointMarker, serializeClassification, serializeRecoveryReport, } from "../src/error-recovery.js";
// ---------------------------------------------------------------------------
// Generators
// ---------------------------------------------------------------------------
const actionOption = () => fc.record({
    index: fc.integer({ min: 1, max: 5 }),
    description: safeStr(1, 50).filter((s) => s.length > 0),
    isDefault: fc.boolean(),
});
const inconsistencyItem = () => fc.record({
...
const safeStr = (min, max) => fc.array(safeChar, { minLength: min, maxLength: max }).map((a) => a.join("").trim());
const recoveryReport = () => fc.record({
    header: fc.record({
        ...
        interruptionCategory: fc.constantFrom("task-completed-not-committed", "committed-not-progress-updated", "progress-updated-not-phase-advanced", "subagent-mid-execution", "clean-state"),
    }),
...
const interruptionClassification = () => fc.record({
    category: fc.constantFrom("task-completed-not-committed", "committed-not-progress-updated", "progress-updated-not-phase-advanced", "subagent-mid-execution", "clean-state"),
    evidence: safeStr(1, 80).filter((s) => s.length > 0),
    tddPhase: fc.oneof(fc.constant(null), fc.constantFrom("red", "green-incomplete", "refactor-incomplete")),
});
...
const checkpointMarker = () => fc
    ...
    .filter((m) => m.taskId.length > 0 && m.intendedCommitMessage.length > 0 && m.timestamp.length > 0);
// ---------------------------------------------------------------------------
...
describe("Feature: error-recovery-strategy, Property 16: RecoveryReport round-trip", () => {
    it("serialize then deserialize yields semantically equivalent object", () => {
        fc.assert(fc.property(recoveryReport(), (report) => {
...
describe("Feature: error-recovery-strategy, Property 17: InterruptionClassification round-trip", () => {
    it("serialize then deserialize yields semantically equivalent object", () => {
        fc.assert(fc.property(interruptionClassification(), (classification) => {
...
describe("Feature: error-recovery-strategy, Property 18: CheckpointMarker round-trip", () => {
    it("serialize then deserialize yields semantically equivalent object", () => {
        fc.assert(fc.property(checkpointMarker(), (marker) => {
