...
describe("detectCiCommandDrift", () => {
    ...
    it("returns has_ci_command when frontmatter has ci_check_command set", () => {
        const result = detectCiCommandDrift({ ci_check_command: "npm run check" }, '{"scripts":{"check":"npm run check"}}');
    ...
    it("returns drift_with_npm_check when ci_check_command is empty string", () => {
        const result = detectCiCommandDrift({ ci_check_command: "" }, '{"scripts":{"check":"npm run check"}}');
        expect(result.kind).toBe("drift_with_npm_check");
    });
...
//# sourceMappingURL=ci-command-drift.test.js.map