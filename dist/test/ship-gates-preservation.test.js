...
import { checkReviewFreshness, checkShipGate, checkShipGateWithAcceptance, checkShipGateWithChecklist, checkShipGateWithForceSkip, checkShipGateWithFreshness, } from "../src/ship.js";
// ---------------------------------------------------------------------------
...
describe("Preservation: checkShipGate basic behavior", () => {
    ...
    const completeProgress = { totalTasks: 3, completedTasks: 3 };
    ...
    it("review fails → blocked with review reason", () => {
        const failedReview = { passed: false, p0Count: 1, p1Count: 0 };
    ...
    it("progress incomplete → blocked with progress reason", () => {
        const incompleteProgress = { totalTasks: 5, completedTasks: 3 };
...
describe("Preservation: checkShipGateWithChecklist", () => {
    const passedReview = { passed: true, p0Count: 0, p1Count: 0 };
    const passedTest = { passed: true };
    const completeProgress = { totalTasks: 1, completedTasks: 1 };
...
describe("Preservation: checkShipGateWithFreshness", () => {
    ...
    it("fresh review → allowed, no freshness warning", () => {
        const result = checkShipGateWithFreshness(passedReview, passedTest, completeProgress, "abc1234", []);
        expect(result.allowed).toBe(true);
        expect(result.reasons).toHaveLength(0);
    });
    ...
    it("stale review with only .forge/ changes → allowed, no warning", () => {
        const result = checkShipGateWithFreshness(passedReview, passedTest, completeProgress, "def5678", [".forge/status.md"]);
        expect(result.allowed).toBe(true);
        expect(result.reasons).toHaveLength(0);
    });
});
...
describe("Preservation: checkShipGateWithForceSkip", () => {
    const failedReview = { passed: false, p0Count: 2, p1Count: 1 };
    const failedTest = { passed: false };
    const incompleteProgress = { totalTasks: 5, completedTasks: 1 };
    ...
    it("force skip without reason → throws", () => {
        expect(() => checkShipGateWithForceSkip(failedReview, failedTest, incompleteProgress, {
            forceSkipReview: true,
        })).toThrow();
    });
...
//# sourceMappingURL=ship-gates-preservation.test.js.map