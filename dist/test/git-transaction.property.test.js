...
import { buildCleanCommand, buildCommitCommand, buildResetCommand, buildStashCommand, containsShellMetacharacters, sanitizeBranchName, validatePathSafety, } from "../src/git-transaction.js";
// ---------------------------------------------------------------------------
// Generators
// ---------------------------------------------------------------------------
...
const shellMetacharStringArb = fc
    .tuple(fc.string({ minLength: 0, maxLength: 50 }), fc.constantFrom("`", "$(", '"', ";", "|", "&", "<", ">"), fc.string({ minLength: 0, maxLength: 50 }))
...
const dangerousPathArb = fc.constantFrom("../etc/passwd", "foo/../../bar", "path\0with\0nulls", "`whoami`", "$(rm -rf /)", "-flag-injection", "file;rm -rf /", "file|cat /etc/passwd", "");
...
describe("Feature: gnhf-inspired-enhancements, Property 5: 提交消息格式化", () => {
    ...
    it("commit message arg contains iteration number and summary", () => {
        fc.assert(fc.property(iterationNumberArb, summaryArb, (iteration, summary) => {
...
describe("Feature: gnhf-inspired-enhancements, Property 6: 回滚往返一致性", () => {
    ...
    it("reset command always targets HEAD (no iteration-specific state leaks)", () => {
        fc.assert(fc.property(iterationNumberArb, (_iteration) => {
            const cmd = buildResetCommand();
...
describe("Feature: gnhf-inspired-enhancements, Property 14: Shell 注入安全性", () => {
    ...
    it("containsShellMetacharacters returns false for safe strings", () => {
        const safeStringArb = fc
            .array(fc.constantFrom(..."abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 -_./".split("")), { minLength: 0, maxLength: 100 })
            .map((chars) => chars.join(""));
...
describe("Feature: audit-remediation-v221, Property 5: sanitizeBranchName produces legal Git ref names", () => {
    ...
    it("Git-illegal-only input produces output free of illegal characters", () => {
        fc.assert(fc.property(gitIllegalStringArb, (input) => {
            const result = sanitizeBranchName(input);
