 ...
 */
export function createTempRepo(seed) {
    ...
    writeFileSync(join(cwd, ".forge", "config.md"), ["---", "version: 1", "project_name: e2e-test", "---", ""].join("\n"));
    // Create hooks directory with hooks.json
    mkdirSync(join(cwd, "hooks"), { recursive: true });
    writeFileSync(join(cwd, "hooks", "hooks.json"), JSON.stringify({
        hooks: {
            PreToolUse: [
                {
                    matcher: "Write|Edit",
