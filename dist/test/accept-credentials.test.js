...
describe("resolvePlaceholder (pure)", () => {
    it("returns the env value for a {{VAR}} placeholder", () => {
        expect(resolvePlaceholder("{{FORGE_E2E_PASSWORD}}", { FORGE_E2E_PASSWORD: "secret123" })).toBe("secret123");
    });
...
describe("resolveSecrets (batch)", () => {
    it("resolves a record of placeholders, flagging missing", () => {
        const r = resolveSecrets({ username: "admin", password: "{{FORGE_E2E_PASSWORD}}" }, { FORGE_E2E_PASSWORD: "pw" });
...
describe("agentBrowserRunner — credentials never in argv (R4-AC2)", () => {
    ...
    afterEach(() => {
        if (originalPwd === undefined)
            delete process.env.FORGE_E2E_PASSWORD;
        else
            process.env.FORGE_E2E_PASSWORD = originalPwd;
    });
    it("fill value is resolved from env and recorded in fill call, not as a ref", async () => {
        ...
        const a = (await agentBrowserRunner.run(scenario, ctx));
...
//# sourceMappingURL=accept-credentials.test.js.map