...
describe("methodology parsing invariants (property tests)", () => {
    ...
    it("parser always returns valid methodology enum value", () => {
        const methodologyArb = fc.oneof(fc.constantFrom(...VALID_METHODOLOGIES), fc.string(), fc.constant(undefined), fc.constant(null), fc.integer());
        fc.assert(fc.property(methodologyArb, (methodology) => {
    ...
    it("unavailable invariant: methodology=unavailable forces result=blocked", () => {
        const resultArb = fc.oneof(fc.constantFrom("pass", "fail", "incomplete"), fc.string({ maxLength: 20 }), fc.constant(undefined), fc.constant(null));
        fc.assert(fc.property(resultArb, (result) => {
    ...
    it("legacy and zod paths agree on methodology", () => {
        ...
        fc.constant(""), fc.constant("no frontmatter"), 
        // Valid methodology
        ...VALID_METHODOLOGIES.map((m) => fc.constant([
            "---",
            `result: pass`,
            `methodology: ${m}`,
            "p0_count: 0",
...
//# sourceMappingURL=review-report-methodology.property.test.js.map