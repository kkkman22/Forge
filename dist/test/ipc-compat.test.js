...
describe("diff-ipc-schema script", () => {
    ...
    it("fails when baseline field is missing in new schema", () => {
        const baseline = join(tmpDir, "baseline.ndjson");
        const current = join(tmpDir, "current.ndjson");
        writeFileSync(baseline, [JSON.stringify({ event: "iteration_start", run_id: "r1", important_field: 42 })].join("\n") +
            "\n");
        writeFileSync(current, `${[JSON.stringify({ event: "iteration_start", run_id: "r2" })].join("\n")}\n`);
        expect(() => {
