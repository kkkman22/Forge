...
import { FORBIDDEN_PLACEHOLDERS, scanForPlaceholders, validateAtomicTask, validateDependencies, validatePlanTasks, validateSpecLocked, } from "../src/plan.js";
// ---------------------------------------------------------------------------
...
const filePathArb = fc
    .tuple(fc.constantFrom("src", "lib", "test", "app"), safeStringArb, fc.constantFrom(".ts", ".tsx", ".js", ".jsx"))
    .map(([dir, name, ext]) => `${dir}/${name}${ext}`);
/** A run command like `npx vitest run --grep "something"`. */
const runCommandArb = fc
...
const refactorDescArb = safeStringArb.map((s) => `Extract ${s} into separate module`);
// ---------------------------------------------------------------------------
// Generators — valid TDD steps
// ---------------------------------------------------------------------------
const validTDDStepsArb = fc
    .tuple(filePathArb, // red testFile
...
const validMinutesArb = fc.integer({ min: 2, max: 5 });
/** A fully valid atomic task. */
const validAtomicTaskArb = fc
    .tuple(fc.integer({ min: 1, max: 100 }), // taskNumber
safeStringArb, // title
...
commitMessageArb)
    .map(([taskNumber, title, filePath, estimatedMinutes, tddSteps, verifyCommand, commitMessage]) => ({
    taskNumber,
    title,
    filePath,
    estimatedMinutes,
...
const taskMissingFilePathArb = validAtomicTaskArb.map((task) => ({
    ...task,
    filePath: "",
}));
/** Task with empty title. */
const taskMissingTitleArb = validAtomicTaskArb.map((task) => ({
    ...task,
    title: "",
}));
/** Task with empty verify command. */
const taskMissingVerifyArb = validAtomicTaskArb.map((task) => ({
    ...task,
    verifyCommand: "",
}));
/** Task with empty commit message. */
const taskMissingCommitArb = validAtomicTaskArb.map((task) => ({
    ...task,
    commitMessage: "",
}));
/** Task with empty TDD RED test code. */
const taskMissingRedCodeArb = validAtomicTaskArb.map((task) => ({
    ...task,
    tddSteps: {
        ...task.tddSteps,
...
const taskMissingGreenCodeArb = validAtomicTaskArb.map((task) => ({
    ...task,
    tddSteps: {
        ...task.tddSteps,
...
const taskMissingRefactorArb = validAtomicTaskArb.map((task) => ({
    ...task,
    tddSteps: { ...task.tddSteps, refactor: "" },
}));
/** Any task with one missing required field. */
const taskWithMissingFieldArb = fc.oneof(taskMissingFilePathArb, taskMissingTitleArb, taskMissingVerifyArb, taskMissingCommitArb, taskMissingRedCodeArb, taskMissingGreenCodeArb, taskMissingRefactorArb);
// ---------------------------------------------------------------------------
// Generators — invalid time range
// ---------------------------------------------------------------------------
/** Estimated minutes below the minimum (< 2). */
const tooShortMinutesArb = fc.integer({ min: -10, max: 1 });
/** Estimated minutes above the maximum (> 5). */
const tooLongMinutesArb = fc.integer({ min: 6, max: 120 });
/** Task with time outside the valid 2-5 range. */
const taskWithInvalidTimeArb = fc
...
const forbiddenPlaceholderArb = fc.constantFrom(...FORBIDDEN_PLACEHOLDERS);
/** A task with a forbidden placeholder injected into the title. */
const taskWithPlaceholderInTitleArb = fc
...
const taskWithPlaceholderInCodeArb = fc
    .tuple(validAtomicTaskArb, forbiddenPlaceholderArb)
    .map(([task, placeholder]) => ({
    ...task,
    tddSteps: {
...
const taskWithPlaceholderInCommitArb = fc
    .tuple(validAtomicTaskArb, forbiddenPlaceholderArb)
    .map(([task, placeholder]) => ({
    ...task,
...
const taskWithPlaceholderArb = fc.oneof(taskWithPlaceholderInTitleArb, taskWithPlaceholderInCodeArb, taskWithPlaceholderInCommitArb);
// ---------------------------------------------------------------------------
// Property 9: Plan 任务有效性
// ---------------------------------------------------------------------------
describe("Property 9: Plan 任务有效性", () => {
    ...
    it("tasks with missing required fields fail validation (Req 4.2)", () => {
        fc.assert(fc.property(taskWithMissingFieldArb, (task) => {
            const result = validateAtomicTask(task);
    ...
    it("tasks with forbidden placeholders fail validation (Req 4.4)", () => {
        fc.assert(fc.property(taskWithPlaceholderArb, (task) => {
            const result = validateAtomicTask(task);
            expect(result.valid).toBe(false);
...
describe("scanForPlaceholders", () => {
    it("detects all forbidden placeholders in text (Req 4.4)", () => {
        fc.assert(fc.property(forbiddenPlaceholderArb, safeStringArb, (placeholder, surrounding) => {
...
describe("validateDependencies", () => {
    it("returns empty errors when no tasks have dependsOn", () => {
        fc.assert(fc.property(fc.array(validAtomicTaskArb, { minLength: 1, maxLength: 5 }), (tasks) => {
    ...
    it("returns multiple errors for multiple invalid references", () => {
        const tasks = [{ ...makeMinimalTask(1), dependsOn: [10, 20] }];
        const errors = validateDependencies(tasks);
...
describe("validatePlanTasks with dependsOn", () => {
    it("fails when a task has an invalid dependency reference", () => {
        const tasks = [makeMinimalTask(1), { ...makeMinimalTask(2), dependsOn: [99] }];
        expect(validatePlanTasks(tasks)).toBe(false);
    });
    it("passes when all dependency references are valid", () => {
        const tasks = [makeMinimalTask(1), { ...makeMinimalTask(2), dependsOn: [1] }];
        expect(validatePlanTasks(tasks)).toBe(true);
    });
    it("passes when tasks have no dependsOn field", () => {
        const tasks = [makeMinimalTask(1), makeMinimalTask(2)];
        expect(validatePlanTasks(tasks)).toBe(true);
    });
});
// ---------------------------------------------------------------------------
...
describe("Property 10: dependsOn dependency validation", () => {
     ...
     */
    const tasksWithValidDepsArb = fc
        ...
        const depArbs = baseTasks.map((task, idx) => {
            ...
            if (otherNumbers.length === 0) {
                ...
                return fc.constant({ ...task, taskNumber: taskNumbers[idx], dependsOn: [] });
            }
            return fc
     ...
     */
    const tasksWithInvalidDepsArb = fc
        .tuple(fc.array(validAtomicTaskArb, { minLength: 1, maxLength: 8 }), fc.nat({ max: 50 }), // index selector for which task gets the bad dep
...
// ---------------------------------------------------------------------------
function makeMinimalTask(taskNumber) {
    return {
        taskNumber,
...
//# sourceMappingURL=plan.property.test.js.map