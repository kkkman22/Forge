...
describe("checkGrillGlossaryConflicts", () => {
    ...
    it("flags avoided_term when tree text contains a glossary-avoided synonym", () => {
        const tree = applyAnswer(generateDecisionTree("Use Action pattern for commands.", GLOSSARY_WITH_AVOIDED, FIXED_NOW), "functionality-1", "We dispatch an Action to mutate state. Action is dispatched synchronously.", FIXED_NOW);
    ...
    it("flags relation_violation when answer reverses the glossary-defined relation", () => {
        const tree = applyAnswer(generateDecisionTree("Handle Event in the system.", GLOSSARY_WITH_RELATIONS, FIXED_NOW), "functionality-1", "The Handler dispatches an Event to the queue. Handler triggers Event processing.", FIXED_NOW);
    ...
    it("skips relation entries that do not match the → pattern", () => {
        ...
        const tree = applyAnswer(generateDecisionTree("Handle Event flow.", glossaryNoArrow, FIXED_NOW), "functionality-1", "Handler triggers Event in the pipeline. Handler sends Event downstream.", FIXED_NOW);
