...
import { buildStopContext, } from "../scripts/stop-additional-context.mjs";
describe("buildStopContext", () => {
    ...
    it("auto-advance gap (build done, review should follow) → auto_advance_gap", () => {
        ...
        expect(result.additionalContext.toLowerCase().includes("no-idle") ||
            result.additionalContext.includes("铁律") ||
            result.additionalContext.includes("auto") ||
            result.additionalContext.includes("review")).toBe(true);
    });
...
describe("hooks.json contract: Stop/SubagentStop additionalContext", () => {
    ...
    it("hooks.json has StopFailure event with additionalContext hook (SubagentStop proxy)", async () => {
        ...
        const stopFailureHookArgs = config.hooks.StopFailure.flatMap((group) => (group.hooks ?? []).map((h) => h.args ?? []));
        const hasAdditionalContextHook = stopFailureHookArgs.some((args) => Array.isArray(args) &&
            args.some((a) => typeof a === "string" && a.includes("stop-additional-context")));
        expect(hasAdditionalContextHook).toBe(true);
    });
});
//# sourceMappingURL=stop-additional-context.test.js.map