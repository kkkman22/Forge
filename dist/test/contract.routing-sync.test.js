...
const routerSkill = readFileSync(resolve(ROOT, "skills/forge-router/SKILL.md"), "utf-8");
...
describe("Tier names in SKILL.md match router.ts", () => {
    ...
    for (const tier of tiers) {
        ...
        it(`CLAUDE.md mentions tier "${tier}" or its Chinese equivalent`, () => {
            ...
            const found = claudeMd.toLowerCase().includes(tier) ||
...
describe("Task type names in SKILL.md match router.ts", () => {
    const taskTypes = ["frontend", "backend", "fullstack", "data", "infra", "docs"];
...
describe("Project phase names in SKILL.md match router.ts", () => {
    const phases = ["greenfield", "iteration", "refactor", "bugfix"];
...
//# sourceMappingURL=contract.routing-sync.test.js.map