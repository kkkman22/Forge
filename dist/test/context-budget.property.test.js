...
const testOutputSummaryArb = fc.record({
...
const _gitDiffSummaryArb = fc.record({
...
const _gitStatusSummaryArb = fc.record({
    staged: fc.record({
...
const subagentSummaryArb = fc
    .tuple(fc.constantFrom("DONE", "DONE_WITH_CONCERNS", "NEEDS_CONTEXT", "BLOCKED"), fc.string({ minLength: 1, maxLength: 100 }), fc.array(fc.string({ minLength: 1, maxLength: 60 }), { maxLength: 10 }), fc.record({
    passed: fc.integer({ min: 0, max: 500 }),
    failed: fc.integer({ min: 0, max: 50 }),
}), fc.string({ minLength: 1, maxLength: 100 }), fc.string({ minLength: 1, maxLength: 100 }), fc.option(fc.string({ minLength: 1, maxLength: 100 })), fc.option(fc.array(fc.string({ minLength: 1, maxLength: 80 }), { maxLength: 5 })))
...
describe("Feature: context-budget-management, Property 2: Explore summary format constraints", () => {
    it("serialized output does not exceed 300 tokens (approximated by char count)", () => {
        fc.assert(fc.property(exploreSummaryArb, (summary) => {
...
describe("Feature: context-budget-management, Property 3: Review summary format constraints", () => {
    it("serialized output does not exceed 400 tokens", () => {
        fc.assert(fc.property(reviewSummaryArb, (summary) => {
...
describe("Feature: context-budget-management, Property 4: Test output trimmer correctness", () => {
    it("all-pass produces single summary line ≤150 tokens", () => {
        fc.assert(fc.property(testOutputSummaryArb, (summary) => {
...
describe("Feature: context-budget-management, Property 6: Subagent summary format completeness", () => {
    it("contains all required fields and does not exceed 200 tokens", () => {
        fc.assert(fc.property(subagentSummaryArb, (summary) => {
