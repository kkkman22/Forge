...
describe("hook-stdin-router PBT", () => {
    it("never throws and always returns a valid RouterDecision for arbitrary input", {
        ...
        fc.assert(fc.property(fc.string({ maxLength: 1000 }), (stdinStr) => {
            // Filter out strings with null bytes (env vars can't contain them)
            if (stdinStr.includes("\0"))
                return true;
    ...
    it("JSON with agent_id (non-empty string) always classifies as subagent", {
        ...
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 50 }), fc.string({ maxLength: 100 }), (agentId, sessionId) => {
            if (agentId.includes("\0") || sessionId.includes("\0"))
                return true;
    ...
    it("JSON with hook_event_name but no agent_id always classifies as main", {
        ...
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 50 }), fc.string({ maxLength: 100 }), (eventName, sessionId) => {
            if (eventName.includes("\0") || sessionId.includes("\0"))
                return true;
            const payload = JSON.stringify({
                session_id: sessionId,
...
//# sourceMappingURL=hook-stdin-router.property.test.js.map