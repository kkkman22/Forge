...
describe("T8: SdkDriver replacement — agent-sdk removal", () => {
    it("forge-loop-cli.ts has no runtime import from agent-sdk", () => {
        ...
        const agentSdkImports = lines.filter((line) => line.includes("@anthropic-ai/claude-agent-sdk") && !line.trim().startsWith("//"));
    ...
    it("no runtime agent-sdk import across the 6 target files (except deprecated adapter)", () => {
        ...
        try {
            const result = execSync(`rg "from '@anthropic-ai/claude-agent-sdk'" ${filesToCheck.map((f) => `"${join(ROOT, f)}"`).join(" ")}`, { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] }).trim();
            if (result) {
...
//# sourceMappingURL=sdk-driver-replacement.test.js.map