...
describe("buildShipGateBlockArtifacts — Requirement 8.7", () => {
    it("maps 'uncommitted' reason to a partial outcome", () => {
        const { episode, markerText } = buildShipGateBlockArtifacts("order-export", "standard", "uncommitted", "ship gate 拦截：存在未提交的本地修改", FIXED_NOW, 1);
    ...
    it("maps 'checklist_failed' reason to a failure outcome", () => {
        const { episode, markerText } = buildShipGateBlockArtifacts("order-export", "full", "checklist_failed", "ship gate 拦截：P1 Fix Checklist 未全部验证", FIXED_NOW, 2);
    ...
    it("is deterministic under identical inputs", () => {
        const a = buildShipGateBlockArtifacts("deterministic", "standard", "uncommitted", "situation", FIXED_NOW, 3);
        const b = buildShipGateBlockArtifacts("deterministic", "standard", "uncommitted", "situation", FIXED_NOW, 3);
        expect(a.episode).toEqual(b.episode);
        expect(a.markerText).toBe(b.markerText);
    });
    it("propagates tier from the call site", () => {
        const { episode: light } = buildShipGateBlockArtifacts("topic", "light", "uncommitted", "x", FIXED_NOW, 1);
        const { episode: full } = buildShipGateBlockArtifacts("topic", "full", "checklist_failed", "x", FIXED_NOW, 1);
        expect(light.tier).toBe("light");
        expect(full.tier).toBe("full");
    });
});
//# sourceMappingURL=ship-gate-evolution.test.js.map