...
// ---------------------------------------------------------------------------
function formatEvolvedRules(file) {
    const lines = [];
    // YAML frontmatter
    lines.push("---");
...
// ---------------------------------------------------------------------------
function parseEvolvedRules(text) {
...
const sourceArb = fc
    .tuple(fc.constantFrom("known-failures", "instincts", "skill-feedback", "metrics", "session-journal"), fc.stringMatching(/^[a-zA-Z0-9_-]+$/).filter((s) => s.length >= 1 && s.length <= 20))
    .map(([file, entry]) => `.forge/knowledge/${file}.md#${entry}`);
/** Generate a single evolved rule with a given index. */
function ruleArb(index) {
    return fc
...
describe("Feature: claude-md-self-evolution, Property 1: Evolved rules file round-trip", () => {
    ...
    it("format then parse produces equivalent rules file", () => {
        fc.assert(fc.property(evolvedRulesFileArb, (file) => {
