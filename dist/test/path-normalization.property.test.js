...
// specs/ paths
fc
    .tuple(fc.stringMatching(/^[a-z][a-z0-9-]{0,20}$/), fc.stringMatching(/^[a-z][a-z0-9-]{0,20}\.md$/))
...
const nonFrozenRelativePathArb = fc.oneof(fc.constant("progress/topic.md"), fc.constant("reviews/review.md"), fc.constant("knowledge/instincts.md"), fc.constant("scratch/notes.md"), fc.stringMatching(/^[a-z][a-z0-9-]{0,15}\.md$/).map((f) => `scratch/${f}`));
/** Absolute path prefix generator */
const absolutePrefixArb = fc.oneof(fc.constant("/home/user/project/"), fc.constant("/abs/path/to/repo/"), fc.constant("/var/data/workspace/"), fc.constant("/Users/dev/code/"));
// ---------------------------------------------------------------------------
...
describe("Property 6: Path normalization produces consistent frozen zone judgments", () => {
    ...
    it("non-frozen paths remain non-frozen regardless of path variant", () => {
        fc.assert(fc.property(nonFrozenRelativePathArb, (relativePath) => {
            const variants = [
                relativePath,
                `.forge/${relativePath}`,
                `/abs/path/.forge/${relativePath}`,
...
//# sourceMappingURL=path-normalization.property.test.js.map