...
describe("AgentBrowserCliClient.fill — real CLI: value in argv (not stdin)", () => {
    ...
    it("calls fill @<ref> <value> with --session global flag", async () => {
        ...
        const fillCall = execFileMock.mock.calls.find((cl) => Array.isArray(cl[1]) && cl[1].includes("fill"));
        expect(fillCall).toBeDefined();
        const args = fillCall[1];
...
describe("AgentBrowserCliClient.click / get url / screenshot / close", () => {
    ...
    it("click uses @ref prefix", async () => {
        ...
        const c = new AgentBrowserCliClient();
        await c.click("s1", "e5");
        const clickCall = execFileMock.mock.calls.find((cl) => Array.isArray(cl[1]) && cl[1].includes("click"));
        expect(clickCall[1]).toEqual(["--session", "s1", "click", "@e5"]);
    });
});
//# sourceMappingURL=agent-browser-cli-client.test.js.map