...
import { extractBacktickPathsWithSections, parseInfraRefs, validateInfraRefs, } from "../src/evolved-rules-infra-refs.js";
describe("extractBacktickPathsWithSections", () => {
    ...
    it("extracts path with §section", () => {
        expect(extractBacktickPathsWithSections("`skills/forge-pack/SKILL.md` §Subcommands")).toEqual([{ path: "skills/forge-pack/SKILL.md", section: "Subcommands" }]);
    });
...
describe("validateInfraRefs", () => {
    ...
    it("flags non-existent files", () => {
        const mockFs = {
            fileExists: (p) => p === "skills/foo/SKILL.md" || p === "config.json",
    ...
    it("flags missing sections in markdown refs", () => {
        const mockFs = {
            fileExists: () => true,
            readFile: (p) => p === "skills/foo/SKILL.md" ? "## Other Section\n\nNo Subcommands here." : "",
        };
        const verdicts = validateInfraRefs(refs, mockFs);
...
//# sourceMappingURL=evolved-rules-infra-refs.test.js.map