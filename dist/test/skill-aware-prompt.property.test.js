...
/** Optional taskType. */
const taskTypeArb = fc.option(fc.constantFrom("frontend", "backend", "fullstack", "data", "infra", "docs"), { nil: undefined });
/** Optional projectPhase. */
const projectPhaseArb = fc.option(fc.constantFrom("greenfield", "iteration", "refactor", "bugfix"), { nil: undefined });
// ---------------------------------------------------------------------------
...
describe("Feature: loop-skills-fusion, Property 4: buildSkillAwarePrompt 字段注入完整性", () => {
    ...
    it("output contains phase value and tier value when phase is provided", () => {
        fc.assert(fc.property(baseParamsArb, skillPhaseArb, tierArb, (base, phase, tier) => {
            const params = {
                base,
                skill: { phase, tier },
            };
    ...
    it("output contains routing instruction (forge-router) when phase is empty/missing", () => {
        fc.assert(fc.property(baseParamsArb, tierArb, fc.constantFrom("", "  ", "   "), (base, tier, emptyPhase) => {
            const params = {
                base,
    ...
    it("output contains all hint descriptions when hints are provided", () => {
        fc.assert(fc.property(baseParamsArb, skillPhaseArb, tierArb, fc.array(hintArb, { minLength: 1, maxLength: 5 }), (base, phase, tier, hints) => {
            const params = {
                base,
    ...
    it("output contains all fix issue descriptions when fixIssues are provided", () => {
        fc.assert(fc.property(baseParamsArb, skillPhaseArb, tierArb, fc.array(fixIssueArb, { minLength: 1, maxLength: 5 }), (base, phase, tier, fixIssues) => {
            const params = {
                base,
    ...
    it('output always contains "mode: autonomous"', () => {
        fc.assert(fc.property(baseParamsArb, fc.option(skillPhaseArb, { nil: "" }), tierArb, fc.option(fc.array(hintArb, { minLength: 0, maxLength: 3 }), { nil: undefined }), fc.option(fc.array(fixIssueArb, { minLength: 0, maxLength: 3 }), { nil: undefined }), taskTypeArb, projectPhaseArb, (base, phase, tier, hints, fixIssues, taskType, projectPhase) => {
            const params = {
                base,
                skill: {
                    phase: phase ?? "",
    ...
    it("output contains taskType and projectPhase when provided", () => {
        fc.assert(fc.property(baseParamsArb, skillPhaseArb, tierArb, fc.constantFrom("frontend", "backend", "fullstack", "data", "infra", "docs"), fc.constantFrom("greenfield", "iteration", "refactor", "bugfix"), (base, phase, tier, taskType, projectPhase) => {
            const params = {
                base,
