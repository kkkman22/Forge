...
import { checkCommandPolicy, checkFilesystemPolicy, checkNetworkPolicy, DEFAULT_SANDBOX_CONFIG, loadSandboxConfig, resolveProfile, } from "../src/sandbox-phased.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function makeTmpDir() {
    const dir = join(tmpdir(), `sandbox-phased-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe("SandboxCheckResult type", () => {
    ...
    it("has optional matchedRule string", () => {
        const result = { allowed: false, reason: "test", matchedRule: "*.env" };
...
describe("checkFilesystemPolicy", () => {
    ...
    it("glob deny blocks paths that would match allow", () => {
        ...
        expect(checkFilesystemPolicy("src/foo/secret.ts", "read", config).matchedRule).toBe("src/**/secret.ts");
    });
...
//# sourceMappingURL=sandbox-phased.test.js.map