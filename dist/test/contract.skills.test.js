...
describe("Contract: skills/*/SKILL.md has YAML frontmatter", () => {
    for (const { dir, path: skillPath } of skillMdFiles) {
        it(`skills/${dir}/SKILL.md starts with YAML frontmatter`, () => {
            ...
            expect(content.startsWith("---\n"), `skills/${dir}/SKILL.md missing opening frontmatter delimiter`).toBe(true);
            const closingIdx = content.indexOf("---", 4);
            expect(closingIdx, `skills/${dir}/SKILL.md missing closing frontmatter delimiter`).toBeGreaterThan(0);
        });
...
describe("Contract: skills/*/SKILL.md contains required content sections", () => {
    for (const { dir, path: skillPath } of skillMdFiles) {
        it(`skills/${dir}/SKILL.md has substantive content after frontmatter`, () => {
            ...
            expect(bodyContent.length, `skills/${dir}/SKILL.md has no content after frontmatter`).toBeGreaterThan(0);
        });
        it(`skills/${dir}/SKILL.md has a heading structure (## sections)`, () => {
            ...
            expect(hasHeadings, `skills/${dir}/SKILL.md has no ## section headings after frontmatter`).toBe(true);
        });
        it(`skills/${dir}/SKILL.md has an overview or instructions section`, () => {
            ...
            expect(instructionsPattern.test(bodyContent), `skills/${dir}/SKILL.md has no overview/instructions section (expected ## Instructions, ## 概述, or numbered ## heading)`).toBe(true);
        });
    }
});
