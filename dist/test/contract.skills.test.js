...
describe("Contract: skills/forge/lib/*/instructions.md has YAML frontmatter", () => {
    for (const { dir, path: skillPath } of instrFiles) {
        it(`skills/forge/lib/${dir}/instructions.md starts with YAML frontmatter`, () => {
            ...
            expect(content.startsWith("---\n"), `skills/forge/lib/${dir}/instructions.md missing opening frontmatter delimiter`).toBe(true);
            const closingIdx = content.indexOf("---", 4);
            expect(closingIdx, `skills/forge/lib/${dir}/instructions.md missing closing frontmatter delimiter`).toBeGreaterThan(0);
        });
...
describe("Contract: skills/forge/lib/*/instructions.md contains required content sections", () => {
    for (const { dir, path: skillPath } of instrFiles) {
        it(`skills/forge/lib/${dir}/instructions.md has substantive content after frontmatter`, () => {
            ...
            expect(frontmatterEnd, `skills/forge/lib/${dir}/instructions.md has no closing frontmatter delimiter`).toBeGreaterThan(0);
            const bodyContent = content.slice(frontmatterEnd + 3).trim();
            expect(bodyContent.length, `skills/forge/lib/${dir}/instructions.md has no content after frontmatter`).toBeGreaterThan(0);
        });
        it(`skills/forge/lib/${dir}/instructions.md has a heading structure (## sections)`, () => {
            ...
            expect(hasHeadings, `skills/forge/lib/${dir}/instructions.md has no ## section headings after frontmatter`).toBe(true);
        });
        it(`skills/forge/lib/${dir}/instructions.md has an overview or instructions section`, () => {
            ...
            expect(instructionsPattern.test(bodyContent), `skills/forge/lib/${dir}/instructions.md has no overview/instructions section (expected ## Instructions, ## 概述, or numbered ## heading)`).toBe(true);
        });
    }
});
// ---------------------------------------------------------------------------
