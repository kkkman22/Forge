...
import { classifyL0Failure, dispatch, isolatePartialFindings, probeL0Eligibility, updateStatusMd, writeDispatchRecord, } from "../src/workflow-dispatcher.js";
// ---------------------------------------------------------------------------
// Helper: set up a valid L0 workflow environment
// ---------------------------------------------------------------------------
function setupL0Workflow(tmpDir) {
    ...
    writeFileSync(join(wfDir, "review.js"), "import { chunkedParallel } from './lib/concurrency.js';\nexport const meta = {};\n");
}
describe("WorkflowDispatcher", () => {
    ...
    describe("classifyL0Failure", () => {
        ...
        it("classifies schema_validation_failed", () => {
            expect(classifyL0Failure(new Error("schema validation failed for output"))).toBe("schema_validation_failed");
        });
        ...
        it("classifies frozen_zone_blocked", () => {
            expect(classifyL0Failure(new Error("FrozenZoneViolation: .forge/specs/locked"))).toBe("frozen_zone_blocked");
        });
    });
    // ---------------------------------------------------------------------------
    ...
    describe("dispatch auditWriter integration", () => {
        it("calls auditWriter.write for L1 result", async () => {
            ...
            expect(auditWriter.write).toHaveBeenCalledWith(expect.objectContaining({
                subcommand: "review",
                runId: "run-001",
                topic: "test-topic",
            }));
...
//# sourceMappingURL=workflow-dispatcher.test.js.map