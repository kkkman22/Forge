...
describe("scheduleWave", () => {
    it("executes all tasks in a wave", async () => {
        const wave = { wave: 1, tasks: ["T-01", "T-02", "T-03"] };
        const executed = [];
    ...
    it("tracks failed tasks", async () => {
        const wave = { wave: 1, tasks: ["T-01", "T-02"] };
        const result = await scheduleWave(wave, {
            maxConcurrency: 6,
    ...
    it("degrades concurrency on 429 signal", async () => {
        const wave = { wave: 1, tasks: ["T-01", "T-02", "T-03", "T-04"] };
        let callCount = 0;
        const batches = [];
        const result = await scheduleWave(wave, {
            maxConcurrency: 4,
...
//# sourceMappingURL=build-wave-three-strike.test.js.map