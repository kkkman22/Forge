...
import { generateResumeOutput, } from "../src/resume.js";
// ---------------------------------------------------------------------------
// Generators
// ---------------------------------------------------------------------------
 ...
 */
const progressContextFromPlanArb = (plan) => {
    const taskCount = plan.tasks.length;
    return fc
        .tuple(fc.integer({ min: 0, max: taskCount }), fc.integer({ min: 0, max: Math.min(1, taskCount) }), fc.array(fc.string({ minLength: 3, maxLength: 30 }).filter((s) => s.trim().length >= 3), { minLength: 0, maxLength: 3 }))
...
const projectStateWithFindingsArb = planContextArb.chain((plan) => fc
    .tuple(progressContextFromPlanArb(plan), nonEmptyFindingsArb)
    .map(([progress, findings]) => ({ plan, progress, findings })));
/** A project state where some tasks are in progress. */
const projectStateWithInProgressArb = planContextArb
    .filter((plan) => plan.tasks.length >= 2)
    .chain((plan) => fc
    .tuple(fc.integer({ min: 0, max: plan.tasks.length - 1 }), findingsContextArb, fc.array(fc.string({ minLength: 3, maxLength: 30 }).filter((s) => s.trim().length >= 3), { minLength: 0, maxLength: 3 }))
