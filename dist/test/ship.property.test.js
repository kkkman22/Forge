...
/** Any review result. */
const anyReviewArb = fc.oneof(passedReviewArb, failedReviewArb);
/** A test result that passed. */
const passedTestArb = fc.constant({ passed: true });
/** A test result that failed. */
const failedTestArb = fc.constant({ passed: false });
/** Any test result. */
const anyTestArb = fc.oneof(passedTestArb, failedTestArb);
/** A progress result where all tasks are complete. */
const completeProgressArb = fc
    .integer({ min: 1, max: 20 })
    .map((total) => ({ totalTasks: total, completedTasks: total }));
/** A progress result where some tasks are incomplete. */
const incompleteProgressArb = fc
...
/** Any progress result. */
const anyProgressArb = fc.oneof(completeProgressArb, incompleteProgressArb);
...
describe("Property 11: Ship 门禁——评审通过且测试通过且任务完成", () => {
    it("all three gates pass → ship allowed (Req 8.1, 8.2)", () => {
        fc.assert(fc.property(allPassArb, ({ review, test, progress }) => {
