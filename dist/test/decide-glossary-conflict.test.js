...
import { checkDecideGlossaryConflicts, renderDecideGlossaryConflictPrompt, } from "../src/decide.js";
// ---------------------------------------------------------------------------
// Fixtures
// ---------------------------------------------------------------------------
...
describe("renderDecideGlossaryConflictPrompt", () => {
    ...
    it("renders every conflict, preserves order, and includes the count", () => {
        ...
        expect(lines[1]).toBe(`  - "Spec": existing = "${first.existing.definition}", proposed = "${first.candidate.definition}"`);
        expect(lines[2]).toBe(`  - "Level": existing = "${second.existing.definition}", proposed = "${second.candidate.definition}"`);
        expect(lines[3]).toBe("请澄清：保留现有 / 替换现有 / 新增别名");
        expect(lines).toHaveLength(4);
    });
});
//# sourceMappingURL=decide-glossary-conflict.test.js.map