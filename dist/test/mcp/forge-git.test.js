...
import { formatDiffSummary, formatStatusSummary, registerForgeGit, truncateDiffContent, } from "../../src/mcp/tools/forge-git.js";
// ---------------------------------------------------------------------------
...
function mockGitFailure(stdout, stderr, exitCode = 128) {
    mockedExecFile.mockImplementation((_cmd, _args, _opts, cb) => {
...
describe("formatStatusSummary", () => {
    ...
    it("omits empty categories from file listing", () => {
        ...
        const fileListLines = lines.filter((l) => l.match(/^\s+(Staged|Modified|Untracked):/) && !l.includes("|"));
...
function collectGitHandler() {
    let handler = null;
    const fakeServer = {
        registerTool: (_name, _schema, h) => {
            handler = h;
        },
    };
    registerForgeGit(fakeServer);
    if (!handler)
        throw new Error("forge_git handler not registered");
    return handler;
}
...
//# sourceMappingURL=forge-git.test.js.map