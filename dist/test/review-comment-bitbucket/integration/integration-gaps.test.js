...
describe("Integration: post.ts wires side-effect modules", () => {
    ...
    it("gate skip calls recordSkip → writes daily skip file + tool-health", async () => {
        ...
        const result = await postReviewToBitbucket(reviewMd, "pr-1", { ...CONFIG, platform_override: "none" }, {
    ...
    it("gate skip also writes metrics.md with gate_skipped_reason", async () => {
        ...
        const reviewMd = join(tmpDir, "review.md");
        writeFileSync(reviewMd, "# Review\n", "utf-8");
        await postReviewToBitbucket(reviewMd, "pr-1", { ...CONFIG, platform_override: "none" }, {
            remoteUrl: "https://bitbucket.org/org/repo",
            mcpBaseUrl: "https://bitbucket.org",
            mcpConfigured: true,
    ...
    it("happy path calls appendRunMetrics with all 10 fields", async () => {
        const bb = mockBitbucketClient();
        await postReviewToBitbucket(join(tmpDir, "review.md"), "pr-1", CONFIG, {
            remoteUrl: "https://bitbucket.org/org/repo",
            mcpBaseUrl: "https://bitbucket.org",
            mcpConfigured: true,
    ...
    it("parseReviewMarkdown: missing file → review-markdown-not-found (not parse-error)", async () => {
        const bb = mockBitbucketClient();
        const result = await postReviewToBitbucket(join(tmpDir, "nonexistent.md"), "pr-1", CONFIG, {
            remoteUrl: "https://bitbucket.org/org/repo",
            mcpBaseUrl: "https://bitbucket.org",
            mcpConfigured: true,
            runId: "run-notfound",
    ...
    it("parseReviewMarkdown: invalid content → parse-error", async () => {
        ...
        const result = await postReviewToBitbucket(badMd, "pr-1", CONFIG, {
            remoteUrl: "https://bitbucket.org/org/repo",
            mcpBaseUrl: "https://bitbucket.org",
            mcpConfigured: true,
            runId: "run-parse-err",
    ...
    it("parse-error path also writes metrics", async () => {
        const bb = mockBitbucketClient();
        await postReviewToBitbucket(join(tmpDir, "nonexistent.md"), "pr-1", CONFIG, {
            remoteUrl: "https://bitbucket.org/org/repo",
            mcpBaseUrl: "https://bitbucket.org",
            mcpConfigured: true,
    ...
    it("applyCliOverrides: --no-post-comments disables + writes skip + metrics", async () => {
        const bb = mockBitbucketClient();
        const result = await postReviewToBitbucket(join(tmpDir, "review.md"), "pr-1", CONFIG, {
            remoteUrl: "https://bitbucket.org/org/repo",
            mcpBaseUrl: "https://bitbucket.org",
            mcpConfigured: true,
            runId: "run-cli-001",
...
//# sourceMappingURL=integration-gaps.test.js.map