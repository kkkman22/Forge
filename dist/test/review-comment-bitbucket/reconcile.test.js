...
describe("reconcile", () => {
    ...
    const createFinding = (message) => ({
        ...finding,
        message,
    });
    const createTask = (hash, status, taskId = "task-123") => ({
        ...taskRecord,
        task_id: taskId,
        status,
        marker_hash: hash,
    });
    const createComment = (hash, commentId = "comment-789") => ({
        ...commentRecord,
    ...
    describe("P9: creates/dones/reopens — same hash at most once (mutual exclusivity)", () => {
        it("should not have same hash in multiple action sets", { timeout: 30000 }, () => {
            ...
            const hasOverlap = [...createHashes].some((h) => doneHashes.has(h) || reopenHashes.has(h)) ||
