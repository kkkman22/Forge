...
import { checkPlatformGate, isBitbucketUrl, isSameHost, parseRemoteUrl, selectRemoteUrl, } from "../../src/review-comment-bitbucket/platform-gate.js";
describe("platform-gate: property tests", () => {
    ...
    it("Row 1: Bitbucket URL, auto, MCP configured, same-host → pass", { timeout: 30000 }, () => {
        fc.assert(fc.property(fc.stringMatching(/^[a-z0-9-]+$/), fc.stringMatching(/^[a-z0-9-]+$/), (org, repo) => {
    ...
    it("Row 2: Bitbucket URL, auto, MCP configured, NOT same-host → mcp-base-url-mismatch", {
        ...
        fc.assert(fc.property(fc.webUrl(), fc.webUrl(), (bitbucketUrl, mcpUrl) => {
    ...
    it("Row 4: Non-Bitbucket URL or null, auto → platform-not-bitbucket", { timeout: 30000 }, () => {
        fc.assert(fc.property(fc.oneof(fc.constant(null), fc.webUrl().filter((url) => !url.includes("bitbucket."))), (remoteUrl) => {
            const input = {
                remoteUrl,
                platformOverride: "auto",
                mcpConfigured: false,
    ...
    it("Row 6: override=bitbucket, MCP configured, NOT same-host → mcp-base-url-mismatch", {
        ...
        fc.assert(fc.property(fc.webUrl(), fc.webUrl(), (remoteUrl, mcpUrl) => {
    ...
    it("Row 7: override=bitbucket, MCP NOT configured → override-but-mcp-missing", {
        ...
        fc.assert(fc.property(fc.webUrl(), (remoteUrl) => {
            const input = {
                remoteUrl,
                platformOverride: "bitbucket",
                mcpConfigured: false,
    ...
    it("Row 8: override=none → platform-disabled-by-config", { timeout: 30000 }, () => {
        fc.assert(fc.property(fc.oneof(fc.constant(null), fc.webUrl()), fc.boolean(), fc.oneof(fc.constant(null), fc.webUrl()), (remoteUrl, mcpConfigured, mcpBaseUrl) => {
            const input = {
                remoteUrl,
    ...
    it("Property 10: Host comparison includes port", { timeout: 30000 }, () => {
        fc.assert(fc.property(fc.webUrl(), fc.nat({ max: 65535 }), (url, port) => {
...
describe("platform-gate: unit tests", () => {
    ...
    it("Unit 4: Multi-remote selection priority", () => {
        ...
        expect(selectRemoteUrl(remotes, "https://bitbucket.example.com/api")).toBe("https://bitbucket.example.com/origin/repo.git");
        // Test 2: no origin, but upstream exists
        const remotesNoOrigin = remotes.filter((r) => r.name !== "origin");
        expect(selectRemoteUrl(remotesNoOrigin, "https://bitbucket.example.com/api")).toBe("https://bitbucket.example.com/upstream/repo.git");
        ...
        expect(selectRemoteUrl(remotesNoOriginUpstream, "https://bitbucket.example.com/api")).toBe("https://bitbucket.example.com/fork/repo.git");
...
//# sourceMappingURL=platform-gate.test.js.map