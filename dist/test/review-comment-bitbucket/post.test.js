...
describe("Property: gate skip => zero MCP calls", () => {
    it("platform gate skip results in 0 MCP tool calls", { timeout: 30000 }, async () => {
        await fc.assert(fc.asyncProperty(fc.constantFrom("platform-not-bitbucket", "mcp-not-configured", "platform-disabled-by-config"), async (reason) => {
            const bb = mockBitbucketClient();
            const ctx = {
                remoteUrl: reason === "platform-not-bitbucket"
            ...
            const config = {
                ...DEFAULT_CONFIG,
                platform_override: reason === "platform-disabled-by-config" ? "none" : "auto",
            };
...
describe("Property: has_p0_p1=false => set_review_status not called", () => {
    it("no P0/P1 findings means set_review_status never called", { timeout: 30000 }, async () => {
        await fc.assert(fc.asyncProperty(fc.array(fc.record({
            priority: fc.constantFrom("P2", "P3"),
        ...
        }), { maxLength: 5 }), async (findings) => {
            const bb = mockBitbucketClient();
            // Mock parseReviewMarkdown to return our findings
            const result = await postReviewToBitbucket("test-fixture", "pr-1", DEFAULT_CONFIG, {
...
describe("Unit: p0_p1_strategy=pr-task creates only task", () => {
    it("P0 finding creates 1 pr_task, no comment", async () => {
        const bb = mockBitbucketClient();
        const config = { ...DEFAULT_CONFIG, p0_p1_strategy: "pr-task" };
        const result = await postReviewToBitbucket("test-fixture", "pr-1", config, {
            remoteUrl: "https://bitbucket.org/org/repo",
            mcpBaseUrl: "https://bitbucket.org",
            mcpConfigured: true,
            runId: "run-1",
...
describe("Unit: p0_p1_strategy=inline-only creates only comment", () => {
    it("P0 finding creates 1 comment, no task", async () => {
        const bb = mockBitbucketClient();
        const config = { ...DEFAULT_CONFIG, p0_p1_strategy: "inline-only" };
        const result = await postReviewToBitbucket("test-fixture", "pr-1", config, {
            remoteUrl: "https://bitbucket.org/org/repo",
            mcpBaseUrl: "https://bitbucket.org",
            mcpConfigured: true,
...
describe("Unit: p2_strategy=none creates nothing for P2", () => {
    it("P2 finding creates no calls", async () => {
        const bb = mockBitbucketClient();
        const config = { ...DEFAULT_CONFIG, p2_strategy: "none" };
        const result = await postReviewToBitbucket("test-fixture", "pr-1", config, {
            remoteUrl: "https://bitbucket.org/org/repo",
            mcpBaseUrl: "https://bitbucket.org",
            mcpConfigured: true,
...
describe("Unit: set_review_status comment format", () => {
    it("comment contains P0=<int> P1=<int> run=<id>", async () => {
        const bb = mockBitbucketClient();
        await postReviewToBitbucket("test-fixture", "pr-1", DEFAULT_CONFIG, {
            remoteUrl: "https://bitbucket.org/org/repo",
            mcpBaseUrl: "https://bitbucket.org",
            mcpConfigured: true,
            runId: "run-42",
        }, bb, [P0_FINDING, P1_FINDING, P2_FINDING, P3_FINDING]);
