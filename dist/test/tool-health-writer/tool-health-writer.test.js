...
import { appendToolHealthRecord, formatToolHealthLine, ToolHealthLockTimeoutError, } from "../../src/tool-health-writer.js";
...
describe("ToolHealthWriter: lock contention", () => {
    it("times out when peer holds the lock past timeoutMs", () => {
        ...
        closeSync(fd);
        const t0 = Date.now();
        expect(() => appendToolHealthRecord(healthPath, {
            subcommand: "review",
            event: "429-degrade",
            details: "x=1",
        }, { timeoutMs: 80, sleepBaseMs: 5, staleLockMs: 60_000 })).toThrow(ToolHealthLockTimeoutError);
...
describe("ToolHealthWriter: 5-process concurrent append safety (R12.7)", () => {
    it("5 child processes × 4 records each → 20 complete distinct lines, no interleaving, prefix-monotone", async () => {
        ...
        for (let p = 0; p < PROCS; p++) {
            for (let i = 0; i < PER_PROC; i++)
                expected.add(`proc${p}#${i}`);
        }
        const seen = new Set(records.map((r) => r.split(" · ")[3]));
        expect(seen).toEqual(expected);
    }, 30_000);
});
//# sourceMappingURL=tool-health-writer.test.js.map