...
import { renderStatusSummary } from "../src/doctor.js";
...
describe("renderStatusSummary (branch coverage)", () => {
    ...
    it("renders enterprise profile", () => {
        expect(renderStatusSummary(baseSnapshot({ policyProfile: "enterprise" }))).toContain("Profile: enterprise");
    });
    ...
    it("renders fail branch status", () => {
        expect(renderStatusSummary(baseSnapshot({ branch: { status: "fail", message: "on main" } }))).toContain("on main");
    });
    it("renders warn worktree status", () => {
        expect(renderStatusSummary(baseSnapshot({ worktree: { status: "warn", message: "multiple wt" } }))).toContain("multiple wt");
    });
});
//# sourceMappingURL=coverage-batch8-branches.test.js.map