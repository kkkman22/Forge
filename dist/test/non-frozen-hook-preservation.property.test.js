...
const EXPECTED_USER_PROMPT_SUBMIT_HOOKS = [
    {
        hooks: [
            {
                type: "command",
                command: "if [ -f .forge/status.md ]; then echo '=== Forge Context ==='; head -50 .forge/plans/*.md 2>/dev/null; echo '=== Recent Progress ==='; tail -20 .forge/progress/*.md 2>/dev/null; fi",
            },
        ],
    },
];
const EXPECTED_POST_TOOL_USE_HOOKS = [
    {
        ...
        hooks: [
            {
                type: "command",
                command: "if [ -f .forge/status.md ]; then echo '📝 代码已修改。请记得更新 .forge/progress/ 中的任务状态。'; fi",
            },
        ],
    },
];
...
const EXPECTED_TEAMMATE_IDLE_HOOKS = [
    {
        hooks: [
            {
                type: "command",
                command: "phase=$(grep '^phase:' .forge/status.md 2>/dev/null | sed 's/phase: *\"\\{0,1\\}//;s/\"\\{0,1\\} *$//'); if [ \"$phase\" = 'review' ] || [ \"$phase\" = 'decide' ]; then echo '队友空闲。请检查是否所有评审/决策维度都已完成输出，未完成的队友应继续工作。'; fi",
            },
        ],
    },
];
...
function getNonFrozenPreToolUseHooks(config) {
    const preToolUseHooks = config.hooks.PreToolUse ?? [];
    return preToolUseHooks.filter((group) => !group.hooks.some((h) => h.command.includes("check-frozen.sh")));
}
// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------
