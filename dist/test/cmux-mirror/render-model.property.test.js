...
import { buildRenderModel, reviewVerdictColor, } from "../../scripts/cmux-mirror/lib/render-model.mjs";
...
describe("render-model: phase totality (R2.2)", () => {
    it("every legal phase maps to exactly one known icon", () => {
        for (const phase of VALID_PHASES) {
            ...
            const model = buildRenderModel({ phase });
            expect(model.phase_region.icon).toBe(PHASE_TO_ICON[phase]);
        }
    });
...
describe("render-model: property — build never throws on arbitrary snapshots (R2.4 fault tolerance)", () => {
    ...
    const arbSnapshot = fc.record({
        ...
        attention: fc.option(fc.array(fc.record({
            kind: fc.string(),
            topic: fc.string(),
    ...
    it("buildRenderModel is total over arbitrary snapshots", () => {
        fc.assert(fc.property(arbSnapshot, (snap) => {
            ...
            const model = buildRenderModel(snap);
...
//# sourceMappingURL=render-model.property.test.js.map