...
function mockExecSuccess(_bin, args, _opts, cb) {
    cb(null, `mock-stdout args=${JSON.stringify(args)}`, "");
}
describe("runCli --window injection", () => {
    beforeEach(() => {
        vi.clearAllMocks();
        mockExecFile.mockImplementation(mockExecSuccess);
        delete process.env.CMUX_WINDOW_ID;
    });
    it("never injects --window when CMUX_WINDOW_ID is absent", async () => {
        delete process.env.CMUX_WINDOW_ID;
        await runCli(["status"]);
        expect(mockExecFile).toHaveBeenCalledWith("cmux", ["status"], expect.any(Object), expect.any(Function));
    });
    ...
    it("injects --window prefix when CMUX_WINDOW_ID is valid", async () => {
        ...
        await runCli(["status"]);
        expect(mockExecFile).toHaveBeenCalledWith("cmux", ["--window", "win-abc123", "status"], expect.any(Object), expect.any(Function));
    });
    it("allows opts.windowId to override env", async () => {
        process.env.CMUX_WINDOW_ID = "env-value";
        await runCli(["status"], { windowId: "opt-value" });
        expect(mockExecFile).toHaveBeenCalledWith("cmux", ["--window", "opt-value", "status"], expect.any(Object), expect.any(Function));
    });
    ...
    it("accepts ids with dots, colons, hyphens", async () => {
        ...
        await runCli(["status"]);
        expect(mockExecFile).toHaveBeenCalledWith("cmux", ["--window", "win.abc:def-123", "status"], expect.any(Object), expect.any(Function));
    });
});
//# sourceMappingURL=cli.window.property.test.js.map