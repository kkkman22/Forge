...
import { buildConsoleArgs, buildErrorsArgs, buildScreenshotArgs, injectSurface, } from "../../scripts/cmux-mirror/lib/browser-q-actions.mjs";
    ...
    it("writes console/errors artifacts and passes the screenshot --out into the findings dir", async () => {
        ...
        mockedRunCli.mockImplementation(async (args) => {
            ...
            if (joined.includes("console"))
                return { exitCode: 0, stdout: "[log] ready", stderr: "" };
            if (joined.includes("errors"))
                return { exitCode: 0, stdout: "[error] none", stderr: "" };
        ...
        const shotCall = mockedRunCli.mock.calls.find((c) => c[0].includes("screenshot"));
        expect(shotCall?.[0]).toEqual(expect.arrayContaining(["screenshot", "--out", join(res.dir, "screenshot.png")]));
    });
...
//# sourceMappingURL=browser-q-actions.test.js.map