...
import { buildCommentSetHandoff, commentToFinding, findingToComment, normalizeSeverity, roundTrip, } from "../../scripts/cmux-mirror/lib/finding-comment-map.mjs";
const SEVERITIES = ["P0", "P1", "P2", "P3"];
const SOURCE_LAYERS = ["spec", "quality", "security", "human", "debug"];
...
describe("finding-comment-map: bidirectional mapping (R2.1)", () => {
    ...
    it("missing/null line falls back to 1 (R2.3 deterministic fallback)", () => {
        ...
        const c2 = findingToComment({ file: "a.ts", severity: "P2", message: "x" });
...
describe("finding-comment-map: round-trip property (R2.3 determinism, R2.4 no info loss)", () => {
    it("roundTrip is pure: same input → same output", () => {
        fc.assert(fc.property(arbFinding, (f) => {
            const a = roundTrip(f);
            const b = roundTrip(f);
            expect(a).toEqual(b);
        }));
    ...
    it("roundTrip applies the deterministic line fallback consistently (null line → 1 in comment form)", () => {
        fc.assert(fc.property(arbFinding, (f) => {
            ...
            expect(rt.line === null || rt.line === undefined || (typeof rt.line === "number" && rt.line > 0)).toBe(true);
            if (f.line === null) {
...
describe("finding-comment-map: buildCommentSetHandoff (R3.1, R3.2)", () => {
    it("produces a payload with topic/source/count/generated_at + ordered comments", () => {
        ...
        const payload = buildCommentSetHandoff(findings, { topic: "topic-a", generatedAt: "2026-06-13T00:00:00Z" });
...
//# sourceMappingURL=finding-comment-map.property.test.js.map