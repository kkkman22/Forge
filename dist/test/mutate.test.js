...
const mockFsExists = vi.fn((..._args) => false);
vi.mock("node:fs", () => ({
    mkdirSync: (...args) => mockFsMkdir(...args),
    writeFileSync: (...args) => mockFsWriteFile(...args),
    readFileSync: (...args) => mockFsReadFile(...args),
    appendFileSync: (...args) => mockFsAppendFile(...args),
...
import { collectMutationTargets, collectTargetGlobs, computeMutationScore, evaluateMutationVerdict, FIRST_PARTY_MUTATION_TARGET_GROUPS, generateStrykerConfig, parseMutationArgs, runMutation, } from "../src/mutate.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function makePackEntry(name, globs) {
    return {
        name,
        displayName: name,
...
function makePackEntryNoGlobs(name) {
    return {
        name,
        displayName: name,
        description: `${name} pack`,
...
function makeEnabledPacks(entries) {
    return {
...
describe("runMutation", () => {
    ...
    it("writes artifact with correct frontmatter", async () => {
        ...
        const artifactCall = mockFsWriteFile.mock.calls.find((call) => typeof call[1] === "string" && call[1].includes("mutationScore:"));
        expect(artifactCall).toBeDefined();
        const writtenContent = artifactCall?.[1];
