...
describe("routing-table renderer", () => {
    it("renders routing table", () => {
        const result = routingTableRenderer(baseInput([{ tier: "Light", condition: "≤1 file", sequence: ["build", "review"] }]));
...
describe("security-tiers renderer", () => {
    it("renders tier hierarchy", () => {
        const result = securityTiersRenderer(baseInput([{ level: 1, name: "Basic", capabilities: ["read"], constraints: ["no write"] }]));
    ...
    it("sorts by level ascending", () => {
        const result = securityTiersRenderer(baseInput([
...
describe("json-list renderer", () => {
    it("renders bullet list", () => {
        const result = jsonListRenderer(baseInput([
...
describe("renderer determinism (P14)", () => {
    ...
    it("json-list PBT: same input → byte-identical output", () => {
        ...
        fc.assert(fc.property(fc.array(entryArb, { maxLength: 20 }), (entries) => {
