...
describe("syncEmbeds", () => {
    ...
    it("replaces a single ssot-block with rendered content", () => {
        ...
        const { content: result, diagnostics } = syncEmbeds(content, P("docs/guide.md"), registryWithEcho(), emptySsotData());
    ...
    it("replaces multiple ssot-blocks independently", () => {
        ...
        const { content: result, diagnostics } = syncEmbeds(content, P("docs/multi.md"), registryWithEcho(), emptySsotData());
        expect(diagnostics.filter((d) => d.severity === "error")).toHaveLength(0);
        expect(result).toContain("<!-- rendered:commands -->");
    ...
    it("replaces #[[file:relative]] with file content from ssotData", () => {
        ...
        const { content: result, diagnostics } = syncEmbeds(content, P("docs/guide.md"), registryWithEcho(), ssotData);
        expect(diagnostics.filter((d) => d.severity === "error")).toHaveLength(0);
    ...
    it("reports error when #[[file:...]] content not found in ssotData", () => {
        ...
        const { diagnostics } = syncEmbeds(content, P("docs/guide.md"), registryWithEcho(), emptySsotData());
        expect(diagnostics.some((d) => d.severity === "error" && d.message.includes("missing.md"))).toBe(true);
    });
    ...
    it("reports diagnostic for unclosed ssot:begin and preserves content", () => {
        ...
        const { content: result, diagnostics } = syncEmbeds(content, P("docs/broken.md"), registryWithEcho(), emptySsotData());
    ...
    it("reports error for unknown renderer", () => {
        ...
        const { diagnostics } = syncEmbeds(content, P("docs/guide.md"), registryWithEcho(), emptySsotData());
        expect(diagnostics.some((d) => d.severity === "error" &&
            d.message.includes("nonexistent") &&
            d.code === "EMBED_UNKNOWN_RENDERER")).toBe(true);
    });
    ...
    it("returns content unchanged when no directives present", () => {
        const content = "# Plain\n\nNo embeds.\n";
        const { content: result, diagnostics } = syncEmbeds(content, P("docs/plain.md"), registryWithEcho(), emptySsotData());
        expect(result).toBe(content);
    ...
    it("handles mix of ssot-block and file-embed directives", () => {
        ...
        const { content: result, diagnostics } = syncEmbeds(content, P("docs/mixed.md"), registryWithEcho(), ssotData);
        expect(diagnostics.filter((d) => d.severity === "error")).toHaveLength(0);
    ...
    it("preserves original content on topic mismatch error", () => {
        ...
        const { content: result, diagnostics } = syncEmbeds(content, P("docs/mismatch.md"), registryWithEcho(), emptySsotData());
    ...
    it("preserves original content when nesting detected", () => {
        ...
        const { content: result, diagnostics } = syncEmbeds(content, P("docs/nested.md"), registryWithEcho(), emptySsotData());
    ...
    it("preserves begin and end marker lines in output", () => {
        ...
        const { content: result } = syncEmbeds(content, P("docs/markers.md"), registryWithEcho(), emptySsotData());
    ...
    it("replaces all occurrences of same topic", () => {
        ...
        const { content: result, diagnostics } = syncEmbeds(content, P("docs/dup.md"), registryWithEcho(), emptySsotData());
        expect(diagnostics.filter((d) => d.severity === "error")).toHaveLength(0);
    ...
    it("handles empty file content", () => {
        const { content: result, diagnostics } = syncEmbeds("", P("docs/empty.md"), registryWithEcho(), emptySsotData());
        expect(result).toBe("");
        expect(diagnostics).toHaveLength(0);
    });
    ...
    it("replaces #[[file:...]] at start of file", () => {
        ...
        const { content: result, diagnostics } = syncEmbeds(content, P("docs/top.md"), registryWithEcho(), ssotData);
        expect(diagnostics.filter((d) => d.severity === "error")).toHaveLength(0);
...
//# sourceMappingURL=embed-sync.test.js.map