...
describe("severityToExitCode", () => {
    ...
    it("PBT: exit code matches max severity", () => {
        ...
        fc.assert(fc.property(fc.array(fc.constantFrom(...severities)), (sevs) => {
            ...
            if (hasCritical)
                expect(code).toBe(ExitCode.CRITICAL);
            else if (hasError)
...
describe("formatDiagnostics", () => {
    it("sorts by severity desc then file asc", () => {
        const records = [
            { script: "a", severity: "warning", file: "b.md", message: "w" },
            { script: "b", severity: "error", file: "a.md", message: "e" },
            { script: "c", severity: "error", file: "b.md", message: "e2" },
        ];
