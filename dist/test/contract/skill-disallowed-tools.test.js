...
function getDisallowedTools(frontmatter) {
    // Match both array literal [A, B] and multiline list
    const match = frontmatter.match(/disallowedTools:\s*\[([^\]]*)\]|disallowedTools:\s*\n((?:\s*-\s+.*\n?)+)/);
    if (!match)
        return null;
...
function disallowedIncludes(tools, pattern) {
    if (!tools)
        return false;
...
describe("Contract: skill disallowed-tools matrix (R3)", () => {
    ...
    describe.each(AGENT_EXPECTATIONS)("$label agent", ({ file, label, mustInclude }) => {
        ...
        for (const tool of mustInclude) {
            it(`${label} disallows ${tool}`, () => {
                const frontmatter = parseFrontmatter(fullPath);
                const tools = getDisallowedTools(frontmatter);
                expect(disallowedIncludes(tools, tool), `${label} should disallow ${tool}, got: ${JSON.stringify(tools)}`).toBe(true);
            });
        }
    });
    ...
    describe("forge-learn skill instructions", () => {
        ...
        it("forge-learn disallows Bash(git push *)", () => {
            const frontmatter = parseFrontmatter(fullPath);
            const tools = getDisallowedTools(frontmatter);
            expect(disallowedIncludes(tools, "Bash(git push"), `forge-learn should disallow Bash(git push *), got: ${JSON.stringify(tools)}`).toBe(true);
        });
    });
    ...
    describe("forge-review git operation restrictions", () => {
        ...
        it("forge-review disallows Bash(git push *)", () => {
            const frontmatter = parseFrontmatter(fullPath);
            const tools = getDisallowedTools(frontmatter);
            expect(disallowedIncludes(tools, "Bash(git push"), "forge-review should disallow git push").toBe(true);
        });
        it("forge-review disallows Bash(git commit *)", () => {
            const frontmatter = parseFrontmatter(fullPath);
            const tools = getDisallowedTools(frontmatter);
            expect(disallowedIncludes(tools, "Bash(git commit"), "forge-review should disallow git commit").toBe(true);
        });
        it("forge-review disallows Bash(git reset *)", () => {
            const frontmatter = parseFrontmatter(fullPath);
            const tools = getDisallowedTools(frontmatter);
            expect(disallowedIncludes(tools, "Bash(git reset"), "forge-review should disallow git reset").toBe(true);
        });
    });
    ...
    describe("forge-plan git push restriction", () => {
        ...
        it("forge-plan disallows Bash(git push *)", () => {
            const frontmatter = parseFrontmatter(fullPath);
            const tools = getDisallowedTools(frontmatter);
            expect(disallowedIncludes(tools, "Bash(git push"), "forge-plan should disallow git push").toBe(true);
        });
    });
});
//# sourceMappingURL=skill-disallowed-tools.test.js.map