...
function setupForgeEnv(opts) {
    ...
    if (opts.statusCurrentTask) {
        writeFileSync(fixture(".forge", "status.md"), `---\ncurrent_task: "${opts.statusCurrentTask}"\nphase: "build"\n---`);
    }
    if (opts.progressContent && opts.statusCurrentTask) {
        writeFileSync(fixture(".forge", "progress", `${opts.statusCurrentTask}.md`), opts.progressContent);
    }
}
describe("PreCompact restate reminder", () => {
    ...
    it("snapshot stays under 10000 characters with max caps", () => {
        ...
        writeFileSync(fixture(".forge", "findings", "big-feature.md"), Array.from({ length: 40 }, (_, i) => `### Finding ${i + 1}: A moderately long finding description with details`).join("\n"));
        const result = runHook();
        expect(result.exitCode).toBe(0);
        const snapshot = readFileSync(fixture(".forge", ".compact-snapshot.md"), "utf-8");
...
//# sourceMappingURL=precompact-restate-reminder.test.js.map