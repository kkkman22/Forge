...
describe("FileChanged hook (R16)", () => {
    ...
    it("outputs systemMessage when spec-lock file changes", () => {
        ...
        try {
            ...
            // Write spec-lock
            writeFileSync(join(stateDir, "spec-lock"), JSON.stringify({ spec: "test-feature", locked: true }));
            const result = runHook(FILE_CHANGED_HOOK, {
    ...
    it("outputs systemMessage when active progress file changes", () => {
        ...
        try {
            // Write status.md with current_task
            writeFileSync(join(forgeDir, "status.md"), ["---", 'current_task: "active-feature"', "tier: standard", "---", "", "# Status"].join("\n"));
            // Write active progress file
            writeFileSync(join(progressDir, "active-feature.md"), "# Progress\n\n- [x] Task 1\n- [ ] Task 2\n");
            const result = runHook(FILE_CHANGED_HOOK, {
...
//# sourceMappingURL=cwd-changed-hook.test.js.map