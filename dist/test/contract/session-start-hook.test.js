...
describe("SessionStart hook (inject-evolved-rules.mjs)", () => {
    describe("additionalContext output", () => {
        it("outputs additionalContext when evolved-rules.md exists", async () => {
            ...
            try {
                ...
                const { stdout } = await runScript(tmpDir, mainInput({ cwd: tmpDir, session_id: "test-123" }));
        ...
        it("truncates additionalContext at 4KB boundary", async () => {
            ...
            try {
                ...
                const { stdout } = await runScript(tmpDir, mainInput({ cwd: tmpDir, session_id: "test-456" }));
                const json = JSON.parse(stdout.trim());
    ...
    describe("hookSpecificOutput", () => {
        it("outputs reloadSkills: true", async () => {
            ...
            try {
                ...
                const { stdout } = await runScript(tmpDir, mainInput({ cwd: tmpDir, session_id: "test-789" }));
                const json = JSON.parse(stdout.trim());
        ...
        it("outputs sessionTitle from spec directory name", async () => {
            ...
            try {
                ...
                const { stdout } = await runScript(tmpDir, mainInput({ cwd: tmpDir, session_id: "test-title" }));
                const json = JSON.parse(stdout.trim());
        ...
        it("outputs sessionTitle from spec-lock when multiple specs exist", async () => {
            ...
            try {
                ...
                const { stdout } = await runScript(tmpDir, mainInput({ cwd: tmpDir, session_id: "test-lock" }));
                const json = JSON.parse(stdout.trim());
        ...
        it("omits sessionTitle when no spec exists", async () => {
            ...
            try {
                makeEvolvedRules(tmpDir, "### R1: Test");
                const { stdout } = await runScript(tmpDir, mainInput({ cwd: tmpDir, session_id: "test-no-spec" }));
                const json = JSON.parse(stdout.trim());
                expect(json.hookSpecificOutput.reloadSkills).toBe(true);
                expect(json.hookSpecificOutput.sessionTitle === undefined ||
                    json.hookSpecificOutput.sessionTitle === "").toBe(true);
            }
            finally {
                rmSync(tmpDir, { recursive: true, force: true });
            }
    ...
    describe("failure / edge cases", () => {
        it("exits 0 with empty output when evolved-rules.md does not exist", async () => {
            const tmpDir = makeTmp();
            try {
                const { stdout, exitCode } = await runScript(tmpDir, mainInput({ cwd: tmpDir, session_id: "test-missing" }));
        ...
        it("exits 0 (not error) when evolved-rules.md is missing", async () => {
            const tmpDir = makeTmp();
            try {
                const { exitCode } = await runScript(tmpDir, mainInput({ cwd: tmpDir, session_id: "test-silent" }));
                expect(exitCode).toBe(0);
            }
            finally {
                rmSync(tmpDir, { recursive: true, force: true });
        ...
        it("exits 0 silently for subagent callers", async () => {
            ...
            try {
                ...
                const { stdout, exitCode } = await runScript(tmpDir, JSON.stringify({
                    cwd: tmpDir,
        ...
        it("completes within 500ms", async () => {
            ...
            try {
                ...
                const { elapsed } = await runScript(tmpDir, mainInput({ cwd: tmpDir, session_id: "test-perf" }));
        ...
        it("outputs valid JSON with all required fields", async () => {
            ...
            try {
                ...
                const { stdout } = await runScript(tmpDir, mainInput({ cwd: tmpDir, session_id: "test-json" }));
                const json = JSON.parse(stdout.trim());
...
//# sourceMappingURL=session-start-hook.test.js.map