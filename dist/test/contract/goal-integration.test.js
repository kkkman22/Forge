...
import { buildGoalCondition, clearGoal, setGoal, shouldClearGoal, } from "../../src/forge/goal-integration";
...
describe("goal-integration (R4)", () => {
    describe("buildGoalCondition", () => {
        ...
        it("returns correct string for full tier", () => {
            ...
            expect(result).toBe("完成 decide→spec→plan→build→review→test→ship→learn 流程，且无 P0/P1 阻断");
        });
    });
    describe("shouldClearGoal", () => {
        ...
        it("returns true when three-strike count >= 3", async () => {
            writeFileSync(THREE_STRIKE_PATH, JSON.stringify({ count: 3, lastFailure: "2026-05-28T00:00:00Z" }));
            expect(await shouldClearGoal(ROOT)).toBe(true);
        });
        it("returns true when three-strike count > 3", async () => {
            writeFileSync(THREE_STRIKE_PATH, JSON.stringify({ count: 5, lastFailure: "2026-05-28T00:00:00Z" }));
            expect(await shouldClearGoal(ROOT)).toBe(true);
        });
        it("returns false when count < 3", async () => {
            writeFileSync(THREE_STRIKE_PATH, JSON.stringify({ count: 2, lastFailure: "2026-05-28T00:00:00Z" }));
...
//# sourceMappingURL=goal-integration.test.js.map