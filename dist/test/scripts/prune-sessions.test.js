...
describe("prune-sessions.sh (spec: session-journal-retention)", () => {
    ...
    const run = (args = []) => execFileSync("bash", [SCRIPT, ...args], {
        cwd: workdir,
    ...
    const sessionPath = (name) => join(workdir, ".forge", "knowledge", "sessions", name);
    // -------------------------------------------------------------------------
    ...
    it("protects journals referenced by solutions/ source_session", () => {
        ...
        writeFileSync(join(solutionsDir, "some-solution.md"), "---\nsource_session: 2026-01-01-referenced.md\n---\n# body\n");
        run();
...
//# sourceMappingURL=prune-sessions.test.js.map