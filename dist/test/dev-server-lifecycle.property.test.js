...
import { buildStartCommand, isTimeoutElapsed, parseTerminalId, withDevServer, } from "../src/dev-server-lifecycle.js";
...
describe("withDevServer — unit", () => {
    it("calls stop even when work throws", async () => {
        ...
        await expect(withDevServer(async () => handle, async () => {
            stopped = true;
        }, async () => {
    ...
    it("returns work result when successful", async () => {
        const handle = { terminalId: "t1", port: 5173, projectRoot: "." };
        const result = await withDevServer(async () => handle, async () => { }, async () => 42);
        expect(result).toBe(42);
    });
});
//# sourceMappingURL=dev-server-lifecycle.property.test.js.map