...
function writeWorkerRuntimeAssets(root) {
...
describe("doctor health snapshot", () => {
    it("reports unknown instead of pass when status is missing", () => {
        ...
        expect(snapshot.nextStep.reasons).toContainEqual(expect.objectContaining({ code: "STATUS_UNKNOWN" }));
    });
    it("includes the full status explainability dimensions", () => {
        ...
        expect(snapshot.spec).toEqual(expect.objectContaining({
            status: "pass",
            message: "Spec status is locked",
            source: ".forge/specs/topic-a/requirements.md",
        }));
        expect(snapshot.plan).toEqual(expect.objectContaining({
            status: "pass",
    ...
    it("explains enterprise ship blockers from missing and stale artifacts", () => {
        ...
        expect(snapshot.nextStep.allowed).toBe(false);
        expect(snapshot.nextStep.reasons).toEqual(expect.arrayContaining([
    ...
    it("renders concise status with active profile and blocker reasons", () => {
        ...
        writeForgeFile(root, "status.md", '---\ncurrent_task: "topic-a"\ntier: "standard"\nphase: "test"\n---\n');
        writeForgeFile(root, "config.md", "policy_profile: enterprise\n");
        const rendered = renderStatusSummary(buildHealthSnapshot({ projectRoot: root, currentHead: "h" }));
...
//# sourceMappingURL=doctor-health.test.js.map