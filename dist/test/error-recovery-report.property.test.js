...
import { buildRecoveryReport, calculateSegmentation, } from "../src/error-recovery.js";
// ---------------------------------------------------------------------------
// Generators
// ---------------------------------------------------------------------------
const _gitCommitEntry = () => fc.record({
    hash: fc.string({ minLength: 7, maxLength: 7 }).map((s) => s.replace(/\s/g, "a")),
    message: fc.string({ minLength: 1, maxLength: 50 }),
    timestamp: fc.string({ minLength: 1, maxLength: 30 }),
});
const progressInconsistency = () => fc.record({
...
const phaseInconsistency = () => fc.record({
...
describe("Feature: error-recovery-strategy, Property 14: report completeness", () => {
    it("includes all inconsistencies with required fields, action options, and correct summary", () => {
        fc.assert(fc.property(header(), fc.array(progressInconsistency(), { minLength: 0, maxLength: 5 }), fc.oneof(fc.constant(null), phaseInconsistency()), fc.record({
            category: fc.constantFrom("task-completed-not-committed", "committed-not-progress-updated", "clean-state"),
            evidence: fc.string({ minLength: 1, maxLength: 50 }),
            tddPhase: fc.constant(null),
        }), fc.record({
            changes: fc.constant([]),
            relevantChanges: fc.constant([]),
            isClean: fc.boolean(),
        }), fc.constant([]), (hdr, progInc, phaseInc, classification, uncommitted, depGaps) => {
            const report = buildRecoveryReport(hdr, progInc, phaseInc, classification, uncommitted, depGaps);
            // Header preserved
...
describe("Feature: error-recovery-strategy, Property 15: task segmentation", () => {
    it("partitions tasks with no duplicates and consistent lastCompletedIndex", () => {
        fc.assert(fc.property(fc
            ...
            .map((ids) => [...new Set(ids)]), fc.array(fc.string({ minLength: 1, maxLength: 5 }), { minLength: 0, maxLength: 5 }), (planTaskIds, completedIds) => {
            ...
            const commitMatches = completed.map((id) => ({
                commit: {
                    hash: `hash${id}`,
                    message: `commit ${id}`,
