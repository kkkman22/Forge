...
import { isArtifactFreshForCommit, queryEvidenceArtifacts, validateArtifactBackedVerdict, validateEvidenceArtifact, writeEvidenceArtifact, } from "../src/evidence-artifact.js";
const tempRoots = [];
function tempRoot() {
...
describe("evidence artifact schema", () => {
    ...
    it("rejects unsupported kind and result values", () => {
        ...
        expect(diagnostics.map((d) => d.code)).toEqual(expect.arrayContaining(["INVALID_KIND", "INVALID_RESULT"]));
    });
    it("rejects invalid identifiers and missing required fields", () => {
        ...
        expect(validateEvidenceArtifact(artifact({ artifact_id: "", run_id: "" })).map((d) => d.code)).toEqual(expect.arrayContaining(["MISSING_ARTIFACT_ID", "MISSING_RUN_ID"]));
    });
});
describe("immutable artifact writer and index", () => {
    ...
    it("queries latest artifacts by topic, kind, and commit", () => {
        const root = tempRoot();
        writeEvidenceArtifact(root, artifact({ artifact_id: "old", created_at: "2026-06-09T01:00:00.000Z" }));
    ...
    it("ignores malformed index records and invalid artifact files", () => {
        ...
        writeFileSync(join(artifactDir, "invalid-artifact.json"), JSON.stringify({ ...artifact(), artifact_id: "" }), "utf-8");
        writeEvidenceArtifact(root, artifact({ artifact_id: "valid" }));
        writeFileSync(join(root, ".forge", "artifacts", "index.jsonl"), [
            "",
            "{",
    ...
    it("marks reviews stale when project files changed and tests stale when input hash differs", () => {
        ...
        expect(isArtifactFreshForCommit(artifact({ kind: "test", commit: "old", input_hash: "old-hash" }), "head", {
            inputHash: "new-hash",
        }).fresh).toBe(false);
    });
});
//# sourceMappingURL=evidence-artifact.test.js.map