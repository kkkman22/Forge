...
const createFsProbe = (existingDirs) => ({
    isDir: (path) => existingDirs.includes(path),
});
...
describe("Feature: plugin-init-experience, Property 1: Plugin priority", () => {
    it("For any input where pluginRoot non-empty + fsProbe says agents exists → result kind === plugin, regardless of other paths", () => {
        fc.assert(fc.property(safePath(), safePath(), safePath(), safePath(), (pluginRoot, scriptDir, homeDir, _extraPath) => {
            const input = {
                pluginRoot,
...
describe("Feature: plugin-init-experience, Property 2: Completeness", () => {
    it("For any input + fsProbe → kind ∈ {plugin,script-relative,global,not-found}, never throws", () => {
        fc.assert(fc.property(fc.option(safePath(), { nil: null }), safePath(), safePath(), fc.array(safePath(), { maxLength: 5 }), (pluginRoot, scriptDir, homeDir, existingDirs) => {
            const input = {
                pluginRoot,
                scriptDir: `${scriptDir}/scripts`,
                homeDir,
...
describe("Feature: plugin-init-experience, Property 3: Fallback order", () => {
    it("When pluginRoot missing agents → won't skip script-relative to go directly to global", () => {
        fc.assert(fc.property(safePath(), safePath(), safePath(), (pluginRoot, scriptDir, homeDir) => {
            const input = {
                pluginRoot,
                scriptDir: `${scriptDir}/scripts`,
                homeDir,
...
//# sourceMappingURL=forge-root-resolver.property.test.js.map