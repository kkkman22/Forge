...
import { computeFailSignature, derivePbtTasksFromUnchanged, triggerThreeStrikeReroute, } from "../src/spec-pbt-derivation.js";
function makeEars(when, shall, suffix = "") {
    return {
        line: 1,
        when,
        shall,
...
function makeBugfixBundle(unchanged) {
    const fm = {
        feature: "test-bugfix",
        status: "locked",
        date: "2026-05-23",
...
describe("derivePbtTasksFromUnchanged", () => {
    ...
    it("PBT: one task per unchanged clause (Property 11)", () => {
        fc.assert(fc.property(fc.array(fc.record({
            ...
            manual: fc.boolean(),
        }), { minLength: 1, maxLength: 15 }), (entries) => {
            const unchanged = entries.map((e, _i) => makeEars(e.when, e.shall, e.manual ? "[manual]" : ""));
...
describe("triggerThreeStrikeReroute", () => {
    it("does not reroute before 3 same-signature failures", () => {
        const failure = { testName: "test-a", firstLine: "Expected 1" };
