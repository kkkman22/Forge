...
import { createFrozenZoneHook, createFrozenZonePostToolUseHook, renderPostHocViolation, writeFrozenBreachRecord, } from "../src/frozen-zone-hook.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function makeTmpDir() {
...
function makeHookInput(toolName, filePath) {
    return {
...
describe("createFrozenZoneHook", () => {
    ...
    it("returns empty object for non-frozen-zone paths", async () => {
        const result = await hook(makeHookInput("Write", join(tmpDir, "src", "main.ts")), "tool-use-1", { signal: new AbortController().signal });
        expect(result).toEqual({});
    });
    it("returns empty object for non-Write/Edit tools", async () => {
        const result = await hook(makeHookInput("Read", join(tmpDir, ".forge", "specs", "test.md")), "tool-use-2", { signal: new AbortController().signal });
        expect(result).toEqual({});
    });
...
describe("createFrozenZonePostToolUseHook (R3 defence-in-depth)", () => {
    ...
    it("overwrites tool output with a revert prompt when a frozen-zone write slipped through (R3.1/R3.2)", async () => {
        ...
        }, "post-tool-1", { signal: new AbortController().signal });
        const output = result;
        expect(output.hookSpecificOutput?.hookEventName).toBe("PostToolUse");
        // R3.2: the marker prefix is required.
        expect(output.hookSpecificOutput?.updatedToolOutput).toContain("⚠ Post-hoc frozen-zone violation detected");
    ...
    it("writes a breach audit record to .forge/runs/<stamp>-frozen-breach.md (R3.3)", async () => {
        ...
        }, "post-tool-2", { signal: new AbortController().signal });
    ...
    it("returns empty for non-Write-class tools (R3.1 scoping)", async () => {
        ...
        }, "post-tool-3", { signal: new AbortController().signal });
        expect(result).toEqual({});
    });
    it("returns empty for a non-frozen file (no false positive)", async () => {
        ...
        const result = await hook({
            ...
            tool_name: "Write",
            tool_input: { file_path: specPath },
        }, "post-tool-4", { signal: new AbortController().signal });
        expect(result).toEqual({});
    });
});
...
//# sourceMappingURL=frozen-zone-hook.test.js.map