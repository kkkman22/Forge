...
import { completeTask, dispatchTasks, failTask, getDispatchBatch, getExecutionSummary, getReadyTasks, simulateParallelExecution, validateGraph, } from "../src/task-graph.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
const dagArb = fc
    ...
    return fc
        .array(fc.tuple(fc.integer({ min: 0, max: n - 1 }), fc.array(fc.integer({ min: 0, max: n - 1 }), { minLength: 0, maxLength: 3 })), { minLength: n, maxLength: n })
        .map((specs) => {
        const tasks = [];
        for (let i = 0; i < n; i++) {
...
describe("Property 26: Dispatch tasks", () => {
    it("dispatched tasks transition to in_progress", () => {
        ...
        const batch = getDispatchBatch(graph, 3);
        dispatchTasks(graph, batch.tasks.map((t) => t.id));
        for (const task of batch.tasks) {
...
describe("Property 26: 并行执行模拟", () => {
    ...
    it("N independent tasks take ceil(N/maxC) waves", () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 10 }), fc.integer({ min: 1, max: 5 }), (n, maxC) => {
...
//# sourceMappingURL=parallel-execution.property.test.js.map