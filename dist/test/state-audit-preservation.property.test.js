...
const frozenPathArb = fc.oneof(topicArb.map((t) => `specs/${t}/spec.md`), topicArb.map((t) => `plans/${t}.md`), fc.constant("config.md"));
...
const openPathArb = fc.oneof(fc.constant("status.md"), topicArb.map((t) => `decisions/${t}.md`), topicArb.map((t) => `findings/${t}.md`), topicArb.map((t) => `debug/${t}.md`), topicArb.map((t) => `knowledge/sessions/${t}.md`));
...
const blockingStatusArb = fc.constantFrom("locked", "approved");
/** Arbitrary status values that do NOT trigger blocking. */
const nonBlockingStatusArb = fc.constantFrom("draft", "in_progress", "new");
/** Arbitrary status values (any). */
const anyStatusArb = fc.constantFrom("draft", "locked", "approved", "in_progress", "new");
// ---------------------------------------------------------------------------
...
describe("Preservation: Open zone write permission behavior", () => {
    it("open zone + any status → { blocked: false, reason: '' }", () => {
        fc.assert(fc.property(openPathArb, anyStatusArb, (path, status) => {
            const result = checkWritePermission(path, contentWithStatus(status));
            expect(result.blocked).toBe(false);
            expect(result.reason).toBe("");
        }), { numRuns: 100 });
...
describe("Preservation: extractFrontmatterStatus() parsing", () => {
    it("correctly parses status from valid frontmatter", () => {
        fc.assert(fc.property(anyStatusArb, (status) => {
    ...
    it("returns null for content starting with non-frontmatter text", () => {
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 50 }).filter((s) => !s.trimStart().startsWith("---")), (content) => {
            expect(extractFrontmatterStatus(content)).toBeNull();
        }), { numRuns: 50 });
    });
});
//# sourceMappingURL=state-audit-preservation.property.test.js.map