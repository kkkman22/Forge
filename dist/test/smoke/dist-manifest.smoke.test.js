...
describe("dist-manifest.json contract tests (P3-3)", () => {
    ...
    it("manifest has all required keys", () => {
        ...
        for (const key of requiredKeys) {
            expect(manifest[key], `Missing key: ${key}`).toBeDefined();
            expect(Array.isArray(manifest[key]), `Key ${key} should be array`).toBe(true);
        }
    });
...
//# sourceMappingURL=dist-manifest.smoke.test.js.map