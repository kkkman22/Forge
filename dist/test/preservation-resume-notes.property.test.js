 ...
 */
function buildNotesContentWithEntries(runId, entries, branchName) {
    return branchName !== undefined
        ? formatNotesDocument({ runId, branchName, entries })
        : formatNotesDocument({ runId, entries });
}
...
describe("Preservation: Notes round-trip without branchName", () => {
    ...
    it("empty NotesDocument round-trips correctly", () => {
        fc.assert(fc.property(runIdArb, (runId) => {
            const doc = { runId, entries: [] };
            const markdown = formatNotesDocument(doc);
            const parsed = parseNotesDocument(markdown);
...
describe("Preservation: Single-run resume returns valid RunSetup", () => {
    ...
    it("single run directory returns valid RunSetup with correct fields", () => {
        fc.assert(fc.property(uuidRunIdArb, branchNameArb, (existingRunId, branchName) => {
            vi.clearAllMocks();
            execFileSync.mockReturnValue(Buffer.from(`${FAKE_SHA}\n`));
            randomUUID.mockReturnValue(FAKE_NEW_UUID);
    ...
    it("single run with entries returns correct lastIteration", () => {
        fc.assert(fc.property(uuidRunIdArb, branchNameArb, fc.array(roundTripEntryArb, { minLength: 1, maxLength: 5 }), (existingRunId, branchName, entries) => {
            vi.clearAllMocks();
            execFileSync.mockReturnValue(Buffer.from(`${FAKE_SHA}\n`));
            randomUUID.mockReturnValue(FAKE_NEW_UUID);
...
describe("Preservation: setupNewRun produces valid RunSetup", () => {
    ...
    it("setupNewRun returns RunSetup with all required fields", () => {
        fc.assert(fc.property(cleanLineArb, (objective) => {
            vi.clearAllMocks();
            randomUUID.mockReturnValue(FAKE_NEW_UUID);
            execFileSync.mockReturnValue(Buffer.from(`${FAKE_SHA}\n`));
    ...
    it("setupNewRun writes parseable notes document with correct runId", () => {
        fc.assert(fc.property(cleanLineArb, (objective) => {
            ...
            const writeCall = writeFileSync.mock.calls.find((call) => typeof call[0] === "string" && call[0].includes("notes.md"));
...
//# sourceMappingURL=preservation-resume-notes.property.test.js.map