...
function apiScenario(overrides = {}) {
    return {
        id: "api-login",
...
function cliScenario(overrides = {}) {
    return {
        id: "cli-build",
        given: "",
...
describe("apiRunner — real execution", () => {
    ...
    it("INCONCLUSIVE when exec rejects (curl crash)", async () => {
        execFileMock.mockImplementation((...rest) => {
            if (rest.length === 0)
                return;
            const cb = rest[rest.length - 1];
...
//# sourceMappingURL=accept-driver-exec.test.js.map