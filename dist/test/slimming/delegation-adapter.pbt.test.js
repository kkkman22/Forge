...
function choosePath(cmd, version) {
...
describe("Property 3: Delegation Adapter", () => {
    it("path selection is deterministic based on version", () => {
        const commands = ["recap", "resume", "abort", "learn", "review"];
        fc.assert(fc.property(fc.constantFrom(...commands), fc.stringMatching(/^\d+\.\d+\.\d+$/), (cmd, ver) => {
    ...
    it("standard path aborts on non-zero native exit code", () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 255 }), (exitCode) => {
    ...
    it("notice per-session dedup: max 1 per command per session", () => {
        fc.assert(fc.property(fc.array(fc.constantFrom("recap", "resume", "learn", "review"), {
            minLength: 1,
            maxLength: 20,
        }), fc.string({ minLength: 1 }), (invocations, sessionId) => {
            ...
            const uniqueLegacy = new Set(invocations.filter((c) => choosePath(c, "1.0.0") === "legacy"));
