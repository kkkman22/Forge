...
import { aggregateVerdicts, classifyPyramid, isE2eHeavy, renderAcceptanceReport, } from "../src/accept-driver.js";
function artifact(id, type, verdict) {
    return {
        scenarioId: id,
        source: "explicit",
        givenWhenThen: "g/w/t",
...
describe("aggregateVerdicts — property (counts invariant)", () => {
    function typedArtifactArb() {
        return fc.record({
            ...
            type: fc.constantFrom("unit", "component", "contract", "api", "ui", "cli", "mixed", "unknown"),
        });
    }
    it("flat counts sum equals total (unchanged)", () => {
        fc.assert(fc.property(fc.array(typedArtifactArb()), (arts) => {
            const r = aggregateVerdicts(arts);
    ...
    it("layer health pass+fail+inconclusive never exceeds total per layer", () => {
        fc.assert(fc.property(fc.array(typedArtifactArb()), (arts) => {
            const r = aggregateVerdicts(arts);
            for (const layer of ["unit", "component", "contract", "e2e"]) {
    ...
    it("pyramidShape is one of the 5 documented values", () => {
        fc.assert(fc.property(fc.array(typedArtifactArb()), (arts) => {
            const r = aggregateVerdicts(arts);
            expect(["healthy", "e2e-heavy", "empty-middle", "no-unit", "empty"]).toContain(r.pyramidShape);
        }));
    });
});
...
describe("isE2eHeavy — shared pure fn (Req5/Req7)", () => {
    const config = { e2eRatioThreshold: 0.3, strictPyramid: true };
    ...
    it("strictPyramid false → always false (advisory mode)", () => {
        expect(isE2eHeavy([
            ...
            { type: "api", tags: [] },
            { type: "api", tags: [] },
        ], { ...config, strictPyramid: false })).toBe(false);
    });
    it("e2eRatioThreshold 0 → always false (gate disabled)", () => {
        expect(isE2eHeavy([
            ...
            { type: "api", tags: [] },
            { type: "api", tags: [] },
        ], { ...config, e2eRatioThreshold: 0 })).toBe(false);
    });
...
//# sourceMappingURL=aggregate-verdicts.property.test.js.map