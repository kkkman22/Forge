...
function createSoftFailureResult() {
    return {
        output: {
            success: false,
...
describe("initialization", () => {
    ...
    it("empty objective throws validation error", () => {
        const executor = createMockEffectExecutor();
        const agent = createMockAgent();
        expect(() => new SdkDriver(createConfig({ objective: "" }), executor, agent)).toThrow("Objective must be a non-empty string");
    });
...
describe("successful iteration", () => {
    it("dispatches iteration_success, commits, and schedules next iteration", async () => {
        ...
        const prompt = agent.run.mock.calls[0][0];
    ...
    it("persists notes after each iteration", async () => {
        ...
        expect(RunManager.persistNotes).toHaveBeenCalledWith(config.notesPath, expect.stringContaining("did stuff"));
    });
    it("logs token usage after each iteration", async () => {
        ...
        const hasTokenLog = logMessages.some((msg) => msg.includes("driver.loop.iterationTokens"));
...
describe("notes persistence", () => {
    it("persistNotes called after each iteration", async () => {
        ...
        for (const call of RunManager.persistNotes.mock.calls) {
...
describe("token usage logging", () => {
    ...
    it("logs cumulative token totals across multiple iterations", async () => {
        ...
        const logMessages = consoleSpy.mock.calls.map((call) => call[0]);
        const tokenLogs = logMessages.filter((msg) => msg.includes("driver.loop.iterationTokens"));
...
describe("empty objective validation", () => {
    ...
    it("accepts non-empty objective", () => {
        const executor = createMockEffectExecutor();
        const agent = createMockAgent();
        expect(() => new SdkDriver(createConfig({ objective: "valid objective" }), executor, agent)).not.toThrow();
    });
});
// ---------------------------------------------------------------------------
...
describe("hooks validation during run()", () => {
    it("throws HooksProtectionMissingError when hooks are absent", async () => {
        ...
        const executor = createMockEffectExecutor();
        const agent = createMockAgent();
        const driver = new SdkDriver(createConfig({ cwd: "/nonexistent/path", forceNoHooks: false }), executor, agent);
    ...
    it("allows startup with forceNoHooks=true when hooks are absent", async () => {
        ...
        const executor = createMockEffectExecutor();
        const agent = createMockAgent();
        const driver = new SdkDriver(createConfig({
    ...
    it("logs warning when hooks are bypassed with forceNoHooks", async () => {
        ...
        const agent = createMockAgent();
        const driver = new SdkDriver(createConfig({ cwd: "/nonexistent/path", forceNoHooks: true }), executor, agent);
        await driver.run();
        const allMessages = [
            ...logSpy.mock.calls.map((call) => call[0]),
            ...errSpy.mock.calls.map((call) => call[0]),
        ];
        const hasHooksWarning = allMessages.some((msg) => msg.includes("hooks_protection_bypassed"));
        expect(hasHooksWarning).toBe(true);
        errSpy.mockRestore();
    });
});
//# sourceMappingURL=sdk-driver.test.js.map