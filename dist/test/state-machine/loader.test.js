...
`;
    ...
    it("includes filePath in error messages when provided", () => {
        expect(() => loadStateMachineDefinition("not: yaml", "reservation.yaml")).toThrow(/reservation\.yaml/);
    });
...
`;
        ...
        expect(thrown.message).toMatch(/transitions/);
    });
...
`;
        ...
        expect(thrown).toBeInstanceOf(Error);
        expect(thrown).not.toBeInstanceOf(TypeError);
        expect(thrown.message).toMatch(/invariants/);
    });
});
