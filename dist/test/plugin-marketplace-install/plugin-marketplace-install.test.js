...
describe("Plugin Marketplace Install (AC 13.3)", () => {
    ...
    beforeAll(() => {
        ...
        if (!forgePlugin)
            throw new Error("marketplace.json missing 'forge' plugin entry");
        ...
        for (const d of includeDirs) {
            ...
            if (existsSync(src))
                cpSync(src, join(installDir, d), { recursive: true });
        }
    ...
    it("installed plugin.json exists and is parseable", () => {
        ...
        const manifest = JSON.parse(readFileSync(installedPluginPath, "utf-8"));
    ...
    it("each workflows[] path resolves to a directory under the install root", () => {
        ...
        for (const entry of manifest.workflows ?? []) {
            ...
            expect(statSync(abs).isDirectory(), `workflows entry '${entry}' is not a directory`).toBe(true);
        }
    });
...
//# sourceMappingURL=plugin-marketplace-install.test.js.map