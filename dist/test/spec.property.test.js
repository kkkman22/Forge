...
/** Purpose text. */
const purposeArb = nonEmptyStringArb;
/** Non-empty list of exclusion strings. */
const exclusionsArb = fc.array(nonEmptyStringArb, {
    minLength: 0,
    maxLength: 5,
});
// ---------------------------------------------------------------------------
...
const validRequirementArb = fc
    .tuple(nonEmptyStringArb, nonEmptyStringArb, fc.array(validScenarioArb, { minLength: 1, maxLength: 4 }))
    .map(([title, description, scenarios]) => ({ title, description, scenarios }));
/** A complete draft SpecDocument (non-brownfield for simplicity). */
const draftSpecArb = fc
...
const testableRequirementArb = validRequirementArb;
/** A scenario in "当...则..." format WITHOUT a verifiable result keyword. */
const untestableScenarioArb = fc
...
const untestableRequirementArb = fc
    .tuple(nonEmptyStringArb, nonEmptyStringArb, fc.array(untestableScenarioArb, { minLength: 1, maxLength: 3 }))
    .map(([title, description, scenarios]) => ({ title, description, scenarios }));
/** A non-empty list of requirements where ALL have valid scenarios. */
const allTestableRequirementsArb = fc.array(testableRequirementArb, {
    minLength: 1,
    maxLength: 5,
});
/**
 ...
 */
const someUntestableRequirementsArb = fc
    .tuple(fc.array(testableRequirementArb, { minLength: 0, maxLength: 3 }), untestableRequirementArb, fc.array(testableRequirementArb, { minLength: 0, maxLength: 3 }))
...
const deltaEntriesArb = fc.array(nonEmptyStringArb, {
    minLength: 1,
    maxLength: 5,
});
/** A valid Delta section with all three non-empty subsections. */
const validDeltaArb = fc
...
const brownfieldSpecWithDeltaArb = fc
    .tuple(draftFrontmatterArb, purposeArb, fc.array(validRequirementArb, { minLength: 1, maxLength: 5 }), exclusionsArb, validDeltaArb)
...
const brownfieldSpecWithoutDeltaArb = fc
    .tuple(draftFrontmatterArb, purposeArb, fc.array(validRequirementArb, { minLength: 1, maxLength: 5 }), exclusionsArb)
    .map(([frontmatter, purpose, requirements, exclusions]) => ({
    frontmatter,
    purpose,
 ...
 */
const incompleteDeltaArb = fc.oneof(
// added is empty
fc
    .tuple(fc.constant([]), deltaEntriesArb, deltaEntriesArb)
    .map(([added, modified, unchanged]) => ({ added, modified, unchanged })), 
// modified is empty
fc
    .tuple(deltaEntriesArb, fc.constant([]), deltaEntriesArb)
    .map(([added, modified, unchanged]) => ({ added, modified, unchanged })), 
// unchanged is empty
fc
    .tuple(deltaEntriesArb, deltaEntriesArb, fc.constant([]))
    .map(([added, modified, unchanged]) => ({ added, modified, unchanged })));
/** A brownfield SpecDocument with an incomplete Delta section. */
const brownfieldSpecIncompleteDeltaArb = fc
...
const greenfieldSpecArb = draftSpecArb;
// ---------------------------------------------------------------------------
// Property 5: Spec 锁定状态转换
// ---------------------------------------------------------------------------
...
describe("Property 7: 棕地 Spec 包含 Delta 章节", () => {
    it("brownfield spec with complete Delta → validation passes (Req 3.6)", () => {
        fc.assert(fc.property(brownfieldSpecWithDeltaArb, (spec) => {
