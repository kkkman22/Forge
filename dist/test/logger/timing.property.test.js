...
describe("Property 3: Iteration timing invariant", () => {
    it("should satisfy total >= agentCall and total >= effectExecution", async () => {
        ...
        fc.assert(fc.property(fc.integer({ min: 0, max: 1_000_000 }), fc.integer({ min: 0, max: 1_000_000 }), fc.integer({ min: 0, max: 1_000_000 }), (startOffset, agentDelta, effectDelta) => {
...
describe("Property 4: Performance baseline statistics correctness", () => {
    it("should satisfy avg = arithmetic mean and max >= min", async () => {
        ...
        fc.assert(fc.property(fc.array(timingArb, { minLength: 1, maxLength: 100 }), (timings) => {
...
describe("Feature: observability-enhancements, Property 7: SubagentTiming 耗时不变量", () => {
    ...
    it("should satisfy durationMs === endMs - startMs when endMs >= startMs", async () => {
        ...
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 50 }), fc.integer({ min: 0, max: 1_000_000_000 }), fc.integer({ min: 0, max: 1_000_000_000 }), (subagentId, startMs, delta) => {
    ...
    it("should clamp durationMs to 0 when endMs < startMs (clock skew)", async () => {
        const { buildSubagentTiming } = await import("../../src/logger/timing.js");
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 50 }), fc.integer({ min: 1, max: 1_000_000_000 }), fc.integer({ min: 1, max: 1_000_000_000 }), (subagentId, startMs, gap) => {
...
describe("Feature: observability-enhancements, Property 9: 退化检测正确性", () => {
    ...
    const iterationTimingArb = fc
        .tuple(fc.integer({ min: 0, max: 1_000_000 }), fc.integer({ min: 1, max: 1_000_000 }), fc.integer({ min: 1, max: 1_000_000 }))
    ...
    it("should return isDegraded: true when currentMs > 2 × rollingAvg (history >= 3)", async () => {
        ...
        fc.assert(fc.property(fc.array(iterationTimingArb, { minLength: 3, maxLength: 50 }), (history) => {
    ...
    it("should return isDegraded: false when currentMs <= 2 × rollingAvg (history >= 3)", async () => {
        const { detectDegradation } = await import("../../src/logger/timing.js");
        fc.assert(fc.property(fc.array(iterationTimingArb, { minLength: 3, maxLength: 50 }), fc.double({ min: 0, max: 1, noNaN: true }), (history, fraction) => {
            const sum = history.reduce((acc, t) => acc + t.totalIterationDurationMs, 0);
            const rollingAvg = sum / history.length;
    ...
    it("should always return isDegraded: false when history has < 3 records", async () => {
        const { detectDegradation } = await import("../../src/logger/timing.js");
        fc.assert(fc.property(fc.array(iterationTimingArb, { minLength: 0, maxLength: 2 }), fc.integer({ min: 0, max: 10_000_000 }), (history, currentMs) => {
            const result = detectDegradation(currentMs, history);
            expect(result.isDegraded).toBe(false);
            expect(result.currentMs).toBe(currentMs);
...
describe("Feature: observability-enhancements, Property 2: SubagentTiming JSON 往返一致性", () => {
    ...
    it("should round-trip SubagentTiming through JSON serialization", async () => {
        const { buildSubagentTiming } = await import("../../src/logger/timing.js");
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 100 }), fc.integer({ min: 0, max: 1_000_000_000 }), fc.integer({ min: 0, max: 1_000_000_000 }), (subagentId, startMs, endMs) => {
            const timing = buildSubagentTiming(subagentId, startMs, endMs);
...
describe("Feature: observability-enhancements, Property 3: 扩展 PerformanceBaseline JSON 往返一致性", () => {
    ...
    it("should round-trip extended PerformanceBaseline through JSON serialization", () => {
        fc.assert(fc.property(fc.integer({ min: 0, max: 1_000_000_000 }), fc.integer({ min: 1, max: 1000 }), fc.integer({ min: 0, max: 1_000_000 }), fc.integer({ min: 0, max: 1_000_000 }), fc.integer({ min: 0, max: 1_000_000 }), fc.integer({ min: 0, max: 1_000_000 }), fc.integer({ min: 0, max: 1000 }), fc.integer({ min: 0, max: 1_000_000 }), fc.integer({ min: 0, max: 1_000_000 }), fc.integer({ min: 0, max: 100 }), (totalRunTimeMs, iterationCount, avgIterationMs, maxIterationMs, minIterationMs, avgAgentCallMs, subagentCallCount, avgSubagentMs, maxSubagentMs, degradationCount) => {
            const baseline = {
                totalRunTimeMs,
                iterationCount,
                avgIterationMs,
...
describe("Feature: observability-enhancements, Property 8: 扩展 PerformanceBaseline 统计正确性", () => {
    ...
    const subagentTimingArb = fc
        .tuple(fc.string({ minLength: 1, maxLength: 50 }), fc.integer({ min: 0, max: 1_000_000_000 }), fc.integer({ min: 0, max: 1_000_000 }))
    ...
    it("should compute correct subagent statistics from non-empty SubagentTiming arrays", async () => {
        ...
        fc.assert(fc.property(fc.array(iterationTimingArb, { minLength: 1, maxLength: 50 }), fc.array(subagentTimingArb, { minLength: 1, maxLength: 50 }), fc.integer({ min: 0, max: 100 }), (timings, subagentTimings, degradationCount) => {
...
describe("Feature: observability-enhancements, Property 10: 格式化 Baseline 包含 Subagent 统计", () => {
    ...
    it("should include Subagent keyword and call count in formatted output", async () => {
        ...
        fc.assert(fc.property(fc.integer({ min: 0, max: 1_000_000_000 }), fc.integer({ min: 1, max: 1000 }), fc.integer({ min: 1, max: 1_000_000 }), fc.integer({ min: 1, max: 1_000_000 }), fc.integer({ min: 1, max: 1_000_000 }), fc.integer({ min: 1, max: 1_000_000 }), fc.integer({ min: 1, max: 1000 }), fc.integer({ min: 0, max: 1_000_000 }), fc.integer({ min: 0, max: 1_000_000 }), fc.integer({ min: 0, max: 100 }), (totalRunTimeMs, iterationCount, avgIterationMs, maxIterationMs, minIterationMs, avgAgentCallMs, subagentCallCount, avgSubagentMs, maxSubagentMs, degradationCount) => {
            const baseline = {
                totalRunTimeMs,
                iterationCount,
                avgIterationMs,
