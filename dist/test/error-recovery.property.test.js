...
import { buildReconciliationPatch, extractCommitPatterns, findDependencyGaps, findPhaseInconsistencies, findProgressInconsistencies, getNextPhase, getPhaseSequence, matchChangesToTask, matchCommitsToTasks, PHASE_SEQUENCES, parseGitStatus, } from "../src/error-recovery.js";
// ---------------------------------------------------------------------------
// Generators
// ---------------------------------------------------------------------------
const gitHash = () => fc
...
const gitCommitEntry = () => fc.record({
    hash: gitHash(),
    message: fc.string({ minLength: 1, maxLength: 100 }),
    timestamp: fc.string({ minLength: 1, maxLength: 30 }),
});
const _taskCommitPattern = () => fc.record({
...
const forgeTier = () => fc.constantFrom("lightweight", "standard", "full");
const forgePhase = () => fc.constantFrom("decide", "spec", "plan", "build", "review", "test", "ship", "learn");
// ---------------------------------------------------------------------------
...
describe("Feature: error-recovery-strategy, Property 1: commit pattern extraction", () => {
    it("extracts patterns for every task with a commit message prefix", () => {
        ...
        }), { minLength: 1, maxLength: 10 }), (tasks) => {
...
describe("Feature: error-recovery-strategy, Property 3: git status parsing", () => {
    it("returns correct file paths and statuses with matching count", () => {
        fc.assert(fc.property(fc.array(fc.record({
            status: fc.constantFrom("M ", "A ", "D ", "??"),
            file: fc
                .string({ minLength: 1, maxLength: 30 })
                .map((s) => `src/${s.replace(/\s/g, "_")}.ts`),
        }), { minLength: 0, maxLength: 20 }), (entries) => {
...
describe("Feature: error-recovery-strategy, Property 4: file change relevance", () => {
    it("returns exactly overlapping changes", () => {
        fc.assert(fc.property(fc.array(fileChange(), { minLength: 0, maxLength: 15 }), fc.array(fc.string({ minLength: 1, maxLength: 30 }), { minLength: 0, maxLength: 10 }), (changes, taskPaths) => {
            const relevant = matchChangesToTask(changes, taskPaths);
            // All relevant changes must overlap with task paths
            for (const c of relevant) {
                const overlaps = taskPaths.some((tp) => tp === c.filePath ||
...
describe("Feature: error-recovery-strategy, Property 5: progress inconsistency detection", () => {
    it("flags exactly committed-but-not-marked tasks with full commit details", () => {
        fc.assert(fc.property(fc.array(gitCommitEntry(), { minLength: 1, maxLength: 10 }), fc.boolean(), fc.integer({ min: 1, max: 5 }), (commits, markCompleted, taskIdNum) => {
            const taskId = String(taskIdNum);
            const matches = commits.map((c) => ({
                commit: c,
                taskId,
                taskTitle: "Test Task",
                confidence: "exact",
...
describe("Feature: error-recovery-strategy, Property 6: reconciliation patch ordering", () => {
    it("produces patches ordered by task order", () => {
        fc.assert(fc.property(fc
            ...
            .map((ids) => [...new Set(ids)]), (taskOrder) => {
            // Create shuffled inconsistencies for a subset
            const inconsistencies = taskOrder.map((id, i) => ({
                taskId: id,
                taskTitle: `Task ${id}`,
                commitHash: `abc${i}`,
...
describe("Feature: error-recovery-strategy, Property 7: dependency gap detection", () => {
    it("identifies missing dependencies in task chains", () => {
        fc.assert(fc.property(fc
            ...
            .filter(({ id1, id2 }) => id1 !== id2), ({ id1, id2 }) => {
...
describe("Feature: error-recovery-strategy, Property 8: phase inconsistency detection", () => {
    it("detects behind, ahead, or consistent state", () => {
        fc.assert(fc.property(fc.boolean(), forgeTier(), forgePhase(), (allCompleted, tier, phase) => {
...
describe("Feature: error-recovery-strategy, Property 9: next phase computation", () => {
    it("returns correct next phase or null", () => {
        const tiers = ["lightweight", "standard", "full"];
...
//# sourceMappingURL=error-recovery.property.test.js.map