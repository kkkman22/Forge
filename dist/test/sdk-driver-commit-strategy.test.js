 ...
 */
function getAllExecutedEffects(executor) {
    const allEffects = [];
    for (const call of executor.executeEffects.mock.calls) {
        const effects = call[0];
 ...
 */
function findCommitEffects(executor) {
    return getAllExecutedEffects(executor).filter((e) => e.type === "commit");
}
/**
 * Find rollback effects from all executed effects.
 */
function findRollbackEffects(executor) {
    return getAllExecutedEffects(executor).filter((e) => e.type === "rollback");
}
// ---------------------------------------------------------------------------
// Setup / Teardown
// ---------------------------------------------------------------------------
...
describe("commit failure triggers hard failure event", () => {
    ...
    it("records commit failure in notes as effect execution failure", async () => {
        ...
        const failureEntry = result.notesDocument.entries.find((e) => e.summary.includes("Effect execution failed"));
...
//# sourceMappingURL=sdk-driver-commit-strategy.test.js.map