...
describe("runSubagentsWithConcurrency property tests", () => {
    ...
    it("complete coverage: all invocations end up in succeeded or failed", async () => {
        await fc.assert(fc.asyncProperty(fc.integer({ min: 1, max: 8 }), fc.integer({ min: 1, max: 5 }), fc.array(fc.boolean(), { minLength: 1, maxLength: 8 }), async (concurrency, _delay, shouldFail) => {
    ...
    it("API equivalence: concurrency=N output multiset equals runSubagentsInParallel", async () => {
        await fc.assert(fc.asyncProperty(fc.integer({ min: 1, max: 6 }), fc.array(fc.boolean(), { minLength: 1, maxLength: 6 }), async (_seed, shouldFail) => {
            const invocations = makeInvocations(shouldFail.length);
            const makeExecutor = () => async (inv) => {
                const idx = parseInt(inv.agentType.split("-")[1], 10);
                if (shouldFail[idx]) {
            ...
            const sort = (arr) => [...arr].sort((a, b) => a.agentType.localeCompare(b.agentType));
            expect(sort(bounded.succeeded).map((s) => s.agentType)).toEqual(sort(parallel.succeeded).map((s) => s.agentType));
            expect(sort(bounded.failed).map((f) => f.agentType)).toEqual(sort(parallel.failed).map((f) => f.agentType));
        }), { numRuns: 200 });
    });
    it("failure isolation: K rejections don't affect N-K successes", async () => {
        await fc.assert(fc.asyncProperty(fc.integer({ min: 1, max: 4 }), fc.array(fc.boolean(), { minLength: 2, maxLength: 6 }), async (concurrency, shouldThrow) => {
...
//# sourceMappingURL=subagent-concurrency-runner.property.test.js.map