...
describe("runSubagentsWithConcurrency", () => {
    it("concurrency >= N behaves like runSubagentsInParallel", async () => {
        ...
        const extract = (r) => [...r.succeeded.map((s) => s.agentType)].sort();
    ...
    it("throws on concurrency=0", async () => {
        await expect(runSubagentsWithConcurrency([], async () => ({ agentType: "x", status: "success", output: "" }), 0)).rejects.toThrow("concurrency must be >= 1");
    });
    it("throws on concurrency=-1", async () => {
        await expect(runSubagentsWithConcurrency([], async () => ({ agentType: "x", status: "success", output: "" }), -1)).rejects.toThrow("concurrency must be >= 1");
    });
    it("throws on concurrency=101", async () => {
        await expect(runSubagentsWithConcurrency([], async () => ({ agentType: "x", status: "success", output: "" }), 101)).rejects.toThrow("concurrency must be <= 100");
    });
...
//# sourceMappingURL=subagent-concurrency-runner.test.js.map