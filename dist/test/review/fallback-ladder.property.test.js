...
const ARBITRARY_AGENT_TYPES = ["spec-check", "quality-check", "security-check"];
function makeInvocation(agentType) {
    return { agentType, prompt: `Review ${agentType}`, permissionMode: "default", maxTurns: 10 };
}
const failureErrorArb = fc.constantFrom("No task found with ID: abc-123", "Error: timeout exceeded", "Error: turn limit reached", "Error: unknown failure");
...
describe("fallback-ladder property tests", () => {
    ...
    it("retry never exceeds 1 — runSubagentsWithConcurrency called at most twice", async () => {
        await fc.assert(fc.asyncProperty(fc.array(subagentResultArb, { minLength: 1, maxLength: 6 }), async (results) => {
            vi.clearAllMocks();
            let callCount = 0;
            runSubagentsWithConcurrency.mockImplementation((_invs, _exec, _concurrency) => {
                callCount++;
    ...
    it("methodology and trace are consistent", async () => {
        await fc.assert(fc.asyncProperty(fc.record({
            ...
            const invocations = ARBITRARY_AGENT_TYPES.map(makeInvocation);
            let callCount = 0;
            runSubagentsWithConcurrency.mockImplementation((invs, _exec, _concurrency) => {
                callCount++;
                if (callCount === 1) {
                    if (l0AllFail) {
                        return {
    ...
    it("failure signature is populated when L0 fails", async () => {
        await fc.assert(fc.asyncProperty(failureErrorArb, async (errorMsg) => {
            vi.clearAllMocks();
            const invocations = ARBITRARY_AGENT_TYPES.map(makeInvocation);
            runSubagentsWithConcurrency.mockResolvedValue({
                succeeded: [],
...
//# sourceMappingURL=fallback-ladder.property.test.js.map