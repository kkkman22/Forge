...
function makeSuccessResult(agentType) {
    return {
        agentType,
        status: "success",
...
function makeFailureResult(agentType, error) {
    return {
        agentType,
        status: "failure",
        error,
...
describe("runReviewFallbackLadder", () => {
    ...
    it("L0 all-fail triggers L1 with concurrency=1", async () => {
        ...
        let callCount = 0;
        mockedRunner.mockImplementation((invs, _exec, concurrency) => {
...
`);
    ...
    it("L1 retry produces visible status output (mock console.warn)", async () => {
        ...
        expect(mockConsoleWarn).toHaveBeenCalledWith(expect.stringContaining("L0 subagent dispatch failed"));
        expect(mockConsoleWarn).toHaveBeenCalledWith(expect.stringContaining("retrying with concurrency=1"));
