...
import { COMPACT_SAFE_ENABLED_LAYERS, compactSafeDedup, DEFAULT_COMPACT_SAFE_THRESHOLD, decideCompactSafe, filterToCompactSafeLayers, formatCompactSafeFinding, renderCompactSafeBanner, } from "../../src/review/compact-safe.js";
...
describe("filterToCompactSafeLayers (R10.2 — keep spec + security only)", () => {
    it("keeps spec-check + security-check, drops quality + adversarial", () => {
        ...
        expect(kept.every((f) => COMPACT_SAFE_ENABLED_LAYERS.includes(f.reviewer))).toBe(true);
...
//# sourceMappingURL=compact-safe.test.js.map