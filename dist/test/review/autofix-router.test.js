...
import { classifyFixRoute, routeForAutofix, } from "../../src/review/autofix-router.js";
...
describe("classifyFixRoute (ce-inspired R9.1)", () => {
    it("naming/missing-import/trivial-null-check → safe_auto (deterministic local fixes)", () => {
        for (const category of ["naming", "missing-import", "trivial-null-check"]) {
    ...
    it("unknown/other category → manual (conservative fallback)", () => {
        expect(classifyFixRoute("other", "P2", true)).toBe("manual");
    });
});
describe("routeForAutofix (ce-inspired R9.2/R9.3/R9.4)", () => {
    it("splits findings into autoApply (safe) / gated / excluded", () => {
        ...
        expect(routing.excluded.every((f) => f.fixRoute === "manual" || f.fixRoute === "advisory")).toBe(true);
    });
...
//# sourceMappingURL=autofix-router.test.js.map