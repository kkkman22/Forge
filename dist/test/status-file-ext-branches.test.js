...
import { clearExecutionMetadata, collectExecutionMetadataFromEnv, extractExecutionMetadata, extractPuaFields, writeExecutionMetadata, writePuaFields, } from "../src/status-file-ext.js";
const FM = (fields) => `---\n${fields.join("\n")}\n---\n\n# Status\n`;
describe("extractExecutionMetadata (branch coverage)", () => {
    ...
    it("extracts claude_version", () => {
        expect(extractExecutionMetadata(FM(['execution_claude_version: "2.1.169"'])).claude_version).toBe("2.1.169");
    });
    it("extracts valid dispatch_mode", () => {
        for (const mode of ["inline", "agents", "auto"]) {
            expect(extractExecutionMetadata(FM([`execution_dispatch_mode: "${mode}"`])).dispatch_mode).toBe(mode);
        }
    });
    it("drops invalid dispatch_mode", () => {
        expect(extractExecutionMetadata(FM(['execution_dispatch_mode: "bogus"'])).dispatch_mode).toBeUndefined();
    });
    it("extracts diagnostic_mode true/false", () => {
        expect(extractExecutionMetadata(FM(["execution_diagnostic_mode: true"])).diagnostic_mode).toBe(true);
        expect(extractExecutionMetadata(FM(["execution_diagnostic_mode: false"])).diagnostic_mode).toBe(false);
    });
    ...
    it("extracts branch", () => {
        expect(extractExecutionMetadata(FM(['execution_branch: "feature/x"'])).branch).toBe("feature/x");
    });
    it("extracts allowlisted forge_flags", () => {
        const m = extractExecutionMetadata(FM(['execution_forge_flags: "FORGE_ROOT,FORGE_DIAGNOSTIC_MODE"']));
    ...
    it("extracts recorded_at", () => {
        expect(extractExecutionMetadata(FM(['execution_recorded_at: "2026-06-14T00:00:00Z"'])).recorded_at).toBe("2026-06-14T00:00:00Z");
    });
});
...
describe("collectExecutionMetadataFromEnv (branch coverage)", () => {
    it("returns diagnostic_mode from FORGE_DIAGNOSTIC_MODE", () => {
        expect(collectExecutionMetadataFromEnv({ FORGE_DIAGNOSTIC_MODE: "1" }).diagnostic_mode).toBe(true);
        expect(collectExecutionMetadataFromEnv({ FORGE_DIAGNOSTIC_MODE: "0" }).diagnostic_mode).toBe(false);
    });
...
//# sourceMappingURL=status-file-ext-branches.test.js.map