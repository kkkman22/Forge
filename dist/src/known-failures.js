...
export function generateAppendBlock(issue, commitSha) {
...
export function mergeKnownFailures(existing, newBlocks) {
    const map = new Map();
...
export function serializeKnownFailures(failures) {
    const lines = [];
...
export function parseKnownFailures(content) {
    const results = [];
...
export function detectRecurrence(failures, diff) {
    ...
    for (const failure of failures) {
        ...
        if (!fixInDiff) {
            results.push(`known-failure recurrence — pattern ${failure.pattern_id}, last seen at ${failure.last_seen}`);
        }
    }
    return results;
}
//# sourceMappingURL=known-failures.js.map