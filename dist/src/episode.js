...
const REQUIRED_FIELDS = ["id", "date", "skill", "tier", "situation", "lesson", "outcome"];
...
export function parseEpisode(content) {
    ...
    const schemaVersion = declaredVersion === 2 ? 2 : 1;
    ...
    const tier = tierRaw !== null && VALID_TIERS.has(tierRaw)
        ? tierRaw
        : "standard";
    if (schemaVersion === 2 && !VALID_TIERS.has(tier))
        return null;
    const outcome = outcomeRaw !== null && VALID_OUTCOMES.has(outcomeRaw)
        ? outcomeRaw
        : "success";
    if (schemaVersion === 2 && !VALID_OUTCOMES.has(outcome))
        return null;
    const episode = {
    ...
    if (relatedSkills.length > 0)
        episode.related_skills = relatedSkills;
    return episode;
}
/**
