const RESERVED_PREFIXES = ["internal-", "debug-", "forge-meta-"];
const DEFAULT_SSOT_ENTRIES = [
...
function hasReservedPrefix(topic) {
...
function diag(severity, message, code) {
    return {
...
export function loadSsotRegistry(config, knownRenderers) {
    const diagnostics = [];
    ...
    for (const item of raw) {
        ...
        if (reserved) {
            diagnostics.push(diag("error", `Topic "${item.topic}" uses reserved prefix "${reserved}"`, "SSOT_RESERVED_PREFIX"));
            continue;
        }
        // Duplicate check
        if (seenTopics.has(item.topic)) {
            diagnostics.push(diag("error", `duplicate topic "${item.topic}" — first occurrence kept`, "SSOT_DUPLICATE_TOPIC"));
            continue;
        }
        ...
        if (knownRenderers && !knownRenderers.has(item.renderer)) {
            diagnostics.push(diag("error", `Renderer "${item.renderer}" for topic "${item.topic}" is not registered`, "SSOT_UNKNOWN_RENDERER"));
            continue;
        }
        entries.push(item);
    }
