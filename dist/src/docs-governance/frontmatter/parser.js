...
// Helpers
// ─────────────────────────────────────────────────────────────
function makeDiagnostic(message, severity = "error", extra) {
    return {
        script: "frontmatter-parser",
        severity,
        file: "", // caller should fill in file path
        message,
        ...(extra ? { extra } : {}),
    };
}
...
function hasNestedMaps(obj) {
    return Object.values(obj).some((v) => isPlainObject(v) || (Array.isArray(v) && v.some(isPlainObject)));
}
// ─────────────────────────────────────────────────────────────
// parseFrontmatter
// ─────────────────────────────────────────────────────────────
