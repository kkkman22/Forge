export function countDocPairs(files) {
    // Exclude INDEX*.md, README.md, .mdx
    const excluded = (name) => name.startsWith("INDEX") || name === "README.md" || name.endsWith(".mdx");
...
export function checkQuota(files, config, options) {
    const diags = [];
