...
let _intentDictCache = null;
function loadIntentDictionary() {
    if (_intentDictCache !== null)
...
export function _resetIntentDictCache() {
        ...
        try {
            ...
            if (matched.length > 0) {
                ...
                if (intentCount > MAX_RUNTIME_INTENT_HINTS) {
                    process.stderr.write(`[intent_overload] ${intentCount} intent hints emitted (threshold: ${MAX_RUNTIME_INTENT_HINTS})\n`);
                }
            }
        }
        catch {
 ...
 */
function generateAssumptions(signals, projectContext) {
    const assumptions = [];
    ...
    if (signals.filesAffected <= 1 && signals.linesChanged <= 20) {
        assumptions.push(`影响范围极小（${signals.filesAffected} 文件 / ${signals.linesChanged} 行），适合轻量路径（基于 filesAffected × linesChanged 信号）`);
    }
    else if (signals.filesAffected > 5 || signals.linesChanged > 200) {
        assumptions.push(`影响范围较大（${signals.filesAffected} 文件 / ${signals.linesChanged} 行），可能涉及多模块协调（基于 filesAffected × linesChanged 信号）`);
    }
    // 3. Requirement clarity assumption
    if (signals.isVagueRequirement) {
        assumptions.push("需求描述存在模糊信号，需在全量路径中通过 decide/spec 阶段澄清（基于需求清晰度信号）");
    }
    ...
    if (signals.hasExistingSpec) {
        ...
        assumptions.push("无现成 Spec，标准路径下 build 需自行推断需求边界（基于 hasExistingSpec 信号）");
    }
    ...
    if (signals.hasNewService || signals.hasNewDatabase || signals.hasAuthChanges) {
        ...
        if (signals.hasAuthChanges)
            changes.push("认证变更");
        assumptions.push(`涉及架构级变更（${changes.join("、")}），需额外关注兼容性审查（基于架构变更信号）`);
    }
    ...
    if (assumptions.length < 3 && projectContext === undefined) {
        assumptions.push("项目上下文未提供，技术栈和影响范围假设基于信号推断（基于 TaskSignals 默认值）");
    }
