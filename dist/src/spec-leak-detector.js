 ...
 */
export function detectSpecLeak(specText, filePath, bannedRegistry, glossary, specContext) {
    if (bannedRegistry.categories.size === 0)
        return [];
    const findings = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (const [category, patterns] of bannedRegistry.categories) {
            for (const bp of patterns) {
                ...
                if (glossaryEntries) {
                    const whitelisted = glossaryEntries.some((e) => e.context === specContext || e.context === "_shared");
                    if (whitelisted)
                        continue;
                }
                findings.push({
 ...
 */
export async function loadBannedPatterns(enabledPacks, fs) {
    ...
    for (const candidate of candidates) {
        ...
        if (!parsed.categories || typeof parsed.categories !== "object")
            continue;
        for (const [catName, rawPatterns] of Object.entries(parsed.categories)) {
            ...
            for (const raw of rawPatterns) {
                ...
                if (catSeen.has(r.pattern))
                    continue;
                catSeen.add(r.pattern);
                catPatterns.push({
                    pattern: r.pattern,
                    description: typeof r.description === "string" ? r.description : "",
                    suggestion_template: typeof r.suggestion_template === "string" ? r.suggestion_template : undefined,
                });
            }
        }
    }
...
//# sourceMappingURL=spec-leak-detector.js.map