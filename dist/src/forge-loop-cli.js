...
async function main() {
    ...
    program
        ...
        if (!opts.noWarmup) {
            ...
            warmup.stdin?.end();
            const warmupExitCode = await new Promise((resolve) => {
        ...
        const resolvedLogConfig = mergeLogConfig({ logFormat: opts.logFormat, logLevel: opts.logLevel, logFile: opts.logFile }, fileLogConfig);
        ...
        if (resolvedLogConfig.logFile) {
            ...
            const fileSinkConfig = { format: "json", level: resolvedLogConfig.level };
        ...
        // ---------------------------------------------------------------
        const hookCheckPath = path.join(process.env.CLAUDE_PLUGIN_ROOT ?? cwd, "scripts/hook-check-frozen.sh");
        ...
        try {
            ...
            try {
                await agentAdapter.shutdown?.("SIGTERM");
            }
            catch (cleanupError) {
