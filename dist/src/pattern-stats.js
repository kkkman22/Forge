...
export function findUpgradableEpisodes(episodes, patterns, now, windowDays = DEFAULT_WINDOW_DAYS, minOccurrences = DEFAULT_MIN_OCCURRENCES) {
    ...
    for (const ep of episodes) {
        ...
        if (episodeMs === null || nowMs - episodeMs > windowMs)
            continue;
    ...
    for (const [key, eps] of clusters) {
        if (eps.length < minOccurrences)
            continue;
        if (isCoveredByExistingPattern(key, coveredKeys))
            continue;
        suggestions.push({
            clusterKey: key,
            episodes: eps,
...
function splitActiveArchived(body) {
 ...
 */
function parsePatternSections(body) {
    const lines = body.split("\n");
    const patterns = [];
    let current = null;
    const flush = () => {
...
// ---------------------------------------------------------------------------
function appendPatternLines(lines, pattern) {
 ...
 */
function buildClusterKey(skill, rootCause) {
...
function coverageKey(pattern) {
    return `${pattern.name.toLowerCase()}|${pattern.pattern_id.toLowerCase()}|${pattern.tags
        .map((t) => t.toLowerCase())
        .join(",")}`;
}
function isCoveredByExistingPattern(key, coveredKeys) {
...
function buildPatternDraft(clusterKey, episodes, now) {
...
//# sourceMappingURL=pattern-stats.js.map