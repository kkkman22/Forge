...
function hasTraversal(segment) {
    return segment.includes("..") || segment.includes("/") || segment.includes("\\");
}
export function resolveLibPath(sub, opts) {
    if (hasTraversal(sub) || isAbsolute(sub)) {
        return { ok: false, code: "E_PATH_INVALID", reason: "traversal or absolute" };
    }
    const pluginRoot = opts && "pluginRoot" in opts ? opts.pluginRoot : process.env.CLAUDE_PLUGIN_ROOT;
