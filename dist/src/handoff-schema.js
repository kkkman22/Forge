...
export function parseHandoffBlock(block) {
    ...
    if (!codeMatch)
        throw new Error("No fenced code block found");
...
function readCommands(lines, start) {
    const entries = [];
    let i = start;
    let currentCmd = "";
    let currentExit;
...
function readMultiline(lines, start) {
    const parts = [];
    let i = start;
    while (i < lines.length && /^\s/.test(lines[i])) {
...
export function validateHandoff(handoff, options = {}) {
...
export function serializeHandoff(handoff) {
    const lines = ["```yaml handoff"];
    lines.push(`task_id: ${handoff.task_id}`);
    lines.push("completed:");
    for (const c of handoff.completed)
        lines.push(`  - ${c}`);
    lines.push("not_completed:");
    for (const c of handoff.not_completed)
        lines.push(`  - ${c}`);
    ...
    for (const item of handoff.issues_found)
        lines.push(`  - ${item}`);
