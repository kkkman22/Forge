 ...
 */
export function parseIntentDictionary(yamlContent) {
    if (!yamlContent?.trim()) {
    ...
    for (const rawLine of lines) {
        ...
        // Parse hint items
        if (inHints) {
            const hintMatch = line.match(/^\s+-\s*\{\s*command:\s*(\S+)\s*,\s*tag:\s*(\S+)\s*,\s*description:\s*["'](.+?)["']\s*\}\s*$/);
            if (hintMatch) {
                currentHints.push({
                    command: hintMatch[1],
                    tag: hintMatch[2],
    ...
    if (definitions.length > 8) {
        process.stderr.write(`[intent_dict_warning] Dictionary has ${definitions.length} intents (threshold: 8). Consider merging or retiring low-usage entries.\n`);
    }
    // R6-2: Soft warning when single intent has > 20 triggers
    for (const def of definitions) {
        if (def.triggers.length > 20) {
            process.stderr.write(`[intent_dict_warning] Intent "${def.name}" has ${def.triggers.length} triggers (threshold: 20). Consider splitting.\n`);
        }
    }
    ...
    for (const def of definitions) {
        for (const trigger of def.triggers) {
            ...
            if (existing) {
                throw new Error(`Duplicate trigger "${trigger}" found in intents "${existing}" and "${def.name}"`);
            }
...
function flushIntent(name, description, triggers, emit_hints) {
 ...
 */
export function matchIntents(description, dictionary) {
    if (!description)
        return [];
...
function findWholeWordMatch(text, word) {
    ...
    const isCJK = (ch) => /[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}]/u.test(ch);
 ...
 */
export function intentsToHints(matched) {
    const hints = [];
 ...
 */
export function detectIntentCancellation(userResponse, knownIntents) {
    if (!userResponse)
...
//# sourceMappingURL=router-intents.js.map