...
function isWithinBase(resolved, expectedBase) {
...
function buildCandidates(relativePath, enabledPacks) {
    const candidates = [];
    // 1. Custom layer
    candidates.push({
 ...
 */
export function resolvePath(relativePath, enabledPacks) {
 ...
 * check whether the files actually exist on disk.
 */
export function resolveAllPaths(relativePath, enabledPacks) {
    const candidates = buildCandidates(relativePath, enabledPacks);
    const results = [];
    for (const candidate of candidates) {
        const resolved = path.resolve(candidate.path);
