...
function sanitizeTaskText(s) {
    return s.replace(TASK_TEXT_DISALLOWED_RE, "");
}
function maxConsecutiveBackticks(s) {
    let max = 0;
    let current = 0;
    for (const ch of s) {
        if (ch === "`") {
...
export function formatFinding(finding, runId, prefix) {
    ...
    if (finding.suggestion) {
        ...
        const maxBackticks = Math.max(maxConsecutiveBackticks(finding.message), maxConsecutiveBackticks(finding.suggestion));
