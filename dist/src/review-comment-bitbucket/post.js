...
export async function postReviewToBitbucket(reviewMarkdownPath, pullRequestId, config, ctx, bitbucket, _testFindings, options) {
    ...
    if (!config.enabled) {
        const reason = "platform-disabled-by-config";
    ...
    if (_testFindings !== undefined) {
        ...
        try {
            ...
            const reason = e instanceof ReviewMarkdownNotFoundError ? "review-markdown-not-found" : "parse-error";
            await persistMetrics(baseDir, ctx, {
                posted: false,
                post_enabled: true,
...
// --- Executor functions ---
async function executeCreatesP0P1(failures, actions, pullRequestId, config, ctx, bitbucket, prefix) {
...
async function executeReopens(failures, actions, pullRequestId, config, ctx, bitbucket, prefix) {
    for (const action of actions) {
...
async function executeDones(failures, actions, pullRequestId, config, ctx, bitbucket, prefix) {
    for (const action of actions) {
        if (action.kind !== "done")
            continue;
        try {
...
async function executeCreatesP2(failures, actions, pullRequestId, config, ctx, bitbucket, prefix) {
    for (const action of actions) {
        if (action.kind !== "create")
            continue;
        if (action.finding.priority !== "P2")
            continue;
...
// --- Helpers ---
async function persistSideEffects(baseDir, fn) {
    if (!baseDir)
        return;
    try {
        await fn();
...
async function persistMetrics(baseDir, ctx, params) {
    if (!baseDir)
        return;
    try {
...
function extractForgeTasks(raw, prefix) {
    return raw
        .map((t) => {
...
function extractForgeComments(rawPr, prefix) {
