 ...
 *   `<!-- Evolution: YYYY-MM-DD | source: <id> | target: <skill>[#<section>] -->`
 */
const MARKER_REGEX = /^\s*<!--\s*Evolution:\s*(\S+?)\s*\|\s*source:\s*(.+?)\s*\|\s*target:\s*(.+?)\s*-->\s*$/;
...
export function aggregateEvolutionMarkers(markersByFile, skillsRegistry, now = new Date()) {
    ...
    const bySkill = Array.from(bySkillMap.entries())
        ...
        const suggestAdr = details.some((m) => (bySectionCount.get(m.target.trim()) ?? 0) >= SUGGEST_ADR_THRESHOLD);
...
//# sourceMappingURL=evolution-marker.js.map