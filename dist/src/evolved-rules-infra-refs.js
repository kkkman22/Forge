 ...
 */
export function parseInfraRefs(body) {
    const refs = [];
    const headingRe = /^###\s+(R\d+):[^\n]*$/gm;
    const matches = [];
    let prev = null;
    let prevStart = 0;
    let m = headingRe.exec(body);
    while (m !== null) {
        if (prev !== null) {
    ...
    if (prev !== null)
        matches.push({ id: prev[1], start: prevStart, end: body.length });
 ...
 */
export function extractBacktickPathsWithSections(line) {
    const out = [];
 ...
 */
export function validateInfraRefs(refs, fs) {
    return refs.map((ref) => {
 ...
 */
function isAnchorLikeSection(needle) {
    if (/\d/.test(needle))
        return true;
    if (needle.includes("§"))
        return true;
...
//# sourceMappingURL=evolved-rules-infra-refs.js.map