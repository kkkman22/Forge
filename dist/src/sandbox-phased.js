 ...
 */
export function checkFilesystemPolicy(filePath, operation, config) {
 ...
 *   3. No rule matched -> allowed: true (default allow)
 */
export function checkCommandPolicy(command, config) {
    // 1. Check deny patterns first (highest priority)
 ...
 */
export function checkNetworkPolicy(url, config) {
    const hostname = extractHostname(url);
    const hasGlob = (s) => s.includes("*") || s.includes("?");
    // 1. Check deny patterns first (highest priority)
 ...
 */
export function loadSandboxConfig(configPath) {
 ...
 */
function isValidSandboxConfig(value) {
    ...
    if (typeof cfg.profile !== "string")
        return false;
    // filesystem
    if (typeof cfg.filesystem !== "object" || cfg.filesystem === null)
        return false;
    const fs = cfg.filesystem;
    if (!Array.isArray(fs.read) || !Array.isArray(fs.write) || !Array.isArray(fs.deny))
        return false;
    // network
    if (typeof cfg.network !== "object" || cfg.network === null)
        return false;
    const net = cfg.network;
    if (!Array.isArray(net.allow) || !Array.isArray(net.deny))
        return false;
    // commands
    if (typeof cfg.commands !== "object" || cfg.commands === null)
        return false;
    const cmd = cfg.commands;
    if (!Array.isArray(cmd.allow) || !Array.isArray(cmd.deny))
        return false;
    return true;
}
/**
...
export function resolveProfile(config, profileName) {
    ...
    throw new Error(`Sandbox profile "${targetProfile}" not found. Available profiles: ${config.profile}`);
}
//# sourceMappingURL=sandbox-phased.js.map