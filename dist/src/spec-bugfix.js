...
// parseBugfixMarkdown
// ---------------------------------------------------------------------------
export function parseBugfixMarkdown(text) {
    ...
    if (!hasSection(body, "Current Behavior") ||
...
// ---------------------------------------------------------------------------
export function parseBugfixDesignMarkdown(text) {
    if (!text || text.trim().length === 0)
        return { errors: [{ message: "Empty input" }] };
    const fm = parseFrontmatter(text);
    if (Array.isArray(fm))
    ...
    if (!rootCause || !fixStrategy || !testProperties) {
        ...
        if (!fixStrategy)
            missing.push("Fix Strategy");
        if (!testProperties)
            missing.push("Test Properties");
...
// ---------------------------------------------------------------------------
export function renderBugfixMarkdown(doc) {
    const parts = [];
...
// ---------------------------------------------------------------------------
export function renderBugfixDesignMarkdown(doc) {
    const parts = [];
    parts.push(renderFrontmatter(doc.frontmatter));
    parts.push("");
...
export function runBugfixSelfChecks(bundle) {
...
function parseFrontmatter(text) {
    ...
    const kind = yaml.match(/kind:\s*(.+)/)?.[1]?.trim();
...
function extractEarsFromSection(body, heading) {
...
//# sourceMappingURL=spec-bugfix.js.map