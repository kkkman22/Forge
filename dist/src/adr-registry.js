 ...
 */
function extractListOrInline(frontmatter, fieldName) {
 ...
 */
export function parseAdrFrontmatter(content) {
 ...
 * they see fit).
 */
export function loadAllAdrs(entries, readFile) {
    const loaded = [];
 ...
 */
export function nextAdrId(existing) {
    let max = 0;
 ...
 */
export function findRelatedAdrs(taskDescription, adrs, limit) {
    if (limit <= 0) {
        return [];
    }
 ...
 */
export function renderAdrIndex(adrs) {
    ...
    const lines = [ADR_INDEX_HEADER, "", ADR_INDEX_TABLE_HEAD, ADR_INDEX_TABLE_SEP];
    for (const entry of sorted) {
        lines.push(`| ${escapeCell(entry.id)} | ${escapeCell(entry.title)} | ${escapeCell(entry.status)} | ${escapeCell(entry.date)} | ${escapeCell(entry.filePath)} |`);
    }
    return `${lines.join("\n")}\n`;
}
// ---------------------------------------------------------------------------
 ...
 */
export function applySupersession(newAdr, allAdrs) {
...
//# sourceMappingURL=adr-registry.js.map