 ...
 */
function parseReviewReportFrontmatter(content) {
    ...
    if (!p0Match && !p1Match && !methodMatch && !resultMatch)
        return null;
...
function isValidMethodology(value) {
    return VALID_METHODOLOGIES.includes(value);
}
/**
 ...
 */
function safeParseP1Fixlist(content) {
    try {
        ...
        for (const issue of obj.p1Issues) {
            ...
            if (i.fixCommit !== null && typeof i.fixCommit !== "string")
                return null;
        }
        return {
            runId: obj.runId,
            p1Issues: obj.p1Issues,
 ...
 */
export function checkReviewGate(reviewDir, _latestCommitHash, gitLogFn) {
    ...
    if (report.methodology === "unavailable") {
        return {
            gate: "review",
            passed: false,
            reason: "Review unavailable: methodology=unavailable (L3 fallback ladder exhausted). HARD-GATE: main agent must not substitute for review.",
 ...
 */
export function checkTestGate(testResultsDir, configCICheck) {
    ...
    const resultFiles = files.filter((f) => f.endsWith(".json") || f.endsWith(".md") || f.endsWith(".txt"));
    ...
    const hasFailure = lower.includes('"passed": false') ||
        ...
        (lower.includes("failures:") &&
            ...
            !lower.includes("failures:0"));
    const hasPass = lower.includes('"passed": true') ||
 ...
 */
export function checkProgressGate(progressDir, featureName) {
 ...
 */
export function parseP1Fixlist(content) {
 ...
 */
export function generateP1Fixlist(runId, findings) {
    const p1Issues = findings
 ...
 */
export function updateFixlistWithCommits(fixlist, gitLogFn) {
    const updatedIssues = fixlist.p1Issues.map((issue) => {
        if (issue.fixCommit !== null)
            return issue;
 ...
 */
export function evaluateFallbackLadder(conditions) {
    // L0 check
    const l0Met = conditions.isInteractive &&
 ...
 */
export function checkFallbackLadderGate(methodology) {
    if (methodology === "unavailable") {
        return {
            gate: "review",
            passed: false,
            reason: "Review unavailable: methodology=unavailable. HARD-GATE (L3): all review paths (L0+L1+L2) exhausted. Main agent must NOT substitute for review. Ship is blocked.",
        };
    }
    return {
        gate: "review",
 ...
 */
export function persistGateResults(report, shipDir) {
 ...
 */
export function validateSkipGateOptions(options) {
    if (options.skipAll) {
    ...
    const validGates = ["review", "test", "progress"];
 ...
 */
export function buildSkipGateAnnotation(options) {
 ...
 */
export function runAllGates(input) {
...
//# sourceMappingURL=ship-gates.js.map