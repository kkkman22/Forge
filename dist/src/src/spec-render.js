...
// ---------------------------------------------------------------------------
export function renderRequirementsMarkdown(doc) {
    const parts = [];
    ...
    if (doc.delta) {
        ...
        parts.push("### 新增");
        parts.push("");
        for (const item of doc.delta.added)
            parts.push(`- ${item}`);
        parts.push("");
        parts.push("### 修改");
        parts.push("");
        for (const item of doc.delta.modified)
            parts.push(`- ${item}`);
        parts.push("");
        parts.push("### 不变");
        parts.push("");
        for (const item of doc.delta.unchanged)
            parts.push(`- ${item}`);
        parts.push("");
    }
    return parts.join("\n");
...
// ---------------------------------------------------------------------------
export function renderDesignMarkdown(doc) {
    const parts = [];
    parts.push(renderFrontmatter(doc.frontmatter));
    parts.push("");
...
// renderTasksMarkdown
// ---------------------------------------------------------------------------
export function renderTasksMarkdown(doc) {
    const parts = [];
    parts.push(renderFrontmatter(doc.frontmatter));
    parts.push("");
...
//# sourceMappingURL=spec-render.js.map