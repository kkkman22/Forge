 ...
 * @internal
 */
export function checkReferenceIntegrity(input) {
    const findings = [];
 ...
 * @internal
 */
export function checkOrphanSolutions(input) {
    const findings = [];
 ...
 * @internal
 */
export function checkContradictions(input) {
    const findings = [];
 ...
 * @internal
 */
export function lintKnowledgeIntegrity(input) {
    return [
...
function extractInstinctSources(content) {
    const results = [];
...
function extractRuleSources(content) {
    const results = [];
    const blocks = splitByH3(content);
    for (const block of blocks) {
...
function extractFailureSources(content) {
    const results = [];
    const blocks = splitByH3(content);
    for (const block of blocks) {
        const headingMatch = block.match(/^###\s+(.+)$/m);
...
function resolveSource(source, solutionTopics, sessionFiles) {
    const normalized = normalizeSource(source);
    // Direct match against solutions/ topics
    if (solutionTopics.has(normalized))
        return true;
    ...
    for (const session of sessionFiles) {
        ...
        if (sessionNorm.includes(normalized) || normalized.includes(sessionNorm))
            return true;
    }
    // Source might be a user-facing description (e.g. "用户反馈 — spec 阶段...")
    // These are not file references, so we accept them as valid.
    if (source.includes("—") || source.includes("用户") || source.includes("审计"))
        return true;
    return false;
}
...
function parsePatternBlocks(content) {
    const blocks = splitByH3(content);
    const results = [];
    for (const block of blocks) {
        const headingMatch = block.match(/^###\s+(.+)$/m);
...
function detectPolarity(text) {
    ...
    if (negScore >= 2 && negScore > posScore)
        return "negative";
...
//# sourceMappingURL=knowledge-integrity.js.map