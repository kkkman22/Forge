...
// ---------------------------------------------------------------------------
export function scoreTaskDescription(text) {
...
// resolveSpecVariant
// ---------------------------------------------------------------------------
const VALID_VARIANTS = ["requirements-first", "design-first", "quick-plan"];
export function resolveSpecVariant(input, eventsPath) {
    ...
    if (fallback) {
        if (!VALID_VARIANTS.includes(fallback)) {
            import("./event-writer.js").then(({ writeEvent }) => {
                if (eventsPath)
                    writeEvent(eventsPath, "invalid_default_variant_config", { defaultVariant: fallback });
            });
        }
        else {
...
function emitVariantEvent(eventsPath, variant, source) {
    if (!eventsPath)
        return;
    import("./event-writer.js").then(({ writeEvent }) => {
...
//# sourceMappingURL=spec-variant.js.map