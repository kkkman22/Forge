...
export const PATTERNS = [
    ...
    {
        id: "io-001",
        pattern: /\bignore\s+(?:all\s+|any\s+)?(?:the\s+)?(?:previous|prior|above|preceding|earlier)\s+(?:instructions?|prompts?|rules?|directives?)\b/i,
    ...
    {
        id: "io-002",
        pattern: /\b(?:forget|erase|wipe)\s+(?:all\s+|everything\s+)?(?:you\s+(?:were|have been)\s+(?:told|instructed|taught)|your\s+(?:instructions?|prompts?|training|guidelines?))\b/i,
        type: "instruction_override",
        severity: "critical",
        baseConfidence: 0.93,
    ...
    {
        id: "io-003",
        pattern: /\b(?:disregard|discard)\s+(?:all\s+|any\s+)?(?:the\s+)?(?:previous|prior|above|system)\s+(?:instructions?|prompts?|context|rules?)\b/i,
        type: "instruction_override",
        severity: "critical",
        baseConfidence: 0.92,
    ...
    {
        id: "io-004",
        pattern: /\b(?:override|overwrite|replace)\s+(?:your\s+)?(?:system\s+|default\s+)?(?:instructions?|prompts?|behavior|rules?)\b/i,
        type: "instruction_override",
        severity: "critical",
        baseConfidence: 0.9,
    ...
    {
        id: "io-005",
        pattern: /\b(?:new|updated|revised)\s+(?:system\s+)?(?:instructions?|rules?|directives?)\s*(?::|\bare\b|\bfollow\b)/i,
        type: "instruction_override",
        severity: "high",
        baseConfidence: 0.8,
    ...
    {
        id: "io-006",
        pattern: /\bstop\s+following\s+(?:the\s+|your\s+)?(?:previous|prior|earlier|system)\s+(?:instructions?|rules?|guidelines?)\b/i,
        type: "instruction_override",
        severity: "critical",
        baseConfidence: 0.9,
    ...
    {
        id: "jb-003",
        pattern: /\bbypass\s+(?:all\s+|any\s+|your\s+)?(?:restrictions?|filters?|safety|guardrails?|content\s+policy)\b/i,
        type: "jailbreak",
        severity: "critical",
        baseConfidence: 0.93,
    ...
    {
        id: "jb-005",
        pattern: /\b(?:with|have|has)\s+no\s+(?:limits?|restrictions?|filters?|rules?|morals?|ethics?)\b/i,
        type: "jailbreak",
        severity: "high",
        baseConfidence: 0.85,
    ...
    {
        id: "jb-007",
        pattern: /\b(?:disable|remove|turn\s+off|switch\s+off)\s+(?:your\s+)?(?:safety|content\s+filter|moderation|guardrails?)\b/i,
        type: "jailbreak",
        severity: "critical",
        baseConfidence: 0.93,
    ...
    {
        id: "jb-008",
        pattern: /\bpretend\s+(?:you|that\s+you)\s+(?:have\s+no|don'?t\s+have)\s+(?:rules?|restrictions?|filters?|guidelines?)\b/i,
        type: "jailbreak",
        severity: "high",
        baseConfidence: 0.88,
    ...
    {
        id: "ea-001",
        pattern: /\b(?:decode|decrypt|run|execute|interpret)\s+(?:this\s+|the\s+following\s+)?base[-\s]?64\b/i,
    ...
    {
        id: "ea-003",
        pattern: /\b(?:decode|convert|interpret)\s+(?:this\s+|the\s+following\s+)?hex(?:adecimal)?\s+(?:string|payload|message|instructions?)\b/i,
        type: "encoding_attack",
        severity: "medium",
        baseConfidence: 0.78,
...
//# sourceMappingURL=prompt-defense-patterns.js.map