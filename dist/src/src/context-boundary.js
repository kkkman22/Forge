...
const ALLOWED_RELATIONSHIPS = new Set(["partnership", "shared-kernel", "acl"]);
/** Relationships that are allowed only when the target is the provider. */
const PROVIDER_RELATIONSHIPS = new Set(["open-host", "published-language"]);
 ...
 */
export function loadOwnershipMap(_projectRoot, ownershipYamlPath) {
    const mappings = {};
...
function parseYamlMappings(content, mappings) {
    ...
    for (const rawLine of lines) {
        ...
        if (plainMatch) {
            ...
            if (glob && context && glob !== "schema_version")
                mappings[glob] = context;
        }
    }
}
 ...
 */
export function extractJsdocContext(fileContent) {
 ...
 *  3. No match → null
 */
export function resolveFileContext(filePath, ownershipMap, jsdocContext) {
    ...
    for (const imp of imports) {
        ...
        if (relationship === null) {
            ...
            violations.push({
                ...
                suggestion: `Add a declared relationship between "${sourceContext}" and "${targetContext}" in your context map, ` +
        ...
        if (BLOCKED_RELATIONSHIPS.has(relationship)) {
            const suggestion = relationship === "customer-supplier"
...
//# sourceMappingURL=context-boundary.js.map