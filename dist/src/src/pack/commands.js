...
export function commandList(registry, enabled) {
...
export function commandEnable(name, config, registry) {
...
export function commandDisable(name, config) {
    const { frontmatter, body } = splitConfig(config);
    const existingPacks = parsePacksField(frontmatter);
    if (!existingPacks.includes(name)) {
...
export function commandInspect(name, registry) {
...
export function commandOverride(relativePath, enabled, _force) {
...
export function commandValidate(name, registry) {
...
export function commandNew(name) {
    const manifest = [
        `name: ${name}`,
        `display_name: "${name}"`,
...
function splitConfig(config) {
    const match = /^---\s*\n([\s\S]*?)\n---\n?([\s\S]*)$/.exec(config);
    if (!match)
        return { frontmatter: "", body: config };
    return { frontmatter: match[1], body: match[2] };
}
function parsePacksField(frontmatter) {
    const match = /^packs:\s*\n((\s+-\s+.+\n?)*)/m.exec(frontmatter);
    if (!match)
        return [];
    return match[1]
...
function rebuildFrontmatter(original, packs) {
