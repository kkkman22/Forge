 ...
 */
export function validateHandoffEntry(entry) {
    const errors = [];
 ...
 */
export function handoffPath(from, to) {
 ...
 * ```
 */
export function renderHandoff(doc) {
    const lines = [];
    lines.push("---");
...
export function parseHandoff(content) {
    ...
    if (!fromStage || !toStage || !createdAt)
        return null;
    // Parse body sections
 ...
 */
export function priorHandoffPaths(currentStage) {
    const paths = [];
