 ...
 */
import { advanceMethodology, buildPressurePrompt, detectFailurePattern, determinePressureLevel, getMethodologyChain, getStallResponse, MAX_SUMMARY_HISTORY, selectMethodology, } from "./pua-engine.js";
...
export class PuaStateManager {
     ...
     */
    constructor(deps, taskType = "general") {
        this.deps = deps;
    ...
    handleSuccess() {
        try {
            ...
            this.deps.warn(`Warning: PUA success cleanup failed: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
        }
    }
    /**
     ...
     * StatusFile.
     */
    handleFailure(summary, consecutiveFailures) {
        try {
            ...
            if (pressureLevel !== "L0") {
                if (failurePattern !== null) {
                    if (this.currentMethodologyChain === null) {
                        ...
                        const next = advanceMethodology(this.currentMethodologyChain, this.methodologyChainIndex);
                        if (next !== null) {
            ...
            this.deps.warn(`Warning: PUA failure handling failed, continuing without PUA: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
        }
    }
    /**
    ...
    safeWriteFields(fields) {
        try {
            ...
            this.deps.warn(`Warning: failed to write PUA fields to StatusFile: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
        }
    }
    /**
    ...
    safeClearFields() {
        try {
            ...
            this.deps.warn(`Warning: failed to clear PUA fields from StatusFile: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
        }
    }
    /**
     ...
     */
    restoreContext(statusContent, consecutiveFailures) {
        try {
            ...
            if (puaFields.puaFailurePattern !== undefined && this.currentMethodologyChain === null) {
                try {
                    ...
                    this.deps.warn(`[debug] PUA failure pattern restoration failed for pattern "${puaFields.puaFailurePattern}": ${err instanceof Error ? err.message : String(err)}`);
                }
            }
            ...
            // PUA engine error — degrade gracefully, continue without PUA
            this.deps.warn(`Warning: PUA state restoration failed, continuing without PUA: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
            return undefined;
        }
    }
}
//# sourceMappingURL=pua-state-manager.js.map