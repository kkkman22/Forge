 ...
 * Pure; no IO.
 */
export function checkSkillLength(filePath, content, limit = DEFAULT_LIMIT) {
 ...
 */
export function validateAllSkillLengths(fs, skillsDir, limit = DEFAULT_LIMIT) {
    const paths = fs.listSkillMdFiles(skillsDir);
    return paths.map((path) => checkSkillLength(path, fs.readFile(path), limit));
}
//# sourceMappingURL=skill-length.js.map