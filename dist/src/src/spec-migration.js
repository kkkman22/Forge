...
import { renderDesignMarkdown, renderRequirementsMarkdown, renderTasksMarkdown, } from "./spec-render.js";
// ---------------------------------------------------------------------------
// migrateLegacySpec
// ---------------------------------------------------------------------------
export function migrateLegacySpec(featureDir, eventsPath) {
    ...
    try {
        ...
        if (p0Findings.length > 0) {
            throw new Error(`Post-migration Analyze surfaced ${p0Findings.length} P0 finding(s); rolling back. ` +
        ...
        if (eventsPath) {
            try {
                const { writeEvent } = require("./event-writer.js");
 ...
 */
function migratePlansFile(featureDir, feature) {
    ...
    if (taskMatches.length > 0) {
        ...
        const existingIds = new Set([...existingContent.matchAll(/###\s*(T-\d+(?:\.\d+)?)\s+/g)].map((m) => m[1]));
...
//# sourceMappingURL=spec-migration.js.map