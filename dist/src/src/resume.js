 ...
 */
export function generateResumeOutput(state) {
    // Q1: What problem are we solving?
    const q1Answer = state.plan.objective || "未找到计划目标";
    // Q2: Where are we now?
    const q2Answer = state.progress.inProgressTasks.length > 0
        ? state.progress.inProgressTasks.join("、")
        : "未找到进行中的任务";
    // Q3: What do we know?
    const q3Answer = state.findings.findings.length > 0 ? state.findings.findings.join("\n") : "暂无发现";
    // Q4: What's next?
    const nextTask = findNextTask(state.plan.tasks, state.progress);
    const q4Answer = nextTask || "所有任务已完成";
    // Q5: What's blocking?
    const q5Answer = state.progress.blockers.length > 0 ? state.progress.blockers.join("、") : "无阻塞";
    const questions = [
    ...
    const autoLocateTask = state.progress.inProgressTasks.length > 0 ? state.progress.inProgressTasks[0] : nextTask;
    return {
        questions,
        autoLocateTask,
    };
 ...
 */
function findNextTask(planTasks, progress) {
