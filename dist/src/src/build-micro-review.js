...
function hasPassIndicator(output) {
 ...
 */
function findEvidence(criterion, diff) {
...
// Main function
// ---------------------------------------------------------------------------
export function runMicroReview(input) {
    ...
    if (planVersion === "legacy") {
        ...
        if (diffEmpty || noPass) {
            const missing = [];
            if (diffEmpty)
                missing.push("git diff is empty — no changes detected");
            if (noPass)
                missing.push("verify output does not contain PASS indicator");
...
//# sourceMappingURL=build-micro-review.js.map