 ...
 */
export function maxParallelism(graph) {
    if (graph.tasks.length === 0)
        return 0;
    // Simulate execution: repeatedly get ready tasks, mark them completed
    const simGraph = {
        tasks: graph.tasks.map((t) => ({ ...t, status: "pending" })),
    };
    let maxWidth = 0;
 ...
 */
export function completeTask(graph, taskId) {
 ...
 */
export function isGraphStuck(graph) {
 ...
 */
export function getDispatchBatch(graph, maxConcurrency = DEFAULT_MAX_CONCURRENCY) {
 ...
 */
export function dispatchTasks(graph, taskIds) {
 ...
 */
export function failTask(graph, taskId) {
    const task = graph.tasks.find((t) => t.id === taskId);
    if (!task)
        return [];
    task.status = "failed";
    ...
    const blocked = [];
    const failedIds = new Set([taskId]);
 ...
 */
export function getExecutionSummary(graph) {
    ...
    return {
        ...
        isStuck: counts.completed < graph.tasks.length &&
 ...
 */
export function simulateParallelExecution(graph, maxConcurrency = DEFAULT_MAX_CONCURRENCY) {
    ...
    while (!isGraphComplete(sim) && waves < maxWaves) {
        ...
        dispatchTasks(sim, batch.tasks.map((t) => t.id));
