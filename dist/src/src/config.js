...
export function parseReviewConfig(configContent) {
    ...
    if (envValue !== undefined) {
        ...
        console.warn(`FORGE_REVIEW_CONCURRENCY invalid (${envValue}); falling back to config.md or default=${DEFAULT_CONCURRENCY}`);
    }
    if (configContent) {
        ...
        if (match) {
            ...
            // biome-ignore lint/suspicious/noConsole: config parser warning is intentional (no logger access)
            console.warn(`subagent_concurrency invalid in config.md (${match[1]}); falling back to default=${DEFAULT_CONCURRENCY}`);
        }
    }
