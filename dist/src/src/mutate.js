...
export function generateStrykerConfig(globs, configPath) {
    return {
        mutate: globs,
        testRunner: "vitest",
        reporters: ["json", "html"],
 ...
 */
export function collectTargetGlobs(enabled) {
    const seen = new Set();
    const result = [];
...
function countByStatus(mutants) {
    let killed = 0;
    let survived = 0;
    let noCoverage = 0;
    let runtimeErrors = 0;
 ...
 */
export function computeMutationScore(strykerJsonOutput) {
    let report;
    try {
        report = JSON.parse(strykerJsonOutput);
    }
    catch {
        return {
            total: 0,
...
// Artifact writing
// ---------------------------------------------------------------------------
function writeArtifact(projectRoot, summary) {
 ...
 */
export async function runMutation(enabled, options) {
    ...
    const verdict = mutationScore >= threshold ? "pass" : "warn";
    const summary = {
        packSource,
        targetedGlobs,
        total: scores.total,
