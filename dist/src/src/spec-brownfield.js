...
// ---------------------------------------------------------------------------
export function detectBrownfieldSignals(input, eventsPath) {
    const signals = [];
    if (input.hasGitHistory)
        signals.push("git-history");
    if (input.hasPriorSpec)
...
// ---------------------------------------------------------------------------
export function runBrownfieldSelfChecks(bundle) {
    ...
    else if (req.delta.added.length === 0 ||
    ...
    if (!design?.reversibility) {
        ...
        const hasRollback = design.reversibility.includes("回滚") ||
...
//# sourceMappingURL=spec-brownfield.js.map