 ...
 */
function setField(lines, pattern, newLine) {
 ...
 */
export function extractLoopFields(statusContent) {
    const result = {};
 ...
 */
export function writeLoopFields(statusContent, fields) {
    const parsed = parseFrontmatter(statusContent);
    if (!parsed) {
 ...
 */
export function clearLoopFields(statusContent) {
    const parsed = parseFrontmatter(statusContent);
    if (!parsed) {
        return statusContent;
    }
 ...
 * @returns Updated StatusFile content string.
 */
export function updateIterationStatus(statusContent, phase, iteration) {
    const parsed = parseFrontmatter(statusContent);
    if (!parsed) {
...
const VALID_PRESSURE_LEVELS = new Set(["L0", "L1", "L2", "L3", "L4"]);
/**
 ...
 */
export function extractPuaFields(statusContent) {
    const result = {};
    const parsed = parseFrontmatter(statusContent);
    if (!parsed) {
        return result;
 ...
 * @returns Updated StatusFile content string.
 */
export function writePuaFields(statusContent, fields) {
    const parsed = parseFrontmatter(statusContent);
    if (!parsed) {
    ...
    if (fields.puaFailurePattern !== undefined) {
        setField(lines, /^pua_failure_pattern:\s/, `pua_failure_pattern: "${fields.puaFailurePattern}"`);
    }
    return buildContent(lines, parsed.body, parsed.leadingWhitespace);
}
/**
 ...
 */
export function clearPuaFields(statusContent) {
    const parsed = parseFrontmatter(statusContent);
    if (!parsed) {
        return statusContent;
    }
...
//# sourceMappingURL=status-file-ext.js.map