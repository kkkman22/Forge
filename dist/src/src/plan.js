 ...
 */
function buildReplacementTable(glossary) {
    const table = [];
 ...
 */
function replaceWholeWord(text, surface, canonical) {
 ...
 * @public
 */
export function checkPlanStructure(tasks, headings, executionStrategy) {
    const reasons = [];
    // (a) task count > 15
    if (tasks.length > 15) {
        reasons.push("任务数 > 15");
 ...
 */
export function checkExpectedOutput(planContent, isLegacy) {
    const errors = [];
    const warnings = [];
