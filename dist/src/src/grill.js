 ...
 */
const GLOSSARY_FOLLOWUP_PARENT = "dependency";
// ---------------------------------------------------------------------------
// Public API
// ---------------------------------------------------------------------------
/**
 ...
 */
export function generateDecisionTree(description, existingGlossary, now = new Date()) {
    const timestamp = now.toISOString();
    const mentioned = findMentionedTerms(description, existingGlossary);
    const nodes = CATEGORY_ORDER.map((category) => {
        const root = {
            id: `${category}-1`,
            category,
 ...
 */
function findMentionedTerms(description, glossary) {
 ...
 */
export function selectNextQuestion(tree) {
 ...
 */
export function applyAnswer(tree, nodeId, answer, now = new Date()) {
    const updated = updateNodeById(tree.nodes, nodeId, answer);
    if (updated === tree.nodes)
        return tree;
    return {
        ...tree,
        nodes: updated,
 ...
 */
function findEligibleNode(node, ancestorsResolved) {
    if (ancestorsResolved && node.status === "pending")
        return node;
 ...
 */
function updateNodeById(nodes, nodeId, answer) {
    let found = false;
    const next = [];
...
function hasPendingNode(nodes) {
    for (const node of nodes) {
        if (node.status === "pending")
            return true;
        if (hasPendingNode(node.children))
            return true;
    }
    return false;
}
 ...
 */
export function extractNewGlossaryCandidates(tree, existingGlossary) {
 ...
 */
export function renderGrillFindings(tree, alignmentSummary) {
 ...
 */
function renderGrillFindingsWithCandidates(tree, alignmentSummary, candidates) {
 ...
 */
const EMPTY_GLOSSARY_FOR_RENDER = {
    schema_version: 1,
    updated: "",
    terms: [],
};
 ...
 *   请澄清：保留现有 / 替换现有 / 新增别名
 */
export function renderGrillConflictPrompt(result) {
    if (!result.hasConflict || result.conflictingTerms.length === 0)
        return "";
    const lines = [];
    ...
    for (const conflict of result.conflictingTerms) {
        lines.push(`  - "${conflict.candidate}" vs "${conflict.existing.term}": ${conflict.existing.definition}`);
    }
 ...
 */
function truncateDefinition(context) {
    const trimmed = context.trim();
    if (trimmed.length <= GRILL_CONFLICT_DEFINITION_MAX_LENGTH)
        return trimmed;
 ...
 */
const DECISION_CATEGORY_SET = new Set([
    "functionality",
    "boundary",
    "dependency",
    "assumption",
 ...
 */
const DECISION_STATUS_SET = new Set([
    "pending",
    "resolved",
    "deferred",
    "skipped",
 ...
 */
export function parseGrillFindings(content, now = new Date()) {
 ...
 * **Validates: Requirements 4.10**
 */
export function resumeGrillFromFindings(content, now = new Date()) {
 ...
 * the offending line.
 */
function parseFindingsNodeLine(match) {
    ...
    if (!DECISION_STATUS_SET.has(status))
        return null;
    if (!DECISION_CATEGORY_SET.has(category))
        return null;
    return {
        depth,
        node: {
