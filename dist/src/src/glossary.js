...
export function parseGlossary(content) {
    ...
    const out = { schema_version, updated, terms };
 ...
 */
function appendTermLines(lines, term) {
 ...
 */
function splitBodyByArchiveSentinel(body) {
    const lines = body.split("\n");
 ...
 */
export function findTerm(glossary, query) {
    const needle = normalize(query);
    if (needle.length === 0)
        return null;
    for (const term of glossary.terms) {
        if (normalize(term.term) === needle)
            return term;
 ...
 */
export function detectConflict(glossary, candidate) {
 ...
 */
export function mergeTerm(glossary, candidate, strategy) {
    ...
    switch (strategy) {
        ...
        case "add_alias": {
            ...
            if (aliasesEqual(existing.aliases, merged))
                return glossary;
            const nextTerm = { ...existing, aliases: merged };
            const next = glossary.terms.slice();
 ...
 */
export function findStaleterms(glossary, now, maxAgeDays = 30) {
 ...
 */
export function archiveTerm(glossary, termName) {
