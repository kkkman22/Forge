 ...
 */
export function compactNotesContent(markdown, charBudget = DEFAULT_NOTES_CHAR_BUDGET) {
    ...
    if (entries.length <= MIN_FULL_DETAIL_ENTRIES)
        return markdown;
    ...
    const compactedLines = compacted.map((e) => `### Iteration ${e.number} (compacted): ${e.summary}`);
