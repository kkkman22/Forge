...
describe("releaseFileLock debug logging", () => {
    it("logs a [debug] message when closeSync throws", () => {
        ...
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("[debug] closeSync failed for lock fd=42"));
    ...
    it("logs a [debug] message when unlinkSync throws", () => {
        ...
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("[debug] unlinkSync failed for lock path=/tmp/test.lock"));
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("ENOENT: no such file or directory"));
    });
...
describe("SdkDriver PUA state restoration debug logging", () => {
    ...
    it("logs a warning when PUA state restoration fails with outer catch", async () => {
        ...
        // spurious debug logs.
        const warnCalls = warnSpy.mock.calls.map((c) => String(c[0]));
        const puaRestorationFailLog = warnCalls.find((msg) => msg.includes("PUA state restoration failed"));
...
describe("effect executor onLog optional chaining in catch blocks", () => {
    ...
    it("does not throw when onLog is undefined and stash ref capture fails", async () => {
        ...
        expect(onLog).toHaveBeenCalledWith(expect.stringContaining("[debug] Failed to capture stash ref"));
    });
    it("handles missing onLog gracefully when commit has nothing to commit after frozen zone filtering", async () => {
        ...
        await executor.executeEffect({ type: "commit", message: "test" });
        expect(onLog).toHaveBeenCalledWith("Commit skipped: no changes to commit after frozen zone filtering");
    });
});
//# sourceMappingURL=debug-logging.test.js.map