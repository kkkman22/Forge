...
if (!source.includes('from "./state.js"')) {
    throw new Error("Source sync failed: check-frozen.ts must delegate to state.ts for protection zone rules");
}
...
if (!source.includes("extractFrontmatterStatus")) {
    throw new Error("Source sync failed: check-frozen.ts must use extractFrontmatterStatus from state.ts");
}
...
//# sourceMappingURL=frozen-protection.integration.test.js.map