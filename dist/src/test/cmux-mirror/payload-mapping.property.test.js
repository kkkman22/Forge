...
import { DEFAULT_ICON, LOOP_STATE_TO_ICON, PHASE_TO_ICON, phaseToIcon, TIER_TO_COLOR, tierToColor, } from "../../scripts/cmux-mirror/lib/payload.mjs";
const VALID_PHASES = [
    "decide",
    "spec",
    "plan",
...
describe("payload: icon mapping totality (R12.3)", () => {
    ...
    it("any out-of-domain phase maps to circle (R12.3)", () => {
        fc.assert(fc.property(fc.string().filter((s) => !VALID_PHASES.includes(s)), (unknown) => {
...
describe("payload: color mapping totality (R12.4)", () => {
    ...
    it("out-of-domain tier returns null (emitter omits --color)", () => {
        fc.assert(fc.property(fc.string().filter((s) => !VALID_TIERS.includes(s)), (unknown) => {
...
//# sourceMappingURL=payload-mapping.property.test.js.map