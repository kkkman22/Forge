...
import { generateEvolutionReport, renderEvolutionReport, } from "../src/learn.js";
// ---------------------------------------------------------------------------
// Fixtures
// ---------------------------------------------------------------------------
...
function markerBlock(date, source, target, description) {
    return [
        ...
        const files = new Map([
            [
                ...
                [
                    ...
                    markerBlock("2026-05-01", "ep-2026-05-01-001", "forge-review#new_review_pattern", "Review discovered an untracked failure pattern"),
                ].join("\n\n"),
            ],
            [
                ...
                [
                    ...
                    markerBlock("2026-05-02", "ep-2026-05-02-001", "forge-build#three_strike", "TDD failed three times in a row"),
                    markerBlock("2026-05-02", "ep-2026-05-02-002", "forge-build#three_strike", "Another three-strike run"),
                    markerBlock("2026-05-03", "ep-2026-05-03-001", "forge-build#three_strike", "Third occurrence should trigger ADR suggestion"),
                ].join("\n\n"),
            ],
            [
                ".forge/findings/grill-auth.md",
                markerBlock("2026-05-04", "grill-auth", "forge-nonexistent", "Orphan marker pointing at an unknown skill"),
            ],
            // Archive entry must be ignored — it lives under .forge/archive/
            [
                ".forge/archive/2026-04-20-old/reviews/old.md",
                markerBlock("2026-04-20", "ep-2026-04-20-001", "forge-build", "Archived marker must not leak"),
            ],
        ]);
...
describe("renderEvolutionReport — Task 8.7 (markdown output)", () => {
    ...
    it("is deterministic for the same inputs", () => {
        ...
        const fs = new InMemoryFs(files);
        const first = renderEvolutionReport(generateEvolutionReport(fs, ".forge", SKILLS_REGISTRY, FIXED_NOW));
        const second = renderEvolutionReport(generateEvolutionReport(fs, ".forge", SKILLS_REGISTRY, FIXED_NOW));
...
describe("generateEvolutionReport — Task 8.9 (no historical snapshot)", () => {
    it("drops markers from deleted files on the next run", () => {
        ...
        const afterFiles = new Map(initialFiles);
    ...
    it("drops markers when the marker comment is edited away but the file survives", () => {
        ...
        const before = new InMemoryFs(new Map([
            [markerFile, markerBlock("2026-05-03", "ep-2026-05-03-001", "forge-ship", "temp marker")],
        ]));
        expect(generateEvolutionReport(before, ".forge", SKILLS_REGISTRY, FIXED_NOW).totalMarkers).toBe(1);
        const after = new InMemoryFs(new Map([[markerFile, "# Review — topic-c\n\n(no markers anymore)\n"]]));
        expect(generateEvolutionReport(after, ".forge", SKILLS_REGISTRY, FIXED_NOW).totalMarkers).toBe(0);
    });
});
//# sourceMappingURL=evolution-report.test.js.map