...
function makeBannedRegistry(categories) {
    const map = new Map();
...
describe("detectSpecLeak property: empty banned → empty results", () => {
    it("always returns empty array when banned registry has no categories", () => {
        fc.assert(fc.property(textArb, (specText) => {
            const banned = { categories: new Map() };
...
describe("detectSpecLeak property: glossary monotonicity", () => {
    it("adding glossary entries never increases findings count", () => {
        ...
        fc.assert(fc.property(specTextArb, (specText) => {
            ...
            const findingsWithout = detectSpecLeak(specText, "spec.md", banned, emptyGlossary(), "booking");
            const findingsWith = detectSpecLeak(specText, "spec.md", banned, makeGlossary(glossaryEntries), "booking");
            expect(findingsWith.length).toBeLessThanOrEqual(findingsWithout.length);
        }), { numRuns: 300 });
    });
});
//# sourceMappingURL=spec-leak-detector.property.test.js.map