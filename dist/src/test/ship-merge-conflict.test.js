...
describe("ship_merge conflict-resolver integration", () => {
    it("resolves guarded conflicts via handleMergeConflict", async () => {
        ...
        const result = await handleMergeConflict("Ship merge failed: CONFLICT (content): Merge conflict in .forge/progress/auth.md", "autonomous", {
    ...
    it("aborts on frozen conflict in autonomous via handleMergeConflict", async () => {
        const { handleMergeConflict } = await import("../src/conflict-resolver.js");
        const result = await handleMergeConflict("CONFLICT (content): Merge conflict in .forge/specs/auth/spec.md", "autonomous", {
            statusContent: "current_task: auth\n",
            repoRoot: "/tmp/test",
    ...
    it("resolves reviews conflict and proceeds with merge", async () => {
        const { handleMergeConflict } = await import("../src/conflict-resolver.js");
        const result = await handleMergeConflict("CONFLICT (content): Merge conflict in .forge/reviews/auth.md", "autonomous", {
            statusContent: "current_task: auth\n",
            repoRoot: "/tmp/test",
...
//# sourceMappingURL=ship-merge-conflict.test.js.map