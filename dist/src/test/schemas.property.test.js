...
import { ConfigFileSchema, StatusFileSchema, safeParseConfigFile, safeParseStatusFile, } from "../src/schemas/index.js";
// ---------------------------------------------------------------------------
// Arbitraries
// ---------------------------------------------------------------------------
const phaseArb = fc.constantFrom("decide", "spec", "plan", "build", "build-light", "review", "test", "ship", "learn", "debug", "fix", "refactor");
...
describe("schemas — round-trip", () => {
    ...
    it("StatusFileSchema round-trips any well-formed object", () => {
        fc.assert(fc.property(statusFileArb, (sample) => {
    ...
    it("ConfigFileSchema round-trips any well-formed object", () => {
        fc.assert(fc.property(configFileArb, (sample) => {
            const wireForm = JSON.parse(JSON.stringify(sample));
...
describe("schemas — passthrough unknown fields", () => {
    ...
    it("StatusFileSchema preserves unknown fields", () => {
        fc.assert(fc.property(statusFileArb, fc.stringMatching(/^[a-z_]{3,20}$/), fc.string({ maxLength: 40 }), (sample, key, value) => {
    ...
    it("ConfigFileSchema preserves unknown fields", () => {
        fc.assert(fc.property(configFileArb, fc.stringMatching(/^[a-z_]{3,20}$/), fc.string({ maxLength: 40 }), (sample, key, value) => {
            const knownKeys = new Set([
                "project",
                "stack",
                "security_level",
...
//# sourceMappingURL=schemas.property.test.js.map