...
import { countSentences, secondSentenceStartsWithUseWhen, splitSentences, startsWithImperative, validateDescription, validateDescriptionExtended, } from "../src/skill-description.js";
import { IMPERATIVE_WHITELIST } from "../src/skill-description-imperatives.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function buildSkillDoc(description, name = "forge-test") {
...
describe("startsWithImperative", () => {
    ...
    it("property: pure function — same inputs always same output", () => {
        fc.assert(fc.property(fc.string(), (sentence) => {
...
describe("secondSentenceStartsWithUseWhen", () => {
    it("returns true when second sentence starts with 'Use when'", () => {
        expect(secondSentenceStartsWithUseWhen(["Build things.", "Use when you need to build."])).toBe(true);
    });
    ...
    it("returns false when second sentence does not start with 'Use when'", () => {
        expect(secondSentenceStartsWithUseWhen(["Build things.", "Also does other stuff."])).toBe(false);
    });
...
//# sourceMappingURL=skill-description-extended.property.test.js.map