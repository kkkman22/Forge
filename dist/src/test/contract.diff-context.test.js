...
function matchesNarrativeAntiPattern(text) {
...
describe("Contract: .forge/reviews/.diff-context.md fidelity", () => {
    ...
    it("Patch section contains unified diff hunk markers (except empty diff)", () => {
        ...
        expect(hasHunkMarker(patchSection), "## Patch / ## Diff Content section must contain at least one unified diff marker (@@ ... @@ / --- a/ / +++ b/)").toBe(true);
    });
    it("Patch section does not contain narrative-summary anti-pattern (or co-presence with real hunk)", () => {
        ...
        if (matchesNarrativeAntiPattern(patchSection)) {
            ...
            expect(hasHunkMarker(patchSection), "narrative summary anti-pattern detected without accompanying unified diff hunk markers — see forge-review-diff-context-fidelity bugfix.md Bug Condition").toBe(true);
        }
    });
});
//# sourceMappingURL=contract.diff-context.test.js.map