...
describe("ProcessRegistry", () => {
    ...
    describe("execTracked", () => {
        it("executes with default 30s timeout and killSignal SIGTERM", () => {
            ...
            expect(mockExecFileSync).toHaveBeenCalledWith("git", ["rev-parse", "HEAD"], expect.objectContaining({
                timeout: 30_000,
                killSignal: "SIGTERM",
            }));
        });
        it("allows custom timeout override", () => {
            ...
            expect(mockExecFileSync).toHaveBeenCalledWith("git", ["status"], expect.objectContaining({ timeout: 5000 }));
        });
    });
    ...
    describe("Property 1: register preserves metadata", () => {
        it("for any valid metadata, getAll contains the entry with all fields", () => {
            ...
            }), fc.integer({ min: 1, max: 65535 }), (metadata, pid) => {
                ProcessRegistry.resetInstance();
                const reg = ProcessRegistry.getInstance();
                const child = { pid, on: () => { } };
    ...
    describe("Property 2: unregister removes processes", () => {
        it("for any registered set and any unregister subset, removed PIDs are gone", () => {
            fc.assert(fc.property(fc
                ...
                .filter((arr) => new Set(arr.map((p) => p.pid)).size === arr.length), (entries) => {
                ProcessRegistry.resetInstance();
                const reg = ProcessRegistry.getInstance();
                for (const e of entries) {
                    reg.register({ pid: e.pid, on: () => { } }, {
                        source: e.source,
                        detached: e.detached,
                    });
                }
                ...
                const remainingPids = new Set(entries.slice(Math.ceil(entries.length / 2)).map((e) => e.pid));
                for (const entry of allAfter) {
...
describe("Property 3: shutdownAll terminates all registered processes", () => {
    it("terminated + forcedKill + alreadyExited equals size()", async () => {
        await fc.assert(fc.asyncProperty(fc
            .array(fc.record({
            pid: fc.integer({ min: 1, max: 65535 }),
            source: fc.string({ minLength: 1, maxLength: 20 }),
            detached: fc.boolean(),
            ...
            .filter((arr) => new Set(arr.map((p) => p.pid)).size === arr.length), async (entries) => {
            ProcessRegistry.resetInstance();
            const reg = ProcessRegistry.getInstance();
...
describe("Property 5: deserialize rejects invalid JSON", () => {
    it("throws on any invalid input", () => {
        fc.assert(fc.property(fc.oneof(fc.constant(""), fc.constant("{"), fc.constant("}"), fc.constant("null"), fc.constant("[]"), fc.constant("42"), fc.string({ minLength: 1, maxLength: 20 }).filter((s) => {
            try {
                const p = JSON.parse(s);
                return !(typeof p.sessionPid === "number" &&
