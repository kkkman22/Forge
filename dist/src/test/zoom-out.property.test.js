...
import { buildZoomOutPrompt, isZoomOutTrigger, MAX_LINES_PER_SECTION, pauseForZoomOut, renderZoomOut, resumeFromZoomOut, validateZoomOutOutput, ZOOM_OUT_PAUSED_PHASE, } from "../src/zoom-out.js";
// ---------------------------------------------------------------------------
// Generators
// ---------------------------------------------------------------------------
/** Arbitrary section body with a bounded non-empty line count. */
const sectionBodyArb = (maxEffectiveLines) => fc
    .array(fc.string({ minLength: 1, maxLength: 30 }).map((s) => s.replace(/\n/g, " ")), {
    minLength: 0,
    maxLength: maxEffectiveLines,
})
    .map((lines) => lines.join("\n"));
const zoomOutOutputArb = fc.record({
...
describe("renderZoomOut", () => {
    it("is deterministic (same input → same output)", () => {
        fc.assert(fc.property(zoomOutOutputArb, (output) => {
...
describe("validateZoomOutOutput", () => {
    ...
    it("flags each section that exceeds the 5-line budget", () => {
        ...
        const makeBody = (n) => Array.from({ length: n }, (_, i) => `line ${i + 1}`).join("\n");
        fc.assert(fc.property(fc.integer({ min: 0, max: 10 }), fc.integer({ min: 0, max: 10 }), fc.integer({ min: 0, max: 10 }), (a, b, c) => {
            const output = {
            ...
            const result = validateZoomOutOutput(output);
            const expectedOverflowCount = (a > MAX_LINES_PER_SECTION ? 1 : 0) +
...
// ---------------------------------------------------------------------------
function makeStatus(phase, extra = "") {
    return [
        "---",
...
describe("pauseForZoomOut / resumeFromZoomOut", () => {
    ...
    it("round-trips many concrete phases (property)", () => {
        fc.assert(fc.property(fc
            ...
            .filter((s) => s !== ZOOM_OUT_PAUSED_PHASE), (phase) => {
...
//# sourceMappingURL=zoom-out.property.test.js.map