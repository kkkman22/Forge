...
import { checkSandboxAccess, detectNetworkCommand, extractTargetFromBash, } from "../src/check-sandbox.js";
// ---------------------------------------------------------------------------
// Test fixtures
// ---------------------------------------------------------------------------
...
describe("checkSandboxAccess", () => {
    it("allows Write to permitted path", () => {
        const result = checkSandboxAccess("Write", JSON.stringify({ file_path: `${PROJECT_ROOT}/src/index.ts`, content: "" }), ACTIVE_CONFIG_PATH);
        expect(result.allowed).toBe(true);
    });
    it("denies Write to denied path", () => {
        const result = checkSandboxAccess("Write", JSON.stringify({ file_path: `${PROJECT_ROOT}/.env`, content: "" }), ACTIVE_CONFIG_PATH);
        expect(result.allowed).toBe(false);
        expect(result.reason).toContain("deny");
    });
    it("denies Write to path outside project root", () => {
        const result = checkSandboxAccess("Write", JSON.stringify({ file_path: "/etc/passwd", content: "" }), ACTIVE_CONFIG_PATH);
        expect(result.allowed).toBe(false);
    });
    it("allows Edit to permitted path", () => {
        const result = checkSandboxAccess("Edit", JSON.stringify({ file_path: `${PROJECT_ROOT}/src/test.ts` }), ACTIVE_CONFIG_PATH);
        expect(result.allowed).toBe(true);
    });
    it("allows non-network Bash commands", () => {
        const result = checkSandboxAccess("Bash", JSON.stringify({ command: "npm test" }), ACTIVE_CONFIG_PATH);
        expect(result.allowed).toBe(true);
    });
    it("denies network Bash command not in allow list", () => {
        const result = checkSandboxAccess("Bash", JSON.stringify({ command: "curl https://evil.com/api" }), ACTIVE_CONFIG_PATH);
        expect(result.allowed).toBe(false);
        expect(result.reason).toContain("evil.com");
    });
    it("allows network Bash command in allow list", () => {
        const result = checkSandboxAccess("Bash", JSON.stringify({ command: "curl https://api.anthropic.com:443/v1/messages" }), ACTIVE_CONFIG_PATH);
        expect(result.allowed).toBe(true);
    });
    it("allows all when no sandbox config file exists", () => {
        rmSync(ACTIVE_CONFIG_PATH);
        const result = checkSandboxAccess("Write", JSON.stringify({ file_path: "/anything", content: "" }), ACTIVE_CONFIG_PATH);
        expect(result.allowed).toBe(true);
    });
    ...
    it("denies when sandbox config is malformed JSON", () => {
        ...
        const result = checkSandboxAccess("Write", JSON.stringify({ file_path: `${PROJECT_ROOT}/src/test.ts` }), ACTIVE_CONFIG_PATH);
        expect(result.allowed).toBe(false);
...
//# sourceMappingURL=check-sandbox.test.js.map