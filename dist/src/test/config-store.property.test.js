...
const localeArb = fc.oneof(fc.constantFrom("zh", "en", "ja", "ko", "fr", "de", "es", "pt", "ru", "ar"), fc.stringMatching(/^[a-z]{2,3}$/).filter((s) => s.length >= 2));
 ...
 */
function buildConfigContent(otherFields, lang, body) {
    const lines = ["---"];
...
describe("Feature: i18n-support, Property 9: Config lang 字段往返与字段保留", () => {
    ...
    it("round-trip: extractConfigLang(writeConfigLang(content, lang)) === lang", () => {
        fc.assert(fc.property(otherFieldsArb, localeArb, bodyArb, (fields, lang, body) => {
    ...
    it("round-trip works on content with no frontmatter", () => {
        fc.assert(fc.property(localeArb, bodyArb, (lang, body) => {
    ...
    it("writeConfigLang preserves all other frontmatter fields", () => {
        fc.assert(fc.property(otherFieldsArb, localeArb, bodyArb, (fields, lang, body) => {
            if (fields.length === 0)
                return; // skip when no other fields to check
    ...
    it("updating existing lang preserves other fields", () => {
        fc.assert(fc.property(otherFieldsArb, localeArb, localeArb, bodyArb, (fields, oldLang, newLang, body) => {
            if (fields.length === 0)
                return;
...
const validLogFormatArb = fc.constantFrom("text", "json");
/** Valid log level values. */
const validLogLevelArb = fc.constantFrom("debug", "info", "warn", "error");
/**
...
describe("Feature: observability-enhancements, Property 4: 配置解析正确性", () => {
    ...
    it("parseLogConfig returns matching values for valid log_format, log_level, log_file", () => {
        fc.assert(fc.property(validLogFormatArb, validLogLevelArb, logFilePathArb, (format, level, filePath) => {
    ...
    it("parseLogConfig returns null for missing fields, valid values for present fields", () => {
        fc.assert(fc.property(validLogFormatArb, validLogLevelArb, (format, level) => {
    ...
    it("parseLogConfig extracts log fields correctly even with extra frontmatter fields", () => {
        fc.assert(fc.property(validLogFormatArb, validLogLevelArb, logFilePathArb, otherFieldsArb.filter((fields) => fields.every(([k]) => k !== "log_format" && k !== "log_level" && k !== "log_file")), (format, level, filePath, extraFields) => {
            const content = buildLogConfigFrontmatter({
                logFormat: format,
                logLevel: level,
                logFile: filePath,
...
describe("Feature: observability-enhancements, Property 5: CLI 参数优先于配置文件", () => {
    ...
    it("CLI logFormat overrides config file logFormat", () => {
        fc.assert(fc.property(validLogFormatArb, validLogFormatArb, validLogLevelArb, logFilePathArb, (cliFormat, fileFormat, fileLevel, fileLogFile) => {
            const fileConfig = {
    ...
    it("CLI logLevel overrides config file logLevel", () => {
        fc.assert(fc.property(validLogLevelArb, validLogFormatArb, validLogLevelArb, logFilePathArb, (cliLevel, fileFormat, fileLevel, fileLogFile) => {
            const fileConfig = {
                logFormat: fileFormat,
                logLevel: fileLevel,
                logFile: fileLogFile,
    ...
    it("CLI logFile overrides config file logFile", () => {
        fc.assert(fc.property(logFilePathArb, validLogFormatArb, validLogLevelArb, logFilePathArb, (cliLogFile, fileFormat, fileLevel, fileLogFile) => {
            const fileConfig = {
                logFormat: fileFormat,
                logLevel: fileLevel,
                logFile: fileLogFile,
    ...
    it("all CLI parameters override all config file values simultaneously", () => {
        fc.assert(fc.property(validLogFormatArb, validLogLevelArb, logFilePathArb, validLogFormatArb, validLogLevelArb, logFilePathArb, (cliFormat, cliLevel, cliLogFile, fileFormat, fileLevel, fileLogFile) => {
            const fileConfig = {
                logFormat: fileFormat,
                logLevel: fileLevel,
                logFile: fileLogFile,
            };
            const result = mergeLogConfig({ logFormat: cliFormat, logLevel: cliLevel, logFile: cliLogFile }, fileConfig);
            expect(result.format).toBe(cliFormat);
            expect(result.level).toBe(cliLevel);
            expect(result.logFile).toBe(cliLogFile);
        }), { numRuns: 50 });
...
describe("Feature: observability-enhancements, Property 6: 无效配置回退到默认值", () => {
    ...
    it("parseLogConfig returns null logFormat for invalid log_format values", () => {
        fc.assert(fc.property(invalidLogFormatArb, (invalidFormat) => {
    ...
    it("parseLogConfig returns null logLevel for invalid log_level values", () => {
        fc.assert(fc.property(invalidLogLevelArb, (invalidLevel) => {
    ...
    it("mergeLogConfig returns all defaults when both config fields are invalid and CLI is empty", () => {
        fc.assert(fc.property(invalidLogFormatArb, invalidLogLevelArb, (invalidFormat, invalidLevel) => {
            const content = buildLogConfigFrontmatter({
