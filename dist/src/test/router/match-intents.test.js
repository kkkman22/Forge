    ...
    it("never returns intents whose triggers are absent from input", () => {
        fc.assert(fc.property(fc.string({ minLength: 0, maxLength: 200 }), (text) => {
            ...
            for (const matched of result) {
                const hasTrigger = matched.triggers.some((t) => text.toLowerCase().includes(t.toLowerCase()));
                expect(hasTrigger).toBe(true);
            }
        }));
    });
    it("matches when trigger appears as standalone word in arbitrary text", () => {
        fc.assert(fc.property(fc.string({ minLength: 0, maxLength: 50 }), fc.constantFrom(...TEST_DICT.flatMap((d) => d.triggers)), fc.string({ minLength: 0, maxLength: 50 }), (prefix, trigger, suffix) => {
...
//# sourceMappingURL=match-intents.test.js.map