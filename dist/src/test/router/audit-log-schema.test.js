...
describe("Audit log schema validation (T-12)", () => {
    it("ClassificationResult hints still use RouteHint schema (no new fields)", () => {
        const result = classifyTask(BASE_SIGNALS, "standard", undefined, "backend", "iteration", "feature", "深思熟虑地实现");
        for (const hint of result.hints) {
            expect(Object.keys(hint).sort()).toEqual(expect.arrayContaining(["command", "tag", "description"]));
    ...
    it("intent hints appear alongside taskType hints in the same array", () => {
        const result = classifyTask(BASE_SIGNALS, "standard", undefined, "backend", "iteration", "feature", "深思熟虑地做后端 API");
...
//# sourceMappingURL=audit-log-schema.test.js.map