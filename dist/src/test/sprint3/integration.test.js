...
import { loadStormState, saveStormState, serializeStormMarkdown, } from "../../src/storm.js";
...
describe("Context boundary violation detection", () => {
    ...
    it("resolveFileContext maps file paths to contexts via ownership map", () => {
        expect(resolveFileContext("src/domain/reservations/aggregates/booking.ts", ownershipMap, null)).toBe("reservations");
        expect(resolveFileContext("src/domain/folio-billing/invoice.ts", ownershipMap, null)).toBe("folio-billing");
...
describe("Pack lint rules load and apply", () => {
    it("loads rules from packs/pms/lint-rules/manifest.yaml and applies money/no-number-for-money rule", () => {
        ...
        fs.writeFileSync(path.join(rulesDir, "manifest.yaml"), [
            "rules:",
...
describe("Living doc generation and rendering from temp specs", () => {
    it("generates living doc data and renders index.html with expected content", () => {
        ...
        fs.writeFileSync(path.join(specsDir, "reservations.md"), [
            "---",
        ...
        fs.writeFileSync(path.join(specsDir, "billing.md"), [
            "---",
            "feature: Billing",
            "status: draft",
            "date: 2026-05-10",
...
describe("core_subdomains triggers business-analyst", () => {
    ...
    it("getCoreSubdomains returns deduplicated union", () => {
        ...
        expect(subdomains).toEqual(expect.arrayContaining(["reservations", "folio-billing", "housekeeping"]));
