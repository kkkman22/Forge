 ...
 */
const failureSequenceArb = fc.array(summaryArb, { minLength: 1, maxLength: 10 }).map((summaries) => summaries.map((summary, i) => ({
    summary,
...
describe("Property 2: PUA state manager — success resets state", () => {
    ...
    it("handleSuccess clears all PUA fields from StatusFile after arbitrary failure sequences", () => {
        fc.assert(fc.property(failureSequenceArb, (failures) => {
    ...
    it("after handleSuccess, next handleFailure behaves identically to a fresh manager", () => {
        fc.assert(fc.property(failureSequenceArb, summaryArb, (failures, freshSummary) => {
...
describe("Property 3: PUA state manager — failure escalation monotonicity", () => {
    ...
    it("pressure level is non-decreasing for increasing consecutiveFailures", () => {
        fc.assert(fc.property(fc.integer({ min: 2, max: 10 }), fc.array(fc.string({ minLength: 1, maxLength: 50 }), { minLength: 10, maxLength: 10 }), (sequenceLength, summaryPool) => {
            const { manager, getStatusContent } = createMockManager();
    ...
    it("pressure level eventually reaches L4 and stays there for high failure counts", () => {
        fc.assert(fc.property(fc.array(fc.string({ minLength: 1, maxLength: 50 }), { minLength: 8, maxLength: 8 }), (summaries) => {
            const { manager, getStatusContent } = createMockManager();
            let reachedL4 = false;
            for (let i = 1; i <= 8; i++) {
