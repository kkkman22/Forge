...
const oldFormatAgentOutputArb = fc.oneof(fc.record({
    success: fc.boolean(),
    summary: fc.string(),
...
const validGateResultArb = fc.constantFrom("passed", "blocked", "skipped");
/** Arbitrary valid AgentOutput WITH skill fields (new format). */
const newFormatAgentOutputArb = fc.record({
    success: fc.boolean(),
    summary: fc.string(),
    key_changes_made: fc.array(fc.string()),
    key_learnings: fc.array(fc.string()),
...
describe("Feature: loop-skills-fusion, Property 11: AgentOutput 扩展向后兼容", () => {
    ...
    it("invalid gate_result values cause validation failure", () => {
        fc.assert(fc.property(fc.record({
            ...
            key_changes_made: fc.array(fc.string()),
            key_learnings: fc.array(fc.string()),
        }), invalidGateResultArb, (base, badGateResult) => {
