...
import { aggregateEvolutionMarkers, parseEvolutionMarkers, validateEvolutionTarget, } from "../src/evolution-marker.js";
// ---------------------------------------------------------------------------
// Generators
// ---------------------------------------------------------------------------
...
const targetArb = fc.oneof(skillArb, fc.tuple(skillArb, innerScalarArb).map(([s, sec]) => `${s}#${sec}`));
// ---------------------------------------------------------------------------
...
describe("evolution-marker — validateEvolutionTarget", () => {
    it("returns orphan=true for any target whose base name is not in the registry", () => {
        fc.assert(fc.property(innerScalarArb.filter((s) => !SKILLS_REGISTRY.includes(s) && !s.includes("#")), (unknownSkill) => {
...
describe("evolution-marker — aggregateEvolutionMarkers", () => {
    ...
    it("is deterministic for the same input", () => {
        fc.assert(fc.property(fc.array(fc.tuple(dateArb, innerScalarArb, targetArb, innerScalarArb), {
            minLength: 0,
            maxLength: 12,
        }), (rows) => {
...
//# sourceMappingURL=evolution-marker.property.test.js.map