...
describe("resolveConflicts", () => {
    it("resolves all guarded conflicts automatically", async () => {
        ...
        const result = await resolveConflicts([".forge/progress/auth.md", ".forge/reviews/auth.md"], "autonomous", {
            statusContent: "current_task: auth\n",
            repoRoot: "/tmp/test",
            readFileContent: async (_p) => "- [ ] task-a: Do A",
    ...
    it("handles mixed zones: guarded resolved, frozen refused, source skipped", async () => {
        const { resolveConflicts } = await import("../src/conflict-resolver.js");
        const result = await resolveConflicts([".forge/progress/auth.md", ".forge/specs/auth/spec.md", "src/main.ts"], "autonomous", {
            statusContent: "current_task: auth\n",
            repoRoot: "/tmp/test",
...
describe("applyGuardedMerge", () => {
    ...
    it("merges instincts by confidence=max, count=sum", async () => {
        const { applyGuardedMerge } = await import("../src/conflict-resolver.js");
        const result = applyGuardedMerge("known-failures", "p1: confidence=0.5 count=3 | Text", "p1: confidence=0.8 count=2 | Text");
    ...
    it("merges reviews by append + sort", async () => {
        const { applyGuardedMerge } = await import("../src/conflict-resolver.js");
        const result = applyGuardedMerge("reviews", "[quality][P2] src/a.ts: Issue", "[security][P0] src/b.ts: Issue");
