...
import { hasMarkdownExtension, hasYamlFrontmatter, validateStateFile, } from "../src/state.js";
// ---------------------------------------------------------------------------
// Generators
// ---------------------------------------------------------------------------
...
const validFrontmatterContentArb = fc
    .tuple(fc
    .string({ minLength: 1, maxLength: 20 })
    .filter((s) => s.trim().length > 0 && !s.includes("---")), fc.string({ minLength: 0, maxLength: 50 }).filter((s) => !s.includes("---")))
    .map(([key, value]) => `---\n${key}: ${value}\n---\n`);
...
const invalidExtensionPathArb = fc
    .tuple(fc.constantFrom(...forgeSubdirs), fc.constantFrom(".txt", ".json", ".yaml", ".yml", ".html", ".xml"))
    .map(([base, ext]) => `${base}${ext}`);
/** Content without YAML frontmatter. */
const noFrontmatterContentArb = fc
    .string({ minLength: 1, maxLength: 100 })
    .filter((s) => !s.trimStart().startsWith("---"));
/** A state file with invalid extension (not .md). */
const invalidExtensionFileArb = fc
...
const dynamicValidPathArb = fc
    .tuple(fc.constantFrom("decisions/", "specs/", "plans/", "findings/", "progress/", "reviews/", "knowledge/solutions/", "debug/"), topicArb)
    .map(([dir, topic]) => `${dir}${topic}.md`);
/** A dynamically generated valid state file. */
const dynamicValidStateFileArb = fc
