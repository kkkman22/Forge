...
function writeToolInputFile(tempDir, filePath, content) {
    const toolInput = {
        file_path: filePath,
        content,
    };
...
function writeOwnershipMap(tempDir, map) {
...
function writeContextMap(tempDir, edges) {
...
function runHook(tempDir, toolInputPath) {
    try {
...
describe("check-context-boundary.mjs hook", () => {
    ...
    it("allows import within the same context (exit 0)", () => {
        ...
        const inputPath = writeToolInputFile(tempDir, "src/domain/reservation/services/booking.ts", 'import { BookingRepo } from "./booking-repo";\n');
    ...
    it("allows import with declared partnership relationship (exit 0)", () => {
        ...
        const inputPath = writeToolInputFile(tempDir, "src/domain/reservation/services/booking.ts", 'import { GuestInfo } from "../../guest/models/guest";\n');
        const result = runHook(tempDir, inputPath);
        expect(result.exitCode).toBe(0);
    });
    ...
    it("blocks import with undeclared cross-context dependency (exit 1)", () => {
        ...
        const inputPath = writeToolInputFile(tempDir, "src/domain/reservation/services/booking.ts", 'import { Invoice } from "../../billing/models/invoice";\n');
        const result = runHook(tempDir, inputPath);
    ...
    it("blocks import with customer-supplier relationship (exit 1)", () => {
        ...
        const inputPath = writeToolInputFile(tempDir, "src/domain/reservation/services/booking.ts", 'import { BillingService } from "../../billing/service";\n');
        const result = runHook(tempDir, inputPath);
        expect(result.exitCode).toBe(1);
    ...
    it("allows cross-context import with escape hatch comment (exit 0)", () => {
        ...
        const inputPath = writeToolInputFile(tempDir, "src/domain/reservation/services/booking.ts", [
    ...
    it("skips files outside src/ directory (exit 0)", () => {
        ...
        const inputPath = writeToolInputFile(tempDir, "test/some-test.ts", 'import { Invoice } from "../src/domain/billing/models/invoice";\n');
        const result = runHook(tempDir, inputPath);
        expect(result.exitCode).toBe(0);
    });
    // --- Non-TypeScript file ---
    it("skips non-TypeScript files (exit 0)", () => {
        ...
        const inputPath = writeToolInputFile(tempDir, "src/domain/reservation/config.yaml", "key: value\n");
        const result = runHook(tempDir, inputPath);
        expect(result.exitCode).toBe(0);
    });
    ...
    it("allows package (non-relative) imports (exit 0)", () => {
        ...
        const inputPath = writeToolInputFile(tempDir, "src/domain/reservation/services/booking.ts", ['import { z } from "zod";', 'import * as fs from "fs";', ""].join("\n"));
        const result = runHook(tempDir, inputPath);
        expect(result.exitCode).toBe(0);
    });
    // --- Edit tool ---
    ...
    it("allows import when file is not in any defined context (exit 0)", () => {
        ...
        const inputPath = writeToolInputFile(tempDir, "src/utilities/helper.ts", 'import { Invoice } from "../domain/billing/models/invoice";\n');
        const result = runHook(tempDir, inputPath);
        expect(result.exitCode).toBe(0);
    });
