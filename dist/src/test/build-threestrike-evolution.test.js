...
describe("buildThreeStrikeFailureArtifacts — Requirement 8.6", () => {
    it("produces a failure Episode with forge-build trigger metadata", () => {
        const { episode, markerText } = buildThreeStrikeFailureArtifacts("widget-refactor", "standard", "连续三次 TDD 失败于同一任务", undefined, FIXED_NOW, 1);
    ...
    it("attaches the root cause when provided", () => {
        const { episode } = buildThreeStrikeFailureArtifacts("widget-refactor", "full", "三次连续失败", "任务拆得太粗", FIXED_NOW, 2);
    ...
    it("omits root_cause when not provided", () => {
        const { episode } = buildThreeStrikeFailureArtifacts("widget-refactor", "light", "三次连续失败", undefined, FIXED_NOW, 3);
    ...
    it("is deterministic under identical inputs", () => {
        const a = buildThreeStrikeFailureArtifacts("deterministic", "standard", "三次失败", "cause", FIXED_NOW, 5);
        const b = buildThreeStrikeFailureArtifacts("deterministic", "standard", "三次失败", "cause", FIXED_NOW, 5);
        expect(a.episode).toEqual(b.episode);
        expect(a.markerText).toBe(b.markerText);
    });
});
//# sourceMappingURL=build-threestrike-evolution.test.js.map