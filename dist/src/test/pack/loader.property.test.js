...
function createMockFs(files) {
    return {
        ...
        writeFile: async () => { },
        exists: async (f) => f in files && files[f] !== null,
...
const kebabArb = fc
    .tuple(fc.constantFrom("a", "b", "c", "demo", "pack", "core", "my", "test"), fc.option(fc.constantFrom("svc", "lib", "mod", "ext", "base"), { nil: undefined }))
...
describe("loadPackRegistry property: idempotence", () => {
    it("same inputs always produce identical registries (serialized to JSON)", async () => {
        await fc.assert(fc.asyncProperty(validPacksFileMapArb, async (files) => {
...
describe("loadPackRegistry property: no crash", () => {
    it("random string YAML content never throws, only produces warnings", async () => {
        ...
        await fc.assert(fc.asyncProperty(fc.array(randomYamlArb, { minLength: 0, maxLength: 5 }), async (entries) => {
            const files = {};
...
describe("loadPackRegistry property: empty input stability", () => {
    ...
    it("packs directory with subdirs but no pack.yaml returns empty registry", async () => {
        ...
        await fc.assert(fc.asyncProperty(dirNamesArb, async (dirNames) => {
...
describe("validateManifest property", () => {
    it("never throws for any arbitrary object input", () => {
        const arbitraryObjArb = fc.dictionary(fc.string({ minLength: 1, maxLength: 20 }), fc.anything());
        fc.assert(fc.property(arbitraryObjArb, (obj) => {
...
//# sourceMappingURL=loader.property.test.js.map