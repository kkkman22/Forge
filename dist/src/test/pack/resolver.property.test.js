...
const arbPackName = fc
    .tuple(fc.constantFrom("a", "b", "alpha", "beta", "demo", "hotel", "core", "pms"), fc.constantFrom("ops", "svc", "lib", "ext", "base"))
    .map(([a, b]) => `${a}-${b}`);
...
describe("resolvePath properties", () => {
    ...
    it("custom always wins over pack for any safe path", () => {
        fc.assert(fc.property(arbSafeRelativePath, arbUniquePackEntries, (relPath, enabled) => {
            if (enabled.entries.length === 0)
                return; // need at least one pack to compare
    ...
    it("path traversal attempt always returns null (Unix-style)", () => {
        ...
        fc.assert(fc.property(fc.constantFrom(...traversalPaths), arbUniquePackEntries, (traversalPath, enabled) => {
...
//# sourceMappingURL=resolver.property.test.js.map