...
import { buildIterationPrompt, buildSkillAwarePrompt, formatIterationEntry, formatListSection, formatNotesDocument, parseListSection, parseNotesDocument, } from "../src/context-accumulator.js";
// ---------------------------------------------------------------------------
// Generators
// ---------------------------------------------------------------------------
/**
...
describe("Feature: gnhf-inspired-enhancements, Property 1: Prompt 构建完整性", () => {
    ...
    it("prompt contains iteration number, runId, notes content, and objective text", () => {
        fc.assert(fc.property(iterationNumberArb, runIdArb, objectiveArb, notesContentArb, (iteration, runId, objective, notesContent) => {
    ...
    it("prompt includes stop condition section when stopWhen is provided", () => {
        fc.assert(fc.property(iterationNumberArb, runIdArb, objectiveArb, notesContentArb, cleanLineArb, (iteration, runId, objective, notesContent, stopWhen) => {
            const prompt = buildIterationPrompt({
                iteration,
                runId,
                objective,
...
describe("Feature: gnhf-inspired-enhancements, Property 7: Notes 条目格式化", () => {
    ...
    it("formatted entry contains ### Iteration N header and summary text", () => {
        fc.assert(fc.property(iterationEntryArb, (entry) => {
    ...
    it("successful entries contain all key_changes items", () => {
        fc.assert(fc.property(iterationEntryArb.filter((e) => e.success && e.keyChanges.length > 0), (entry) => {
            const md = formatIterationEntry(entry);
    ...
    it("entries contain all key_learnings items when non-empty", () => {
        fc.assert(fc.property(iterationEntryArb.filter((e) => e.keyLearnings.length > 0), (entry) => {
            const md = formatIterationEntry(entry);
 ...
 */
const regexSpecialTitleArb = fc
    .tuple(fc
    .array(fc.constantFrom(...REGEX_SPECIAL_CHARS), { minLength: 1, maxLength: 4 })
    .map((a) => a.join("")), cleanLineArb, fc.integer({ min: 0, max: 80 }))
...
describe("Feature: v2-2-legacy-fixes, Property 1: parseListSection regex special character round-trip", () => {
    ...
    it("round-trips arbitrary titles with regex special characters through format → parse", () => {
        fc.assert(fc.property(regexSpecialTitleArb, fc.array(cleanLineArb, { minLength: 1, maxLength: 8 }), (title, items) => {
...
describe("Feature: v2-2-legacy-fixes, Property 2: parseListSection returns empty array for non-matching titles with special characters", () => {
    ...
    it("returns empty array when block does not contain the generated title as a bold section header", () => {
        fc.assert(fc.property(regexSpecialTitleArb, cleanLineArb, fc.array(cleanLineArb, { minLength: 1, maxLength: 5 }), (title, otherTitle, items) => {
...
const phaseArb = fc.constantFrom("router", "plan", "build", "review", "test", "ship", "learn", "refactor-scan", "refactor-apply", "fix-analyze", "fix-apply");
/** Tier string. */
...
const skillPromptParamsArb = fc
    .tuple(baseParamsArb, phaseArb, tierArb, fc.option(cleanLineArb, { nil: undefined }), fc.option(cleanLineArb, { nil: undefined }), fc.option(cleanLineArb, { nil: undefined }), fc.option(fc.array(hintArb, { minLength: 0, maxLength: 3 }), { nil: undefined }), fc.option(fc.array(fixIssueArb, { minLength: 0, maxLength: 4 }), { nil: undefined }))
...
const skillPromptParamsWithFixIssuesArb = fc
...
describe("Feature: loop-skills-fusion, Property 10: buildSkillAwarePrompt content completeness", () => {
    ...
    it("all fixIssue descriptions appear in the output when fixIssues are provided", () => {
        fc.assert(fc.property(skillPromptParamsWithFixIssuesArb, (params) => {
            const output = buildSkillAwarePrompt(params);
