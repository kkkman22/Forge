...
import { checkSkillLength, countEffectiveLines, DEFAULT_LIMIT, validateAllSkillLengths, } from "../src/skill-length.js";
// ---------------------------------------------------------------------------
...
describe("countEffectiveLines (property)", () => {
    ...
    it("is invariant under inserting blank separator lines", () => {
        ...
        fc
            ...
            .filter((s) => !s.includes("\n") && s.trim() !== ""), { minLength: 1, maxLength: 10 })
            .chain((realLines) => fc
            .array(fc.integer({ min: 0, max: 3 }), {
            minLength: realLines.length + 1,
            maxLength: realLines.length + 1,
        })
            .map((blankCounts) => ({ realLines, blankCounts }))), ({ realLines, blankCounts }) => {
...
//# sourceMappingURL=skill-length.property.test.js.map