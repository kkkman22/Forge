...
describe("Branch Gate PBT", () => {
    it("alreadyCheckedThisPhase=true always returns skipped", () => {
        fc.assert(fc.property(branchSkillArb, branchNameArb, fc.string({ minLength: 1 }), (skill, branch, task) => {
    ...
    it("currentTask=null always returns skipped", () => {
        fc.assert(fc.property(branchSkillArb, branchNameArb, (skill, branch) => {
            const result = runBranchGate(baseInput({
                skill,
                currentBranch: branch,
                currentTask: null,
    ...
    it("main/master branch always passes regardless of other inputs", () => {
        fc.assert(fc.property(branchSkillArb, fc.constantFrom("autonomous", "interactive"), (skill, mode) => {
            const result = runBranchGate(baseInput({
                skill,
                mode: mode,
                currentBranch: "main",
    ...
    it("auto_fixed only when autonomous + clean tree + valid format + topic mismatch", () => {
        fc.assert(fc.property(branchSkillArb, fc.string({ minLength: 1, maxLength: 20 }).filter((t) => t !== "target-task"), (skill, otherTopic) => {
            ...
            if (resultClean.kind !== "auto_fixed")
                return false;
...
//# sourceMappingURL=branch-gate.property.test.js.map