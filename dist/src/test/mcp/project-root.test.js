...
describe("logResolvedRoot", () => {
    it("logs env source to stderr", () => {
        ...
        expect(spy).toHaveBeenCalledWith(expect.stringContaining("[forge-context] resolved project root: /abs/project (env)"));
        spy.mockRestore();
    });
    it("logs cwd fallback to stderr", () => {
        const spy = vi.spyOn(console, "error").mockImplementation(() => { });
        logResolvedRoot({ path: "/some/cwd", source: "cwd" });
        expect(spy).toHaveBeenCalledWith(expect.stringContaining("[forge-context] resolved project root: /some/cwd (cwd fallback)"));
        spy.mockRestore();
    });
});
