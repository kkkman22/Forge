...
import { decideOutputTarget, evaluateAdrCriteria, renderCriteriaCheck, } from "../src/adr-criteria.js";
// ---------------------------------------------------------------------------
// Generators
// ---------------------------------------------------------------------------
/**
 ...
 */
const neutralDecisionArb = fc.record({
...
const signalsArb = fc.record({
    reversalCostAssessment: fc.constantFrom("low", "medium", "high"),
...
describe("evaluateAdrCriteria — property-based", () => {
    ...
    it("any 'no' on the three questions blocks shouldBecomeAdr", () => {
        fc.assert(fc.property(neutralDecisionArb, signalsArb, (decision, signals) => {
    ...
    it("empty alternatives forces tradeOff=false", () => {
        fc.assert(fc.property(signalsArb, (signals) => {
            const decision = {
                title: "t",
                context: "ctx",
                decision: "adopt X",
...
describe("evaluateAdrCriteria — unit", () => {
    ...
    it("reversalCostAssessment=low maps to reversibility=soft", () => {
        const result = evaluateAdrCriteria({ title: "t", context: "c", decision: "d", consequences: "" }, { reversalCostAssessment: "low", hasExplicitTradeoff: false, inferFromKeywords: false });
        expect(result.reversibility).toBe("soft");
    });
    it("reversalCostAssessment=medium maps to reversibility=hard", () => {
        const result = evaluateAdrCriteria({ title: "t", context: "c", decision: "d", consequences: "" }, { reversalCostAssessment: "medium", hasExplicitTradeoff: false, inferFromKeywords: false });
...
describe("decideOutputTarget", () => {
    function resultWithVerdict(verdict) {
        return {
...
//# sourceMappingURL=adr-criteria.property.test.js.map