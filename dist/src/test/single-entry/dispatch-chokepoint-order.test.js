...
describe("R5.2: chokepoint orchestrator executes steps 1-9 in order", () => {
    it("steps execute in order: resolveMode → validateTopic → resolvePath → checkIntegrity → resolveTools → resolveDispatchMode → wrapContext → dispatch → auditLog", async () => {
        const callOrder = [];
        const mocks = {
            resolveDispatcherMode: vi.fn(() => {
                callOrder.push("resolveDispatcherMode");
                return "collapsed";
            }),
            validateTopic: vi.fn(() => {
                callOrder.push("validateTopic");
                return { ok: true, value: "build" };
            }),
            resolveLibPath: vi.fn(() => {
                callOrder.push("resolveLibPath");
                return { ok: true, path: "/mock/build/instructions.md" };
            }),
            checkIntegrity: vi.fn(() => {
                callOrder.push("checkIntegrity");
                return { ok: true };
            }),
            resolveAllowedTools: vi.fn(() => {
                callOrder.push("resolveAllowedTools");
                return { ok: true, tools: ["Read"] };
            }),
            resolveDispatchMode: vi.fn(() => {
                callOrder.push("resolveDispatchMode");
                return "fork";
            }),
            wrapWorkspaceContext: vi.fn(() => {
                callOrder.push("wrapWorkspaceContext");
                return "";
            }),
            dispatch: vi.fn(() => {
    ...
    it("step 2 reject stops pipeline (steps 3-9 not called)", async () => {
        ...
        const mocks = {
            ...
            validateTopic: vi.fn(() => {
                callOrder.push("validateTopic");
                return { ok: false, code: "E_UNKNOWN_SUB" };
            }),
            resolveLibPath: vi.fn(() => {
                callOrder.push("resolveLibPath");
            }),
...
//# sourceMappingURL=dispatch-chokepoint-order.test.js.map