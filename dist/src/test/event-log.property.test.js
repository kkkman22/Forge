...
import { buildEntry, hashState, parseEventLog, replay, serializeEntry, stableStringify, } from "../src/event-log.js";
import { createInitialState, transition } from "../src/orchestrator.js";
// ---------------------------------------------------------------------------
// Arbitraries
// ---------------------------------------------------------------------------
const eventArb = fc.oneof(fc.constant({ type: "start", limits: {} }), fc.record({
...
}), fc.constant({ type: "backoff_elapsed" }));
const eventSequenceArb = fc.array(eventArb, { minLength: 1, maxLength: 12 });
// ---------------------------------------------------------------------------
// Helper
// ---------------------------------------------------------------------------
function buildLog(initial, events) {
    let state = initial;
    const entries = [];
    for (let i = 0; i < events.length; i += 1) {
        const before = state;
        const { state: after, effects } = transition(before, events[i]);
        entries.push(buildEntry("run-prop", i, events[i], before, after, effects, `2026-05-06T00:00:${String(i).padStart(2, "0")}.000Z`));
        state = after;
    }
...
//# sourceMappingURL=event-log.property.test.js.map