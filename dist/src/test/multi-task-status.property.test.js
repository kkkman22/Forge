...
import { hasTaskName, parseStatusEntries, removeTaskEntry, serializeStatusEntries, upsertTaskEntry, } from "../src/state.js";
const safeString = (min, max) => fc
    .string({ minLength: min, maxLength: max })
    .filter((s) => s.trim().length > 0 && !s.includes('"') && !s.includes("\n") && !s.includes("#"));
const taskEntryArb = fc.record({
...
describe("Feature: forge-review-fix-optimization, Property 21: Multi-task status round-trip", () => {
    it("serializeStatusEntries → parseStatusEntries produces equivalent entries", () => {
        fc.assert(fc.property(fc.array(taskEntryArb, { maxLength: 10 }), (entries) => {
...
describe("Feature: forge-review-fix-optimization, Property 22: Multi-task upsert preserves other entries", () => {
    it("new entry is added and existing is updated while preserving others", () => {
        fc.assert(fc.property(fc.array(taskEntryArb, { maxLength: 5 }).map((arr) => {
            const seen = new Set();
            return arr.filter((e) => {
                if (seen.has(e.taskName))
                    return false;
                seen.add(e.taskName);
                return true;
            });
        }), taskEntryArb, (entries, newEntry) => {
...
describe("Feature: forge-review-fix-optimization, Property 23: Multi-task remove preserves other entries", () => {
    it("removing an entry preserves all others", () => {
        fc.assert(fc.property(fc.array(taskEntryArb, { minLength: 2, maxLength: 5 }).map((arr) => {
            const seen = new Set();
            return arr.filter((e) => {
                if (seen.has(e.taskName))
                    return false;
...
describe("Feature: forge-review-fix-optimization, Property 24: Multi-task conflict detection", () => {
    it("detectConflict returns true iff taskName exists in entries", () => {
        ...
        }), fc.string({ minLength: 1, maxLength: 30 }), (entries, taskName) => {
...
//# sourceMappingURL=multi-task-status.property.test.js.map