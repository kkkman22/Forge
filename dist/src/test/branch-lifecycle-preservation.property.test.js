...
import { checkBuildGate } from "../src/build.js";
import { BranchValidationError, buildBranchDeleteCommand, buildCheckoutCommand, buildMergeCommand, buildPushCommand, sanitizeBranchName, validateBranchName, } from "../src/git-transaction.js";
// ---------------------------------------------------------------------------
// Generators
// ---------------------------------------------------------------------------
...
const remoteArb = fc.constantFrom("origin", "upstream");
...
const illegalBranchCharArb = fc
    .tuple(fc.string({ minLength: 0, maxLength: 10 }), fc.constantFrom("~", "^", "*", "[", ":", "?", "\\", " ", "{", "}"), fc.string({ minLength: 0, maxLength: 10 }))
...
describe("Property 2: sanitizeBranchName preservation (Req 3.2)", () => {
    ...
    it("strips @{ reflog syntax", () => {
        fc.assert(fc.property(fc.tuple(fc
            ...
            .map((chars) => chars.join("")), fc.constant("@{")), ([prefix, reflog]) => {
...
describe("Property 4: Branch name validation preservation (Req 3.5, 3.6)", () => {
    ...
    it("BranchValidationError has code BRANCH_VALIDATION_ERROR", () => {
        try {
            ...
            expect(e.code).toBe("BRANCH_VALIDATION_ERROR");
        }
    });
});
//# sourceMappingURL=branch-lifecycle-preservation.property.test.js.map