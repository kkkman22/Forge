...
import { deserializeExploreSummary, deserializeGitDiff, deserializeGitStatus, deserializeReviewSummary, deserializeSubagentSummary, deserializeTestOutput, serializeExploreSummary, serializeGitDiff, serializeGitStatus, serializeReviewSummary, serializeSubagentSummary, serializeTestOutput, } from "../src/context-budget.js";
// ---------------------------------------------------------------------------
...
const exploreSummaryArb = fc.record({
    entryPoints: fc.array(entryPointArb, { maxLength: 4 }),
    dependencyChain: fc.array(fc.string({ minLength: 1, maxLength: 30 }).filter((s) => s.trim().length > 0), { maxLength: 6 }),
    relatedTests: fc.array(fc.record({
        filePath: pathArb,
        testCount: fc.integer({ min: 0, max: 100 }),
    }), { maxLength: 5 }),
    keyInterfaces: fc.array(fc.record({
        name: fc.string({ minLength: 1, maxLength: 40 }).filter((s) => /^[a-zA-Z_]\w*$/.test(s)),
        filePath: pathArb,
        line: fc.integer({ min: 1, max: 9999 }),
    }), { maxLength: 4 }),
    fileGroups: fc.array(fc.record({
        moduleName: fc
            .string({ minLength: 1, maxLength: 30 })
            .filter((s) => s === s.trim() && !s.includes(", ") && !s.includes("（") && !s.includes("）")),
        fileCount: fc.integer({ min: 1, max: 50 }),
    }), { maxLength: 5 }),
});
const reviewSummaryArb = fc.record({
    filePath: pathArb,
...
const gitDiffSummaryArb = fc.record({
    fileCount: fc.integer({ min: 1, max: 50 }),
...
const gitStatusSummaryArb = fc.record({
    staged: fc.record({
...
const subagentSummaryArb = fc
    .tuple(fc.constantFrom("DONE", "DONE_WITH_CONCERNS", "NEEDS_CONTEXT", "BLOCKED"), fc.string({ minLength: 1, maxLength: 100 }), fc.array(pathArb, { maxLength: 5 }), fc.record({
    passed: fc.integer({ min: 0, max: 500 }),
    failed: fc.integer({ min: 0, max: 50 }),
}), fc.string({ minLength: 1, maxLength: 100 }), fc.string({ minLength: 1, maxLength: 100 }), fc.option(fc.string({ minLength: 1, maxLength: 100 })), fc.option(fc.array(fc.string({ minLength: 1, maxLength: 80 }), { maxLength: 3 })))
    .map(([status, taskDescription, changedFiles, testResult, commitMessage, selfCheckResults, blockingReason, concerns,]) => {
    const result = {
        status,
        taskDescription,
        changedFiles,
...
describe("Feature: context-budget-management, Property 7: Subagent summary round-trip", () => {
    it("serialize then deserialize yields semantically equivalent object", () => {
        fc.assert(fc.property(subagentSummaryArb, (original) => {
...
describe("Feature: context-budget-management, Property 8: Explore summary round-trip", () => {
    it("serialize then deserialize yields semantically equivalent object", () => {
        fc.assert(fc.property(exploreSummaryArb, (original) => {
...
describe("Feature: context-budget-management, Property 9: Review summary round-trip", () => {
    it("serialize then deserialize yields semantically equivalent object", () => {
        fc.assert(fc.property(reviewSummaryArb, (original) => {
...
describe("Feature: context-budget-management, Property 10: Test output summary round-trip", () => {
    it("serialize then deserialize yields semantically equivalent object", () => {
        fc.assert(fc.property(testOutputSummaryArb, (original) => {
...
describe("Feature: context-budget-management, Property 11: Git output summary round-trip", () => {
    it("GitDiffSummary round-trip", () => {
        fc.assert(fc.property(gitDiffSummaryArb, (original) => {
    ...
    it("GitStatusSummary round-trip", () => {
        fc.assert(fc.property(gitStatusSummaryArb, (original) => {
...
//# sourceMappingURL=context-budget-roundtrip.property.test.js.map