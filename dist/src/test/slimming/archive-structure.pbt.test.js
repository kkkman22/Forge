...
function simulateArchive(files, slug, date) {
...
describe("Property 6: Archive Structure Preservation", () => {
    it("archived tree has same relative paths under archive prefix", () => {
        fc.assert(fc.property(fc.string({ minLength: 1 }).filter((s) => /^[a-z-]+$/.test(s)), fc.record({ date: fc.stringMatching(/^\d{4}-\d{2}-\d{2}$/) }), fc.array(fc.record({
            relativePath: fc.stringMatching(/^[a-z0-9/_.-]+$/),
            content: fc.string({ minLength: 1 }),
        }), { minLength: 1, maxLength: 10 }), (slug, { date }, files) => {
    ...
    it("cross-reference rewrite maps to archived location", () => {
        fc.assert(fc.property(fc.string({ minLength: 1 }).filter((s) => /^[a-z-]+$/.test(s)), fc.record({ date: fc.stringMatching(/^\d{4}-\d{2}-\d{2}$/) }), fc.array(fc.record({ relativePath: fc.stringMatching(/^[a-z0-9/_.-]+$/), content: fc.string() }), {
            minLength: 1,
            maxLength: 5,
        }), (slug, { date }, files) => {
...
//# sourceMappingURL=archive-structure.pbt.test.js.map