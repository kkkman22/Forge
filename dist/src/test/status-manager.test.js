...
import { archiveTaskStatus, getMostRecentActiveTask, listActiveTasks, migrateToMultiTask, readTaskStatus, writeTaskStatus, } from "../src/status-manager.js";
// ---------------------------------------------------------------------------
// In-memory IO implementation for testing
// ---------------------------------------------------------------------------
function createInMemoryIO(files = {}) {
    const store = { ...files };
    return {
        exists: (p) => p in store,
...
function makeStatusContent(task, phase, tier = "standard", updated = "2026-04-30") {
    return `---\ncurrent_task: "${task}"\ntier: "${tier}"\nphase: "${phase}"\nupdated: "${updated}"\n---\n`;
}
function _makeTaskFile(taskName, phase) {
