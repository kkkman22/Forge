...
describe("parseReviewConfig", () => {
    ...
    it.each([1, 5, 10])("should parse review.subagent_concurrency: %i", (n) => {
        ...
        expect(result).toEqual({ subagent_concurrency: n });
    });
    it.each([
        0, 11, -1,
    ...
    it("FORGE_REVIEW_CONCURRENCY env overrides config.md", () => {
        ...
        try {
            ...
            expect(result).toEqual({ subagent_concurrency: 7 });
        }
        finally {
    ...
    it("invalid env falls through to config.md", () => {
        ...
        try {
            const result = parseReviewConfig("review.subagent_concurrency: 5\n");
            expect(result).toEqual({ subagent_concurrency: 5 });
            expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("FORGE_REVIEW_CONCURRENCY invalid"));
        }
        finally {
            warnSpy.mockRestore();
            if (original === undefined) {
...
//# sourceMappingURL=subagent-concurrency-config.test.js.map