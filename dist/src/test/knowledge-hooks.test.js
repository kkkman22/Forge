...
describe("knowledge-hooks pure scheduling", () => {
    describe("hashEvent", () => {
        it("produces deterministic hash for same event", () => {
            const event = { kind: "adr_written", path: ".forge/decisions/ADR-0042.md" };
            expect(hashEvent(event)).toBe(hashEvent(event));
        });
        it("produces different hash for different events", () => {
            const a = { kind: "adr_written", path: ".forge/decisions/ADR-0042.md" };
            const b = { kind: "adr_written", path: ".forge/decisions/ADR-0043.md" };
            expect(hashEvent(a)).not.toBe(hashEvent(b));
        });
        it("produces different hash for same path but different kind", () => {
            const a = { kind: "adr_written", path: "x" };
            const b = { kind: "solution_written", topic: "x", path: "x" };
            expect(hashEvent(a)).not.toBe(hashEvent(b));
        });
    });
...
//# sourceMappingURL=knowledge-hooks.test.js.map