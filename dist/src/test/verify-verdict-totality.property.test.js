...
// Generators
// ---------------------------------------------------------------------------
const VALID_VERDICTS = ["VERIFIED", "NOT_VERIFIED", "INCONCLUSIVE"];
...
const corruptedContent = fc.oneof(fc.constant(""), fc.constant("---\nverdict: INVALID\n---"), fc.constant("---\nmissing_fields: true\n---"), fc.constant("not yaml at all <<<<>>>>"), fc.constant("<script>alert('xss')</script>"), fc.constant("---\nverdict: null\n---"), fc.constant("---\nverdict: []\n---"), fc.constant("\x00\x01\x02\x03"), fc.constant('---\nverdict: "VERIFIED\nmulti\nline"\n---'), fc.constant("---\nverdict: maybe_verified\n---"), fc.constant("---\nverdict: 42\n---"));
// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------
describe("parseVerdict totality property [R13.3]", () => {
    it("returns a valid verdict for any string input (200 iterations)", () => {
        const validSet = new Set(VALID_VERDICTS);
        fc.assert(fc.property(fc.string({ maxLength: 2000 }), (content) => {
...
//# sourceMappingURL=verify-verdict-totality.property.test.js.map