...
import { checkBranchTopicGate, checkCommitTopicMatch, detectStaleBranches, detectUnshippedBranches, extractBranchTopic, recordPendingDelivery, } from "../src/branch-lifecycle.js";
// ---------------------------------------------------------------------------
// Generators — Shared
// ---------------------------------------------------------------------------
 ...
 */
const matchedTopicPairArb = branchNameArb.chain((branchName) => {
 ...
 */
const mismatchedTopicPairArb = fc
 ...
 */
const matchedCommitPairArb = matchedTopicPairArb.map(({ branchName, taskTopic }) => ({
    branchName,
 ...
 */
const mismatchedCommitPairArb = mismatchedTopicPairArb.map(({ branchName, taskTopic }) => ({
    branchName,
    commitTopic: taskTopic,
}));
// ---------------------------------------------------------------------------
...
describe("Property 1: Topic Mismatch Detection", () => {
    it("extractBranchTopic returns correct topic for valid feature/<topic> branches", () => {
        fc.assert(fc.property(branchNameArb, (branchName) => {
    ...
    it("checkBranchTopicGate extracts nested topic from feature/foo/bar as 'foo/bar'", () => {
        ...
        const result = checkBranchTopicGate(nestedBranch, "foo/bar");
        expect(result.allowed).toBe(true);
        expect(result.reasons).toHaveLength(0);
    });
    it("checkBranchTopicGate blocks when nested topic does not match task topic", () => {
        const nestedBranch = "feature/foo/bar";
        const result = checkBranchTopicGate(nestedBranch, "baz");
        expect(result.allowed).toBe(false);
        expect(result.reasons.length).toBeGreaterThan(0);
    });
});
...
describe("Property 4: Stale Branch Detection", () => {
    it("detectStaleBranches returns exactly records with different topic and older timestamp", () => {
        fc.assert(fc.property(topicArb, topicArb, timestampArb, fc.integer({ min: 1, max: 100000 }), (currentTopic, otherTopic, now, delta) => {
    ...
    it("detectStaleBranches returns all records when all are stale", () => {
        fc.assert(fc.property(topicArb, fc
            ...
            .filter((topics) => topics.every((t) => t !== "current-topic")), timestampArb, fc.integer({ min: 60001, max: 1000000 }), (_currentTopic, otherTopics, now, delta) => {
            const currentTopic = "current-topic";
            const threshold = 60000;
            const records = otherTopics.map((topic, i) => ({
...
describe("Property 5: Cross-Topic Commit Prevention", () => {
    ...
    it("checkCommitTopicMatch with nested slash topic does not match 'a'", () => {
        const result = checkCommitTopicMatch("feature/a/b", "a");
        expect(result.allowed).toBe(false);
    });
});
// ---------------------------------------------------------------------------
...
describe("Property 6: Unshipped Branch Detection", () => {
    it("detectUnshippedBranches returns warnings for different-topic records", () => {
        fc.assert(fc.property(fc.array(fc.tuple(branchNameArb, topicArb, timestampArb), { minLength: 1, maxLength: 10 }), topicArb, (records, currentTopic) => {
            fc.pre(records.some(([, topic]) => topic !== currentTopic));
            const pendingDeliveries = records.map(([branchName, topic, timestamp]) => ({ branchName, topic, timestamp }));
...
//# sourceMappingURL=branch-lifecycle-bug-condition.property.test.js.map