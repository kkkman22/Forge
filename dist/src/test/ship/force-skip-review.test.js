...
import { checkShipGateWithForceSkip, recordForceSkip } from "../../src/ship.js";
...
describe("checkShipGateWithForceSkip", () => {
    ...
    it("--force-skip-review without reason throws", () => {
        const options = { forceSkipReview: true };
        const review = { passed: true, p0Count: 0, p1Count: 0 };
        const test = { passed: true };
        const progress = { totalTasks: 5, completedTasks: 5 };
        expect(() => checkShipGateWithForceSkip(review, test, progress, options)).toThrow("--force-skip-review requires --reason='<non-empty>'");
    });
    it("--force-skip-review with empty reason throws", () => {
        const options = { forceSkipReview: true, forceSkipReason: "   " };
        const review = { passed: true, p0Count: 0, p1Count: 0 };
        const test = { passed: true };
        const progress = { totalTasks: 5, completedTasks: 5 };
        expect(() => checkShipGateWithForceSkip(review, test, progress, options)).toThrow("--force-skip-review requires --reason='<non-empty>'");
...
//# sourceMappingURL=force-skip-review.test.js.map