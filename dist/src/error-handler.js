...
export function classifyExitCode(code) {
    if (code === 0)
        return { retryable: false, category: "success" };
    if (!RETRYABLE_EXIT_CODES.has(code))
        return { retryable: false, category: "fatal" };
    const categoryMap = {
        1: "general_error",
        2: "usage_error",
        137: "sigkill",
...
export function shouldRetry(exitCode, currentAttempt) {
