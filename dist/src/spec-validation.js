...
export function validateContractGate(bundle) {
    const req = bundle.primary;
 ...
 */
export function deriveLenientPatterns() {
    const sourceFiltered = STRICT_PATTERNS.filter((p) => !STRUCTURAL_ONLY.some((s) => s.source === p.source));
 ...
 */
export function detectSpecLeakFromBundle(bundle, scope) {
    const req = bundle.primary;
    const findings = [];
    if (scope === "strict") {
...
function scanLines(text, file, patterns, out) {
...
export function enforceEarsSyntax(text, options) {
...
//# sourceMappingURL=spec-validation.js.map