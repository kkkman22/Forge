...
When cmux is installed, `/forge test` can use `cmux browser` commands for browser-based QA verification. This is a fallback — not required for Forge to function.

## Integration Point

- **Script**: `scripts/cmux-mirror/browser-qa.mjs`
- **Trigger**: `/forge test` phase, only when `cmuxAvailable()` returns true
