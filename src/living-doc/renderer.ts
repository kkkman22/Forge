...
function passRateClass(pass: number, total: number): { rate: string; cls: string } {
...
export function renderIndexPage(data: LivingDocData): string {
...
export function renderContextPage(
  ...
  const { rate: ctxRate, cls: ctxRateClass } = passRateClass(
    context.stats.pass,
    context.stats.total,
  );
...
export function renderLivingDoc(data: LivingDocData, outputDir: string): void {
  ...
  for (const [contextName, context] of data.contexts) {
    ...
    fs.writeFileSync(
      ...
      renderContextPage(context, contextName, data.generatedAt),
      "utf-8",
    );
  }
}
