 ...
 */
export function lookupKey(data: TranslationData, keyPath: string): string | null {
  if (keyPath === "") return null;

  const segments = keyPath.split(".");

  let current: string | TranslationData = data;
  ...
  for (const segment of segments) {
    ...
    const next: string | TranslationData | undefined = (current as TranslationData)[segment];
 ...
 * non-string leaf.
 */
export function validateTranslationData(data: unknown): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
 ...
 */
export function parseTranslationFile(jsonString: string, filePath: string): TranslationData {
  let parsed: unknown;
  try {
  ...
  if (!result.valid) {
    throw new Error(
      `i18n: Invalid translation structure in ${filePath}: non-string values at ${result.errors.join(", ")}`,
    );
  }
