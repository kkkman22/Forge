...
export class PromptDefenseError extends ForgeError {
  readonly code = "PROMPT_DEFENSE_REJECTED" as const;
  ...
  constructor(message: string, threats: ReadonlyArray<Threat>) {
    ...
    this.threats = threats.map((t) => {
      const summary: PromptThreatSummary = { type: t.type, pattern: t.pattern };
...
export class SchemaValidationError extends ForgeError {
  ...
  constructor(rawIssues: ReadonlyArray<z.ZodIssue | SchemaValidationIssue>) {
    const issues = rawIssues.map<SchemaValidationIssue>((issue) => {
      if ("path" in issue && typeof issue.path === "string" && "message" in issue) {
        return { path: issue.path, message: issue.message };
      }
      const z = issue as z.ZodIssue;
      return {
        path: z.path.join("."),
        message: z.message,
      };
...
export class EventLogReplayError extends ForgeError {
  ...
  constructor(expectedHash: string, actualHash: string) {
    super(
      `Event log replay produced hash ${actualHash}, expected ${expectedHash}. ` +
