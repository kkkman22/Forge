...
/** Known invariant expression patterns. */
type TemplateType =
  | "terminal_no_outgoing"
  | "state_before_state_only"
  | "state_requires_condition"
  | "no_state_requires_condition";

/** Match an expression to a template type. */
function classifyInvariant(expression: string): TemplateType | null {
...
export function deriveStatePropertyTests(def: StateMachineDefinition): string {
  ...
  for (const inv of def.invariants) {
    ...
    if (templateType === null) {
      lines.push(
        `  // TODO: manually implement test for: ${inv.description}`,
    ...
    switch (templateType) {
      case "terminal_no_outgoing": {
        ...
        lines.push(
          ...
          `      fc.constantFrom<string>(${stateList}),`,
          `      (state) => {`,
          `        const m = new ${machineName}Machine(state as any);`,
      ...
      case "state_before_state_only": {
        lines.push(
          `  it("${inv.description}", () => {`,
          `    // Verify ${parts.state} is only reachable from ${parts.before} states`,
          `    fc.assert(fc.property(`,
          `      fc.constantFrom(...Object.values(m.states)),`,
          `      (fromState) => {`,
          `        if (fromState === "${parts.before}") return;`,
          `        const canReach = m.canTransitionTo("${parts.state}", fromState);`,
      ...
      case "no_state_requires_condition": {
        lines.push(
          `  it("${inv.description}", () => {`,
          `    fc.assert(fc.property(`,
          `      fc.constantFrom(...m.nonTerminalStates),`,
          `      (fromState) => {`,
          `        const m = new ${machineName}Machine(fromState);`,
          `        if (!m.hasCondition("${parts.condition}")) {`,
          `          expect(() => m.transition("To${toPascal(parts.state)}" as any))`,
          `            .toThrow(/${parts.condition}/i);`,
          `        }`,
          `      },`,
          `    ));`,
          `  });`,
