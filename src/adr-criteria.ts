 ...
 */
const OBVIOUS_KEYWORDS: readonly string[] = [
  "obvious",
  "standard",
  "conventional",
  "common practice",
 ...
 *      "surprising" signal.
 */
function detectSurprising(decision: DecisionCandidate, signals: DecisionSignals): boolean {
...
function buildReasoning(
  ...
  switch (verdict) {
    ...
    case "INLINE_NOTE": {
      const parts: string[] = [];
      if (reversibility === "hard") parts.push("hard to reverse");
      if (surprising) parts.push("surprising");
      if (tradeOff) parts.push("real trade-off");
...
export function evaluateAdrCriteria(
  ...
  const reversibility: "hard" | "soft" = signals.reversalCostAssessment === "low" ? "soft" : "hard";
 ...
 * reads like prose.
 */
const VERDICT_LABELS: Record<AdrCriteriaResult["verdict"], string> = {
  WRITE_ADR: "WRITE ADR",
...
export function renderCriteriaCheck(result: AdrCriteriaResult): string {
  const alternatives =
    result.alternatives.length === 0 ? "none" : `[${result.alternatives.join(", ")}]`;
