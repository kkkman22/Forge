 ...
 */
export function evaluateGateForPhase(
  phase: string,
  readers: QualityFileReaders,
): GateResult | null {
  try {
    switch (phase) {
      case "review": {
      ...
      case "ship": {
        ...
        if (!reviewContent && !testContent && !progressContent) return null;
    ...
    console.warn(
      `Warning: quality gate evaluation failed for phase "${phase}": ${err instanceof Error ? err.message : String(err)}`,
    );
    return null;
  }
}
