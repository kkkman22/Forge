...
export function buildSubagentInvocations(
  ...
  options?: { permissionMode?: "default" | "acceptEdits"; maxTurns?: number },
): SubagentInvocation[] {
  for (const at of agentTypes) {
    if (!VALID_AGENT_TYPES.includes(at as (typeof VALID_AGENT_TYPES)[number])) {
      throw new Error(
        `Invalid agentType: "${at}". Must be one of: ${VALID_AGENT_TYPES.join(", ")}`,
      );
    }
  }
...
export async function runSubagentsInParallel(
  ...
  executor: (invocation: SubagentInvocation) => Promise<SubagentResult>,
