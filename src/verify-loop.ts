 ...
 */
export function parseVerifyConfig(frontmatterContent: string): VerifyConfig {
  ...
  const timeoutMs =
    timeoutSeconds !== null && timeoutSeconds > 0 ? timeoutSeconds * 1000 : DEFAULT_TIMEOUT_MS;

  const maxAttempts = extractNumericField(raw, "verify_max_attempts");
  const resolvedMaxAttempts =
    maxAttempts !== null && maxAttempts > 0 ? maxAttempts : DEFAULT_MAX_ATTEMPTS;
 ...
 */
export function shouldRetryVerify(result: VerifyResult, config: VerifyConfig): boolean {
 ...
 */
export function advanceVerifyLoop(
