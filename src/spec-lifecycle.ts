...
const VALID_PRIORITIES: ReadonlySet<string> = new Set<string>(["P1", "P2", "P3"]);

const VALID_TIERS: ReadonlySet<string> = new Set<string>(["light", "standard", "full"]);
 ...
 */
export function parseSpecFrontmatter(content: string): SpecFrontmatter | null {
  ...
  const status = (extractStringField(raw, "status") ?? "in_progress") as SpecStatus;
 ...
 */
export function validateSpecFrontmatter(fm: SpecFrontmatter): ValidationResult {
  const errors: string[] = [];
  ...
  if (fm.status === "deferred") {
    ...
    if (!fm.deferred_date || !ISO_DATE_RE.test(fm.deferred_date)) {
      errors.push(
        `Invalid deferred_date: "${fm.deferred_date}" (must be YYYY-MM-DD, required when status is deferred)`,
      );
    }
  }
...
function extractStringField(frontmatter: string, fieldName: string): string | null {
