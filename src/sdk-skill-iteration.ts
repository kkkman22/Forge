 ...
 */
function evaluateQualityGateForPhase(phase: string, ctx: SkillIterationContext): GateResult | null {
 ...
 */
export function readFileContent(reader: (() => string) | undefined): string | null {
 ...
 */
export async function executeSkillAwareIteration(
  ctx: SkillIterationContext,
  ...
  if (puaEnabled && ctx.puaStateManager) {
    puaContext = ctx.puaStateManager.restoreContext(
      statusContent,
  ...
  try {
    ...
    if (isContextOverflowError(error)) {
      ...
      try {
        ...
        const output: AgentOutput = retryAgentResult.output;
        ...
        const retryCommitResult = applySkillAwareCommitStrategy(
          retryDispatchResult.effects,
          retryEffectivePhase,
          iterationSuccess,
          iterationNumber,
          iterationSummary,
          ctx.config.objective,
          orchestratorState.commitCount,
        );
        ...
        const retryLogEntry = buildEntry(
          ...
          event,
          retryPreState,
          retryDispatchResult.state,
          lastEffects,
        );
        lastEffects = [...lastEffects, { type: "write_event_log" as const, entry: retryLogEntry }];
