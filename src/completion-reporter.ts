...
import { formatPerformanceBaseline, type PerformanceBaseline } from "./logger/index.js";
...
export function formatCompletionSummary(
  ...
  if (ctx.loopCompletedNormally) {
    ...
    lines.push(
      ctx.t("driver.summary.fixAttemptsExhausted", {
 ...
 */
export function buildPhaseStatusSummary(notesDocument: NotesDocument, t: TranslateFn): string[] {
 ...
 */
export function collectUnresolvedIssues(readReviewFile?: () => string): string[] {
  try {
    if (!readReviewFile) return [];
 ...
 */
export function getLastFailureReason(notesDocument: NotesDocument): string | null {
