...
- **init/hooks**: 修复 Claude Code hooks schema + init.sh 韧性 (#122)
  ...
  - **⚠️ 老项目迁移说明**: 仅升级 Forge 插件**不能**修复 v3.4.0–v3.6.0 已初始化项目里的 `.claude/settings.json`,因为 `init.sh` 检测到已有 `"hooks"` 键就跳过同步(init.sh:728),坏 hook 是快照拷贝。**受影响用户必须 (a) 删除 `.claude/settings.json` 的 `"hooks"` 键后重跑 `forge init`,或 (b) 手动把 15 条 `args` 写法改成 `{type:"command", command:"..."}`。** 新项目自动拿到修复版。
