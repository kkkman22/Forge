---
description: "Use when running `/tinkerman review`, build completes, or code changes need quality gate before ship"
updated: 2026-08-11
...
当用户以 `/tinkerman review <pr-url-or-number>` 或 `/tinkerman review --from-pr <value>` 调用时：
...
| 2 | `warn` | 输出警告，建议重新运行 `/tinkerman review` |
...
## 8. Gate: P0/P1 → Block `/tinkerman ship`
...
**Flag**: `/tinkerman review --canvas <topic>`
...
| # | Check | Failure Route |
|---|-------|-----------|
| 1 | 有代码变更待评审 | → `/tinkerman build` |
| 2 | build 阶段已完成 | → `/tinkerman build` |
...
When user triggers `/tinkerman review`, follow this dispatch protocol for workflow eligibility:
...
Review 完成后，如果后续还有 test/ship 阶段且 Read 预算 >50KB（`${TMPDIR}/tinkerman-read-budget-<session>.json`），输出：

`⚠️ Read budget >50KB after review. Suggest /clear + /tinkerman resume before test phase.`
