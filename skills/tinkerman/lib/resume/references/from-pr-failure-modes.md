...
| Slug resolution all sources fail | Interactive: prompt; Non-interactive: exit 1 | List `.tinkerman/specs/` for selection |
| CC `--from-pr` not supported | Continue Forge-only, print warning | "CC version <2.1.29, session not restored, Forge state only" |
| `.tinkerman/status.md` conflict | Interactive: prompt; Non-interactive: exit 1 | Show diff between current and proposed slug |
