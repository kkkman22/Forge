...
|-------------|-----------------|
| `.tinkerman/progress/<topic>.md` | First-pass / repeated failures / blocked tasks |
| `.tinkerman/reviews/<topic>.md` | P0/P1 count and recurring issue types |
| `.tinkerman/debug/<topic>.md` | Whether debug was triggered and root cause |
| `.tinkerman/plans/<topic>.md` | Estimated vs actual time deviation |
| `.tinkerman/specs/<feature>/spec.md` | Scenario coverage rate, Scope Creep |

## Function Calls

**`analyzeSkillFeedback(entries)`**
- 参数：`entries` — 从 `.tinkerman/knowledge/skill-feedback.md` 解析的反馈条目数组（`SkillFeedbackEntry[]`，每条含 command、scenario、suggestion、frequency）
...
- 参数：`feedbackReasons` — 从 `analyzeSkillFeedback` 结果中提取的失败原因（`string[]`）；`knownFailureDescriptions` — 从 `.tinkerman/knowledge/known-failures.md` 解析的已知失败描述（`string[]`）
...
将本次会话指标追加到 `.tinkerman/knowledge/metrics.md`：命令使用统计、路由准确度、四维度执行质量趋势、验证命令健康度。`/tinkerman plan` Research 阶段可读取历史指标校准预估。
