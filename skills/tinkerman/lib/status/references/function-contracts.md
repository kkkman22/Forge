...
- **用途**：`/tinkerman status`、`forge-status` 和 `forge-doctor --json` 的共同状态模型
...
- **用途**：`/tinkerman status` 与 `forge-status` 的终端输出
...
- **用途**：扫描 `.tinkerman/status.md` + `.tinkerman/status/*.md`，返回所有活跃任务的状态摘要
