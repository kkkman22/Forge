---
description: "Use when /tinkerman ship runs on a spec with acceptance_eval true, user runs /tinkerman accept explicitly, or /tinkerman ship --with-acceptance flag is provided"
updated: 2026-08-11
...
# /tinkerman accept — Acceptance Scenario Eval

**Use when** you need to validate that user-facing acceptance scenarios (defined in spec) actually pass against the running system. This is *behavioral validation from the user's perspective* — running scenario scripts and recording pass/fail with evidence. Do not confuse with `/tinkerman verify` (evidence-based three-state verdict) or `/tinkerman ship` (merge + release).
