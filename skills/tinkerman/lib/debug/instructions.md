...
# /tinkerman debug — Scientific Debugging Engine
...
`/tinkerman debug` uses the **Scientific Method** for debugging: falsifiable hypothesis testing + persistent debug files + append-only knowledge base. Each phase has clear entry/exit criteria. No phase may be skipped.
...
**Not For**:
- Known root-cause simple fixes (use `/tinkerman fix`)
...
**Three-Strike Rule**: Same hypothesis direction fails 3 times consecutively → stop fixing, question the architecture. Enter `/tinkerman debug` with fresh perspective.
