...
> 主 agent 在阶段边界（wave 间 / 跨阶段切换）fire-and-forget spawn 本 subagent，把会话状态增量写入 `.tinkerman/checkpoint.md`。
...
Your only job: incrementally update .tinkerman/checkpoint.md in-place.
...
ABSOLUTE PATHS — USE VERBATIM, NEVER INFER:
  ...
  CHECKPOINT_PATH = <cwd>/.tinkerman/checkpoint.md
  STATUS_PATH     = <cwd>/.tinkerman/status.md
  PROGRESS_DIR    = <cwd>/.tinkerman/progress/
...
见 `.tinkerman/templates/checkpoint.md`（§1 当前阶段与意图 / §2 下一步 / §3 本会话指令 / §4 当前工作 / §5 文件与代码 / §6 问题与修复 / §7 活跃资源 / §8 设计决策 / §9 待迁移知识 / §10 开放笔记 / §11 EXACT-FORM 值）。

每 section 有 token 预算（见模板 italic 标注），增量更新时不要超出预算。

## 与 PreCompact/PostCompact 的关系

checkpoint-writer 写的 `.tinkerman/checkpoint.md` 是 compact 再生的数据源：
