...
name: tinkerman
updated: 2026-08-11
description: "Use when the user types /tinkerman <subcommand> or /tinkerman <task description>. Unified dispatcher entry point that routes to 36 sub-skills via lib instructions.md. Handles tier routing, branch protection, and phase auto-advance."
...
Forge's sole registered skill. All 36 sub-skills live under `skills/tinkerman/lib/<sub>/instructions.md` and are dispatched through the 10-step chokepoint in `src/forge-dispatcher.ts`. Users invoke via `/tinkerman <sub>` (direct) or `/tinkerman <description>` (router-analyzed).
...
Tier is logical, not physical — a sub can appear in multiple tiers. Routing logic in `skills/tinkerman/lib/router/instructions.md`.
...
9. **appendAuditLog** — `${CLAUDE_PLUGIN_DATA}/tinkerman/audit/dispatch.log` (R2.7)

## 4. Fork vs Inline

Each sub's `dispatch_mode` is declared in `skills/tinkerman/lib/<sub>/instructions.md` frontmatter. `registry.toml` is the derived index verified by CI.
...
See `skills/tinkerman/lib/router/instructions.md` for full routing logic. Summary:
...
Usage: `/tinkerman decide --no-gate <topic>` or `/tinkerman spec --no-gate <topic>`.
...
| Value | Behavior |
|-------|----------|
| `collapsed` (default) | Reads `skills/tinkerman/lib/<sub>/instructions.md` |
| `legacy` | v2.4 compatibility (migration-period only) |

## 8. Audit Log

Every dispatch appends a tamper-evident HMAC-chained NDJSON entry to `${CLAUDE_PLUGIN_DATA}/tinkerman/audit/dispatch.log` (outside workspace). See `src/forge-dispatcher/audit-log.ts`.
