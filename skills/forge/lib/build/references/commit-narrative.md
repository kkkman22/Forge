...
`<run_id>` 从 loop-state.json 的 `id` 字段读取（loop 场景）；非 loop 场景（手动 `/forge build`）用 `manual-<timestamp>`，落到 `.forge/runs/manual-<timestamp>/commit-narrative.md`。
...
- **`/forge learn`** 从 runs/ 提取经验时，提示用户至少复述一条关键改动意图。
