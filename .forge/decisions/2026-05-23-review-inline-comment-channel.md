...
**Predecessor**: `.forge/decisions/2026-05-16-claude-code-uplift-2.1.143.md`
...
Forge 当前 review 阶段产物是 `.forge/reviews/<run-id>.md`，依赖人工查看和 ship 阶段 gate（参见 ADR-0005 fallback ladder）。Bitbucket 是 Forge 团队的主要代码协作平台，现有 `bitbucket` MCP power 已经提供完整的 PR 操作能力。
...
**采纳**：默认 per-project（`.forge/config.md`），CLI flag `--post-comments` 可临时覆盖。

优先级（高 → 低）：
1. CLI flag `--post-comments` / `--no-post-comments`（per-run，最高优先级）
2. `.forge/config.md` 的 `review.comment_channel.enabled`（per-project）
...
- Forge 视角：`.forge/reviews/*.md` 是 source of truth，ship 看 markdown 决定是否阻断
...
优先级（A4）：CLI flag > `.forge/config.md` > 内置默认（关闭）
...
- `.forge/decisions/2026-05-16-claude-code-uplift-2.1.143.md`
