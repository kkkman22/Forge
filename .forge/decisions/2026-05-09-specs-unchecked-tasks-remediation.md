...
- `.forge/findings/` has retention policy
...
**OWASP Check**:
- Injection: Low — prune-event-logs.sh uses `mv` with hardcoded `.forge/` paths. Path traversal risk minimal if paths are sanitized (reject `..` sequences).
