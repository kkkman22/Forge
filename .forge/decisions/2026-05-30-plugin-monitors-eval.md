...
1. **evolved-rules stale 检测**：Stop hook 检查 `.forge/knowledge/evolved-rules.md` 中的 PENDING 状态
2. **任务完成提醒**：Stop hook 检查 `.forge/progress/*.md` 中未完成任务
...
1. **evolved-rules stale 检测**：Stop hook 在会话结束时触发，时机合适。改为 monitor 需要独立轮询 `.forge/knowledge/evolved-rules.md`，增加复杂度但无额外收益。当前方案足够。
