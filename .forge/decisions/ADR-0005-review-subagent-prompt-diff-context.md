...
`/forge review` orchestrator (`src/review.ts buildReviewSubagents()`) generated subagent prompts containing only a changed-file list (`Changed files: src/a.ts, src/b.ts, ...`). The SKILL §2.0 contract requires the `.forge/reviews/.diff-context.md` path be passed so subagents read unified diff hunks directly.
...
1. `buildReviewSubagents()` now generates a 5-line preamble for spec-check/quality-check/security-check prompts containing:
   - `.forge/reviews/.diff-context.md` literal path
