...
On commit `d1ee44b`, `/forge review` executed a three-layer review for `forge-single-entry-skills-collapse`. All three subagents failed with Claude Code Agent SDK's task registry purge bug (`Error: No task found with ID: <id>`). In response, the main agent **self-took over** the review and produced `.forge/reviews/forge-single-entry-skills-collapse.md` with `result: blocked` and 7 P0/P1 findings.
...
| **L2** | CI ultrareview async evidence | L1 all-fail + `.forge/reviews/<pr>-ci.md` exists | Medium | `methodology: ci-evidence` |
...
- Writes audit record to `.forge/findings/force-skip-review-<date>.md` with commit hash, reason, timestamp, and user identity
