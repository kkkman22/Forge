...
- metric 落盘到 `.forge/state/triage-state.json`（已有的 state 文件），不新增独立 metric 系统。
- 零新依赖，纯扩展现有 state 结构。
...
**Tech Selection**：扩展 `.forge/state/triage-state.json`（PR #98 已建）的 schema，新增 `last_run` 对象。纯 schema 扩展 + triage skill 写入，零新依赖。
...
**STRIDE Analysis**：
- **Information Disclosure**：`last_run` 含 sources/finding 计数，已在 `.forge/state/`（PR #98 已加入 .gitignore，不进 git）。无新增泄露面。
