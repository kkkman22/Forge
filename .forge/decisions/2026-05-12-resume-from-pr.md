...
Claude Code 2.1.29 起提供 `--from-pr` 标志恢复 PR 关联会话。Forge 的 `/forge resume` 通过 `.forge/status.md` 恢复上下文，但无法跨机器/跨人恢复。典型场景：同事接手未完成 PR，或在新机器上继续 review。
...
- 缓存 `.forge/.pr-slug-cache.json` → 增加一个 git-ignored 文件，但避免重复远程调用
