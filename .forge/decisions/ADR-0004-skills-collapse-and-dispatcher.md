...
PoC evidence (`.forge/poc/single-entry-dispatch/RESULTS.md`) demonstrated that `Agent` tool + `lib/<sub>/instructions.md` achieves functional parity with the original `SKILL.md` + `context: fork` behavior across V1/V2/V3 test scenarios.
...
1. Set `dispatcher_mode = legacy` in `.forge/config.md` — advisory mode, physical revert required for full rollback
