...
- **Current command**: `if [ -f .forge/knowledge/evolved-rules.md ]; then echo '=== Evolved Rules ==='; cat .forge/knowledge/evolved-rules.md; fi`
...
- **Current command**: `if [ -f .forge/.sandbox-active.json ]; then node forge/dist/src/check-sandbox.js Bash "$TOOL_INPUT_FILE" ...; fi`
...
- **Current command**: `if [ -d .forge/status ] || [ -f .forge/status.md ]; then echo '📝 ...'; fi`
- **Can migrate to `if:`**: NO — checks project state
- **Action**: Keep inline, already optimal

### Entry 4: Stop — incomplete tasks check
- **Location**: Line 139 (Stop hook 1, sub-hook 1)
- **Current command**: `if [ -f .forge/progress/*.md ] 2>/dev/null; then incomplete=$(grep -c '- \[ \]' .forge/progress/*.md ...); ...; fi`
- **Can migrate to `if:`**: NO — checks project state
- **Action**: Keep inline

### Entry 5: Stop — evolved rules pending check
- **Location**: Line 152 (Stop hook 2)
- **Current command**: `if [ -f .forge/knowledge/evolved-rules.md ] && grep -q 'PENDING' ...; then ...; fi`
- **Can migrate to `if:`**: NO — checks project state + file content
- **Action**: Keep inline

### Entry 6: Stop — phase verification nudge
- **Location**: Line 188 (Stop hook 6)
- **Current command**: `if [ -f .forge/status.md ]; then phase=$(grep '^phase:' ...); ...; fi`
- **Can migrate to `if:`**: NO — checks project state
- **Action**: Keep inline

### Entry 7: TeammateIdle — phase check
- **Location**: Line 199 (TeammateIdle hook)
- **Current command**: `if [ -d .forge/status ]; then status_file=...; else status_file='.forge/status.md'; fi; phase=$(grep ...); ...`
- **Can migrate to `if:`**: NO — checks project state
- **Action**: Keep inline
...
|------|----------------|---------------------------|
| PreToolUse Write\|Edit → frozen check | `Write\|Edit` | `if: "Write(.forge/**)\|Edit(.forge/**)"` |
...
| PostToolUse Write\|Edit → progress reminder | `Write\|Edit` | `if: "Write(.forge/**)\|Edit(.forge/**)"` |
| PostToolUse Write\|Edit → cmux sync | `Write\|Edit` | `if: "Write(.forge/**)\|Edit(.forge/**)"` |
| PostToolUse Write\|Edit → feature dossier | `Write\|Edit` | `if: "Write(.forge/**)\|Edit(.forge/**)"` |

## Migration Actions

1. Add `if: "Write(.forge/**)|Edit(.forge/**)"` to frozen-zone check PreToolUse hook
2. Add `if: "Write(.forge/**)|Edit(.forge/**)"` to PostToolUse progress/cmux/dossier hooks
