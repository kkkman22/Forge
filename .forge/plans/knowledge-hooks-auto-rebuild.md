...
status: "draft"
date: "2026-05-14"
spec_ref: ".forge/specs/knowledge-hooks-auto-rebuild/spec.md"
format: "full"
---

# Plan: Knowledge Integrity / Catalog Auto Hook

> 来源: `.forge/specs/knowledge-hooks-auto-rebuild/spec.md`
...
describe("knowledge-hooks pure scheduling", () => {
  describe("hashEvent", () => {
    ...
    it("produces different hash for same path but different kind", () => {
      const a: KnowledgeEvent = { kind: "adr_written", path: ".forge/decisions/ADR-0042.md" };
      const b: KnowledgeEvent = { kind: "solution_written", topic: "ADR-0042", path: ".forge/decisions/ADR-0042.md" };
      expect(hashEvent(a)).not.toBe(hashEvent(b));
    });
  });
...
> solutions/ 写入完成后，hooks.json PostToolUse 自动触发 integrity lint。findings 写入 `.forge/findings/integrity-<timestamp>.md`。
