...
- **Knowledge KB 命中**：`.forge/knowledge/skill-style-guide.md` 显式要求所有 forge-* 必须 `disable-model-invocation: true` —— 本 plan 必须更新该指南
...
|------|-----------|--------|
| `.forge/findings/worktree-spike-2026-05-17.md` | CREATE | Wave 0 spike 报告（R2.8） |
...
| `.forge/config.md` | MODIFY | 新增 `skills.dispatcher_mode: collapsed` |
...
| `.forge/knowledge/skill-style-guide.md` | MODIFY | 删除 `disable-model-invocation: true` 强制要求 |
| `README.md` | MODIFY | SKILL 数量声明 + /forge-X 例子重写 |
| `ROADMAP.md` | MODIFY | 同上 |
| `CHANGELOG.md` | MODIFY | v2.5.0 breaking change 条目 |
| `.forge/decisions/ADR-0004-skills-collapse-and-dispatcher.md` | CREATE | ADR-0004（R6.1）|
| `.forge/decisions/ADR-0003-single-entry-command-consolidation.md` | MODIFY | 标注 supersedes_partial 范围 |
...
**Depends On**: []
**Files**:
- Create: `.forge/findings/worktree-spike-2026-05-17.md`
...
**`no-skill-x-references.test.ts`**：grep 全仓除 `.forge/decisions/`、`.kiro/specs/`、`.forge/archive/`、`.forge/reviews/` 外的文件，断言无 `Skill(forge-` 形式调用。
...
- Modify: `.forge/config.md`（追加 `skills.dispatcher_mode` 字段说明）
...
- `resolveDispatcherMode()` 读 `.forge/config.md`，默认 `collapsed`；`legacy` 模式输出提示 `"legacy mode requires Forge < 2.6"` 并继续走 chokepoint（R2.10 chokepoint 不被绕过）
...
**Depends On**: [9]
**Files**:
- Modify: `.forge/config.md`（在 Task 4d 已添加字段，本任务补完整文档块）

**实现**：在 `.forge/config.md` 现有结构追加（如 4d 未完整写）：
```markdown
...
- Modify: `.forge/knowledge/skill-style-guide.md`
- Modify: `README.md`
- Modify: `ROADMAP.md`
- Modify: `CHANGELOG.md`
...
- `.forge/knowledge/skill-style-guide.md` 删除"必须 `disable-model-invocation: true`"段落，新增"lib instructions.md frontmatter 字段表"段落（含 `allowed_tools`、`dispatch_mode`）
...
**Run**: `grep -rn 'skills/forge-' docs/ agents/ README.md ROADMAP.md CHANGELOG.md .forge/knowledge/skill-style-guide.md`
...
**Depends On**: [12]
**Files**:
- Create: `.forge/decisions/ADR-0004-skills-collapse-and-dispatcher.md`
- Modify: `.forge/decisions/ADR-0003-single-entry-command-consolidation.md`
...
**Evidence**: 全测试 PASS；R1.2 截图归档到 `.forge/findings/menu-screenshot-2026-05-17.png`（或文本记录）
...
1. 复制 `.kiro/specs/forge-single-entry-skills-collapse/spec.md` 到 `.forge/specs/forge-single-entry-skills-collapse/spec.md`（forge-build §2.2 spec-lock gate 在 `.forge/specs/` 找）
2. 复制本 plan 到 `.forge/plans/forge-single-entry-skills-collapse.md`
3. 写入 `.forge/status.md`（current_task / tier=full / phase=plan / approved=true / branch=feature/forge-single-entry-poc）
