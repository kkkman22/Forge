...
将 Forge 的冻结区保护从 `exit 2` 硬阻断升级为结构化 JSON 反馈 middleware。PreToolUse hook 从退出码决策改为 JSON stdout 决策；新增 PostToolUse hook 作为 defence-in-depth 兜底；Zone_Registry 从 `.forge/config.md` 动态解析；结构化审计日志写 `.forge/runs/`。
...
| `.forge/decisions/ADR-0001-frozen-structured-feedback.md` | CREATE | ADR：exit-code → JSON feedback 迁移 |
...
- **Depends On**: Task 1
- **Verify**: `echo '{"tool_name":"Write","tool_input":{"file_path":".forge/config.md"}}' | FORGE_STRUCTURED_FROZEN=1 bash scripts/hook-check-frozen-structured.sh`
...
- **Depends On**: Task 1
- **Verify**: `echo '{"tool_name":"Write","tool_input":{"file_path":".forge/config.md"},"tool_response":{"success":true}}' | FORGE_STRUCTURED_FROZEN=1 bash scripts/hook-check-frozen-post.sh`
- **Commit**: `feat(frozen): PostToolUse defence-in-depth hook`

### Task 5: hooks.json 调整
- **Goal**: 修改 hooks/hooks.json，PreToolUse 增加 `if` 过滤器（.forge/** 路径）；新增 PostToolUse 条目带 `if` 过滤；保持现有 hook 行为不变
...
- **Design Reference**: `design.md#component-5-audit-log` — 单行 JSON 追加到 `.forge/runs/<YYYY-MM-DD>-frozen-events.jsonl`，10MB 轮转，findings_retention_days 过期
- **Depends On**: Task 1
- **Verify**: `source scripts/zone-registry.sh && log_event "pre" ".forge/config.md" "frozen-config" "CONFIG_ROOT" "denied" && cat .forge/runs/*-frozen-events.jsonl | tail -1`
...
- **File**: `.forge/decisions/ADR-0001-frozen-structured-feedback.md`, `CHANGELOG.md`, `README.md`, `SECURITY.md`
- **Design Reference**: `design.md#testing-strategy` — R9.1-R9.5 文档要求
- **Depends On**: Task 5（需要最终 hooks.json 确认）
- **Verify**: `test -f .forge/decisions/ADR-0001-frozen-structured-feedback.md && grep -c "structured" CHANGELOG.md`
