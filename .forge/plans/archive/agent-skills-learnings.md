...
| 假设写入 status | `skills/forge-router/SKILL.md` §5 | 假设列表写入 `.forge/status.md` 的新字段 `assumptions`，下游 skill 可读取 |
...
来源包括：package.json、项目路由扫描、现有代码模式、.forge/config.md 技术栈、git log 等。
...
```
1. 读取 .forge/reviews/<topic>.md 的 reviewed_at_commit
2. 获取当前 HEAD commit hash
3. 如果相同 → 通过
4. 如果不同 → 计算 diff：
   - diff 仅涉及 .forge/ 文件 → 通过（状态更新不影响代码质量）
