...
- **`.forge/config.md` frontmatter 缺 `ci_check_command`**：仅 body 有说明文字，SKILL Layer 3 走逐项回退分支
...
| `.forge/config.md` | MODIFY | frontmatter 加 `ci_check_command: "npm run check"`（冻结区，需用户解锁） |
...
### Task 6: 补齐 .forge/config.md frontmatter（冻结区）

**Depends On**: ["3", "4"]
**Files**: `.forge/config.md` (MODIFY)

- 用户解锁后，frontmatter 新增 `ci_check_command: "npm run check"`
**Verify**: `grep '^ci_check_command:' .forge/config.md` + `npm run typecheck` + `npm run check`
...
**Depends On**: ["6", "5"]
**Files**: `.forge/knowledge/known-failures.md` (MODIFY)
...
- Task 6 涉及冻结区 `.forge/config.md`，需用户明确授权解锁
