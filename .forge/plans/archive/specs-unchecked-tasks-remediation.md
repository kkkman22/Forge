...
status: "approved"
date: "2026-05-09"
spec_ref: ".forge/specs/specs-unchecked-tasks-remediation/spec.md"
format: "full"
---
...
| `.forge/knowledge/evolved-rules.md` | MODIFY | R3 Source 改为存在的路径 |
| `.forge/config.md` | MODIFY | 开放区追加 ship、frontmatter 追加字段 |
...
将 R3 的 `Source` 字段从不存在的 `.forge/knowledge/glm-summary-ending.md` 改为 `.forge/specs/phase-advance-hardening/spec.md`（该 spec 驱动了 R3 规则的创建）。
...
**验证命令**：`grep 'glm-summary-ending' .forge/knowledge/evolved-rules.md` 应无输出
...
**文件**：`.forge/config.md`, `templates/config.md`
**需求**：需求 4, 8
...
**验证命令**：`grep -E 'findings_retention_days|post_push_verify_enabled|\.forge/ship' .forge/config.md templates/config.md`
...
setup() {
  ...
  export CONFIG_FILE="${TEST_DIR}/.forge/config.md"
  export FINDINGS_DIR="${TEST_DIR}/.forge/findings"
  export ARCHIVE_DIR="${TEST_DIR}/.forge/archive/findings"
  mkdir -p "${FINDINGS_DIR}"
  mkdir -p "${ARCHIVE_DIR}"
  mkdir -p "${TEST_DIR}/.forge/config.md"
  cat > "${CONFIG_FILE}" <<'EOF'
---
findings_retention_days: 30
---
