...
status: "approved"
date: "2026-05-15"
spec_ref: ".forge/specs/conflict-resolver-hook/spec.md"
format: "full"
---
...
describe("classifyConflictZone", () => {
  it("classifies frozen paths from statusContent", () => {
    ...
    expect(classifyConflictZone(".forge/specs/auth/spec.md", status)).toBe("frozen");
    expect(classifyConflictZone(".forge/config.md", status)).toBe("frozen");
  });
  ...
  it("classifies guarded paths", () => {
    const status = "";
    expect(classifyConflictZone(".forge/progress/auth.md", status)).toBe("guarded");
    expect(classifyConflictZone(".forge/reviews/auth.md", status)).toBe("guarded");
    expect(classifyConflictZone(".forge/knowledge/instincts.md", status)).toBe("guarded");
    expect(classifyConflictZone(".forge/decisions/ADR-001.md", status)).toBe("guarded");
  });
...
describe("ship_merge conflict-resolver integration", () => {
  it("calls conflict-resolver on merge conflict", async () => {
    ...
    const resolveSpy = vi.fn().mockResolvedValue({
      ...
      frozenRefused: false,
      escalateToDebug: false,
      resolvedPaths: [".forge/progress/auth.md"],
      refusedPaths: [],
