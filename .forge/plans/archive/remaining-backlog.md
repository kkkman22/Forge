...
- **Group A**（SKILL 压缩）→ 已有批准计划 `.forge/plans/token-budget-compression.md`，独立执行
- **Group B**（Agent 语言转换）→ 已有批准计划 `.forge/plans/token-language-optimization.md` Task 12，独立执行
...
- 包含 `.forge/` 完整配置（status.md, config.md, specs/, plans/, reviews/）
- 包含示例 spec 和 plan
