...
为 Forge 增加可选的 cmux 集成层：通过独立守护进程 Mirror_Daemon 观察 `.forge/` 状态文件，将 Forge 生命周期（路由、DAG 进度、评审结果、Forge Loop 迭代、冻结拦截）以结构化形式投射到 cmux 侧边栏、通知和浏览器 surface。同时追加 Events_NDJSON 事件流（服务多个未来消费者）和 Reviews Frontmatter 扩展。严格遵守 Zero-Impact 不变量：未安装 cmux 时 Forge 行为零变化。
...
| `scripts/cmux-mirror/sync-once.mjs` | CREATE | hook 触发：一次性读 .forge/ → 推 cmux；respawn 受限自愈 |
...
| `scripts/cmux-mirror/lib/reader.mjs` | CREATE | 纯函数：从 .forge/ 读状态合成 payload |
...
- **Goal**: reader 从 .forge/ 合成 CanonicalSidebarPayload；emitter 将 payload 变化翻译为 cmux CLI 命令序列
...
- **Goal**: 追加 find .forge/.cmux-dedupe -type f -mmin +60 -delete
...
- **File**: `.forge/knowledge/sessions/cmux-integration-acceptance.md`
