...
- **Goal**: Add pure function implementing 4 cases: undefined→fresh, same→fresh, .forge/ only→fresh, project code→not fresh
- **File**: `src/ship.ts`
...
- **Goal**: Unit tests covering empty file list, empty commit hash, mixed .forge/ and project files
...
| Req 2 AC4: Diff only `.forge/` → pass; project code → warning | Task 2, Task 6 |
