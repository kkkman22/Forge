...
> Spec: `.forge/specs/domain-knowledge-threading/` (locked)
> Decision: `.forge/decisions/2026-06-27-domain-example-reference-impl.md` §7 (slice B)
...
runtime 路径读 `.forge/config.md` 的 `packs:` 字段，四个 phase（decide/plan/build/review）
都不消费 pack 数据，glossary 检查仍读扁平 `.forge/glossary.md`，`src/index.ts` 不导出
