...
> **复核日期**：2026-06-11
> **复核范围**：`.forge/specs/` 下所有 spec（排除 `_archived/`）
...
| **`sandbox-phased-implementation`** | `sandbox-phased.ts` 纯函数 + 测试齐全 | `forge init` 不生成 `.forge/sandbox.json`；无 `--sandbox <profile>` CLI；零 SKILL 引用 sandbox 检查点。 |
...
| 67 | parallel-status-tracking | ✅ IMPL | HIGH | status-resolver.ts/status-manager.ts 全函数 + 多任务 .forge/status/ |
...
### 4.1 流程
1. 枚举 `.forge/specs/` 全部 113 个非 archived 目录（`find` 纯名，规避 `ls` 颜色码干扰）。
...
基于本复核结论，以下 2 个 spec 已于复核当日归档至 `.forge/archive/2026-06-11-*/`，INDEX.md 同步更新（active 112→110，archived 11→13）：
