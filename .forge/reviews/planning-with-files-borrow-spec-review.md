...
> 三层 subagent 并行评审，对象是 `.forge/specs/planning-with-files-borrow/{requirements,tasks}.md`（纯文档 draft，无代码）。按 §3.1 执行与评估分离，评审者独立于 spec 作者。
...
**根因**：R1 把 `.forge/progress/*.md`（受保护区，可追加的自由文本）的"未完成任务清单"直接拼入 agent 续做指令，无任何清洗/边界标记。
...
| ID | 问题 | 修复方向 |
|----|------|----------|
| **S-3** | R6 写 attestation 到 plan frontmatter，**违反冻结区**（approved plan 属冻结区不可改）。frozen-zone hook 会拦截或被迫开口子。 | attestation 元数据移出冻结区：写入 `.forge/state/attestations.json`（开放区），plan frontmatter 保持只读。 |
...
| **S-10**（R3 plan_path 缺路径穿越校验） | R3.AC3 增路径校验：`resolve(plan_path)` 落在 `.forge/plans/` 内，否则拒绝注入退化，对应 VAL-R3-002 |
...
- **N-3**：R3 路径校验用 `path.resolve`（词法）而非 `fs.realpathSync`（物理），symlink 可逃出 `.forge/plans/` 词法围栏。
