...
本设计将 `requirements.md` 中 17 条 Requirement 映射为 **1 个守护进程 + 1 个一次性同步脚本 + 1 个推送瘦包装 + 1 套共享库 + 1 个 hook 瘦封装 + 1 套 starter 布局 + 1 个可选技能包**，采用**观察者架构 + 可选主动推送**（Observer + opt-in Push）：Forge 核心对 cmux 零认知，默认由 Mirror_Daemon 通过 `fs.watch` 观察 `.forge/` 状态文件与 `Events_NDJSON` 事件流并翻译为 cmux CLI 调用；在少数需要毫秒级响应的场景下，SKILL 可通过 `scripts/cmux-mirror/push.sh` 主动推送至 Mirror_Push_Socket 加速投影（R17）。
...
| R3 DAG 并行进度 | Mirror_Daemon 观察 `.forge/progress/<topic>.md` + `parseProgressDag()` 纯函数 |
...
| R6 冻结拦截侧边栏与通知 | `scripts/cmux-mirror/hook-notify.sh` + `.forge/.cmux-dedupe/` 文件系统 TTL |
...
| R17 Mirror Push 主动推送通道 | Mirror_Daemon 监听 `.forge/.cmux-mirror.sock` + `scripts/cmux-mirror/push.sh` 瘦包装 |
...
| R16 Forge Session 边界 | Mirror_Daemon 的会话状态机 + `.forge/config.md` `cmux_session_idle_minutes` |
...
1. **观察者架构是一等公民**：Forge 写 `.forge/` 是既有行为；Mirror_Daemon 只从外部观察并翻译。Forge 核心代码路径不包含任何 cmux 条件分支。
...
- **不改写 `.forge/status.md` 为 cmux 单一数据源**：status.md 仍是 SoT，cmux sidebar 是投影（Out of Scope #7）。
...
| Node CLI | `scripts/cmux-mirror/sync-once.mjs` | hook 触发：一次性读 `.forge/` → 推 cmux；respawn 受限自愈 | R2.7, R2.8, R13.12–14 | 160 |
...
| Node 纯库 | `scripts/cmux-mirror/lib/reader.mjs` | 从 `.forge/status.md`、`.forge/progress/`、`.forge/reviews/` 读取并合成 payload | R2.1, R12.10 | 200 |
...
| Node 纯库 | `scripts/cmux-mirror/lib/respawn.mjs` | Respawn_Budget 计数：`.forge/.cmux-respawn-count` 原子读写 + session 边界重置 | R13.12–14 | 80 |
...
| `forge-build` | 说明 Mirror_Daemon 从 `.forge/progress/<topic>.md` 读 DAG；SKILL 无需主动推送 | R3 |
...
#### 3.2.1 `.forge/runs/<id>/events.ndjson` [R14]
...
**读取规则**（R14.6、R14.7）：
- Mirror_Daemon 维护 `<cwd>/.forge/.cmux-mirror-cursor.json`：
  ```json
  {
...
#### 3.2.2 `.forge/reviews/<topic>.md` frontmatter [R15]
...
#### 3.2.3 `.forge/.cmux-last-sync.json` [R2.5]
...
#### 3.2.4 `.forge/.cmux-dedupe/<sha1>.ts` [R6.2]

最简形式：文件名 = `createHash('sha1').update(absPath).digest('hex')`；内容 = `Date.now().toString()`（unix millis ASCII）。

```
$ cat .forge/.cmux-dedupe/3a1b2c4d5e6f7g8h9i0j1k2l3m4n5o6p7q8r9s0t.ts
1746705023412
```
...
if [ -d "$REPO/.forge/.cmux-dedupe" ]; then
  find "$REPO/.forge/.cmux-dedupe" -type f -mmin +60 -delete 2>/dev/null || true
fi
```

#### 3.2.5 `.forge/findings/<topic>/browser-qa/verdict.md` [R8.3]
...
### 3.3 `.forge/config.md` frontmatter 新增字段（4 个，R11.9 + R16.7）
...
**向后兼容保证**：5 个字段全部 optional，已有项目无需 edit `.forge/config.md`。
...
- **`CMUX_INTEGRATION` 环境变量**是给 Mirror_Pane 启动命令用的：bash 脚本里 `grep 'cmux_integration: off' .forge/config.md` 然后 `export CMUX_INTEGRATION=off` 再 `exec node mirror.mjs`；这样 availability 判断是纯函数、不读 `.forge/config.md`，测试更简单。
...
mirror.mjs 启动
  ...
  │      失败则按 polling 兜底。
  ├── 9. 启动文件监听主循环
  │      - watch .forge/status.md（change）
  │      - watch .forge/progress/（recursive）
  │      - watch .forge/reviews/（recursive）
  │      - watch .forge/runs/（recursive，发现新 run_id 则添加 .ndjson watch）
  ├── 10. 绑定 Mirror_Push_Socket（.forge/.cmux-mirror.sock, mode 0600）
...
sync-once.mjs 启动（每次独立进程，由 hook 触发）
  ...
  ├── 2. .forge/status.md 不存在 → exit 0（R2.9）
  ├── 3. 尝试文件锁 .forge/.locks/cmux-sync.lock（1s 超时）
  │      失败 → exit 0（R2.10）
  ├── 4. 检查 .forge/.cmux-mirror.pid（R13.12）
  │      - 文件不存在 OR pid 不存活 → 进入受限 respawn 流程：
  │          读 .forge/.cmux-respawn-count（原子 open+read+close）
  │          读 .forge/config.md frontmatter 取 cmux_respawn_budget（默认 3）
  │          IF count < budget:
  │            count++ 原子写回
  ...
  │            （exactly once per session；用 .forge/.cmux-respawn-exhausted-flag 去重）
  │      - 文件存在且 pid 存活 → 不 respawn
  ├── 5. 调用 lib/reader.mjs 读 .forge/ → payload
  ├── 6. 调用 lib/emitter.mjs → cmux CLI 命令序列
  ├── 7. 与 .forge/.cmux-last-sync.json 做 diff（R2.5）
  ├── 8. 变更字段 → cmux set-status / set-progress / log
  └── 9. 更新 .cmux-last-sync.json + 释放锁 + exit 0
```

Respawn_Budget 计数的 session 边界重置（R13.14）由 Mirror_Daemon 在 `session.onBoundary("start")` 时通过 `fs.writeFileSync(".forge/.cmux-respawn-count", "0")` 完成；Sync_Once 自己不重置（避免并发竞态）。
...
export async function runBrowserQA({ topic, targetUrl, assertions }) {
  ...
  const artifactsDir = `.forge/findings/${topic}/browser-qa`;
...
Mirror_Daemon 启动序列 §4.2.1 的 step 10 调用 `createPushServer({ socketPath: ".forge/.cmux-mirror.sock", onEvent: dispatch.bind(null, { source: "push" }) })`；SIGINT/SIGTERM 处理器里调 `close()` 清理 socket 文件。
...
export function initReviewFrontmatter(topic: string): void {
  const path = `.forge/reviews/${topic}.md`;
...
export function markLayerStatus(topic: string, layer: "spec_check" | "quality_check" | "security_check", status: LayerStatus): void {
  atomicUpdateFrontmatter(`.forge/reviews/${topic}.md`, fm => {
...
| 3. Sync_Once（hook） | 第一行即检测 `cmuxAvailable()`，false → exit 0；`.forge/status.md` 不存在 → exit 0 | R1.5, R2.9 |
...
| 场景 | 策略 | Requirement |
|---|---|---|
| 多 Sync_Once 并发 | `.forge/.locks/cmux-sync.lock` + 1s timeout，后来者 skip | R2.10 |
| 多 Mirror_Daemon 实例 | `.forge/.locks/cmux-mirror.lock` exclusive，后来者 exit 0 | R2.10 |
| 多 worktree 并发 Forge Loop | 每个 worktree 一个 `.forge/runs/<id>/events.ndjson`；Mirror_Daemon 按 Workspace_Ref 分离 counter | R16.9 |
...
cat .forge/.cmux-mirror.pid  # 若 pid 存活即 running
ps -p $(cat .forge/.cmux-mirror.pid) 2>/dev/null

# 2. 看 cmux sidebar 当前状态
cmux sidebar-state --json | jq '.'

# 3. 看最近事件流
jq -c '.' .forge/runs/*/events.ndjson | tail -20

# 4. 看上次 sync 发送了什么
cat .forge/.cmux-last-sync.json | jq '.'

# 5. 看 respawn 计数
cat .forge/.cmux-respawn-count 2>/dev/null
...
- **SKILL 调 push.sh 是可选优化**：任何使用 push 的 SKILL 必须在 push.sh 返回后继续原路径（即仍要写 `.forge/status.md` 等），让 fs.watch 作为权威路径兜底。
...
```ascii
User         /forge build    src/review etc.   fs (.forge/)    Mirror_Daemon    cmux CLI
 │                │                │                 │                │               │
 │ /forge build   │                │                 │                │               │
 ├───────────────▶│                │                 │                │               │
...
```ascii
User     /forge review     src/review.ts    fs (.forge/reviews/)    Mirror_Daemon   cmux
 │             │                 │                     │                    │          │
 │ /forge review                  │                     │                    │          │
 ├────────────▶│                 │                     │                    │          │
...
```ascii
User   /forge test   browser-qa.mjs    cmux browser   dev server   fs (.forge/findings/)
 │           │              │                │              │               │
 │           │ engage? (R8.1)                │              │               │
 │           ├─────────────▶│                │              │               │
 │           │              │ cmuxAvailable? │              │               │
...
| 1.3 `.forge/config.md` 新增 5 个 optional 字段说明 + `templates/config.md` 同步 | R11.9, R16.7, R13.12 | 文档 patch |
...
| # | 风险 | 影响 | 概率 | 缓解 |
|---|---|---|---|---|
| 1 | **守护进程挂了但用户不知道** | 中：长任务无 sidebar 更新用户以为 Forge 卡死 | 中 | R13.12 hook 自愈；`.cmux-mirror.pid` + `cmux log warning "respawning after crash"` 让用户能在 sidebar 看到；文档教 `ps -p $(cat .forge/.cmux-mirror.pid)` 自检 |
...
| 11 | **Respawn 风暴掩盖真问题** | 中：mirror 反复崩溃用户看不出来 | 中 | R13.12 受限 budget 默认 3；耗尽后每 session 发一次 warning 日志而非每次都发；用户能通过 `cat .forge/.cmux-respawn-count` 诊断 |
...
| forge-spec | 否 | 无 |
| forge-plan | 否 | 无 |
| forge-build | 否（仅通过观察 `.forge/progress/` 被 Mirror 消费） | 无 |
...
#   - 要禁用 → .forge/config.md 设 cmux_integration: off
```
...
# 布局自动拉起 4 个 pane：
#   - Claude Code（左）
#   - tail -f .forge/status.md（右上）
#   - tail -c 20000 -f .forge/progress/*.md（右中）
...
- 观察者方案让 Forge 仅对 `.forge/` 负责（已有行为），Mirror_Daemon 作为外部消费者。cmux 不可用时 Mirror_Daemon **根本不被启动**，Zero-Impact 是物理保证。
...
### D3. 为什么 Events_NDJSON 而非写更多 `.forge/status.md` 字段？

**选择**：Events_NDJSON 作为 Forge Loop 事件源，`.forge/status.md` 维持既有 /forge 会话状态。

**理由**：
- `.forge/status.md` 是"快照"型状态（phase、tier、current_topic），不是事件流。塞事件进去会污染快照语义。
...
- 每轮写一个 `.forge/runs/<id>/iter-<n>.md` 文件 → 拒绝（文件数量爆炸）
...
**备选**：
- 另起 `.forge/reviews/<topic>.events.ndjson` → 拒绝（双倍路径复杂度，收益不显著）
...
**理由**：
- 两者的核心逻辑（读 `.forge/` → 生成 payload → 翻译为 cmux 命令）完全相同；只是触发方式不同（fs.watch vs. hook 一次性）。
