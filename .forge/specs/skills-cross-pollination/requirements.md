...
- **Glossary（共享语言）**：以 `.forge/glossary.md` 为单一事实来源的领域术语表，被 forge-spec / plan / learn 等多个 skill 消费
...
1. THE 项目 SHALL 在 `.forge/glossary.md` 维护单一领域术语表，位于开放区（Open zone）
2. THE 每个术语条目 SHALL 包含字段：`term`（string）、`definition`（string，≤ 2 行）、`aliases`（string[]，可选）、`last_updated`（ISO 8601）、`source_session`（string，可选，指向 `.forge/knowledge/sessions/` 中的文件）
3. WHEN `.forge/glossary.md` 文件不存在时，THE `forge-spec` 或 `forge-plan` SHALL 在首次检测到新术语候选时懒创建该文件，写入表头与首个术语
...
**用户故事**：作为 Forge 维护者，我希望 `/forge decide` 在落盘 ADR 之前，必须通过三条硬门槛筛选，以便防止 `.forge/decisions/` 变成灌水目录、每个决策都有真实价值。
...
7. THE 三问筛 SHALL 对旧 ADR 无回溯影响——已存在于 `.forge/decisions/` 的文件不需要补填 reversibility 等字段
...
5. THE 输出 SHALL 生成 `.forge/findings/grill-<topic>.md`，包含字段：
...
3. THE 工作流 SHALL：
   ...
   - Step 2：调用 explore agent 或直接读取 `.forge/` 状态文件，构建整体视角
   - Step 3：输出三段式摘要
...
4. THE 输出 SHALL 采用固定三段式 Markdown，每段 ≤ 5 行：
   ...
   [与上下游模块的接口、不变量、职责边界]
   ```
5. THE zoom-out SHALL 不产生文件副作用（不写入 `.forge/` 任何文件），仅输出到对话
...
**用户故事**：作为 Forge 用户与维护者，我希望 `.forge/knowledge/sessions/` 的会话记录是结构化的 episode（含结果、评分、关联 pattern），`instincts.md` 的每条模式都带 applications/successes/failures 计数和衰减阈值，以便 `/forge learn` 能自动聚合同类经验、自动识别高频问题升级为 instinct、自动剪枝陈旧规则。
...
**关系说明**：本需求与需求 1（glossary 归档机制）同家族——glossary 30 天未引用提示归档、instinct 衰减阈值剪枝，可共用 `.forge/knowledge/` 的维护通道。与 engineering-governance-hardening 需求 3（Event Sourcing）互补——event log 是机器视图（jsonl 机读），episode 是人类视图（md 可读），两者不重复。

#### 验收标准

1. THE `.forge/knowledge/sessions/<date>-<topic>.md` 的 frontmatter SHALL 扩展为结构化 episode 字段：
...
5. THE `.forge/knowledge/instincts.md` 的每条模式 frontmatter SHALL 扩展为：
...
1. THE `.forge/reviews/<topic>.md` 与 `.forge/progress/<topic>.md` SHALL 支持 Evolution 标记行，格式为：
   ```markdown
   <!-- Evolution: YYYY-MM-DD | source: <episode_id | review_id | progress_id> | target: <skill_name>[#<section>] -->
   <具体描述：发现了什么模式，建议怎么改>
   ```
2. THE Evolution 标记 SHALL 只出现在 `.forge/reviews/*.md`、`.forge/progress/*.md`、`.forge/findings/*.md` 三类受保护区文件——不允许出现在 `skills/**/SKILL.md`（冻结区）、`.forge/config.md`、锁定的 spec
...
9. WHEN `/forge learn` 运行时，THE learn SKILL SHALL：
   - 扫描全仓 Evolution 标记（忽略 `.forge/archive/`）
...
- 不废弃 Forge 现有的 `.forge/` 状态系统换成 mattpocock 的 CONTEXT.md
...
- `.forge/glossary.md` 缺失时，forge-spec / plan / learn 行为与现状一致
