...
layout: tasks
created: 2026-05-05
spec_ref: ".forge/specs/skills-cross-pollination/requirements.md"
---
...
- [x] 2. Phase 1.2 — ADR 三问筛（需求 2）
  ...
  - [x] 2.4 行内注释落点实现
    ...
    - 上游文件识别：优先读 `.forge/status.md` 的 `current_task` 关联的 spec/plan/progress
...
- [x] 6. Phase 3.1 — Zoom-out（需求 6）
  ...
  - [x] 6.4 确保不产生文件副作用
    - zoom-out 仅输出对话，不写 `.forge/` 任何文件
    - Integration test：触发前后 `.forge/` 内容不变
...
- [x] 7. Phase 4.1 — Episode & Confidence Lifecycle（需求 7）
  ...
  - [x] 7.4 集成到 forge-learn（自动生成 episode）
    ...
    - 写入 `.forge/knowledge/sessions/<date>-<topic>.md`（Guarded zone 追加）
...
- [x] 8. Phase 4.2 — Evolution 标记 & 失败自动沉淀（需求 8）
  ...
  - [x] 8.7 集成到 forge-learn（生成 evolution-report）
    ...
    - 扫描 `.forge/reviews/**`、`.forge/progress/**`、`.forge/findings/**`（忽略 `.forge/archive/`）
    - 输出 `.forge/knowledge/evolution-report.md`（开放区，每次覆盖）
  ...
  - [x] 8.8 标记位置校验
    ...
    - 如在 `skills/**/SKILL.md` 或 `.forge/config.md` 或 locked spec 中发现标记 → 报错并退出
    - 加入 `npm run check` 执行序列
...
- [x] `.forge/glossary.md` 初始预置 12 个核心术语
