...
为 Forge plugin 新增 `ConfigChange` hook，监听 `.forge/config.md` 和 `.claude/settings.json` 的变化，通过 `additionalContext` 提示 Claude 重新读取配置。
...
2. **手动验证**：修改 `.forge/config.md` 后观察 hook 是否触发提示
