...
Claude Code 2.1.49 引入 `ConfigChange` 生命周期事件：当 `.claude/settings.json`、`CLAUDE.md` 等配置文件变化时自动触发 hook。Forge 依赖 `.forge/config.md` 作为项目级配置（`ci_check_command`、`decide_dispatch_mode`、`max_parallel_agents` 等），但配置变更后 Claude 不感知，导致行为与配置不一致。
...
- **Forge Config**：`.forge/config.md`，Forge 项目的配置单一事实源。
...
**User Story:** 作为 Forge 用户，我希望修改 `.forge/config.md` 后 Claude 能感知变化并重新加载配置，以避免使用过时的配置继续工作。

#### 验收标准

1. THE `scripts/config-changed-hook.mjs` SHALL 作为 `ConfigChange` 事件的 hook 脚本运行。
2. WHEN hook 被触发且变化文件匹配 `.forge/config.md`，THE script SHALL 输出 JSON `{ "additionalContext": "📝 Forge 配置已变更（<changed_files>），建议重新读取 .forge/config.md" }`。
...
2. THE 监听列表 SHALL 至少包含：`.forge/config.md`、`.claude/settings.json`。
