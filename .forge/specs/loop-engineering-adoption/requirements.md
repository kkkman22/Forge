...
**已经对齐、不做**：Skills（整个 `/forge` 即 skill 编排）、Worktrees（`worktree-manager.ts` + 分支门禁）、Memory（`.forge/` 文件系统交接 + `events.ndjson`）、验证与持久化（三层 review + progress 文件）、调度（`ScheduleWakeup`/`CronCreate`）。Forge 把论文"难的是往循环里放能说不的东西"写成了不可违反的 IRON-LAW 系统，是论文该思想的极致体现。
...
- **triage inbox**：自动发现的、值得处理的事项落盘文件（`.forge/triage-inbox.md`），对应论文 Memory 零件。"agent 会忘，仓库不会"。
...
4. THE triage skill SHALL 将每个值得处理的发现写入 `.forge/triage-inbox.md`，条目格式包含：`id`、`source`（jira-sprint/bitbucket-pr/bitbucket-branch/git-fallback）、`external_ref`（Jira case key 如 `CH-1234` / PR URL / commit sha）、`summary`、`severity`（high/medium/low）、`detected_at`、`status`（open/in-progress/done/skip）、`suggested_action`。
...
6. THE `.forge/state/triage-state.json` SHALL 记录 `last_triage_at` 时间戳，供下次 triage 增量扫描（只看上次之后的变更），避免重复报告已记录的 case/PR。
...
8. THE `.forge/config.md` SHALL 新增 triage 配置块（`forge init` 模板含默认值）：
...
9. THE triage skill SHALL 在 MCP 未配置时给出清晰指引（如何配置 `mcp-atlassian` 和 Bitbucket MCP），而非静默失败。指引指向 `.forge/config.md` 的 MCP 工具映射配置。
...
2. THE 关键改动复述段 SHALL 对 loop 期间的每个 build commit 生成一条：`commit_sha` + `subject` + `一句话人类可读说明`（这个改动解决了什么、改了什么）。说明由 loop 在 commit 时落盘到 `.forge/runs/<run_id>/commit-narrative.md`，而非事后从 git log 重构。
...
| triage 发现写入 inbox | 跑 `/forge triage`，`.forge/triage-inbox.md` 出现结构化条目（含 Jira case key / Bitbucket PR 引用） |
