...
>
> **已知背景（研究成果约束）**：
> - `.forge/config.md` 在**冻结区**，build 阶段 AI 不得修改。所有 config 变更只能落进 `scripts/init.sh` 的 heredoc 模板（生成新项目的默认值）+ 文档指引让用户手加。
...
| `.forge/templates/loop-state.json` | MODIFY（加 commitNarrativePath 字段） | R3 |
...
| `.forge/templates/triage-inbox.md` | CREATE | R2 |
| `.forge/templates/triage-state.json` | CREATE | R2 |
...
- **行为变更**：在 §6.2 原子提交处，commit 前向 `.forge/runs/<run_id>/commit-narrative.md` 追加一节：`commit_sha` + `subject` + `what`（干了什么）+ `why`（为什么）。在 commit 上下文最丰富时生成。
...
- **Commit**：`feat(build): commit-narrative landing to .forge/runs (anti understanding-rot)`

### Task 5: loop-state 模板加 commitNarrativePath 字段
- **目标文件**：`.forge/templates/loop-state.json`
- **行为变更**：加 `"commitNarrativePath": ""` 字段，loop init 时填为 `.forge/runs/<id>/commit-narrative.md`。
...
- **目标文件**：`.forge/templates/triage-inbox.md`（CREATE）+ `.forge/templates/triage-state.json`（CREATE）
...
- **行为变更**：在 config heredoc（L354-422）里加 `triage:` 默认配置块（enabled:false / cron 示例 / sources / stale_days / assignee / mcp 工具映射默认值）；在复制模板步骤加 triage-inbox.md + triage-state.json 复制到用户 `.forge/`。
...
| commit narrative 落盘 | 4,5 | build commit 后 `.forge/runs/<id>/commit-narrative.md` 出现条目 |
