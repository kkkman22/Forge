...
本特性为 Forge 引入**可插拔领域包（Domain Pack）机制**，把领域无关的方法论能力（DDD / ATDD / BDD / TDD / SDD）留在 Core 层，把领域特定知识（酒店 PMS、金融、电商等）下放到 `packs/<name>/` 子目录，业务项目通过 `.forge/config.md` 声明启用哪些 Pack。Forge 从"单用途 AI 工作流 skill 包"升级为"**通用工作流框架 + 可插拔领域生态**"。
...
3. **项目可定制**：同一个 PMS Pack 在不同酒店集团可通过 `.forge/custom/` 覆盖术语、场景、规则。
...
架构选择（分层覆盖）：本特性采用 **Core < Pack < Custom** 三层优先级。Core 提供纯方法论能力的引擎（Bounded Context 注册、Glossary 查询、Leak Detector 执行、Scenario Linter 规则），Pack 提供领域数据（Context 定义、术语表、禁用词清单、场景模板），Custom 提供项目级覆盖（`.forge/custom/<...>` 优先读取）。同名条目按优先级覆盖，不同名条目并存。Pack 和 Custom 均为 opt-in，未声明即不生效。

关键约束（贯穿所有需求）：

- **Zero-Pack-Zero-Impact**：未在 `.forge/config.md` 启用任何 Pack 时，Forge 核心代码路径行为与 Sprint 1 改造前完全一致；`npm run check` 跑完整回归测试通过。
...
- **Forge_Core**：Forge 领域无关的方法论能力层，包含现有 18 个命令、三维路由、`.forge/` 状态系统、以及本特性新增的 Bounded Context 引擎、Glossary 引擎、Leak Detector 引擎、Scenario Linter 引擎。
...
- **Enabled_Packs**：业务项目在 `.forge/config.md` YAML frontmatter 中通过 `packs:` 字段声明启用的 Pack 列表，按声明顺序加载。
- **Custom_Layer**：`.forge/custom/` 目录下的项目级覆盖文件，路径与 Pack 目录结构镜像（例如 `packs/pms/glossary/folio.md` 的覆盖位于 `.forge/custom/glossary/folio.md`）。
- **Resolution_Order**：条目查询的优先级：Custom_Layer → Enabled_Packs（按声明顺序）→ Forge_Core 默认。先命中先返回。
- **Bounded_Context**：DDD 限界上下文，一个独立的领域子域，拥有自己的统一语言和模型边界。在 Forge 中表现为 `packs/<name>/contexts/<context>.md` 或 `.forge/contexts/<context>.md` 文件。
...
- **Glossary**：分 Context 组织的术语表，结构为 `packs/<pack>/glossary/<context>.md` 或 `.forge/custom/glossary/<context>.md`。每条术语含名称、定义、别名、所属 Context、来源、更新日期。
...
- **Zero_Pack_Invariant**：未启用任何 Pack 时 Forge 行为与本特性改造前完全一致，包括：所有现有测试通过、所有 SKILL.md 外部契约不变、`.forge/` 文件读写行为不变、`npm run check` 退出码不变。
- **Pack_Loader**：`src/pack/loader.ts` 提供的纯函数，输入为仓库根路径和 `.forge/config.md` 解析结果，输出为 `{ registry, enabledPacks, resolutionOrder }`。
...
**User Story:** As a business project developer, I want to enable packs by declaring them in `.forge/config.md`, so that my project configuration makes its domain dependencies explicit and version-controlled.

#### Acceptance Criteria

1. THE `.forge/config.md` YAML frontmatter SHALL support a `packs:` field accepting a list of strings (pack names).
...
4. THE Spec Leak Detector engine (Requirement 7) SHALL be a no-op when `Enabled_Packs` is empty AND `.forge/custom/banned-patterns.yaml` does not exist.
...
6. THE Glossary engine (Requirement 6) SHALL fall back to the existing single-file `.forge/glossary.md` behavior when no per-context glossary files exist under any enabled pack or custom layer.
...
3. WHEN invoked with `enable <name>`, THE skill SHALL add `<name>` to `.forge/config.md` frontmatter `packs:` list (create the field if absent); idempotent (re-enable is no-op).
4. WHEN invoked with `disable <name>`, THE skill SHALL remove `<name>` from `.forge/config.md` frontmatter `packs:` list; idempotent.
...
6. WHEN invoked with `override <path>` where `<path>` is a relative path under any enabled pack (e.g., `glossary/folio.md`), THE skill SHALL copy the file from the first-hit pack to `.forge/custom/<path>` for user editing; refuse if destination already exists unless `--force` is passed.
...
6. WHEN `Enabled_Packs` is empty and no `.forge/custom/contexts/` exists, `loadContexts()` SHALL return an empty registry; calls to `registry.get(name)` SHALL return `null` for any name.
...
2. EACH glossary file SHALL follow the format `packs/<pack>/glossary/<context>.md` or `.forge/custom/glossary/<context>.md` where `<context>` matches a context name (or the special value `_shared` for cross-context terms).
...
5. WHEN `Enabled_Packs` is empty and no per-context glossary exists, THE `loadGlossary()` SHALL fall back to loading `.forge/glossary.md` as a single `_shared` context.
...
3. THE banned pattern registry SHALL be loaded from (in Resolution_Order): `.forge/custom/banned-patterns.yaml` → `packs/<enabled>/banned-patterns.yaml` → no defaults. Patterns are unioned across layers, not overridden.
...
**User Story:** As a hotel chain owner using the PMS pack, I want to override a specific glossary term, scenario, or banned pattern in `.forge/custom/`, so that my chain-specific semantics don't require forking the pack.

#### Acceptance Criteria

1. THE `.forge/custom/` directory SHALL mirror the structure of a pack's `extends` categories (`contexts/`, `glossary/`, `scenarios/`, `state-machines/`, `banned-patterns.yaml`, `templates/`).
2. WHEN the same relative path exists in both `.forge/custom/` and an enabled pack, THE `Override_Resolver` SHALL return the Custom_Layer file.
...
4. WHEN `.forge/custom/banned-patterns.yaml` exists, its categories SHALL be **unioned** with pack banned-patterns (not overridden), since multiple banned categories are additive.
5. WHEN `.forge/custom/` does not exist or is empty, the resolver SHALL fall through to pack layer transparently; Zero_Pack_Invariant SHALL hold.
