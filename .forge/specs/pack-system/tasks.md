...
layout: tasks
created: 2026-05-09
spec_ref: ".forge/specs/pack-system/requirements.md"
---
...
- [x] 9. `src/context/registry.ts` — Context 加载
  ...
  - [x] 9.2 GREEN：实现 `loadContexts(enabledPacks, fs)`
    - 遍历 `.forge/custom/contexts/*.md` 和每个 pack 的 `contexts/*.md`
...
- [x] 11. `src/glossary/registry.ts` — 分 Context 术语加载
  ...
  - [x] 11.2 GREEN：实现 `loadGlossary(enabledPacks, fs)`
    ...
    - 向后兼容：空输入且无 custom → 读 `.forge/glossary.md`
...
- [x] 25. `test/pack/zero-pack-invariant.test.ts`
  - [x] 25.1 设置 `packs:` 为空的 `.forge/config.md` fixture
...
- [x] 27. README 与 CHANGELOG
  ...
  - [x] 27.3 `.forge/knowledge/adr-index.md` 追加"Pack 机制引入"ADR 条目
  - [x] 27.4 生成 ADR `decide` 文档，`.forge/decisions/ADR-NNNN-pack-system.md`
