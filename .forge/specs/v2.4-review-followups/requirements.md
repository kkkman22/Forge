...
- `.forge/findings/dogfooding-observations.md` 发现 6「Context Window 压力」
- `.forge/findings/skill-function-audit.md`（30 函数映射审计）
...
4. WHEN `--force-no-hooks` 被使用时，THE 当次运行 SHALL 在 `.forge/runs/<runId>/` 目录下写入一个 `force-no-hooks.flag` 文件，记录时间戳与 CLI 参数，供审计追溯
...
3. THE E2E 套件 SHALL 覆盖以下 5 条路径，每条路径独立测试文件：
   ...
   - **worktree 模式**（`e2e-worktree.test.ts`）：`--worktree` 创建隔离分支 → 运行完成 → 删除 worktree 前 `backupWorktreeNotes` 写入主仓库 `.forge/runs/<runId>/notes.md`
...
4. THE 每条 E2E SHALL 至少包含两类断言：
   ...
   - **状态文件快照断言**：`.forge/status.md` 或 `.forge/runs/<runId>/notes.md` 内容匹配预期 frontmatter 字段
...
**用户故事**：作为 Forge 用户，我希望每次 prompt 注入的 plan 上下文被限制在 token 预算内，以便避免 `.forge/plans/` 膨胀时把大量无关 plan 的头部塞进每条 prompt。

#### 验收标准

1. THE `hooks/hooks.json` 中 `UserPromptSubmit` hook 的 `head -50 .forge/plans/*.md` 命令 SHALL 被替换为调用 `scripts/inject-plan-context.mjs`
2. THE `scripts/inject-plan-context.mjs` 脚本 SHALL 具备以下行为：
   - 扫描 `.forge/plans/*.md`，仅选取 frontmatter 中 `status: active` 的 plan（其他忽略）
   - 按 mtime 倒序取最多 3 个
...
7. THE 改动 SHALL 关闭 `.forge/findings/dogfooding-observations.md` 中发现 6「Context Window 压力」的 plan 注入部分
...
- `forge-v2.3-technical-review.md` §7.1, §8, §9（主要来源）
- `forge-v2.3-executive-audit.md`（交叉验证）
- `.forge/findings/skill-function-audit.md`（需求 5）
- `.forge/findings/dogfooding-observations.md` 发现 6（需求 8）
- `ROADMAP.md` v2.2.1 H-1 `validateHooksPresence` 的基础（需求 1 在其上继续强化）
- `.forge/decisions/ADR-TEMPLATE.md`（每条需求实施时需补 ADR）
