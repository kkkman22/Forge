...
layout: tasks
created: 2026-05-07
spec_ref: ".forge/specs/v2.4-review-followups/requirements.md"
---
...
- [x] 1. Phase 1.1 — Hook 阻断强化（需求 1）
  ...
  - [x] 1.4 写 `force-no-hooks.flag` 审计文件
    ...
    - 写入 JSON 格式（timestamp、cliArgs、reason、runId）到 `.forge/runs/<runId>/`
    - _Requirements: 1.4_
...
- [~] 4. Phase 2.1 — E2E 测试套件（需求 2）
  ...
  - [x] 4.1 新增 `test/e2e/helpers/temp-repo.ts`
    ...
    - `copyForgeSkeleton(cwd)`：从 fixture 复制最小 `.forge/` 与 `hooks/` 结构
  ...
  - [x] 4.3 新增 `test/e2e/helpers/snapshot.ts`
    ...
    - `assertStatusFile(cwd, expected)`：解析 `.forge/status.md` frontmatter 比对
...
- [x] 5. Phase 2.2 — API 面收敛（需求 3）
  - [x] 5.1 盘点 `src/index.ts` 现有 51 个 export
    ...
    - 输出到 `.forge/findings/public-api-audit-2026-05.md` 供 review
    - _Requirements: 3.3_

  - [x] 5.2 决策保留清单
    - 按"文档 + examples + SKILL"三选二标准决定保留集
    - 维护者评审后锁定 ≤ 20 个保留符号
    - 更新 `.forge/findings/public-api-audit-2026-05.md` 记录决策
...
- [x] 6. Phase 2.3 — Plan 注入收敛（需求 8）
  - [x] 6.1 新增 `scripts/inject-plan-context.mjs`
    - 读取 `.forge/plans/*.md`，过滤 `status: active`
    - 按 mtime 倒序取 3 个
  ...
  - [x] 6.4 Dogfooding 发现 6 跟进
    - 更新 `.forge/findings/dogfooding-observations.md`，标记发现 6 的 plan 注入部分为"已解决"
