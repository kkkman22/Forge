...
- **StatusFile**：`.forge/status.md`，YAML frontmatter 格式的中央状态协调文件
...
- **PlanFile**：`.forge/plans/*.md`，任务拆解计划文件
- **ProgressFile**：`.forge/progress/<topic>.md`，任务进度追踪文件
...
1. WHEN `.forge/` 目录不存在, THE Loop SHALL 输出提示信息并拒绝启动
