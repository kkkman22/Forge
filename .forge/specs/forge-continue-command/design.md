...
新增 `/forge continue` 子 skill,读取 `.forge/status.md` 当前 phase/tier,调用已有的 `getNextPhase` + `buildNextForgeArgs` 生成下一个 `Skill(skill="forge", args="<phase>")` 调用。把 loop 路径(自主后台)已验证的推进逻辑接到交互式路径,顺带处置 `determineNextSkill` 死代码。
