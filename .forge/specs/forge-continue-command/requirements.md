...
- `src/status-manager.ts:54` `readTaskStatus`——已能从 `.forge/status.md` 提取 `phase`/`tier`/`current_package`/`review_result`。

本 spec 新增 `/forge continue` 命令,**把 loop 路径已有的推进逻辑接到交互式路径**,让用户只需反复敲 `continue` 即可推进工作流,无需记忆命令序列。这与 Trellis 的 `continue` 命令理念一致。

## Goals

- 提供单一入口 `/forge continue`,读取 `.forge/status.md` 当前 phase/tier,自动推进到下一个合法 phase。
...
1. THE continue skill SHALL 调用 `readTaskStatus`(`src/status-manager.ts:54`)读取 `.forge/status.md` 的 `phase`、`tier`、`work_nature`、`current_package`、`review_result`、`testPassed` 字段。
