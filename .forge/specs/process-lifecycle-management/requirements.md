...
- **PID_File**：PID 文件，存储在 `.forge/.pids/` 目录下的文件，记录某个会话所管理的子进程 PID 列表，用于跨会话的孤儿进程检测
...
1. WHEN 一个新的 Session 启动时，THE Forge_CLI SHALL 在 `.forge/.pids/` 目录下创建一个 `session-<sessionId>.pid` 文件，记录当前会话的 PID、PGID 和所有已注册子进程的 PID
2. WHEN Process_Registry 中的子进程列表发生变化时，THE Process_Registry SHALL 更新对应的 PID 文件内容
3. WHEN 一个新的 Session 启动时，THE Forge_CLI SHALL 扫描 `.forge/.pids/` 目录下的所有 PID 文件
