...
- `.forge/status.md` stores task, tier, phase, branch, and updated date, but not the execution environment metadata needed to reconstruct dispatch choices after resume.
...
**User Story:** As a Forge user resuming a task, I want `.forge/status.md` or associated progress metadata to record key execution environment choices, so `/forge resume`, debug, and review can explain how the previous run was dispatched.
...
5. THE docs SHALL mention metadata as diagnostic context, not as a behavior source of truth that overrides `.forge/config.md`.
   - Verify-By: manual
...
- No secrets may be written to `.forge/status.md`, `.forge/progress/`, or `.forge/debug/`.
