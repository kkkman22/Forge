...
Forge 当前将知识库热缓存（processed rules、evolved-rules 缓存等）存储在项目内 `.forge/` 目录或临时目录。plugin 更新后这些缓存丢失，需要重新计算。`audit-log.js` 已使用 `process.env.CLAUDE_PLUGIN_DATA` 但作为回退模式，缺少统一的路径解析策略。
...
- **热缓存**：处理后用于 hook 注入的缓存版本（非 `.forge/knowledge/` 中的源文件），如 evolved-rules 编译缓存。
- **源数据**：存储在 `.forge/knowledge/` 中的项目级知识文件，不迁移。
...
3. WHEN 源文件（`.forge/knowledge/evolved-rules.md`）的 mtime 比缓存文件新，THE script SHALL 重新编译并更新缓存。
