...
- **Goal**: `forge-doctor` 报告 `FORGE_ROLLBACK_NONCE` / `FORGE_ALLOW_DESTRUCTIVE` env 是否被设置;env 被设置但 nonce 文件缺失 → warn(检测潜在伪造)。同时报告 guard 是否实际激活(检测 `.forge/.sandbox-active.json`,P1-1)。
- **TDD Steps**:
