 ...
 * v3:来自一次性随机生成的 .forge/.guard-secret(0600,首次自动生成,后续不变);
...
export function issueRollbackNonce(projectRoot: string): string;  // 写 .forge/.rollback-nonce
export function issueAllowNonce(projectRoot: string): string;     // 写 .forge/.allow-destructive-nonce
...
R4 不新增 src 模块——它在 `skills/forge/lib/learn/` 的实现里调用一个已有的计数能力(扫 `.forge/knowledge/solutions/` 文件数),与 `knowledge_limit` 比较后输出 `[knowledge-near-limit]` 提醒。为可测,把判定抽成纯函数放 `src/knowledge-quota.ts`:
...
- **`.forge/.rollback-nonce`** — loop/事务回滚 skill 执行 `git reset --hard` 前由 `issueRollbackNonce()` 写入(内容:nonce 值 + HMAC)。guard 的 `contextFromNonce` 校验文件存在 + HMAC 合法后置 `rollbackActive=true`,**校验后删除文件**(即焚,确保单次有效且不可重放)。
- **`.forge/.allow-destructive-nonce`** — 用户单次放行,同理即焚。
...
- `.forge/config.md` — frontmatter 字段(check-sandbox 入口新增读取):
...
- `.forge/knowledge/tool-health.md` — 追加 spawn-policy-error / max-depth-exceeded 事件类型。
