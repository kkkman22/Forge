...
| R4 evolved-rules R6 条目 | `.forge/knowledge/evolved-rules.md` frontmatter + 新段 | 15 分钟 |
...
│  └── .forge/knowledge/evolved-rules.md R6 新条目                │
└────────────────────────────────────────────────────────────────┘
                              ↓ 消费
┌────────────────────────────────────────────────────────────────┐
│                   现有 Forge 基础设施（不改）                    │
 ...
 *   - `npx tsc --outDir .forge/.dist-sync-check/` (may need temporary tsconfig)
...
npm run check
  ...
  → node scripts/check-dist-sync.mjs
    ...
    → npx tsc --outDir .forge/.dist-sync-check/
    → 比较 tracked dist/ vs .forge/.dist-sync-check/
...
- 临时 outDir `.forge/.dist-sync-check/` 在 `.gitignore` 已存在的 `.forge/` 覆盖下自动忽略
...
5. `.forge/knowledge/evolved-rules.md` 含 R6 条目，rule_count=6
