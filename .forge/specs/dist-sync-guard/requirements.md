...
1. THE `scripts/check-dist-sync.mjs` SHALL be a Node.js script runnable via `node scripts/check-dist-sync.mjs` that performs these checks in sequence:
   ...
   - (C) Run `tsc --noEmit` first to ensure TS compiles cleanly. Then run `tsc` (with emit) to a **temporary outDir** (e.g., `.forge/.dist-sync-check/`); diff the temp output against the tracked `dist/` to detect compilation drift.
...
4. THE script SHALL clean up its temp outDir (`.forge/.dist-sync-check/`) before exiting, regardless of success or failure.
...
1. THE `.forge/knowledge/evolved-rules.md` SHALL gain a new entry R6 titled "src/dist 同步是 PR 合约一部分".
