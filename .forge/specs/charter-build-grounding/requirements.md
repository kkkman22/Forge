...
Forge 的 `.forge/charter.md`(项目宪章,cross-spec 工程约束锚点)目前在 decide/spec/plan/review 四个阶段被读取并作为 grounding 约束,但**在 build 阶段是盲区**。
...
1. THE `skills/forge/lib/build/instructions.md` SHALL 在 §2 Pre-build Checks 之后新增一节(建议 §2.5 或 §1.7)"Charter Grounding",内容为:当 `.forge/charter.md` 存在且 frontmatter `status: active` 时,读取并注入其 grounding 摘要(格式同 `charter/instructions.md:55-64` 定义的核心问题/架构边界/INV-NNN 列表,≤500 tokens)。
