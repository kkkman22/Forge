...
layout: tasks
created: 2026-05-29
spec_ref: ".forge/specs/sandbox-phased-implementation/requirements.md"
---
...
- [x] 4. 实现配置加载和 Profile 解析
  - `loadSandboxConfig(configPath?)` 从 .forge/sandbox.json 读取
  - 文件不存在时返回默认配置（全部允许）
...
- [x] 6. 集成 forge init
  - 更新 forge init SKILL 文档，在生成 .forge/ 时同时生成 sandbox.json
  - 从 templates/sandbox.json 复制到 .forge/sandbox.json
  - _Requirements: 1.3_

- [x] 7. 增强 --sandbox CLI 选项
  - `--sandbox` 无参数 → 使用 .forge/sandbox.json 的 default profile
