...
| `.forge/charter.md` | 工程策略锚定（约束、边界、基线） | 低（季度级） | 团队 |
| `.forge/decisions/*.md` | 单次架构决策 | 中（每 feature 1–2 次） | AI + 用户 |
| `.forge/specs/*/spec.md` | 功能需求规格 | 高（每 feature） | AI + 用户 |
...
```
/forge decide "加一个直接查数据库的性能优化接口"
→ decide agent 读取 .forge/charter.md
...
**回滚**：`status: deprecated` 或直接删除 `.forge/charter.md`。下游 skill 自动降级。
...
### D3 — 为什么用 `.forge/` 而非项目根目录？

**选择**：`.forge/charter.md`

**理由**：
- CE 的 `STRATEGY.md` 放在项目根目录是因为它面向人类+AI 双重阅读
- Forge 的 `.forge/` 是受保护区域，charter 不容易被意外修改
- `.forge/` 已经是 Forge 状态管理的标准位置
