  ...
  用户在 Claude Code 内: /forge loop "做X"
    → Forge dispatcher (fork dispatch)
      → loop skill (instructions.md)
        → 读取 .forge/progress/loop-{id}.json
...
ScheduleWakeup 触发 → prompt: "/forge loop continue {id}"
  │
  ├─ 1. 读取 loop state (.forge/progress/loop-{id}.json)
  │
  ├─ 2. 质量门禁检查
...
**文件**: `.forge/progress/loop-{id}.json`
...
| `.forge/progress/build-goal-replace-loop.md` | 历史记录保留 |
