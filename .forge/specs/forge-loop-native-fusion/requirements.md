...
| **保留**（工作流基础设施） | Forge dispatcher（ADR-0004）、`.forge/` 文件系统状态、TDD 铁律、分支保护、review 三层评审 | 不变 |
...
| **loop state** | `.forge/progress/loop-{id}.json`，记录 loop 实例的阶段、计数器、目标等状态 |
...
1. WHEN 用户输入 `/forge loop "<goal>"` THEN 系统 SHALL 创建 `.forge/progress/loop-{id}.json` 状态文件，含字段 `{id, goal, phase, consecutiveFailures, totalIterations, createdAt, stopWhen}`
...
3. WHEN 用户输入 `/forge loop status` THEN 系统 SHALL 列出所有活跃的 loop 实例（从 `.forge/progress/loop-*.json` 读取），显示每个实例的 id、goal、当前 phase、失败次数、下次调度时间
...
2. **状态一致**：loop state JSON 的 `phase` 始终与 `.forge/` 目录下的实际工作产物（spec、plan、review 结果等）一致
...
| 风险 | 概率 | 影响 | 缓解 |
|------|------|------|------|
| ScheduleWakeup 在 context compaction 后行为异常 | 中 | 高 | loop state 持久化到 `.forge/progress/`；`--resume` + `/forge loop continue` 恢复 |
