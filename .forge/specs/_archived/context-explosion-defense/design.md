...
修改后：
  主 agent spawn spec-check →
    subagent Write .forge/reviews/spec-check-20260530-061500.md
    subagent 返回: "status:pass findings:3 p0:0 p1:0 report:.forge/reviews/..."
...
你的最后一步必须：
1. Write 完整报告到 .forge/reviews/<layer>-<timestamp>.md
2. 最终返回文本限制在 800 chars 以内，格式：

   status: <pass|fail>
   findings: <total_count>
   p0: <count> | p1: <count>
   report: .forge/reviews/<path>
...
4. 综合评审报告仍然输出到 .forge/reviews/<timestamp>-combined.md
```
...
新增产物：
1. `.forge/reviews/<layer>-<timestamp>.md` — subagent 文件化返回
