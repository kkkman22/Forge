...
- **File-Based Return**：Subagent 将完整结果写入 `.forge/` 文件系统，仅向主 agent 返回结构化摘要。
...
6. THE phase-advance 触发的 `/clear` SHALL 在 `.forge/progress/` 中记录阶段完成状态和 context 快照，以便 resume 时恢复。
...
1. THE `spec-check.md`、`quality-check.md`、`security-check.md` agent 定义 SHALL 在末尾增加指令：将完整 Layer 报告 Write 到 `.forge/reviews/<layer>-<timestamp>.md`。
...
5. THE 报告文件路径 SHALL 遵循 `.forge/reviews/<layer>-<YYYYMMDD-HHmmss>.md` 命名约定。
...
2. THE `/forge resume` SHALL 从 `.forge/progress/<topic>.md` 读取当前阶段和任务状态，而非从对话历史恢复。
