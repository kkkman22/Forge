...
1. `.forge/config.md` SHALL 支持 `ship_default_method` 配置项，可选值为 `merge`、`push-pr`、`keep-branch`、`prompt`
...
1. WHEN `.forge/config.md` 中未配置 `ship_default_method` 时，autonomous 模式 SHALL 保持 "keep branch" 行为
