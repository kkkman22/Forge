...
- **R21 横跨三层**：任何阶段发现需要改 `src/` 就立刻 defer 到 `.forge/features/`，不在本 spec 内执行。
...
flowchart TD
    ...
    M --> N[basename + session_id 写入<br/>.forge/.deprecation-notice/&lt;sid&gt;/&lt;cmd&gt;.lock]
    N --> O{锁文件是否已存在?}
    O -- 是 --> P[静默跳过]
...
| 维度 | 设计选型 | 原因 |
|------|---------|------|
| 唯一性标识 | `session_id`（从 Claude Code 环境变量或 `.forge/status.md` 的 `runId` 取）+ 命令名 basename | 保证跨子会话不会重复提醒；进程崩溃重启后 session_id 变化，允许再次提醒一次 |
| 存储位置 | `.forge/.deprecation-notice/<session_id>/<command>.lock`（0 字节文件） | 与现有 `.forge/.stop-hook-dedupe/` 约定一致；目录自动 GC（session 结束后由现有 cleanup hook 清理） |
...
**Edge case**：如果 `.forge/` 被 hooks 暂时阻断（极少见），改写到 `/tmp/.forge-deprecation-<session_id>/<cmd>.lock`；Notice 本身不应成为阻断项。
...
findings:
  ...
  - id: "F-006"
    ...
    severity: "P0"
    violates_spec: ".forge/specs/foo-bar/requirements.md#R3.2"
    message: "..."
merged_summary:
  P0_blockers: 2
  P1: 8
...
- **输出格式保持向后兼容**：`.forge/reviews/*.yaml` 的 top-level schema 原有的字段全部保留，`sources` / `merged_summary` 为新增字段（R19.5）。
...
```mermaid
flowchart TD
    A[packs/*/pack.yaml] --> B[loadEnabledPacks<br/>.forge/config.md:pack_activation]
    ...
    G -- 否 --> I[skipSilently<br/>写入 .forge/audit/pack-conditional-skipped.log]
...
运行时短路实现写在 SKILL.md 的正文第一段（不进入 `src/`）：若 `.forge/config.md` 未激活对应 pack，打印 `fallback_message` 并终止执行。
...
- `scripts/install-dist.sh` 在目标项目初始化后调用 `gen-plugin-commands.mjs`，按目标项目 `.forge/config.md` 的 pack 激活状态决定是否创建 `commands/forge-mutate.md`。
...
scripts/gen-plugin-commands.mjs
  ...
  - 输出 3: 命令总数 N (纯数字, 写入 .forge/.command-count)
        │
        ▼
...
for each plan or spec dir:
  evidence = {
    ...
    in_active_progress: .forge/progress/ 是否有未完成项引用这个 spec/plan
    in_status_current: .forge/status.md 的 current_task 是否指向这个
  }
...
`.forge/archive/.audit-YYYY-MM-DD.md`：

```markdown
| path | status | evidence | action |
|------|--------|----------|--------|
| .forge/plans/foo.md | shipped | roadmap:v2.4 + changelog:2026-05-01 + no-active | moved to .forge/archive/2026-05-20-foo/ |
| .forge/specs/bar/ | active | progress:.forge/progress/bar-build.md | keep in place |
| .forge/specs/baz/ | ambiguous | no-roadmap-mention + no-progress | logged to .audit-pending.md |
```

未决项追加到 `.forge/archive/.audit-pending.md`（R3.3），供人工复核。

#### 4.4 交叉引用更新

`scripts/audit-archive-candidates.mjs --fix-refs` 扫描 `docs/`、`README.md`、`.forge/features/` 下对已归档路径的引用，将 `.forge/plans/foo.md` 自动改写为 `.forge/archive/<date>-foo/foo.md`。修改逐条写入 audit log 的 `action` 字段。

**不改动 `src/` 核心在本节的生效点**：
- 审计脚本是纯 Node 脚本，不 touch `src/`。
- 写入的内容只在 `.forge/` 下，遵循 Frozen_Zone 语义：归档前检查原路径是否被 Spec_Lock 标记 `locked`；是则跳过并报告（等待 spec 结束锁定状态）。
...
新增 `.forge/decisions/TEMPLATE-integration-evaluation.md`：
...
2. **Hooks ndjson writer**（新增，轻量）：`scripts/metrics-recorder.mjs`，被 `plugin.json` 的 UserPromptSubmit hook 调用，写入 `.forge/.metrics/<YYYY-MM>.ndjson`。
...
- Hooks 的 ndjson writer 是**额外一条写入通道**，独立文件（`.forge/.metrics/`），不影响现有 events.ndjson 消费者（cmux Mirror_Daemon、未来 IDE 插件等）。

#### 5.3 决策归档路径

- `.forge/decisions/<ISO-date>-forge-maintenance-evaluation.md`（R14.1）
- `.forge/decisions/<ISO-date>-grill-zoomout-usage.md`（R16.2）
- `.forge/decisions/<ISO-date>-skill-count-deviation.md`（R17.2，仅当最终数量落在 18-22 范围外时生成）
...
| 典型输出产物 | `.forge/accept/<scenario>.md` | `.forge/verify/<task>.md`（three-state） | PR merge + tag + CHANGELOG entry |
...
| Usage_Metrics 采集压力 | hooks 每次 prompt 都写一行 ndjson，14 天下来文件过大 | 按月滚动（`.forge/.metrics/<YYYY-MM>.ndjson`），合并评估只读取需要的时间窗 |
...
```yaml
# .forge/audit/pack-conditional-skipped.log（append-only）
...
数值来源: scripts/gen-plugin-commands.mjs 导出 .forge/.command-count 纯数字文件
...
*For any* 启用的 pack 集合 P、聚合得到的 feature_flag 集合 F(P) = ⋃_{p ∈ P} flags(p) 与声明 `pack_conditional.required_flag = flag` 的 skill K，`gen-plugin-commands.mjs` 的注册函数 `shouldRegister(K, P)` 满足 `shouldRegister(K, P) ⟺ flag ∈ F(P)`；无 `pack_conditional` 字段的 skill 对任意 P 恒等注册（`shouldRegister(K, P) = true`）；同时输出的 `commands/` 目录集合、`plugin.json.commands` 数组、`.forge/audit/pack-conditional-skipped.log` 记录三者对同一 (K, P) 输入的判定一致。

**Validates: Requirements 13.2, 13.3, 18.4, 20.4**

### Property 5: 命令调用语法前后兼容

*For any* 瘦身前合法的 `/forge <subcommand> [args...]` 调用语法 I ∈ pre-slimming 合法集合（包括 `/forge control-cli`、`/forge control-ui`、所有 T2 受影响命令的原生调用形式、Pack_Conditional_Skill 在 pack 启用状态下的调用），post-slimming 的入口编排（`commands/forge.md` + 各 SKILL.md）都接受 I 不抛 syntax error；对消费 `.forge/reviews/*.md` / `.forge/runs/*.md` 等产物的下游工具，以 pre-slimming schema 解析 post-slimming 输出，解析成功且原有字段值保持功能等价（可能新增 `sources[]` / `merged_summary` 等字段但不删除既有字段）。

**Validates: Requirements 12.3, 12.4, 19.1, 19.5**

### Property 6: 归档保持原目录结构

*For any* 被 `classify` 判定为 `shipped` 的源路径 src（可能是 `.forge/plans/<slug>.md` 或 `.forge/specs/<slug>/` 子树）与目标 `.forge/archive/<ISO-date>-<slug>/`，归档操作 `archive(src, dst)` 满足：(a) `tree(dst)` 与归档前 `tree(src)` 的相对路径 / 内容 / 权限 bit 逐项相等；(b) 归档前指向 src 的任意跨引用（docs/ / README.md / .forge/features/ 中的 Markdown 链接）在归档后被重写为指向 dst 的等价链接，且重写后的链接解析到的内容与归档前一致。
...
| 场景 | 处理 |
|------|------|
| `packs/*/pack.yaml` 解析失败 | `gen-plugin-commands.mjs` 以错误退出；`.forge/audit/pack-conditional-skipped.log` 记录失败原因；CI 阻断 |
...
- `FrozenZoneViolation` 错误类保持原样抛出；任何 `.forge/` 下的归档 / 生成写入都经过现有 hooks。
