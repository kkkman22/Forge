...
layout: tasks
created: 2026-05-17
spec_ref: ".forge/specs/subagent-result-truncation/requirements.md"
---
...
- [x] 4. **Decision Gate Stage 1** — Real `/forge review` 主 agent 会话 dogfood
  ...
  - 步骤：
    ...
    4. **记录**：把 spec-check 完整输出 + tool_uses + duration 追加到 `.forge/findings/subagent-result-truncation-stage1.md`。
  ...
  - Verify: 手工读 `.forge/findings/subagent-result-truncation-stage1.md`，确认 spec-check Layer 1 报告完整。
...
- [x] 7. GREEN — 修改 `.claude/agents/quality-check.md`（② 三处改动）
  ...
  - 实现细节（与 task 2 同模板，差异点）：
    ...
    3. Method 合并：quality-check 当前没有 Step 0.5 contract，仅 Step 0.5 known-failures Read + Step 0.6 append。改为 "known-failures Read 可选"语义：`如果 .forge/knowledge/known-failures.md 存在 AND review scope ≥ 1 文件，进行 Read；否则跳过`。Step 0.6 byte-equal 保留。
...
- [x] 12. **Decision Gate Stage 2** — Real `/forge review` 主 agent 会话 dogfood + spec closure (PARTIAL — 2026-05-17)
  ...
  - 步骤：
    ...
    4. **附加 Quality-check Preservation 检查（Property 2）**：把本次 quality-check 输出与 `.forge/findings/subagent-hook-context-budget-smoke.md` § Real Smoke Run 的 quality-check baseline 做对比。允许 ≤ 5% 自然语言波动，但 Layer 标题 / severity 表格列数 / 行数必须一致。
    5. **记录**：把三个 subagent 完整输出 + tool_uses + duration + Quality-check preservation diff 追加到 `.forge/findings/subagent-result-truncation-stage2.md`，并更新 `.forge/findings/subagent-hook-context-budget-smoke.md` frontmatter 的 `status` 从 `partial-closure` 改为 `complete`（因为前序 spec acceptance criterion 2 现在被本 spec closure 一并解锁）。
  ...
  - Verify: 手工读 `.forge/findings/subagent-result-truncation-stage2.md`，确认三个 subagent Layer 报告完整且 quality-check preservation 通过。
  ...
  **Closure Note (2026-05-17 — three-strike reroute)**:
  ...
  on main + Pack/Loader Integration Evidence）需要枚举 `.forge/plans/` 进行
  存在性验证；fixture 含 ≥ N 个 plan 文件时打满 maxTurns 在 Plans-enumeration
  loop 中无法进入文本生成阶段。详细决策与四个候选根因方向见
  `.forge/findings/subagent-result-truncation-stage3.md` § Closure Note。
...
> cause: spec-check's R6/R7 checks enumerate `.forge/plans/` and exhaust
...
2. **Stage 1 + Stage 2 dogfood smoke pass**：`.forge/findings/subagent-result-truncation-stage1.md` 与 `.forge/findings/subagent-result-truncation-stage2.md` 都已生成并记录三个 review subagent 完整 Layer 报告。
...
- **Real Smoke 的 fixture 依赖**：task 4 / task 12 的 `/forge review` 必须在 `.forge/plans/` ≥ 5 个 ≥ 4 KB active plan 且 `.forge/knowledge/evolved-rules.md` ≥ 8 KB 的环境下执行（与前序 spec 同 fixture，已就位）。Stage 1 / Stage 2 之间不刷新 fixture，可对比 spec-check 在 Stage 1 单文件状态 vs Stage 2 三文件状态下的行为差异。
...
- **外部参考**：
  - 前序 spec：`.forge/specs/subagent-hook-context-budget/{bugfix,design,tasks}.md`
  - 前序 spec findings：`.forge/findings/subagent-hook-context-budget-smoke.md`
