...
前序 spec `subagent-hook-context-budget` 修掉了 hook 注入字节量（subagent 路径已经 0 字节，量化证据见 `.forge/findings/subagent-hook-context-budget-smoke.md` § Mock Smoke）。但 Real Smoke 揭示 `tool_uses ≥ 5` 的 review subagent（`spec-check` / `security-check`）仍只返回 preamble 而非结构化 Layer 报告。本 spec 修复这残留现象。
...
- Step 0.5 — Known-failures Recurrence Detection（Read `.forge/knowledge/known-failures.md`）
...
当前 Step 0.5 仅有 Known-failures Read。合并目标：把 Step 0.5 的 known-failures Read **改为可选**（`if .forge/knowledge/known-failures.md exists AND review scope ≥ N files`），让 review scope 小（如本次 metrics.md 一行）的场景跳过 known-failures，直接进主流程。Step 0.6 保留。
...
**Test Plan**：跑一次 baseline `/forge review`（在 Stage 1 commit 之前）。预期：`spec-check` / `security-check` 在 `tool_uses ≥ 5` 时返回 preamble-only。这本身就是 Real Smoke 已经记录的现象，无需重复——直接引用 `.forge/findings/subagent-hook-context-budget-smoke.md` § Real Smoke Run 作为 counterexample。
...
**Test Plan**：在 Stage 2 完成后跑一次 `/forge review` 在与 Real Smoke 相同 fixture 上，断言 quality-check 的输出 byte-equal `.forge/findings/subagent-hook-context-budget-smoke.md` 的 quality-check § Real Smoke Run 引用。允许 ≤ 5% 差异（reviewer 自然语言波动），但 severity 表格列数 / 行数 / Layer 标题必须一致。
...
# .forge/findings/subagent-result-truncation-stage1.md
```
...
# .forge/findings/subagent-result-truncation-stage2.md
```
...
| Real Smoke 在 fixture 改变后表现不一致 | findings 文件锚定 `.forge/plans/` 文件数与 `.forge/knowledge/evolved-rules.md` 字节数为 fixture 状态戳；Smoke 前必须 echo 这两个值 | findings 文件 frontmatter |
