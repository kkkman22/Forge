...
- **CI_Check_Command**: 项目在 `.forge/config.md` 的 YAML frontmatter 中通过 `ci_check_command` 字段配置的完整 CI 检查命令（如 `npm run check`），一条命令覆盖所有 CI 检查步骤。
- **Verify_Commands**: `.forge/config.md` 的 YAML frontmatter 中通过 `verify_commands` 字段配置的逐条验证命令列表（如 `npm run lint`、`npm run typecheck`），供 Forge Loop 的 TDD 循环使用。
...
- **Init_Script**: `scripts/init.sh`，Forge 项目初始化脚本，交互式收集项目配置并生成 `.forge/config.md`。
- **Config_Template**: `templates/config.md`，`.forge/config.md` 的模板文件，定义所有可配置字段的默认值和说明。
...
1. WHEN all tasks are completed and Final_Validation begins, THE Forge_Build_SKILL SHALL instruct the AI to read the `ci_check_command` field from `.forge/config.md` YAML frontmatter and execute that command as the full validation step.
2. WHILE `ci_check_command` is non-empty in `.forge/config.md`, THE Forge_Build_SKILL SHALL prohibit the AI from substituting, omitting, or partially reconstructing the CI check command.
3. IF `ci_check_command` is empty or absent in `.forge/config.md`, THEN THE Forge_Build_SKILL SHALL fall back to executing each command in the `verify_commands` list sequentially; IF `verify_commands` is also empty or absent, THEN THE Forge_Build_SKILL SHALL fall back to AI auto-detection of verification commands.
...
1. WHEN executing Layer 3 Completion_Checklist, THE Forge_Test_SKILL SHALL instruct the AI to read the `ci_check_command` field from `.forge/config.md` and use it as the primary verification command for checklist items 1-4 (test execution, test pass, type check, lint).
...
3. WHEN the user provides a CI_Check_Command value, THE Init_Script SHALL write it to the `ci_check_command` field in `.forge/config.md` YAML frontmatter and include the CI check command section in the config body.
4. WHEN the user leaves CI_Check_Command empty (presses Enter without input), THE Init_Script SHALL set `ci_check_command` to an empty string in `.forge/config.md`, and the system SHALL fall back to verify_commands or AI auto-detection.
...
1. WHILE `ci_check_command` is absent from `.forge/config.md` YAML frontmatter, THE Forge_Build_SKILL SHALL operate identically to the current behavior (AI auto-detection or verify_commands fallback).
2. WHILE `ci_check_command` is absent from `.forge/config.md`, THE Forge_Test_SKILL SHALL operate identically to the current behavior (individual commands for each checklist item).
3. WHILE `ci_check_command` is absent from `.forge/config.md`, THE Forge_Ship_SKILL SHALL operate identically to the current behavior (Test gate based on Layer 1 + Layer 3 results).
