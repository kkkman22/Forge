...
预检不持久化数据,仅在 build 启动时运行并输出结果。豁免记录追加到 `.forge/progress/<topic>.md` 的预检日志段(纯文本追加)。
...
| Verify-By 白名单 | `.forge/config.md` | 被 spec-check R6 规则使用,本 spec 复用 |
...
- `.forge/config.md` 加 `preflight_enabled` 开关(默认 true)。
