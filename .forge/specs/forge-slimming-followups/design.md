...
- 检查 Deprecation_Notice 锁文件：.forge/.deprecation-notice/<sid>/forge-recap.lock
...
| `.forge/decisions/*.md` | 同上模式，但匹配时**跳过**含 `(historical:` 的行 |

**历史决策文件处理**：
- 检测到 `.forge/decisions/*.md` 中的裸数字 ≠ SST 时，若该行已含 `(historical:` 括注则视为合规（跳过）。
...
- `.forge/decisions/2026-05-12-plugin-distribution.md`：保留原文 "28"，追加括注。
...
- Deprecation_Notice 锁文件（`.forge/.deprecation-notice/<sid>/<cmd>.lock`）
- Command_Count_Declaration（`{FORGE_COMMAND_COUNT}` 占位符 + `.forge/.command-count`）
