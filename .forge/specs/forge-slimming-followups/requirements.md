...
1. THE task SHALL fix the following four locations that still claim "28":
   ...
   - `.forge/decisions/2026-05-12-plugin-distribution.md` line referencing "28 slash command wrappers"
2. FOR historical decision files (`.forge/decisions/`), THE task SHALL append a parenthetical note "(historical: count at time of writing was 28; current SST={FORGE_COMMAND_COUNT})" rather than silently changing the number。
...
4. THE task SHALL extend `scripts/gen-plugin-commands.mjs --verify-count` to scan: marketplace.json plugins[0].description, CHANGELOG.md, and `.forge/decisions/*.md`。
