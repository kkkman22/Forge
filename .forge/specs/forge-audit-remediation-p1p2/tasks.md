...
- [ ] `checkDestructive` 无条件检测(移除 `.forge/.sandbox-active.json` 条件)
...
- [ ] `redactOnWrite(filePath, content)` 拦截 `.forge/**/*.md` write
- [ ] `.gitignore` 增加 `.forge/reviews/`
- [ ] 已有 tracked `.forge/reviews/*.diff-context.md` `git rm --cached`
...
- [ ] 断言产物文件(`.forge/status.md`, `.forge/specs/`)
...
- .forge/config.md (新增 config field)
...
- .forge/decisions/ADR-0009-audit-remediation-scope.md (new)
...
- [ ] .forge/knowledge/adr-index.md 更新(自动 hook)
