...
import_source: ".forge/decisions/2026-07-15-audit-remediation-p1p2.md"
health:
  score: 0
  verdict: "pending"
---
...
- 不动 `.forge/audit-keep.md` / `.forge/audit-hmac*` (审计日志模块独立,本轮不触)
...
**当** `.forge/**/*.md` 文件被 `writeFileSync` 写入 **则** 内容经 `redactSecrets()` 处理后才落盘。
...
**当** `.forge/reviews/.diff-context.md` 被生成 **则** 同时 (a) `git rm --cached` + `.gitignore` 增加 `.forge/reviews/`; (b) 已有 reviews 重新 redact; (c) review 流程改用 tmp + symlink 或 export-only 模式。

**Verify-By**: vitest:unit
**Evidence**: `test/secret-redactor.test.ts` 新增 4 类 pattern 正向/反向测试;`test/fs-write-redact.test.ts` 验证 `.forge/**/*.md` write 路径自动 redact;`test/forge/reviews-gitignore.test.ts` 验证 `.gitignore` 增量与已有文件 untrack

**Current State**: `src/secret-redactor.ts:22-72` 6 类 pattern 均上下文绑定(JSON field name / env 赋值 / Bearer 前缀),自由文本里的裸 vendor token 和裸 DSN 不脱敏。`.gitignore` 排除 `.forge/{debug,archive,cache,specs,...}` 但不排除 `.forge/reviews/`。

**Proposed Change**:
- `redactSecrets()` 增加 3 个新 pattern:vendor prefix、URL-with-creds、Sentry DSN
- 新增 `redactOnWrite(filePath, content)` 包装器,在 `fs.writeFileSync` 调用 site 拦截 `.forge/**/*.md`
- `.gitignore` 增加 `.forge/reviews/`(或更精细:.forge/reviews/*.diff-context.md 排除但保留 review-summary.md)
- `git rm --cached` 已有 `.forge/reviews/*.diff-context.md` 跟踪文件

**明确不改变**: `redactSecrets()` 函数签名可保持;既有的 6 类 pattern 不删(向后兼容);`.forge/reviews/*.md` 之外的 review artifact 不动。
...
**当** `test/e2e/spec-kiro-style.test.ts` 跑 `describe("E2E")` block **则** 必须用 `createTempRepo()` 创建真实 git harness,通过 `child_process.spawn` 跑 `/forge` 命令二进制,断言 `.forge/status.md` 文件内容。

**当** 现有 e2e test 仍进程内 import 纯函数 **则** 删除或改为 unit test(`test/unit/spec-*.test.ts`)。

**Verify-By**: bash:contract + forge_exec:e2e
**Evidence**: `test/e2e/helpers/temp-repo.ts` (已存在)被至少 1 个真实 spawn test 引用;`npm run test:e2e` 至少 1 条测试启动 `/forge plan` 验证 `.forge/specs/` 创建
...
**明确不改变**: 既有 docs 内容不变;中文为主语言原则(`.forge/decisions/2026-06-23-i18n-source-language-chinese.md`)不变。
...
**当** `.forge/.sandbox-active.json` 不存在 **则** hook 仍挂载并工作(无条件激活,非 sandbox-only)。
...
**Current State**: `src/check-sandbox.ts:181` `checkDestructive` 仅在 `.forge/.sandbox-active.json` 存在时经 Bash 路径触发;repo 内 `.claude/settings.json` PreToolUse Bash 未挂载它。
...
**明确不改变**: `checkDestructive` 函数签名不变(仅去掉条件);`.forge/.sandbox-active.json` 行为不变(sandbox 模式仍可激活);其他 hook 配置不变。
...
- `.forge/decisions/2026-07-15-audit-remediation-p1p2.md` (full decision context)
- `Forge 项目审核报告-codex.md` (audit basis 1)
- `Forge 项目审核报告-claude code.md` (audit basis 2)
- `.forge/specs/audit-remediate-p0p1/` (prior remediation pattern)
