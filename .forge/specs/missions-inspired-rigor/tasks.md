...
layout: tasks
created: 2026-05-16
spec_ref: ".forge/specs/missions-inspired-rigor/requirements.md"
---
...
- [ ] 12. Dogfooding：用本 spec 自身跑一次完整 standard tier 流程 (deferred)
  ...
  - **2026-05-16 部分完成**：本次审计后通过"修复审计发现的 P2 偏差"作为最小闭环，产出 `.forge/progress/missions-inspired-rigor.md` 真实 handoff block + `.forge/knowledge/known-failures.md` 真实条目。完整 standard tier dogfooding 仍 deferred。
  - 验证：build 阶段 progress 文件中每个原子任务都有合法 handoff block；review 阶段命中并新增 known-failures；如果跑了 forge-loop，events.ndjson 至少有 N 条 phase_start/phase_end 配对
  - 把 dogfooding 报告写入 `.forge/findings/missions-inspired-rigor-dogfooding.md`
  - **对应需求**：所有 R 的端到端验证
  - **Verify-By**: manual
