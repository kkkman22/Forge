...
- **不引入新的外部依赖**：复用 forge-spec / forge-build / forge-review / forge-loop 既有 SKILL 文件 + `.forge/` 状态目录 + `forge-context` MCP。
...
每完成一个原子任务并准备 commit 前，build agent 必须在 `.forge/progress/<topic>.md`
...
forge-review SKILL 在收到三层评审输出后：
  ...
  2. 读取 .forge/knowledge/known-failures.md
  3. 按 pattern_id 去重：
     - 不存在 → append 新条目
...
`.forge/archive/known-failures-<date>.md`，known-failures.md 保留最近 80 条。
```
...
读取 .forge/knowledge/known-failures.md，对当前 diff 中的代码模式检索匹配。
...
**验证方法**：解析 `.forge/progress/<topic>.md`，校验 commit 数 = handoff block 数；schema 校验通过。
...
forge-loop 每次 SKILL 调用前后 `.forge/status.md` + events.ndjson 都构成状态完整快照；`/forge resume` 在新会话内能从最新 cursor 完整恢复。
...
```bash
bash scripts/mark-legacy-contracts.sh  # 扫描 .forge/specs/*/spec.md
...
**处理**：超过 100 条时自动归档：按 `last_seen` 升序，归档最旧 20 条到 `.forge/archive/known-failures-<date>.md`，正文保留最新 80 条。归档动作由 forge-review SKILL 在每次 review 结束时执行（仅在 count > 100 时触发）。

### E4：forge-loop fresh-context 调用导致 cache miss 暴增

**原因**：每次 SKILL 调用都重新加载状态文件，prefix cache 命中可能下降。

**处理**：forge-loop 在 phase_end 事件中记录 `token_budget_used` 和 `cache_hit_rate`（如 SDK 暴露）。监控指标进入 `.forge/knowledge/metrics.md`。若实测 cache 命中率 < 70%，重新评估状态文件分区策略。
