...
当前 Forge 的 `.forge/status.md` 是单任务状态快照，无法支持多个 worktree 并行开发。本设计引入多文件状态追踪模式（`.forge/status/<task-id>.md`），使每个并行任务拥有独立的状态文件，同时保持单任务场景下的完全向后兼容。
...
| 决策 | 选择 | 理由 |
|------|------|------|
| 多任务存储方式 | `.forge/status/<task-id>.md` 多文件 | 文件级隔离，无并发写冲突，shell 命令易读取 |
| 模式检测 | 基于 `.forge/status/` 目录是否存在 | 零配置，目录即信号 |
...
graph TD
    ...
    subgraph "存储层"
        Legacy[".forge/status.md"]
        Multi[".forge/status/<task-id>.md"]
    end
...
stateDiagram-v2
    ...
    state SingleTask {
        [*] --> ReadWriteLegacy
        ReadWriteLegacy: 读写 .forge/status.md
    }

    state MultiTask {
        [*] --> ReadWriteTaskFile
        ReadWriteTaskFile: 读写 .forge/status/task-id.md
    }
```
 ...
 *
 * 模式检测逻辑：
 * - 如果 .forge/status/ 目录存在 → multi 模式
 * - 否则 → single 模式
 *
 * 路径解析：
 * - single 模式 → .forge/status.md
 * - multi 模式 → .forge/status/<task-id>.md
 */
export function resolveStatusPath(ctx: ResolverContext): ResolvedStatus;

/**
 * 检测当前是否处于多任务模式。
 * 基于 .forge/status/ 目录是否存在。
 */
...
/**
 * 读取指定任务的状态。
 * 优先读取 .forge/status/<task-id>.md，
 * 不存在则回退到 .forge/status.md。
 */
 ...
 * 1. 从 .forge/status.md 读取 current_task
 * 2. slugify 得到 task-id
 * 3. 创建 .forge/status/ 目录
 * 4. 将内容写入 .forge/status/<task-id>.md
 * 5. 清空 .forge/status.md（保留空 frontmatter）
 */
...
if [ -d .forge/status ]; then
  # 多任务模式：读取最近更新的任务文件
  latest=$(ls -t .forge/status/*.md 2>/dev/null | head -1)
  if [ -n "$latest" ]; then
    echo '=== Forge Context ==='; head -50 .forge/plans/*.md 2>/dev/null
    echo '=== Recent Progress ==='; tail -20 .forge/progress/*.md 2>/dev/null
  fi
elif [ -f .forge/status.md ]; then
  # 单任务模式：保持原有行为
  echo '=== Forge Context ==='; head -50 .forge/plans/*.md 2>/dev/null
  echo '=== Recent Progress ==='; tail -20 .forge/progress/*.md 2>/dev/null
fi
...
```bash
phase=$(grep '^phase:' .forge/status.md 2>/dev/null | sed '...')
```
...
**PostToolUse hook**（当前）：
```bash
if [ -f .forge/status.md ]; then echo '📝 ...'; fi
```

**PostToolUse hook**（修改后）：
```bash
if [ -d .forge/status ] || [ -f .forge/status.md ]; then echo '📝 ...'; fi
```
...
*For any* task name, if a task-specific file (`.forge/status/<task-id>.md`) exists, `readTaskStatus` SHALL return its content; if it does not exist but `.forge/status.md` exists, `readTaskStatus` SHALL return the legacy file's content.
...
*For any* new task name, when one or more Active_Tasks already exist, `writeTaskStatus` SHALL write to `.forge/status/<task-id>.md` (not `.forge/status.md`).
...
*For any* valid Legacy_StatusFile content with a non-empty `current_task`, after migration to multi-task mode, the content of `.forge/status/<task-id>.md` SHALL be identical to the original Legacy_StatusFile content.
...
| 场景 | 处理策略 |
|------|---------|
| `.forge/status/` 目录创建失败 | 回退到单文件模式，log warning |
