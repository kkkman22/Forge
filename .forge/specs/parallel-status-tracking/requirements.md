...
Forge 当前的 `.forge/status.md` 被设计为单任务状态快照，只能记录一个任务的 phase、tier、task name 等信息。当用户通过 Git Worktree 并行开发多个任务时，多个 worktree 共享同一个 `.forge/status.md`，导致状态互相覆盖、`/forge resume` 只能恢复最后写入的任务、第三个任务启动时被提示有未完成的旧任务。

本功能引入多文件状态追踪模式（`.forge/status/<task-id>.md`），使每个并行任务拥有独立的状态文件，同时保持单任务场景下的完全向后兼容。
...
- **Legacy_StatusFile**: 位于 `.forge/status.md` 的传统单文件状态，用于单任务场景的向后兼容。
- **Task_StatusFile**: 位于 `.forge/status/<task-id>.md` 的独立任务状态文件，用于多任务并行场景。
...
2. WHEN operating in multi-task mode, THE Status_Resolver SHALL resolve the status file path to `.forge/status/<task-id>.md`.
3. WHEN operating in single-task mode (no `.forge/status/` directory exists and only one task is active), THE Status_Resolver SHALL resolve the status file path to `.forge/status.md` (Legacy_StatusFile).
...
1. WHEN writing status for a task in multi-task mode, THE Status_Manager SHALL create the `.forge/status/` directory if it does not exist, and write the task's StatusFile to `.forge/status/<task-id>.md`.
2. WHEN reading status for a specific task, THE Status_Manager SHALL first look for `.forge/status/<task-id>.md`; IF that file does not exist, THE Status_Manager SHALL fall back to reading `.forge/status.md`.
...
4. WHEN listing all active tasks, THE Status_Manager SHALL scan both `.forge/status.md` and all files in `.forge/status/`, returning a list of Active_Task entries with their task names and phases.
...
1. WHEN a new task is started and other Active_Tasks exist, THE Forge_Router SHALL write the new task's status to a Task_StatusFile (`.forge/status/<task-id>.md`) instead of overwriting Legacy_StatusFile.
2. WHEN a new task is started and no other Active_Tasks exist, THE Forge_Router SHALL write to Legacy_StatusFile (`.forge/status.md`) for backward compatibility.
...
1. WHILE the system is in single-task mode (Legacy_StatusFile only), THE Context_Hook SHALL read `.forge/status.md` and inject context identically to the current behavior.
2. WHILE the system is in multi-task mode, THE Context_Hook SHALL read all Task_StatusFiles in `.forge/status/` and inject context for the most recently updated Active_Task.
...
1. WHILE no `.forge/status/` directory exists, THE Status_Manager SHALL operate in single-task mode using Legacy_StatusFile exclusively, with behavior identical to the current implementation.
2. WHEN the first parallel task is started (second Active_Task detected), THE Status_Manager SHALL migrate the existing Legacy_StatusFile content into `.forge/status/<task-id>.md` for the original task, then create a new Task_StatusFile for the new task.
3. WHEN all parallel tasks complete or are aborted (zero Active_Tasks in `.forge/status/`), THE Status_Manager SHALL NOT automatically remove the `.forge/status/` directory (cleanup is deferred to `/forge abort` or manual action).
...
4. WHEN the last Active_Task is aborted from `.forge/status/`, THE Forge_Abort SHALL reset Legacy_StatusFile to its initial empty state.
