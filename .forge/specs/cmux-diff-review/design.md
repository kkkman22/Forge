...
│ 读 .forge/reviews/*.md │   (R2 Finding_To_Comment_Map)    │ diff (--branch/--last-turn)
...
方向 A 的价值核心：把人评审从"写一段自由文本 prompt"升级为"在 diff 行上标 P-level + 评论 → 结构化 Forge_Finding 载荷直送 agent"。agent 收到后按既有 P0/P1 阻断规则（宪法 §3.3）处理——与 `.forge/reviews/<topic>.md` 同等地位。这让"人机协作评审"的语义统一。
...
| 方案 | 拒绝理由 |
|------|---------|
| 把人评论直接写进 `.forge/reviews/*.md` | 改 source of truth、混淆 agent 产物与人输入；评论应作为 agent 输入，落盘与否由既有逻辑决定（Out of Scope #3） |
