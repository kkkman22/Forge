...
2. WHEN the Mixed_Language_Strategy is applied to a SKILL_File, THE Mixed_Language_Strategy SHALL convert all table cell content that is structural data from Chinese to English (e.g., `扫描 .forge/specs/ 下所有 spec.md 的 YAML status` → `Scan all spec.md YAML status under .forge/specs/`).
