...
## 批次 3：`.forge/` → `.tinkerman/`（`feature/rename-dir`）⚠️ 最大风险
- [ ] 写 `scripts/migrate-forge-to-tinkerman.mjs`（递归复制 + 文件数校验 + SHA256 + dry-run + 失败不删原）
- [ ] `src/` + `scripts/` 160 处 `.forge/` 引用 → `.tinkerman/` + fallback 读取函数
- [ ] `scripts/init.sh`：创建 `.tinkerman/`（新装）；检测旧 `.forge/` 提示迁移
