...
execution_packages:
  - id: P1
    ...
    handoff_path: ".forge/runs/<run-id>/packages/P1.md"
  ...
  - id: P2
    ...
    handoff_path: ".forge/runs/<run-id>/packages/P2.md"
```
...
/forge build --package P2
  ...
  -> write .forge/runs/<run>/packages/P2.md
...
commands:
  - cmd: "npx vitest run test/plan-package.test.ts"
    result: pass
    evidence_path: ".forge/runs/<run-id>/packages/P2-verify.log"
findings:
  p0: 0
  p1: 0
  highest_risk: "none"
blockers: []
report_path: ".forge/runs/<run-id>/packages/P2.md"
...
The current project still has `/forge loop` as a Native Fusion Loop skill. It uses `.forge/loop-state.json`, status fields, and native scheduling tools. The package design plugs into that loop:
...
4. The next iteration starts from fresh context and reads `.forge/` state.
...
| `/forge resume` | Reconstructs phase/task state from `.forge/` | Must reconstruct current package and next package from package fields. |
...
- `scripts/hook-postcompact.sh` prints and removes `.forge/.compact-snapshot.md`; therefore any package fields omitted from the snapshot are unavailable to the compact recovery prompt.
...
- `scripts/inject-plan-context.mjs` supports `--phase`, but the active PreToolUse setting calls it with no explicit phase. It must infer phase and package from `.forge/status.md`; otherwise it can fall back to `extractHead()` and re-inject broad plan context.
