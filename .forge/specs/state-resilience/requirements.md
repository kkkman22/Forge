...
Forge 的每个阶段都依赖前一阶段写入的状态文件（`.forge/status.md`、`.forge/progress/`、`.forge/reviews/` 等）。当状态文件格式不对、frontmatter 缺字段、或 phase 值不在预期范围内时，流程会卡住或行为不确定。
...
2. WHEN `.forge/config.md` is missing or unparseable, THE parser SHALL use the complete default config and emit a warning.
...
2. THE function SHALL infer the current phase from: presence of `.forge/plans/` files (→ at least plan phase), presence of `.forge/progress/` files with completed tasks (→ at least build phase), presence of `.forge/reviews/` files (→ at least review phase).
