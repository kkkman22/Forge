...
- 当需求被显式拒绝时 系统应当 写入 `.forge/knowledge/out-of-scope/`(或等价路径),并在原决策处链接。
- 当三分机制落地时 系统应当 与现有 `.forge/knowledge/` 共存,不破坏 §4 knowledge loop 的五维度提取。
...
- 当体检结论被记录时 系统应当 纳入 `.forge/specs/` 或 findings,作为后续 build 的依据。
...
- **向后兼容**:所有改动与现有 `.forge/` 结构、frontmatter、铁律共存。
...
**Verify-By**: `bash:contract`
**Evidence**: `grep -ri "out-of-scope\|rejected-requests" .forge/knowledge/ docs/` 命中机制描述或目录约定
...
**Verify-By**: `bash:contract`
**Evidence**: 存在体检清单(spec 附件或 `.forge/findings/`),含每条疑似 no-op + 建议
**Covers**: R7.AC1
