...
**需求描述**：把 `.forge/config.md` 的 flat YAML frontmatter 重构为分层结构（`[build]` / `[review]` / `[ship]` / `[loop]` section），同时：
...
- 安全模型（plugin 能否执行任意 Bash、能否访问 `.forge/` 状态）
...
# 汇总报告写入 .forge/decisions/<date>-agent-teams-poc.md
```
