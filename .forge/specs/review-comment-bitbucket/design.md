...
> **权威来源**：`.forge/decisions/2026-05-23-review-inline-comment-channel.md`（决策已锁定，4 个开放问题已回答）
> **配套约束**：`.forge/decisions/2026-05-18-review-fallback-ladder.md`（ADR-0005）、`.forge/decisions/2026-05-16-claude-code-uplift-2.1.143.md`、`AGENTS.md`、`.forge/config.md`
> **本文档原则**：只描述"如何实现"，不重复"为什么这样设计"。论证请回到决策记录。

## Overview

本 SKILL 在 `/forge review` 产出 `.forge/reviews/<run-id>.md` 之后，作为**独立的交付层**把 finding 投递到 Bitbucket PR：P0/P1 双层（PR Task + inline comment），P2 单层（仅 inline comment），P3 不投递。整体流程串接在 ADR-0005 fallback ladder 之后，不修改 fallback ladder 自身行为，不影响 ship gate 判定逻辑（ship 仍以 review markdown 为 source of truth）。
...
graph TD
    ...
    L0 -->|review.md 产出| RM[.forge/reviews/run-id.md]
    L1 -->|review.md 产出| RM
    L2 -->|review.md 产出| RM
    ...
    Skip --> Health[.forge/knowledge/tool-health.md]
    Skip --> RunMD[run-id.md 末尾追加 skip 段]
    Skip --> Findings[.forge/findings/comment-channel-skipped-*.md]
...
sequenceDiagram
    ...
    alt post_enabled = false
        ...
        alt gate skip
            ...
            CLI-->>CLI: 写入 run summary 到 .forge/knowledge/metrics.md
        end
    end
```
...
P3 finding（开发者决定）
    └─ 仅写入 .forge/reviews/<run-id>.md（本 spec 不投递）
...
- 单条动作失败不中断后续动作（partial success 语义），错误聚合写入 `.forge/findings/comment-channel-error-<date>.md`
...
完整移植自决策记录 §5.2，写入 `.forge/config.md`（附属于 `review:` 节）。
...
**优先级解析（A4）**：CLI flag `--post-comments` / `--no-post-comments` > `.forge/config.md` `review.comment_channel.enabled` > 内置默认 `false`。
...
- `.forge/findings/comment-channel-skipped-<date>.md` 记录 `reason`、`remote_url`、`mcp_base_url`
...
**Recovery**：失败聚合写入 `.forge/findings/comment-channel-error-<date>.md`，含 finding hash、工具名、错误信息；`PostResult` 返回 `posted: true` + `partial_failures` 数组
...
**Recovery**：错误信息引导用户回到 `.forge/config.md` 修正；不写 fallback 静默路径（无效配置必须暴露）
...
**Recovery**：错误写入 `.forge/findings/comment-channel-error-<date>.md`；ship gate 仍照常根据 review markdown 决定阻断（与本通道解耦）

### MCP 网络超时 / rate limit

**Condition**：MCP 工具调用因 429 / 5xx / 超时失败
**Response**：单条动作走 partial-failure 路径；不全局重试（避免与 `rate_limit_interval_ms` 节流互相放大）
**Recovery**：429 频次写入 `.forge/knowledge/tool-health.md`；下一次 review run 时 reconcile 会自动重试缺失的 create

## Out of Scope（显式声明）

按决策记录 §3.1（P3 处理规则）和 §3.3 A3（平台支持范围）：

- **P3 finding 投递**：本 spec 不实现。P3 仅写入 `.forge/reviews/<run-id>.md`，不创建 PR Task、不创建 inline comment、不影响 `set_review_status`。schema 中保留 `p3_strategy: none` 字段供未来扩展，但**值只能是 `none`**（其它值在配置解析阶段报错）
...
- **ship gate 阻断逻辑修改**：ship gate 仍以 `.forge/reviews/<run-id>.md` 为 source of truth；`set_review_status(request_changes)` 只是协作信号，不替代 ship gate
...
1. **review markdown 解析契约**：当前 `.forge/reviews/<run-id>.md` 是否已有稳定的 finding schema？若没有，需要定义最小 schema（priority / file_path / line_number / finding_type / message / suggestion?），并在 review SKILL 中保证产出符合该 schema
