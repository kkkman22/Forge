...
│   存在 .forge/config.md          → skip:already_initialized    │
│   存在 .forge/.bootstrap-dismissed → skip:user_dismissed       │
...
- `.forge/.bootstrap-dismissed`：用户主动创建的空文件，告诉 bootstrap-check 不再提示。
...
*For any* `(env, fsExists)`，若结果是 `skip`，则 `reason` 唯一对应一个先决条件——`already_initialized` 蕴含 `.forge/config.md` 存在；`user_dismissed` 蕴含 `.bootstrap-dismissed` 存在且 `config.md` 不存在；`no_plugin_context` 蕴含两文件都不存在且 `pluginRoot` 为空。

**Validates: Requirements 3.2, 3.3, 3.4, 3.5**

### Property 4: shouldShowBootstrap show 触发条件

`shouldShowBootstrap(env, fs).kind === "show"` 当且仅当 `.forge/config.md` 不存在 AND `.forge/.bootstrap-dismissed` 不存在 AND `env.pluginRoot` 非空字符串。
...
| `.forge/.bootstrap-dismissed` 是目录（非文件） | `existsSync` 返回 true，跳过提示 | 与"用户已忽略"等价 |
...
|---|---|---|
| 1 | `.forge/config.md` 存在 | `skip:already_initialized` |
| 2 | 仅 `.forge/.bootstrap-dismissed` 存在 | `skip:user_dismissed` |
| 3 | `pluginRoot` 为空；两文件均不存在 | `skip:no_plugin_context` |
| 4 | `pluginRoot=/x`；两文件均不存在 | `show` |
| 5 | `.forge/config.md` 与 `.bootstrap-dismissed` 同时存在 | `skip:already_initialized`（优先级最高） |
...
- 在 tmp 项目目录下，对 `${CLAUDE_PLUGIN_ROOT}` 设置桩，运行 `node scripts/bootstrap-check.mjs`，断言：
  - 无 `.forge/` 时 stdout 含 "Forge plugin 已激活"
  - 有 `.forge/config.md` 时 stdout 为空
  - 有 `.forge/.bootstrap-dismissed` 时 stdout 为空
...
| 用户在已初始化项目误删 `.forge/config.md`，bootstrap 误报"未初始化" | 误导用户运行 init 覆盖剩余文件 | init.sh 已有"重新初始化将覆盖"二次确认；不修改该交互 |
