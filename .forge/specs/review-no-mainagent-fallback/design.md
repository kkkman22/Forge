...
└────────────────────────────────────────────┘
     ...
     ▼
┌─ L2: CI 证据 ──────────────────────────────┐
│  if (.forge/reviews/<pr>-ci.md exists     │
...
export async function runReviewFallbackLadder(
  ...
  const config = parseReviewConfig(/* read .forge/config.md */);
...
export function recordForceSkip(commitHash: string, reason: string, user: string): void {
  ...
  const path = `.forge/findings/force-skip-review-${date}.md`;
...
3. 检查 `.forge/findings/agent-sdk-task-id-purge-2.1.143.md` 列出的上游 issue 是否标记 closed
