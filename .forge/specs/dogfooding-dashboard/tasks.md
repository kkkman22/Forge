...
- **TDD Steps**:
  - 调研真实 `.forge/`：spec 如何标记 ship 痕迹、episode 如何标记证据链
...
- **TDD Steps**:
  - RED: 断言对空 `.forge/` 调用 main 退出码 1 且报错友好；对缺子目录的 fixture 不崩溃且显示"无数据"——失败
  - GREEN: 实现错误处理与编排
...
### T-07 真实仓库试跑 + 数字核对
- **Goal**: 在真实 `.forge/` 上跑一次，人工核对数字合理性
- **TDD Steps**:
  - 运行 `node scripts/build-dogfooding-dashboard.mjs`
  - 人工核对：完整链路率是否合理、评审拦截总数与 `.forge/findings/` 文件数是否同数量级
  - 把首版产物附到 design.md 作为验证记录
- **Verify Command**: `node scripts/build-dogfooding-dashboard.mjs && cat .forge/dashboards/dogfooding.md`
