...
将 evolved-rules 的生成流程从"统计阈值触发"改为 TDD 驱动（先验证问题存在再写规则），并为 skill/instructions 变更增加验证步骤。修改 `skills/forge/lib/learn/instructions.md` 和 `.forge/knowledge/evolved-rules.md` 格式模板。
...
5. **记录结果**：在 `.forge/knowledge/skill-feedback.md` 中记录
