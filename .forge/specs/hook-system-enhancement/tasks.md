...
layout: tasks
created: 2026-05-30
spec_ref: ".forge/specs/hook-system-enhancement/requirements.md"
---
...
- [ ] 3.1 新建 `scripts/task-created-hook.mjs`：读取 `.forge/plans/` 输出 task 摘要
...
- [ ] 4.1 新建 `scripts/worktree-create-hook.mjs`：记录 worktree 信息到 `.forge/progress/worktrees.json`
- [ ] 4.2 新建 `scripts/worktree-remove-hook.mjs`：移除对应记录
- [ ] 4.3 在 `plugin.json` 注册两个事件
- [ ] 4.4 实现 `.forge/progress/` 和 `.forge/debug/` 的自动 mkdir
...
- [ ] 5.1 新建 `scripts/stop-failure-hook.mjs`：追加 API 错误到 `.forge/debug/failures.jsonl`
...
- [ ] 8.2 追加到 `.forge/runs/` 下的 JSONL 日志
- [ ] 8.3 添加 `--help` 支持

**Verify-By**: bash — 运行 hook 后检查 `.forge/runs/` 中的 JSONL
**关联需求**: R8
