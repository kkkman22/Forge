...
| 当前 command | 新脚本 |
|-------------|--------|
| `if [ -f .forge/progress/*.md ]...` | `scripts/stop-incomplete-tasks.mjs` |
| `bash persistent-loop.sh` | 保留（已有独立脚本） |
| `if [ -f .forge/knowledge/evolved-rules.md ]...` | `scripts/stop-pending-rules.mjs` |
| `record-evolved-rule-violation.mjs` | 保留（已有独立脚本） |
| `flag-stale-evolved-rules.mjs` | 保留（已有独立脚本） |
| `cmux-mirror/sync-once.mjs` | 保留（已有独立脚本） |
| `if [ -f .forge/status.md ]...phase check` | `scripts/stop-phase-verify.mjs` |
...
scripts/worktree-create-hook.mjs   — 记录 worktree 到 .forge/progress/worktrees.json
scripts/worktree-remove-hook.mjs   — 移除 worktree 记录
scripts/stop-failure-hook.mjs      — 追加 API 错误到 .forge/debug/failures.jsonl
...
| `.forge/debug/` 不存在 | mkdirSync recursive |
