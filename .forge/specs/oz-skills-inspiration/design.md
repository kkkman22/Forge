...
│                                    │ 汇总引用                    │
│                                    v                             │
│                         `.forge/knowledge/skill-style-guide.md`  │
│                                                                  │
│  ┌──────────────┐                                                │
...
[CI: npm run check]
   ...
   │
   v
[作者查阅] .forge/knowledge/skill-style-guide.md   ← R3：单一事实来源
   │
   v
...
|---|-------|-----------------|-------|
| 1 | Spec Gate — `.forge/specs/<topic>/spec.md` status | Not `locked` | → `/forge spec` |
| 2 | Plan Gate — `.forge/plans/<topic>.md` status | Not `approved` | → `/forge plan` |
...
```
.forge/knowledge/skill-style-guide.md   # 主文档（开放区）
...
- 大版本（2.x）：破坏性变更（如必填字段调整）——伴随 `.forge/decisions/ADR-XXXX-skill-style-guide-v2.md`
- 每次变更追加到 `.forge/knowledge/skill-style-guide-changelog.md`
...
首版审计产出 `.forge/findings/scripts-help-audit.md`：
...
.forge/cache/login-state-<project>.json               # 登录态缓存，.gitignore 排除
```
...
- **Screenshots**: <.forge/reviews/assets/ 路径列表>
...
| 状态 | Agent 行为 |
|---|---|
| 无缓存 + 页面需登录 | 提示用户在 cmux browser 内登录一次，然后运行 `cmux browser $SURFACE state save .forge/cache/login-state-<project>.json` |
...
.forge/acceptance/<topic>/<scenario-id>/  # 产物
```
...
async run(scenario, ctx) {
  ...
  const artifact = {
    ...
    evidence: [`.forge/acceptance/${ctx.topic}/${scenario.id}/response.json`],
...
`aggregateVerdicts` → 写入 `.forge/reviews/<topic>-acceptance.md`：

```markdown
---
topic: <topic>
executed_at: <ISO>
spec_ref: .forge/specs/<topic>/spec.md
...
- **Given/When/Then**: ...
- **Verdict**: PASS
- **Evidence**: `.forge/acceptance/<topic>/scenario-001/`

### scenario-002 — @derived
- **Given/When/Then**: ...
- **Verdict**: FAIL
- **Failure Reason**: API returned 500 instead of 200
- **Evidence**: `.forge/acceptance/<topic>/scenario-002/`
...
| `.forge/reviews/assets/` 约定 | 需求 5 | 需求 6 evidence 存放 |
...
- `.forge/reviews/<topic>.md` 的 Layer 4 段落结构化
- `.forge/acceptance/<topic>/summary.md` 总览入口
