...
- `staleness.ts`：用 UTC 当日基准计算天数差，分级输出报告与 `.forge/staleness-report.json`（R5）。
...
- `ssot/registry.ts`：从 `.forge/config.md` 加载 `docs.ssot_sources`，校验保留前缀、来源存在、渲染器已注册（R16）。
...
function classify(path):
  ...
  for prefix in DOMAIN_C_PREFIXES (.forge/, .kiro/):  # R1.3 优先级最高
...
let touchesConfig      = staged contains ".forge/config.md"
...
`.forge/config.md` frontmatter 完整示例：
...
export function loadConfigWithDefaults(
  configPath?: string                 // 默认 .forge/config.md
...
pre_hook:
  - timeout: 10s                    # R10.AC1 时间预算
    ...
    aggregate_to: .forge/knowledge/sessions/<session>.md "文档增量" 小节
```
...
- 同时生成 `docs/audits/` 目录骨架 + `.forge/archive/` 索引（不迁移文件，只占位）。
...
- 在 `.forge/config.md` 加 `docs.grace_period_until` 字段（YYYY-MM-DD），覆盖未来 7 个自然日。
