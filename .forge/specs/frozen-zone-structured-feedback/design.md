...
- Zone_Registry 从 `.forge/config.md` 单一事实源动态解析
- 结构化审计日志写 `.forge/runs/`
...
**关键不变**：`.forge/` 目录结构、现有 config.md 的 YAML schema、frozen 路径规则。
...
Claude → Write tool
    ...
    PreToolUse hook
         │
         ├── parse .forge/config.md → Zone_Registry
         │
    ...
    PostToolUse hook
         ...
         │     写 .forge/runs/<date>-frozen-events.jsonl
         │
...
Zone_Registry (in-memory)
     │
     ├── .forge/specs/*/spec.md | frozen-spec | SPEC_LOCKED | requires status: locked
     ├── .forge/plans/*.md | frozen-plan | PLAN_APPROVED | requires status: approved
     ├── .forge/config.md | frozen-config | CONFIG_ROOT | unconditional
     ├── .forge/progress/*.md | guarded | append-only
     ├── .forge/reviews/*.md | guarded | no-overwrite
     └── ...
```
...
```bash
# parse_zone_registry
# Reads .forge/config.md, outputs normalized rule lines on stdout.
    ...
    "PreToolUse": [
      {
        ...
        "if": "Write(.forge/**)|Edit(.forge/**)|MultiEdit(.forge/**)",
        "hooks": [
          {
            "type": "command",
...
**文件**：`.forge/runs/<YYYY-MM-DD>-frozen-events.jsonl`

**单行格式**：
```json
{"timestamp":"2026-05-12T08:00:00Z","session_id":"abc","tool_name":"Write","path":".forge/specs/foo/spec.md","category":"frozen-spec","reason_code":"SPEC_LOCKED","decision":"pre","outcome":"denied"}
```
...
运行 `bash scripts/summarize-frozen-events.sh --days=7`，读取 `.forge/runs/*-frozen-events.jsonl`：
...
| 场景 | hook 行为 |
|---|---|
| `.forge/config.md` 缺失 | warning to stderr，使用 hard-coded default rules，继续 |
| `.forge/config.md` 解析失败 | 同上，`reason_code = ZONE_OVERRIDE_MISSING` |
...
3. **Contract test** `test/contract.test.ts` 扩展：
   ...
   - `.forge/config.md` 包含 `<HARD-GATE name="frozen-zone-protection">` 块

4. **手动 e2e**：
   - 在干净 session 中设置 `FORGE_STRUCTURED_FROZEN=1`，尝试编辑 locked spec
   - 验证 CC 输出包含 Frozen_Diagnostic
   - 临时改 `.forge/config.md` 加一个自定义 frozen 规则，验证即时生效
