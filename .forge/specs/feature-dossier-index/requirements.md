...
本特性为 Forge 增加一份**功能维度的索引文档**（Feature Dossier），解决 `.forge/` 目录按阶段平铺导致的"一个功能的产物散落在 5-7 个目录"的回顾痛点。今天回顾一个功能从需求 → 决策 → 计划 → 评审 → 进度的全貌时，需要用户在 `decisions/`、`specs/<feature>/`、`plans/`、`reviews/`、`progress/`、`findings/`、`debug/` 这些目录之间来回跳转按 topic 拼凑，摩擦大且容易遗漏。
...
1. **一处回顾**：打开一份 `.forge/features/<topic>.md` 即可看到该功能的全部阶段链接 + 状态 + 摘要
...
架构选择（Hook 驱动的派生视图）：本特性采用 **派生视图（derived view）** 而非**物理重组（physical reorg）**。派生视图意味着索引是可再生的衍生物，物理文件仍由各 skill 写入各自的阶段目录；Hook 监听这些写入事件并重建索引。**不做**物理重组因为会涉及 40+ 文件路径改写、冻结规则重构、存量迁移和 `.forge/specs/` 互操作等大面积代价，且不可逆。
...
- **零目录迁移**：`.forge/specs/`、`.forge/plans/`、`.forge/decisions/` 等现有目录保持现状
- **派生可重建**：`.forge/features/` 下所有文件都可以通过扫描其它目录一次性重建；即使被手动删除也不影响 Forge 正常工作
- **开放区语义**：`.forge/features/` 处于开放区（非冻结、非受保护），避免 Hook 自己写入时被 PreToolUse 拦截

## Glossary

- **Feature_Dossier**：一个 feature 的索引文档，路径为 `.forge/features/<topic>.md`。聚合该 topic 在所有阶段目录中的产物链接 + 状态 + 摘要。派生文件，可再生。
- **Topic_Key**：一个功能的规范标识符，kebab-case（如 `structured-observability`）。Dossier 文件名即 Topic_Key。
- **Stage_Directory**：`.forge/` 下按阶段组织的目录，共 7 个：`decisions/`、`specs/`、`plans/`、`reviews/`、`progress/`、`findings/`、`debug/`。Dossier 的生成依据是这些目录下匹配 Topic_Key 的文件。
...
- **Auto_Rebuild_Hook**：PostToolUse Hook，在 `.forge/{decisions,specs,plans,reviews,progress,findings,debug}/` 下任何 markdown 文件被 Write/Edit 时触发 Dossier_Rebuilder，更新对应 topic 的 Dossier。
...
**User Story:** As a developer calling `buildDossier`, I want a companion function to scan `.forge/` and produce the `StageScanResult` input, so that I can feed `buildDossier` without reimplementing filesystem logic.
...
2. WHEN invoked with a single topic argument:
   ...
   - Write the result to `.forge/features/<topic>.md`, creating `.forge/features/` if absent
   - Print `dossier: wrote .forge/features/<topic>.md (N stages, M files)` on success
...
5. THE script SHALL exit 2 when the current directory has no `.forge/` directory, with message suggesting `forge init`.
...
1. THE `hooks/hooks.json` SHALL gain a new PostToolUse entry:
   - Matcher: `Write|Edit`
   - Trigger condition: the written file path matches the regex `\.forge/(decisions|specs|plans|reviews|progress|findings|debug)/[^/]+` 
...
4. THE hook SHALL NOT trigger when the written file is the dossier itself (`.forge/features/*.md`) to avoid infinite loops.
5. THE hook SHALL NOT trigger on reads; only `Write|Edit` tools.
6. WHEN the Hook fires for a written path it cannot map to a Topic_Key (e.g., `.forge/decisions/ADR-TEMPLATE.md` or an unrecognized filename pattern), it SHALL exit 0 silently without writing anything.
...
**User Story:** As a Forge maintainer, I want `.forge/features/` and its dossier files to live in the open zone, so that the auto-rebuild Hook never gets blocked by frozen-file protection.

#### Acceptance Criteria

1. THE `src/conflict-classifier.ts` SHALL treat `.forge/features/**` paths as **open zone** (same category as `.forge/status.md`, `.forge/findings/`).
2. THE `.forge/features/` paths SHALL NOT appear in any frozen-zone pattern list (`check-frozen.ts`, `scripts/check-frozen.sh`, `templates/config.md`'s frozen examples).
3. THE `.forge/features/` paths SHALL NOT appear in any guarded-zone pattern list (`src/zone-classification.md`-equivalent rules).
...
5. WHEN a user manually deletes `.forge/features/<topic>.md`, the next triggering Write/Edit on that topic's stage files SHALL regenerate it via the Hook.
...
1. WHEN `.forge/features/` does not exist, the CLI SHALL create it as a regular directory (no special permissions).
2. WHEN `scripts/rebuild-feature-dossier.mjs --all` is run for the first time on the current repo, it SHALL succeed (generate a dossier for each discovered topic) without touching any other file under `.forge/`.
...
5. THE `.gitignore` recommendation in README.md SHALL note that `.forge/features/` is safe to include in version control (sharing feature history across the team) OR to exclude (treating as local-only derived state); both options are supported.
...
7. PROPERTY-TESTED: `test/feature-dossier.property.test.ts` SHALL assert:
   - Round-trip: `discoverTopics` output is a superset of the Topic_Key derived from any stage file under `.forge/`
...
8. DOCUMENTATION: `README.md` SHALL gain a short subsection under `.forge/ 目录结构` pointing to `.forge/features/` with one sentence on purpose.
...
1. WHEN `/forge learn` archives a topic to `.forge/archive/<date>-<topic>/`, the archive SHALL include the topic's dossier as `archive/<date>-<topic>/dossier.md` (copy, not symlink).
...
- **物理目录重组**：不把 `.forge/specs/`、`.forge/plans/` 等目录搬进 `.forge/features/<topic>/` 下。所有 skill 的输出路径保持原样。
...
- **跨仓库 dossier**：只在当前 repo 的 `.forge/` 下工作，不处理 worktree 之间的聚合。

## Dogfooding Baseline

本 spec 启用后，应立即对当前 Forge 仓库现有的约 30+ 个 topic（见 `.forge/plans/` 文件列表）执行一次 `dossier:rebuild:all`，生成基线索引。后续通过 Hook 维持同步。
