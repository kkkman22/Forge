...
│  └── 统一 exit code：0 成功 / 1 失败 / 2 缺 .forge/              │
│                                                                 │
│  Layer 3: PostToolUse Hook（hooks/hooks.json）                  │
│  └── Write|Edit 匹配 .forge/{decisions,specs,...}/ → CLI         │
│      └── 失败静默（|| true），timeout=5s                         │
└─────────────────────────────────────────────────────────────────┘
                               ↓ 派生产物
┌─────────────────────────────────────────────────────────────────┐
│  .forge/features/<topic>.md                                     │
...
1. **派生视图，不是真相源**：Dossier 是聚合索引，真相仍然在各 Stage_Directory 的文件里。删除整个 `.forge/features/` 不影响 Forge 任何功能。
...
| `test/fixtures/feature-dossier/` | 测试用的最小 `.forge/` fixture |
...
| `src/conflict-classifier.ts` | 白名单 `.forge/features/**` → open zone（R6） |
| `README.md` | `.forge/ 目录结构` 章节补一行说明 features/（R8 AC8） |
...
- `.forge/specs/` 目录（Forge + Kiro 双轨保持兼容）
- `scripts/init.sh`（`.forge/features/` 由 CLI 按需创建）

## 6. Error Handling & Edge Cases

| Case | 行为 |
|------|------|
| `.forge/` 不存在 | CLI exit 2 + 提示 `forge init`；Hook 静默退出 |
| `.forge/features/` 不存在 | CLI 自动 `mkdir -p`；Hook 同样 |
...
| Hook 触发路径是 `.forge/features/*.md` | 立即 exit 0（防止无限循环） |
...
在 `test/fixtures/feature-dossier/` 下放置一个最小的 `.forge/` 树，脚本调用 CLI 后断言生成的 `features/<topic>.md` 内容等于快照。

### 7.4 Hook 烟测

`test/feature-dossier.hook.test.ts`：模拟 `TOOL_INPUT_FILE=.forge/plans/foo.md` 环境变量，调用 CLI `--from-path` 模式，断言：
- dossier 被生成在 `features/foo.md`
- 对 `.forge/features/foo.md` 本身作为输入时 exit 0 无副作用（防循环）
- 对 `.forge/decisions/ADR-TEMPLATE.md` 作为输入时 exit 0 无副作用
...
| `.forge/features/` 被误判为受保护 | 低 | 中 | R6 有白名单验证 + 单测覆盖 |
