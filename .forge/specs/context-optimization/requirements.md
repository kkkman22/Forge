...
**明确不做的事情**：不引入 FTS5/SQLite 知识库（Forge 的 `.forge/` 文件系统状态已足够）；不做会话事件持久化（Forge 用 Subagent 隔离 + 文件系统状态替代）；不做 Hook 层路由拦截（会与 Forge 现有 Hook 冲突）；不引导安装 context-mode（Agent prompt 注入冲突风险太高）。
...
本 spec 于 2026-05-29 合并了以下 spec 的需求内容。被合并者的原 requirements.md 保留在 `.forge/specs/_archived/` 中供历史参考。
