...
work_nature: feature
brownfield: true
import_source: ".forge/decisions/2026-06-27-packs-plugin-distribution.md"
related_adrs:
  - "待生成（spec lock 后由 decide→ADR 流程）"
related_decisions:
  - ".forge/decisions/2026-06-27-packs-plugin-distribution.md"
health:
  score: 0
  verdict: "pending"
---
...
**Requirement**：
- WHEN 用户执行 `/forge init --pack <name>` 且 pack 成功定位 THEN init.sh SHALL 追加一行使用记录到项目 `.forge/knowledge/tool-health.md`（复用现有运行时日志机制，不引入新文件）。
- THE 记录格式 SHALL 与 tool-health 现有条目一致（时间戳 · 事件类型 · pack 名 · 分发途径 plugin/clone）。
- WHEN 记录失败（如 .forge/ 不可写）THEN SHALL 静默跳过，不阻断 init。
