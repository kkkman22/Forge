...
[Orchestrator transition] ← 需求 3：追加 write_event_log effect
   │
   v
[Effect Executor] ← 需求 3：写 .forge/runs/<id>/events.jsonl
   │
   v
...
.forge/knowledge/adr-index.md 新增索引文件
...
async function resumeRun(runId: string) {
  const events = parseEventLog(await readFile(`.forge/runs/${runId}/events.jsonl`));
  const replayed = replay(createInitialState(), events);
  const persisted = JSON.parse(await readFile(`.forge/runs/${runId}/state-final.json`));
  ...
  update-baseline:
    ...
    steps:
      ...
      - name: commit baseline
        run: |
          git add .forge/knowledge/metrics.md
