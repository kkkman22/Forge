...
- **Verify-By**: manual
- **Evidence**:`grep -A3 "裁决(2026-06-30):撤销" .forge/specs/arch-review-remediate-0626/tasks.md` 命中;`test -d src/state-machine`(目录仍在);frontmatter status_note 含撤销说明。

### AC-2:arch-review T-03/T-08 重评记录
T-03 / T-08 各有 deferred 判定与理由记录。
- **Verify-By**: manual
- **Evidence**:`grep -c "重评(2026-06-30)" .forge/specs/arch-review-remediate-0626/tasks.md` ≥ 2。
...
- **Verify-By**: manual
- **Evidence**:`grep "status 仅" .forge/specs/spec-lifecycle-management/requirements.md` 命中;`rebuild-spec-index.mjs` 的 completed 数 == requirements.md=completed 的真实目录数。
