...
- 修改 `.forge/config.md` + `templates/config.md`（新增字段）
...
│  调用方（外层驱动 / SKILL 指令执行）                            │
│                                                                │
│  1. 读 .forge/status.md → tier (light/standard/full)          │
│  2. 读 .forge/config.md → configContent                        │
...
/**
 * Resolve per-tier agent timeout from `.forge/config.md`.
 *
 ...
 * @param configContent  Raw text of `.forge/config.md`.
