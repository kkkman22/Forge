...
```
现有（v2.5）:
  .forge/knowledge/solutions/<name>.md
    ---
    name: <name>
    confidence: 0.3-0.9
    ---
...
| `.forge/docs/living/review-confidence-guide.md` | 置信度系统使用指南 |
...
| `.forge/config.md` | 增加 `review_force_model` 和 `context_budget` 配置项 |
...
2. 被抑制的 finding 记录在 `.forge/progress/<slug>-review-suppressed.jsonl`，可追溯
3. `/forge learn` 可以从 suppressed findings 中提取"被抑制但后来证实"的模式，调整阈值

**回滚**：confidence gate 的阈值可通过 `.forge/config.md` 的 `review_confidence_threshold` 配置调整，默认 75 可降到 50。
...
**回滚**：`.forge/config.md` 增加 `review_enable_adversarial: false` 开关。
...
**回滚**：`.forge/config.md` 增加 `review_enable_validation: false`。
...
3. `.forge/config.md` 增加 `knowledge_template_version: 2` 标记，区分新旧
...
**选择**：保持 Forge 的 `.forge/knowledge/solutions/`

**理由**：
- Forge 的 `.forge/` 目录是受保护区，知识文档不会被 git 忽略
- `docs/solutions/` 更面向人类阅读，`.forge/knowledge/solutions/` 更面向 agent 检索
