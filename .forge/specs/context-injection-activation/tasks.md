...
- [ ] 1. review subagent prompt 注入 context 清单
  - `src/review/subagent.ts:buildReviewSubagents`(:59-97)调用 `readContextEntries` 读 `.forge/runs/<runId>/context.jsonl` + `mergeContextSources` 合并 plan frontmatter `context_files`
...
- [ ] 4. build 阶段写入 context.jsonl
  - build skill(读 plan 后、spawn subagent 前)调用 `appendContextEntry` 把 plan frontmatter context_files 写入 `.forge/runs/<runId>/context.jsonl`
  - 用现有 O_APPEND 并发语义,不引入锁
  - plan 无 context_files 时跳过
  - runId 来源确认并复用 `.forge/runs/` 现有命名
