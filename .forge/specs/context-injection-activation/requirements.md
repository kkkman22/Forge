...
- 路径约定 `.forge/runs/<runId>/context.jsonl` 已在 `context-injection.ts:23,44,59` 注释定义。
...
- 复用 plan frontmatter 已有的 `context_files` 字段作为静态来源,`.forge/runs/<runId>/context.jsonl` 作为动态运行时来源,经 `mergeContextSources` 去重合并。
...
1. THE `src/review/subagent.ts:buildReviewSubagents`(`:59-97`)SHALL 在构造每个 subagent prompt 时,调用 `readContextEntries` 读取 `.forge/runs/<runId>/context.jsonl`(若存在),并结合 plan frontmatter 的 `context_files` 经 `mergeContextSources` 去重合并,得到本任务的文件清单。
...
3. THE `contextFiles` 的来源 SHALL 由 decide skill 层(plan 已 approved 时读 plan frontmatter `context_files`;或读 `.forge/runs/<runId>/context.jsonl`)填充。
...
1. THE 至少一个生产流程(建议 build skill 在读取 plan 后、spawn subagent 前)SHALL 调用 `appendContextEntry` 把 plan frontmatter 的 `context_files` 条目写入 `.forge/runs/<runId>/context.jsonl`。
...
4. THE `<runId>` 的来源 SHALL 复用现有 run 标识机制(查看 `.forge/runs/` 现有命名约定,不新造 run id 体系)。
