...
7. THE Clarification_Gate SHALL 读取 `.forge/charter.md`（如果存在）来避免提出 charter 已回答的问题（如"技术选型是什么"——charter 已记录）。
...
1. WHEN Reframing_Gate 或 Clarification_Gate 被执行，THE 系统 SHALL 记录以下数据到 `.forge/progress/<slug>-reframing.jsonl`：
   - `timestamp`：ISO 8601
   - `skill`：`decide` 或 `spec`
