...
- 第一次调用时检查 `.forge/contexts/<context>/event-storm.md` 是否存在
...
2. 读 `.forge/contexts/ownership.yaml`（项目级） + enabled packs 的 contexts 路径映射
...
实现：
1. 扫描 `.forge/specs/*/spec.md`，解析 frontmatter.context + `## Scenarios` 段
2. 解析每个 Scenario title / tags / source line（用 Sprint 1 scenario-linter 的解析能力）
3. 扫描 `.forge/acceptance/*/report.md`，解析 `verdicts_summary` 和 scenario-level verdicts
...
```
1. 读 .forge/contexts/reservations/event-storm.md（若存在）
...
5. 5 phase 完成 → serializeStormMarkdown → 写 .forge/contexts/<context>/event-storm.md
...
```
1. 读 .forge/specs/*/spec.md 列表
2. 对每个 spec：
   - 解析 frontmatter.context
   - 解析 ## Scenarios 段（用 scenario-linter 的 parser）
3. 读 .forge/acceptance/*/report.md（若存在）
   - 按 report topic 匹配对应 spec
   - 提取 per-scenario verdicts
4. merge 成 LivingDocData
5. renderLivingDoc(data, '.forge/docs/living/')
6. 输出：✅ Living doc generated at .forge/docs/living/index.html (N scenarios)
```
