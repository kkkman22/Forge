...
本 spec 在 Forge 中集成 `claude ultrareview --json` 作为 CI 评审通道，并打通评审证据到 `.forge/reviews/` 目录的落盘。解决的问题：

- **现状**：Forge `/forge review` 依赖开发者在本地手动触发，CI 侧只跑 `npm run check`，没有 AI 参与的代码评审。
- **目标**：每次 PR 推送自动触发 `claude ultrareview --json`，结果落到 `.forge/reviews/<pr-number>-ci.md`，失败则阻断合并。

本 spec 仅涉及 CI workflow、一个新的包装脚本、`.forge/reviews/` 命名约定更新，不改动任何现有 skill / agent 行为。
...
- **Review_Artifact**：CI 产出的评审落盘文件，路径 `.forge/reviews/<pr-number>-ci.md`。
...
3. WHEN `UltraReview_CLI` exits with code 0, THE Review_Wrapper SHALL parse the JSON output, write a human-readable Markdown report to `.forge/reviews/<pr-number>-ci.md`, and exit 0.
4. WHEN `UltraReview_CLI` exits with a non-zero code, THE Review_Wrapper SHALL still attempt to write the partial JSON to `.forge/reviews/<pr-number>-ci.md` with a clear `[CI review failed]` header, and propagate the original exit code.
...
4. WHEN the Review_Wrapper produces `.forge/reviews/<pr-number>-ci.md`, THE CI_Workflow SHALL upload that file as a workflow artifact named `ultrareview-pr-<pr-number>`.
...
1. THE Review_Artifact SHALL be written to `.forge/reviews/<pr-number>-ci.md` where `<pr-number>` is a positive integer without leading zeros.
...
4. WHEN a Local_Review_Channel artifact already exists for the same PR (e.g. `.forge/reviews/<pr-number>-local.md`), THE Review_Artifact SHALL NOT overwrite it; the two artifacts coexist and may be cross-referenced.
5. THE `.forge/reviews/` directory SHALL remain in the "受保护区" per `.forge/config.md`: CI-produced artifacts are append-only relative to pre-existing reviews.
...
1. WHEN `/forge review` starts and a `.forge/reviews/<pr-number>-ci.md` exists for the current branch, THE Forge_Review_SKILL SHALL read that artifact's frontmatter and include its severity counts in the local review's opening context statement.
...
4. THE Forge_Review_SKILL SHALL NOT modify `.forge/reviews/<pr-number>-ci.md`; all CI artifacts are read-only from the local skill's perspective.
...
1. WHEN `UltraReview_CLI` returns a rate-limit error or authentication failure, THE Review_Wrapper SHALL write a stub `.forge/reviews/<pr-number>-ci.md` noting the failure reason and exit with code 0 (non-blocking), unless `CI_ULTRAREVIEW_STRICT=1` is set.
