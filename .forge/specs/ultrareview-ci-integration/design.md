...
3. **产物层**：`.forge/reviews/<pr-number>-ci.md` 标准化的评审落盘格式。
...
**不涉及**：`/forge review` 本地流程的三层并行评审逻辑、`.forge/reviews/` 权限模型（保持现有"受保护区"语义）。
...
└─────────────────────────────────────────────────────────┘
   ...
   │   if exists .forge/reviews/<pr>-ci.md:     │
    ...
    steps:
      ...
      - uses: actions/upload-artifact@v4
        ...
        with:
          ...
          path: .forge/reviews/${{ github.event.pull_request.number }}-ci.md
      - name: Comment on PR
        if: always()
...
**文件**：`.forge/reviews/<pr-number>-ci.md`
...
2. **`.forge/reviews/*-ci.md` 格式的 contract test**（扩展 `test/contract.test.ts`）：
