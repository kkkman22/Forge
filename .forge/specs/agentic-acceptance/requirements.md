...
1. SPEC scenario Given clauses SHALL use placeholders (`{{FORGE_E2E_PASSWORD}}`); the runner SHALL resolve placeholders from environment / `.forge/secrets.env` (mode 0600, already gitignored) at runtime.
...
3. THE evidence directories `.forge/acceptance/`, `.forge/acceptance/**` SHALL be added to `.gitignore`.
...
6. agent-browser version SHALL be pinned by SHA256 in `.forge/config.md`; unpinned/auto-upgrade SHALL be refused.
