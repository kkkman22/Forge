...
layout: tasks
created: 2026-06-17
spec_ref: .forge/specs/agentic-acceptance/
decision_ref: .forge/decisions/2026-06-17-agentic-acceptance-with-agent-browser.md
format: lightweight
...
## 项目踩坑本能（从 .forge/knowledge/instincts.md 注入，所有外部命令任务必读）
...
- **RED/Verify**: `test/accept-config.test.ts`
  - 读 `.gitignore`，断言含 `.forge/acceptance/` 行。
  - 读 `.forge/config.md`，断言含 `agent_browser_pin_sha256:` 字段（值非空）。
- **GREEN** (MODIFY `.gitignore`, `.forge/config.md`): 加 `.forge/acceptance/`；加 `agent_browser_pin_sha256: "<TBD-by-releaser>"` 字段（releaser 发布时填实际 SHA）。
...
- **GREEN** (CREATE `docs/acceptance-onboarding.md`): 三条硬约束（装 agent-browser / 起 dev server / 占位符凭据走 .forge/secrets.env）+ 命令示例 `/forge accept` + 与 `/forge review` 区分表。
- **Verify**: manual
