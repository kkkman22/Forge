...
3. THE delegate Runner SHALL 经 `forge_exec` 调用项目测试命令(取自 `.forge/config.md` `test_commands` 段,或按约定探测包管理器+`npm/pnpm/yarn run test:unit`),作用域限定 AC 的 `Evidence` 文件。
...
9. **[超时 P4 修复]** THE delegate Runner SHALL 对单次 `forge_exec` 执行设置超时上限(默认 60s,可在 `.forge/config.md` `delegate_timeout` 配置);超时 SHALL 返回 `INCONCLUSIVE` + `failureReason: "delegate timeout after Ns"`,不无限挂起验收流程(对齐 Google Testing Blog 的 per-layer 时间预算要求)。
...
2. WHEN `e2e` 场景占比 > `e2e_ratio_threshold`(默认 0.3,可在 `.forge/config.md` 配置)且 `unit+component` 层场景数为 0,THE 门禁 SHALL 阻断,输出"E2E-heavy 反模式:请将组合下沉到 component 层"。
...
- `.forge/config.md`:新增 `test_commands`、`e2e_ratio_threshold`、`strict_pyramid` 配置项。
