...
- **Source Review:** .forge/reviews/order-export.md
...
Build 阶段的 Final Validation **必须**使用 `.forge/config.md` 中定义的 CI 命令：

1. 读取 `.forge/config.md` 的 `## CI 检查命令` 章节
2. 按顺序执行所有列出的命令
3. 第一个失败的命令即终止验证，报告失败
4. **禁止**自行拼凑验证命令——`.forge/config.md` 是唯一的命令来源
5. 如果 `.forge/config.md` 不包含 CI 命令章节，**终止验证并报错**
```
...
| 决策文档 | Write-and-discard | 完整文档写入 `.forge/decisions/`，context 仅保留结论 |
```
...
| Test | Coverage |
|------|----------|
| Review report file write | R2.1 — full report written to `.forge/reviews/` |
