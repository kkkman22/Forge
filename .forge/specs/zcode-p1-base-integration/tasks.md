...
**目标**：当 `--platform zcode` 时，在 `.forge/` 创建后生成 `.zcode/config.json`，含 `hooks.enabled:true` + `hooks.events.Stop` 注入 status.md 摘要。
...
**设计决策**：status 摘要脚本——若 Forge 已有读 status.md 的脚本（如 stop-additional-context 系列）则复用其摘要逻辑；否则新增一个极简脚本读 `.forge/status.md` 头部输出 additionalContext。P1 取极简路径。
...
2. GREEN：实现脚本：预置临时 `.forge/knowledge/evolved-rules.md`，用 child_process 跑 hook 喂 stdin，捕获 stdout/exit code，跑断言。
...
**Done**：R3 AC4/AC5、R4 AC4/AC5、R5 AC5 满足。文档放 `.forge/specs/zcode-p1-base-integration/` 或 `docs/zcode/` 下。
