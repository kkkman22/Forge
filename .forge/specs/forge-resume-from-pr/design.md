...
本 spec 在 Forge 的 `/forge resume` 命令上新增 `--from-pr <url-or-number>` 标志，实现"从 PR 一键恢复 Claude Code + Forge 双端状态"。核心是把 Claude Code 原生的 `--from-pr` 与 Forge 的 `.forge/status.md` 状态恢复衔接起来。
...
- 新增 `.forge/.pr-slug-cache.json` 到 `.gitignore`
- 修改 `README.md`、`CHANGELOG.md`

**不涉及**：`.forge/specs/`、`.forge/plans/`、`.forge/progress/` 的文件格式；`/forge plan`、`/forge build` 的行为。
...
3. **幂等设计**：重复运行相同的 `--from-pr` 命令不产生额外副作用，除了更新 `.forge/status.md` 的 timestamp。

4. **只写两个文件**：`--from-pr` 只修改 `.forge/status.md` 和 `.forge/.pr-slug-cache.json`，其他所有读取都只读。这与 Forge 的 frozen-zone 规则一致。
...
- `updateStatus(slug, metadata)` → 写 `.forge/status.md`
- `writeRunReport(...)` → 写 `.forge/runs/<ts>-resume-from-pr.md`
...
3. 以结构化格式向用户汇报：
   ...
   ⚠ 缺失：.forge/progress/<slug>.md 已被归档
   ```
...
3. **PR description 链接**：`.forge/specs/([a-z0-9-]+)/` → slug
4. **Decisions 关联**：grep `.forge/decisions/*.md` 中 `pr: <number>` 或 `pull_request: <url>` 的 ADR → frontmatter `slug` 字段
5. **交互提示**：列出 `.forge/specs/` 下的 slug，要求选择；`FORGE_INTERACTIVE=0` 时直接 fail

**缓存**：`.forge/.pr-slug-cache.json`:
```json
{
...
interface ContextBundle {
  ...
  reviews: string[];        // .forge/reviews/<pr>-*.md
...
**加载流程**：
1. `readdir(.forge/specs/<slug>)` → specFiles
2. `access(.forge/plans/<slug>.md)` → planFile
3. `access(.forge/progress/<slug>.md)` → progressFile
4. `readdir(.forge/reviews) | grep ^<pr>-` → reviews
5. 从 spec 文件中 grep `.forge/decisions/ADR-*` 引用 → adrs
...
**冲突处理**：如果 `.forge/status.md` 已有不同 slug，按 Requirement 5.2 交互提示。非交互模式下退出 1。

## Data Models

### PR_Slug_Cache

文件：`.forge/.pr-slug-cache.json`（git-ignored）
...
文件：`.forge/runs/<timestamp>-resume-from-pr.md`
...
| slug 推断所有源均失败 | 交互模式：prompt；非交互：exit 1 | 列出 `.forge/specs/` 供选择 |
| CC `--from-pr` 不支持 | 继续 Forge-only，打印 warning | "CC 版本 <2.1.29，session 未恢复，仅恢复 Forge 状态" |
| `.forge/status.md` 冲突 | 交互模式 prompt；非交互：exit 1 | 展示两个 slug 的差异 |
