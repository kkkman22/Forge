...
Add an artifact layer under `.forge/artifacts/` and route review/test/ship/verify evidence through it. Markdown files remain as views, not source of truth.

## Current State

- Review reports use markdown frontmatter.
- Ship gates write `.forge/ship/<run-id>-gates.json`.
...
```text
.forge/artifacts/<run-id>/<artifact-id>.json
.forge/artifacts/index.jsonl
```
