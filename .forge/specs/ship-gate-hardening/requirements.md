...
#### 1.1 Review 门禁
- 检查 `.forge/reviews/` 中最新 review 报告的状态
...
#### 1.2 Test 门禁
- 检查最近一次测试运行的结果（`.forge/test-results/` 或 CI 状态）
- 如果测试有失败 → 阻断 ship
- 如果无测试记录 → 提示运行测试，阻断 ship
- `ci_check_command` 在 config.md 中定义时，执行该命令验证

#### 1.3 Progress 门禁
- 检查 `.forge/progress/` 中当前 feature 的任务完成状态
- 如果存在 `in_progress` 状态的任务 → 警告（不阻断，可能任务粒度过大）
- 如果所有任务均 `completed` → 通过

### 2. P1 Fix Checklist 集成

- 2.1 Review 报告中的 P1 issue 记录到 `.forge/reviews/<run-id>-p1-fixlist.json`
...
- [ ] `--skip-gate` 机制实现且有文档
- [ ] 门禁结果写入 `.forge/ship/` 目录供审计追溯

## 依赖

- `forge-review` SKILL（review 报告格式）
- `forge-build` SKILL（test 运行）
- `.forge/config.md`（ci_check_command）
