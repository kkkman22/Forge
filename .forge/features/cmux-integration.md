...
- **Plan** (approved, 2026-05-08)：为 Forge 增加可选的 cmux 集成层：通过独立守护进程 Mirror_Daemon 观察 `.forge/` 状态文件，将 Forge 生命周期（路由、DAG 进度、评审结果、Forge Loop 迭代、冻结拦截）以结构化形式投射到 cmux 侧边栏、通知和浏览器 surface。同时...
