...
const bodyContentArb = fc
  .array(
    fc.string({ minLength: 0, maxLength: 60 }).filter((s) => !s.includes("---")),
...
const executionModeArb: fc.Arbitrary<ExecutionMode> = fc.constantFrom(
  "interactive" as const,
  "autonomous" as const,
);

/** Arbitrary ConfirmationPoint value (all 11 values). */
const confirmationPointArb: fc.Arbitrary<ConfirmationPoint> = fc.constantFrom(
  "router_tier" as const,
  "plan_approval" as const,
  "build_pause" as const,
  "review_p0p1" as const,
  "ship_method" as const,
  "refactor_scan_select" as const,
  "refactor_design_review" as const,
  "refactor_apply_step" as const,
  "fix_report_confirm" as const,
  "fix_analyze_confirm" as const,
  "fix_apply_verify" as const,
);

/** Arbitrary ConfirmationPoint value (original 5 only — for backward compat tests). */
const originalConfirmationPointArb: fc.Arbitrary<ConfirmationPoint> = fc.constantFrom(
  "router_tier" as const,
  "plan_approval" as const,
  "build_pause" as const,
  "review_p0p1" as const,
...
const newConfirmationPointArb: fc.Arbitrary<ConfirmationPoint> = fc.constantFrom(
  "refactor_scan_select" as const,
  "refactor_design_review" as const,
  "refactor_apply_step" as const,
  "fix_report_confirm" as const,
...
const statusFileWithInvalidModeArb = fc
  .tuple(
    fc
      ...
      .filter(
        (s) => s !== "interactive" && s !== "autonomous" && !s.includes("\n") && !s.includes("---"),
      ),
    fc.array(yamlFieldArb, { minLength: 0, maxLength: 4 }),
    bodyContentArb,
  )
...
describe("Feature: loop-skills-fusion, Property 1: ExecutionMode round-trip consistency", () => {
  ...
  it("writeExecutionMode then clearExecutionMode results in interactive mode", () => {
    fc.assert(
      fc.property(statusFileArb, (content) => {
  ...
  it("round-trip preserves other frontmatter fields", () => {
    fc.assert(
      fc.property(statusFileArb, executionModeArb, (content, mode) => {
        ...
        if (!originalMatch) return; // skip if no frontmatter
...
describe("Feature: loop-skills-fusion, Property 2: getExecutionMode 解析正確性", () => {
  ...
  it("returns the correct mode when a valid mode field is present", () => {
    fc.assert(
      fc.property(statusFileWithModeArb, ({ content, mode }) => {
  ...
  it("returns interactive for invalid mode values", () => {
    fc.assert(
      fc.property(statusFileWithInvalidModeArb, (content) => {
        const result = getExecutionMode(content);
  ...
  it("returns interactive when mode field is missing", () => {
    fc.assert(
      fc.property(statusFileWithoutModeArb, (content) => {
        const result = getExecutionMode(content);
  ...
  it("returns interactive for content without valid frontmatter", () => {
    fc.assert(
      fc.property(
        fc.string({ minLength: 0, maxLength: 200 }).filter((s) => !s.trimStart().startsWith("---")),
        (content) => {
          const result = getExecutionMode(content);
...
describe("Feature: loop-skills-fusion, Property 2: 自主モード確認点全自動", () => {
  ...
  it("autonomous mode returns auto with a defined preset string for all confirmation points", () => {
    fc.assert(
      fc.property(confirmationPointArb, (point) => {
  ...
  it("ship_method preset in autonomous mode is 'keep branch'", () => {
    fc.assert(
      fc.property(fc.constant("ship_method" as ConfirmationPoint), (point) => {
        const decision = resolveConfirmation("autonomous", point);
  ...
  it("resolveConfirmation is deterministic for any mode and point", () => {
    fc.assert(
      fc.property(executionModeArb, confirmationPointArb, (mode, point) => {
...
describe("Property: 新增 ConfirmationPoint autonomous 全自動", () => {
  ...
  it("autonomous mode returns auto with preset for all new confirmation points", () => {
    fc.assert(
      fc.property(newConfirmationPointArb, (point) => {
        const decision = resolveConfirmation("autonomous", point);
  ...
  it("autonomous mode returns correct preset for each new confirmation point", () => {
    const expectedPresets: Record<string, string> = {
...
describe("Property: 現有 ConfirmationPoint 向後兼容", () => {
  ...
  it("original confirmation points still return auto with correct presets in autonomous mode", () => {
    const originalPresets: Record<string, string> = {
    ...
    fc.assert(
      fc.property(originalConfirmationPointArb, (point) => {
        const decision = resolveConfirmation("autonomous", point);
