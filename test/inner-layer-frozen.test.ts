...
function createExecutor(overrides?: Partial<EffectExecutorDeps>) {
 ...
 */
function setupStagedFilesMock(stagedFiles: Record<string, string | null>): void {
 ...
 */
function outerLayerJudgment(filePath: string, content: string): boolean {
  const relativePath = normalizeForgePath(filePath);
  const zone = getProtectionZone(relativePath);
  if (zone !== "frozen") return false;
...
describe("inner-layer frozen check — unit tests", () => {
  ...
  it("locked status file is blocked by inner-layer check", async () => {
    ...
    try {
      ...
      expect((err as FrozenZoneViolation).files).toContain(".forge/specs/feature/spec.md");
      expect((err as FrozenZoneViolation).code).toBe("FROZEN_ZONE_VIOLATION");
    }

    expect(onLog).toHaveBeenCalledWith(
      expect.stringContaining("Inner-layer frozen zone check blocked commit"),
    );
  ...
  it("approved status file is blocked by inner-layer check", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(FrozenZoneViolation);
      expect((err as FrozenZoneViolation).files).toContain(".forge/plans/my-plan.md");
    }
  ...
  it("git show failure results in warning log and file treated as suspicious", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(FrozenZoneViolation);
      expect((err as FrozenZoneViolation).files).toContain(".forge/specs/broken/spec.md");
    }

    // Should log a warning about the failed git show
    expect(onLog).toHaveBeenCalledWith(
      "⚠️ Could not read staged version of .forge/specs/broken/spec.md — treating as suspicious",
    );
  ...
  it("draft status file in frozen zone is NOT blocked", async () => {
    ...
    const blockedCalls = (onLog.mock.calls as string[][]).filter(
      (call) =>
        typeof call[0] === "string" && call[0].includes("Inner-layer frozen zone check blocked"),
    );
  ...
  it("non-.forge/ files are not checked by inner-layer", async () => {
    ...
    const warningCalls = (onLog.mock.calls as string[][]).filter(
      (call) => typeof call[0] === "string" && call[0].includes("frozen zone"),
    );
...
describe("Property 7: Inner-layer and outer-layer frozen zone checks are consistent", () => {
  ...
  const frozenRelativePathArb = fc.oneof(
    fc
      .tuple(
        ...
        fc.stringMatching(/^[a-z][a-z0-9-]{0,15}\.md$/),
      )
      .map(([dir, file]) => `specs/${dir}/${file}`),
    fc.stringMatching(/^[a-z][a-z0-9-]{0,15}\.md$/).map((f) => `plans/${f}`),
  ...
  const nonFrozenStatusArb = fc.oneof(
    fc.constant("draft"),
  ...
  it("frozen zone path with frozen status: inner-layer matches outer-layer", () => {
    fc.assert(
      fc.property(frozenRelativePathArb, frozenStatusArb, (relativePath, status) => {
  ...
  it("frozen zone path with non-frozen status: inner-layer matches outer-layer", () => {
    fc.assert(
      fc.property(frozenRelativePathArb, nonFrozenStatusArb, (relativePath, status) => {
        const filePath = `.forge/${relativePath}`;
        const content = makeFrontmatter(status);
  ...
  it("non-frozen zone path: inner-layer matches outer-layer (never blocked)", () => {
    fc.assert(
      fc.property(nonFrozenRelativePathArb, frozenStatusArb, (relativePath, status) => {
        const filePath = `.forge/${relativePath}`;
        const content = makeFrontmatter(status);
