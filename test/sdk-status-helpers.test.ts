...
import type { StatusFileIO } from "../src/sdk-status-helpers.js";
import {
  clearLoopFieldsOnShutdown,
  getPhaseFromStatus,
  getTierFromStatus,
  getWorkNatureFromStatus,
  initializeLoopFields,
  safeReadStatusFile,
  safeUpdateIterationStatus,
  safeWriteStatusFile,
} from "../src/sdk-status-helpers.js";
...
function createIO(overrides?: Partial<StatusFileIO>): StatusFileIO {
  return {
    read: vi.fn(() => ""),
    write: vi.fn(),
    ...overrides,
...
function buildStatus(fields: Record<string, string | number>): string {
...
describe("safeReadStatusFile", () => {
  ...
  it("logs a debug warning when IO.read throws", () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining("[debug] safeReadStatusFile failed"),
    );
  });
});
...
describe("safeUpdateIterationStatus", () => {
  it("updates iteration status in StatusFile", () => {
    ...
    expect(io.write).toHaveBeenCalledTimes(1);
    const written = (io.write as ReturnType<typeof vi.fn>).mock.calls[0][0] as string;
...
describe("clearLoopFieldsOnShutdown", () => {
  ...
  it("preserves skill_sequence on abnormal exit", () => {
    ...
    const lastWriteCall = (io.write as ReturnType<typeof vi.fn>).mock.calls;
    const written = lastWriteCall[lastWriteCall.length - 1][0] as string;
