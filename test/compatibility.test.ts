...
import type { ClaudeVersionRange } from "../src/compatibility.js";
...
describe("checkClaudeVersion", () => {
  const range: ClaudeVersionRange = {
    minimum: "2.1.163",
    maximum: "2.1.170",
  ...
  it("no maximum defined → never warns on high version", async () => {
    const { checkClaudeVersion } = await import("../src/compatibility.js");
    const noMaxRange: ClaudeVersionRange = { minimum: "2.1.163", verifiedLatest: "2.1.163" };
