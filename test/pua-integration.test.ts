/**
 * Integration tests — verify end-to-end PUA Quality Engine flow across
 * SdkDriver, PUA Engine, StatusFile, and ContextAccumulator.
 *
 * These tests exercise the full pipeline: failure pattern detection →
 * pressure escalation → methodology switch → prompt injection, using
 * the real SdkDriver with mock agent and effect executor.
 *
...
function createSuccessResult(summary = "did stuff"): AgentResult {
  return {
    output: {
      success: true,
      summary,
...
function createSoftFailureResult(summary = "failed to make progress"): AgentResult {
  return {
    output: {
      success: false,
      summary,
 ...
 */
function createPuaConfig(overrides?: Partial<SdkDriverConfig>): {
  config: SdkDriverConfig;
  getStatusContent: () => string;
} {
  let statusContent = '---\ncurrent_task: "test task"\ntier: "standard"\n---\n# Status\n';

  const config: SdkDriverConfig = {
  ...
  it("escalates pressure and injects PUA prompt after consecutive failures", async () => {
    ...
    const agent = createMockAgent(async (prompt) => {
      ...
      return createSoftFailureResult(
        `Unable to fix the issue, cannot resolve the dependency conflict (attempt ${callNum})`,
      );
    });
...
describe("Complete soft recovery flow", () => {
  ...
  it("pressure escalates and prompt grows monotonically across failure levels", () => {
    const levels: PressureLevel[] = ["L0", "L1", "L2", "L3", "L4"];
    const methodology: Methodology = "huawei-rca";
