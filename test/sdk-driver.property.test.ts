 ...
 */
const syncEffectArb: fc.Arbitrary<OrchestratorEffect> = fc.oneof(
  fc.record({
    type: fc.constant("commit" as const),
    message: fc.string({ minLength: 1, maxLength: 80 }),
  }),
  fc.record({ type: fc.constant("rollback" as const) }),
  fc.record({
  ...
  fc.record({
    type: fc.constant("schedule_iteration" as const),
...
describe("Feature: sdk-autonomous-loop, Property 4: Effect execution order preservation", () => {
  ...
  it("executeEffects processes effects in exact array order", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.array(syncEffectArb, { minLength: 0, maxLength: 20 }),
        async (effects) => {
          ...
          let callIndex = 0;
          executor.executeEffect = async (effect: OrchestratorEffect, signal?: AbortSignal) => {
  ...
  it("git command sequence matches effect array order", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.array(syncEffectArb, { minLength: 0, maxLength: 15 }),
        async (effects) => {
...
describe("Feature: sdk-autonomous-loop, Property 3: Commit effect produces correct git command sequence", () => {
  ...
  it("commit effect calls execFileSync with buildAddAllCommand then buildCommitCommand args", async () => {
    ...
    await fc.assert(
      fc.asyncProperty(commitMessageArb, async (message) => {
...
describe("Feature: sdk-autonomous-loop, Property 5: Git commands executed without shell", () => {
   ...
   */
  const shellMetacharArb: fc.Arbitrary<string> = fc.oneof(
  ...
  it("commit effects call execFileSync without shell: true", async () => {
    await fc.assert(
      fc.asyncProperty(shellMetacharArb, async (message) => {
        vi.clearAllMocks();
  ...
  it("mixed commit and rollback effects never use shell: true", async () => {
    const gitEffectArb: fc.Arbitrary<OrchestratorEffect> = fc.oneof(
      fc.record({
        type: fc.constant("commit" as const),
    ...
    await fc.assert(
      fc.asyncProperty(fc.array(gitEffectArb, { minLength: 1, maxLength: 10 }), async (effects) => {
        vi.clearAllMocks();
...
describe("Feature: sdk-autonomous-loop, Property 1: Driver input validation", () => {
  ...
  function createMockEffectExecutor(): EffectExecutor {
    return {
      aborted: false,
      stopped: false,
      executeEffect: vi.fn(),
  ...
  function buildConfig(objective: string): SdkDriverConfig {
    return {
      objective,
      loopConfig: {
        agent: "claude",
  ...
  it("accepts non-empty objectives and rejects empty/whitespace-only objectives", () => {
    fc.assert(
      fc.property(fc.string(), (objective) => {
        ...
        if (objective.trim() === "") {
          ...
          expect(() => new SdkDriver(config, executor, agent)).toThrow(
  ...
  it("always rejects empty string objectives", () => {
    fc.assert(
      fc.property(fc.constant(""), (objective) => {
        const config = buildConfig(objective);
        const executor = createMockEffectExecutor();
        const agent = createMockAgentInterface();
  ...
  it("always rejects whitespace-only objectives", () => {
    ...
    fc.assert(
      fc.property(whitespaceArb, (objective) => {
        const config = buildConfig(objective);
        const executor = createMockEffectExecutor();
        const agent = createMockAgentInterface();
...
describe("Feature: sdk-autonomous-loop, Property 6: Loop termination matches state machine status", () => {
  ...
  it("state machine reaches terminal status (aborted) via max iterations limit", () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 5 }), (maxIterations) => {
        let state = createInitialState();
  ...
  it("state machine stays in running status when no termination conditions are met", () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 10 }), (iterationCount) => {
        let state = createInitialState();
...
describe("Feature: sdk-autonomous-loop, Property 7: Iteration entry construction correctness", () => {
  ...
  function buildIterationEntry(
    number: number,
    success: boolean,
    output: { summary: string; key_changes_made: string[]; key_learnings: string[] },
  ): IterationEntry {
    return {
      number,
      success,
  ...
  it("successful iteration entry has success=true, matching summary, keyChanges, and keyLearnings", () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 1000 }), agentOutputArb, (iterationNumber, output) => {
  ...
  it("keyChanges is non-empty only when success is true and agent reported changes", () => {
    fc.assert(
      fc.property(
        ...
        fc.boolean(),
        agentOutputArb,
        (iterationNumber, success, output) => {
