...
function term(overrides: Partial<GlossaryTerm> & { term: string }): GlossaryTerm {
  return {
...
function buildGlossary(terms: GlossaryTerm[], archivedTerms?: GlossaryTerm[]): Glossary {
  const out: Glossary = { schema_version: 1, updated: "2026-05-05", terms };
  if (archivedTerms !== undefined) out.archivedTerms = archivedTerms;
  return out;
}
...
describe("parseGlossary + renderGlossary — archived section round-trip", () => {
  ...
  it("omits the archived section when archivedTerms is absent or empty", () => {
    ...
    const withEmpty: Glossary = { ...g, archivedTerms: [] };
