...
describe("renderDesignMarkdown (branch coverage)", () => {
  it("renders minimal design (no optional sections)", () => {
    const out = renderDesignMarkdown({ ...fullDesign } as never);
  ...
  it("renders data model when present", () => {
    const out = renderDesignMarkdown({ ...fullDesign, dataModel: "User { id, name }" } as never);
  ...
  it("renders open questions when present", () => {
    const out = renderDesignMarkdown({ ...fullDesign, openQuestions: ["Q1?", "Q2?"] } as never);
...
describe("renderTasksMarkdown (branch coverage)", () => {
  it("renders minimal tasks (no waves, no tasks)", () => {
    const out = renderTasksMarkdown({ frontmatter: fm, tasks: [] } as never);
