...
function extractKeys(obj: Record<string, unknown>, prefix = ""): string[] {
  ...
  for (const key of Object.keys(obj).sort()) {
    ...
    if (typeof value === "object" && value !== null && !Array.isArray(value)) {
      keys.push(...extractKeys(value as Record<string, unknown>, fullPath));
    } else {
      keys.push(fullPath);
    }
  }
...
describe("Translation parity: en.json ↔ zh.json", () => {
  ...
  it("should preserve {paramName} placeholders across both locales", () => {
    ...
    for (const key of enKeys) {
      ...
      let enValue: unknown = enData;
      let zhValue: unknown = zhData;
      for (const seg of segments) {
        enValue = (enValue as Record<string, unknown>)[seg];
        zhValue = (zhValue as Record<string, unknown>)[seg];
      }

      const enPlaceholders = [...(enValue as string).matchAll(placeholderRe)]
        .map((m) => m[1])
        .sort();
      const zhPlaceholders = [...(zhValue as string).matchAll(placeholderRe)]
        .map((m) => m[1])
        .sort();
