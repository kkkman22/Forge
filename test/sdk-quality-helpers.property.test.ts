...
import { evaluateGateForPhase, type QualityFileReaders } from "../src/sdk-quality-helpers.js";
...
describe("Property 5: Quality gate helper — phase routing consistency", () => {
  ...
  it("returns null when review reader returns null for review phase", () => {
    fc.assert(
      fc.property(testContentArb, progressContentArb, (test, progress) => {
  ...
  it("returns null when test reader returns null for test phase", () => {
    fc.assert(
      fc.property(reviewContentArb, progressContentArb, (review, progress) => {
  ...
  it("review phase only calls readReview reader", () => {
    fc.assert(
      fc.property(reviewContentArb, (review) => {
        const readReview = vi.fn(() => review);
        const readTest = vi.fn(() => null as string | null);
        const readProgress = vi.fn(() => null as string | null);

        const readers: QualityFileReaders = {
          readReview,
          readTest,
          readProgress,
        };
  ...
  it("test phase only calls readTest reader", () => {
    fc.assert(
      fc.property(testContentArb, (test) => {
        const readReview = vi.fn(() => null as string | null);
