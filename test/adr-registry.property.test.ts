...
function entryWithId(id: string): AdrEntry {
  return {
    id,
    title: "t",
    status: "accepted",
...
describe("nextAdrId — property-based", () => {
  ...
  it("returns an id strictly greater than every existing valid id", () => {
    fc.assert(
      fc.property(validEntryListArb, (entries) => {
  ...
  it("is strictly monotonic under iterative generation", () => {
    fc.assert(
      fc.property(validEntryListArb, (entries) => {
        ...
        expect(Number.parseInt(second.slice(4), 10)).toBeGreaterThan(
          Number.parseInt(first.slice(4), 10),
        );
      }),
      { numRuns: 50 },
    );
...
const entryArb: fc.Arbitrary<AdrEntry> = fc
  .record({
    id: adrIdArb,
  ...
  .map(
    ({ id, title, status, date }): AdrEntry => ({
      id,
      title,
      status: status as AdrEntry["status"],
      date,
      deciders: ["@a"],
      filePath: `${id}.md`,
    }),
...
describe("renderAdrIndex — property-based", () => {
  ...
  it("output contains every unique id from the input", () => {
    fc.assert(
      fc.property(uniqueEntryListArb, (entries) => {
  ...
  it("produces a symmetric supersession relationship", () => {
    fc.assert(
      fc.property(
        ...
        entryArb,
        (baseEntries, targetIdx, newAdrBase) => {
          ...
          const newAdr: AdrEntry = { ...newAdrBase, id: candidate, supersedes: target.id };
  ...
  it("returns only the entries that need updating", () => {
    fc.assert(
      fc.property(
        uniqueEntryListArb,
        entryArb,
        fc.option(adrIdArb, { nil: undefined }),
        (baseEntries, newAdrBase, maybeSupersedes) => {
          const usedIds = new Set(baseEntries.map((e) => e.id));
          let candidate = newAdrBase.id;
          let n = 0;
          while (usedIds.has(candidate)) {
  ...
  it("does not mutate its inputs", () => {
    fc.assert(
      fc.property(
        ...
        fc.option(adrIdArb, { nil: undefined }),
        (baseEntries, newAdrBase, maybeSupersedes) => {
          const newAdr: AdrEntry = { ...newAdrBase, supersedes: maybeSupersedes };
