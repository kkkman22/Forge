...
import {
  analyzeHypothesisResults,
  DEBUG_PHASES,
  type DebugPhase,
  getNextPhase,
  HYPOTHESIS_ESCALATION_THRESHOLD,
  type Hypothesis,
  type HypothesisResult,
  type HypothesisSequence,
  isValidPhaseTransition,
  shouldQuestionArchitecture,
  validateHypothesis,
} from "../src/debug.js";
...
const hypothesisResultArb: fc.Arbitrary<HypothesisResult> = fc.constantFrom(
  "confirmed",
  "rejected",
);

const hypothesisSequenceArb: fc.Arbitrary<HypothesisSequence> = fc
  .array(hypothesisResultArb, { minLength: 0, maxLength: 30 })
  .map((results) => ({ results }));

/** Sequence guaranteed to have 3+ consecutive rejections. */
const sequenceWithEscalationArb: fc.Arbitrary<HypothesisSequence> = fc
  .tuple(
    fc.array(hypothesisResultArb, { minLength: 0, maxLength: 10 }),
    fc.constant(["rejected", "rejected", "rejected"] as HypothesisResult[]),
    fc.array(hypothesisResultArb, { minLength: 0, maxLength: 10 }),
  )
  .map(([prefix, block, suffix]) => {
    ...
    for (const r of safePrefix) {
      if (r === "rejected") consecutive++;
      else consecutive = 0;
      if (consecutive >= 3) return { results: safePrefix };
    }
...
const sequenceWithoutEscalationArb: fc.Arbitrary<HypothesisSequence> = fc
  .array(
    fc.tuple(
      fc.array(fc.constant("rejected" as HypothesisResult), { minLength: 0, maxLength: 2 }),
      fc.constant("confirmed" as HypothesisResult),
    ),
...
const validHypothesisArb: fc.Arbitrary<Hypothesis> = fc.record({
  description: nonEmptyStringArb,
  verifyCommand: nonEmptyStringArb,
  expectedOutcome: nonEmptyStringArb,
});

const debugPhaseArb: fc.Arbitrary<DebugPhase> = fc.constantFrom(
  "collect",
  "pattern",
  "hypothesize",
  "fix",
...
describe("Property 21: Debug 假设验证升级", () => {
  it("3 consecutive rejections → escalation triggered", () => {
    fc.assert(
      fc.property(sequenceWithEscalationArb, (sequence) => {
        const result = analyzeHypothesisResults(sequence);
        expect(result.shouldEscalate).toBe(true);
        expect(result.consecutiveRejections).toBeGreaterThanOrEqual(
          HYPOTHESIS_ESCALATION_THRESHOLD,
        );
  ...
  it("fewer than 3 consecutive rejections → no escalation", () => {
    fc.assert(
      fc.property(sequenceWithoutEscalationArb, (sequence) => {
        const result = analyzeHypothesisResults(sequence);
  ...
  it("confirmed hypothesis resets the counter", () => {
    fc.assert(
      fc.property(
        fc.array(
          fc.tuple(
            fc.array(fc.constant("rejected" as HypothesisResult), { minLength: 1, maxLength: 2 }),
            fc.constant("confirmed" as HypothesisResult),
          ),
  ...
  it("shouldQuestionArchitecture matches analyzeHypothesisResults", () => {
    fc.assert(
      fc.property(hypothesisSequenceArb, (sequence) => {
...
describe("Debug phase transitions", () => {
  ...
  it("same phase transition is not allowed", () => {
    fc.assert(
      fc.property(debugPhaseArb, (phase) => {
