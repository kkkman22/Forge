...
describe("evaluateUiVerdict", () => {
  it("PASS when url matches and text contains all keywords", () => {
    const result = evaluateUiVerdict(
      { url: "http://localhost:5173/dashboard", title: "控制台", text: "欢迎 admin" },
      "跳转到 dashboard 且显示 欢迎",
    );
  ...
  it("FAIL when url does not contain expected path", () => {
    const result = evaluateUiVerdict(
      { url: "http://localhost:5173/login", title: "登录", text: "欢迎 admin" },
      "跳转到 dashboard",
    );
  ...
  it("FAIL when text lacks a required keyword", () => {
    const result = evaluateUiVerdict(
      { url: "http://localhost:5173/dashboard", title: "控制台", text: "" },
      "显示 欢迎",
    );
    expect(result).toBe("FAIL");
  });
  ...
  it("PASS on url suffix match (/dashboard matches /dashboard/x)", () => {
    const result = evaluateUiVerdict(
      { url: "http://localhost:5173/dashboard/overview", title: "", text: "" },
      "跳转到 /dashboard",
    );
    expect(result).toBe("PASS");
  });
  ...
  it("PASS when title contains keyword", () => {
    const result = evaluateUiVerdict(
      { url: "http://localhost:5173/", title: "首页 Home", text: "" },
      "显示 首页",
    );
    expect(result).toBe("PASS");
  });
