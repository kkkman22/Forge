...
describe("classifyStaleness", () => {
  ...
  it("returns warning when git mtime is far newer than frontmatter updated (drift)", () => {
    ...
    expect(
      classifyStaleness(
        fm,
        new Date("2026-05-24"),
        { warning_days: 90, critical_days: 180, exempt_paths: [], drift_days: 7 },
        undefined,
        gitMtime,
      ),
    ).toBe("warning");
  ...
  it("ignores drift when no gitMtime provided (backward compatible)", () => {
    ...
    expect(
      classifyStaleness(fm, new Date("2026-05-24"), {
        warning_days: 90,
        critical_days: 180,
        exempt_paths: [],
        drift_days: 7,
  ...
  it("uses default drift_days (7) when config omits the field", () => {
    ...
    expect(
      classifyStaleness(
        fm,
        new Date("2026-05-24"),
        { warning_days: 90, critical_days: 180, exempt_paths: [] },
        undefined,
        gitMtime,
      ),
    ).toBe("warning");
