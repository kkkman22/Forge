...
const makePair = (slug: string, title: string, category: Category = "reference"): DocPair => ({
  slug,
  directory: "docs" as any,
  cn: {
...
describe("buildIndex", () => {
  ...
  it("PBT: gen(gen(input)) === gen(input) (idempotency)", () => {
    ...
    fc.assert(
      fc.property(fc.array(pairArb, { minLength: 1, maxLength: 20 }), (items) => {
