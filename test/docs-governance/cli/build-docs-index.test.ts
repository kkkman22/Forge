...
function createFile(dir: string, relPath: string, content: string): void {
...
describe("build-docs-index core logic", () => {
  const makeDoc = (path: string, fm: Frontmatter): Doc => ({
    path: p(path),
    domain: "A" as Domain,
    frontmatter: fm,
    bodyHash: "hash",
