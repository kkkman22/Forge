...
import type { RendererFn, RenderInput, RenderResult } from "../../../src/docs-governance/types.js";

const noopRenderer: RendererFn = (_input: RenderInput): RenderResult => ({
  markdown: "",
  diagnostics: [],
});
...
describe("createRendererRegistry", () => {
  ...
  it("lists all registered renderer names", () => {
    const reg = createRendererRegistry();
    const fn2: RendererFn = () => ({ markdown: "b", diagnostics: [] });
  ...
  it("overwrites renderer when registering same name twice", () => {
    const reg = createRendererRegistry();
    const fn1: RendererFn = () => ({ markdown: "first", diagnostics: [] });
    const fn2: RendererFn = () => ({ markdown: "second", diagnostics: [] });
  ...
  it("resolves distinct functions for distinct names", () => {
    const reg = createRendererRegistry();
    const fnA: RendererFn = () => ({ markdown: "a", diagnostics: [] });
    const fnB: RendererFn = () => ({ markdown: "b", diagnostics: [] });
    reg.register("a", fnA);
    reg.register("b", fnB);
