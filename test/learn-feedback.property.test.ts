...
const commandArb: fc.Arbitrary<string> = fc.constantFrom(
  "build",
  "review",
  "plan",
  "test",
...
const feedbackEntryArb: fc.Arbitrary<SkillFeedbackEntry> = fc
  .tuple(
    commandArb,
    fc.boolean(),
...
const feedbackListArb: fc.Arbitrary<SkillFeedbackEntry[]> = fc.array(feedbackEntryArb, {
  minLength: 1,
  maxLength: 50,
});
...
describe("Property 24: Skill 反馈分析", () => {
  ...
  it("totalEntries equals input length", () => {
    fc.assert(
      fc.property(feedbackListArb, (entries) => {
  ...
  it("all-success entries produce no alerts", () => {
    fc.assert(
      fc.property(fc.array(commandArb, { minLength: 2, maxLength: 10 }), (commands) => {
        const entries: SkillFeedbackEntry[] = commands.map((cmd) => ({
          command: cmd,
          success: true,
          durationSeconds: 10,
          failureReason: "",
  ...
  it("all-failure entries for a command with >=2 runs produce an alert", () => {
    const entries: SkillFeedbackEntry[] = [
      ...
      fc.property(feedbackListArb, (entries) => {
        ...
        for (const stat of result.commandStats) {
          for (let i = 1; i < stat.topFailureReasons.length; i++) {
            expect(stat.topFailureReasons[i - 1].count).toBeGreaterThanOrEqual(
              stat.topFailureReasons[i].count,
            );
          }
        }
      }),
    ...
    fc.assert(
      fc.property(feedbackListArb, (entries) => {
        ...
        for (let i = 1; i < result.commandStats.length; i++) {
          expect(result.commandStats[i - 1].successRate).toBeLessThanOrEqual(
            result.commandStats[i].successRate,
          );
        }
      }),
      { numRuns: 100 },
...
describe("Property 24: 失败模式交叉验证", () => {
  ...
  it("substring matching works (feedback reason contained in known failure)", () => {
    const result = crossValidateFailures(
      ["type error"],
      ["TypeScript type error in monorepo setup"],
    );
  ...
  it("non-overlapping inputs produce empty result", () => {
    fc.assert(
      fc.property(
        fc.array(fc.constant("aaa-unique-feedback"), { minLength: 1, maxLength: 5 }),
        fc.array(fc.constant("zzz-unique-known"), { minLength: 1, maxLength: 5 }),
        (feedbackReasons, knownFailures) => {
