...
import type { DecisionCandidate, DecisionSignals } from "../src/adr-criteria.js";
import { type CriteriaScreenItem, runCriteriaScreen } from "../src/decide.js";
 ...
 */
const adrWorthyDecision: DecisionCandidate = {
  title: "Adopt SQLite for episode store",
  context:
    "The episode store must survive process restarts and support ad-hoc queries. Keeping the current JSON-lines file forfeits indexed reads; switching to a relational engine introduces a hard dependency we cannot undo without a migration.",
...
const adrWorthySignals: DecisionSignals = {
 ...
 */
const discardDecision: DecisionCandidate = {
...
const discardSignals: DecisionSignals = {
 ...
 */
const inlineDecision: DecisionCandidate = {
  title: "Enable `strict` in tsconfig for new packages",
  context:
    "New packages default to strict TypeScript. Flipping the flag off later requires rewriting any code that leaned on implicit any, which is a non-trivial reversal.",
  decision: "Set `strict: true` in tsconfig for all new packages.",
  consequences: "New code must pass strict null checks from day one.",
  alternatives: [],
};

const inlineSignals: DecisionSignals = {
...
describe("runCriteriaScreen — batching", () => {
  it("preserves input order across a mixed batch of verdicts", () => {
    ...
    expect(items.map((i: CriteriaScreenItem) => i.result.verdict)).toEqual([
      "DISCARD",
      "WRITE_ADR",
      "INLINE_NOTE",
      "WRITE_ADR",
  ...
  it("throws RangeError when parallel arrays have different lengths", () => {
    expect(() =>
      runCriteriaScreen([adrWorthyDecision, inlineDecision], [adrWorthySignals]),
    ).toThrow(RangeError);
    expect(() => runCriteriaScreen([adrWorthyDecision], [adrWorthySignals, inlineSignals])).toThrow(
      /same length/,
    );
  });
});
