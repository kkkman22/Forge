 ...
 */
export function assertGitLog(cwd: string, patterns: RegExp[]): void {
  ...
  for (const pattern of patterns) {
    ...
    if (!found) {
      throw new Error(
        `Git log assertion failed: no message matching /${pattern.source}/.\n` +
 ...
 */
export function assertFileExists(path: string, contentPattern?: RegExp): void {
  if (!existsSync(path)) {
  ...
  if (contentPattern) {
    ...
    if (!contentPattern.test(content)) {
      throw new Error(
        `File content assertion failed: /${contentPattern.source}/ not found in ${path}.\n` +
