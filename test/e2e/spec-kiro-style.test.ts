...
function makeFm(overrides: Partial<SpecFileFrontmatter> = {}): SpecFileFrontmatter {
  return {
    feature: "e2e-test",
    status: "locked",
    date: "2026-05-23",
...
function makeEars(when: string, shall: string, verifyBy?: string, evidence?: string): EarsClause {
  return {
    line: 1,
    when,
    shall,
    raw: `当 ${when} 时 系统应当 ${shall}`,
    ...(verifyBy ? { verifyBy: verifyBy as any } : {}),
    ...(evidence ? { evidence } : {}),
  };
}
...
`;
  ...
  it("analyze passes for valid EARS requirements with user stories", () => {
    const req: RequirementsDocument = {
      frontmatter: makeFm(),
      intro: "Auth system",
      glossary: [],
      userStories: [
