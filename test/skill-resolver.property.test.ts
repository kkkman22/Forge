...
describe("Feature: i18n-support, Property 8: SKILL 文件解析与回退", () => {
  ...
  it("returns only default path when locale equals defaultLocale", () => {
    fc.assert(
      fc.property(skillNameArb, localeArb, (skillName, locale) => {
  ...
  it("resolves to first existing candidate (locale-specific preferred)", () => {
    fc.assert(
      fc.property(
        ...
        fc.boolean(),
        fc.boolean(),
        (skillName, [locale, defaultLocale], localeExists, defaultExists) => {
          ...
          if (localeExists) existingPaths.add(candidates[0]);
          if (defaultExists) existingPaths.add(candidates[1]);
  ...
  it("all candidates contain the skill name in the path", () => {
    fc.assert(
      fc.property(skillNameArb, localeArb, localeArb, (skillName, locale, defaultLocale) => {
        const candidates = buildSkillCandidates(skillName, locale, defaultLocale);
