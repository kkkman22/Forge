...
const VALID_SKILL_PHASES: SkillPhase[] = [
  "router",
  "plan",
  "build",
  "review",
...
const skillPhaseArb: fc.Arbitrary<SkillPhase> = fc.constantFrom(...VALID_SKILL_PHASES);

/** Arbitrary tier value. */
const tierArb: fc.Arbitrary<string> = fc.constantFrom("light", "standard", "full");

/** Arbitrary plan status. */
const planStatusArb: fc.Arbitrary<string> = fc.constantFrom("approved", "draft", "pending");

/** Arbitrary review result. */
const reviewResultArb: fc.Arbitrary<string> = fc.constantFrom("pass", "fail");
 ...
 */
const schedulerInputArb: fc.Arbitrary<SchedulerInput> = fc.record({
 ...
 */
const buildIncompleteArb: fc.Arbitrary<SchedulerInput> = fc
  .record({
    currentPhase: fc.constant("build" as string),
    tier: fc.option(tierArb, { nil: undefined }),
    planStatus: fc.option(planStatusArb, { nil: undefined }),
  ...
  .filter(
    (input) =>
      !(input.reviewResult === "fail" && input.reviewFixAttempts >= input.maxReviewFixAttempts),
  );
 ...
 * reviewResult="fail", reviewFixAttempts < maxReviewFixAttempts.
 */
const buildCompleteArb: fc.Arbitrary<SchedulerInput> = fc
  .record({
    currentPhase: fc.constant("build" as string),
    tier: fc.option(tierArb, { nil: undefined }),
    planStatus: fc.option(planStatusArb, { nil: undefined }),
 ...
 */
const reviewFailNonBreakArb: fc.Arbitrary<SchedulerInput> = fc
  .record({
    currentPhase: fc.constant("review" as string),
    tier: fc.option(tierArb, { nil: undefined }),
    planStatus: fc.option(planStatusArb, { nil: undefined }),
    hasIncompleteTasks: fc.option(fc.boolean(), { nil: undefined }),
  ...
  .chain((base) =>
    fc.record({
      ...(Object.fromEntries(Object.entries(base).map(([k, v]) => [k, fc.constant(v)])) as {
        [K in keyof typeof base]: fc.Arbitrary<(typeof base)[K]>;
      }),
...
const reviewPassArb: fc.Arbitrary<SchedulerInput> = fc.record({
  currentPhase: fc.constant("review" as string),
  tier: fc.option(tierArb, { nil: undefined }),
  planStatus: fc.option(planStatusArb, { nil: undefined }),
  hasIncompleteTasks: fc.option(fc.boolean(), { nil: undefined }),
 ...
 */
const circuitBreakerArb: fc.Arbitrary<SchedulerInput> = fc
  .record({
    currentPhase: fc.option(skillPhaseArb, { nil: undefined }),
    tier: fc.option(tierArb, { nil: undefined }),
    planStatus: fc.option(planStatusArb, { nil: undefined }),
...
const reviewSequenceArb: fc.Arbitrary<Array<"pass" | "fail">> = fc.array(
...
describe("Feature: loop-skills-fusion, Property 5: determineNextSkill 状态转换正确性", () => {
  ...
  it("always returns a valid SkillPhase for any SchedulerInput", () => {
    fc.assert(
      fc.property(schedulerInputArb, (input) => {
        const result: SchedulerResult = determineNextSkill(input);
  ...
  it("build phase with incomplete tasks stays in build", () => {
    fc.assert(
      fc.property(buildIncompleteArb, (input) => {
  ...
  it("build phase with all tasks complete transitions to review", () => {
    fc.assert(
      fc.property(buildCompleteArb, (input) => {
        const result = determineNextSkill(input);
  ...
  it("review phase with fail result and attempts below max transitions to build", () => {
    fc.assert(
      fc.property(reviewFailNonBreakArb, (input) => {
        const result = determineNextSkill(input);
        expect(result.nextPhase).toBe("build");
      }),
      { numRuns: 200 },
  ...
  it("review phase with pass result transitions to test", () => {
    fc.assert(
      fc.property(reviewPassArb, (input) => {
        const result = determineNextSkill(input);
...
describe("Feature: loop-skills-fusion, Property 6: SkillScheduler circuit breaker", () => {
  ...
  it("triggers circuit breaker (aborted) when fix attempts >= max and review fails", () => {
    fc.assert(
      fc.property(circuitBreakerArb, (input) => {
        const result = determineNextSkill(input);
  ...
  it("circuit breaker overrides any currentPhase when attempts >= max and review fails", () => {
    ...
    const allPhasesCircuitBreakerArb: fc.Arbitrary<SchedulerInput> = fc
      .record({
        currentPhase: skillPhaseArb as fc.Arbitrary<string | undefined>,
        tier: fc.option(tierArb, { nil: undefined }),
        planStatus: fc.option(planStatusArb, { nil: undefined }),
        hasIncompleteTasks: fc.option(fc.boolean(), { nil: undefined }),
        reviewResult: fc.constant("fail"),
    ...
    fc.assert(
      fc.property(allPhasesCircuitBreakerArb, (input) => {
        const result = determineNextSkill(input);
        expect(result.nextPhase).toBe("aborted");
      }),
      { numRuns: 200 },
...
describe("Feature: loop-skills-fusion, Property 14: 修复循环计数器状态机", () => {
  ...
  it("counter equals consecutive fails since last pass for any pass/fail sequence", () => {
    fc.assert(
      fc.property(reviewSequenceArb, (sequence) => {
  ...
  it("counter state machine drives correct circuit breaker behavior in scheduler", () => {
    fc.assert(
      fc.property(reviewSequenceArb, fc.integer({ min: 1, max: 5 }), (sequence, maxAttempts) => {
        // Simulate the counter state machine
        let counter = 0;
  ...
  it("counter resets to 0 after any pass", () => {
    fc.assert(
      fc.property(
        fc.array(fc.constantFrom("pass" as const, "fail" as const), {
          minLength: 1,
          maxLength: 20,
        }),
        (sequence) => {
...
describe("Property 3: New command sequences contain only valid SkillPhase values", () => {
  ...
  it("all phases in new sequences are valid SkillPhase values", () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom(...newSequenceNames), (seqName) => {
  ...
  it("existing sequences are unchanged (backward compatibility)", () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom("light", "standard", "full"), (tier) => {
...
describe("Property 4: shouldCommitForPhase returns correct values for new phases", () => {
  ...
  it("refactor-apply and fix-apply return true when successful", () => {
    fc.assert(
      fc.property(fc.constantFrom("refactor-apply", "fix-apply"), (phase) => {
  ...
  it("refactor-scan and fix-analyze return false even when successful", () => {
    fc.assert(
      fc.property(fc.constantFrom("refactor-scan", "fix-analyze"), (phase) => {
  ...
  it("no new phase commits when success is false", () => {
    fc.assert(
      fc.property(
        fc.constantFrom("refactor-scan", "refactor-apply", "fix-analyze", "fix-apply"),
        (phase) => {
  ...
  it("existing commitable phases still return true (backward compatibility)", () => {
    fc.assert(
      fc.property(fc.constantFrom("build", "plan", "fix"), (phase) => {
        expect(shouldCommitForPhase(phase, true)).toBe(true);
      }),
      { numRuns: 100 },
    );
  ...
  it("refactor-scan transitions to refactor-apply", () => {
    fc.assert(
      fc.property(
        fc
          .record({
            currentPhase: fc.constant("refactor-scan" as string),
            tier: fc.option(tierArb, { nil: undefined }),
            planStatus: fc.option(planStatusArb, { nil: undefined }),
            hasIncompleteTasks: fc.option(fc.boolean(), { nil: undefined }),
            reviewResult: fc.option(reviewResultArb, { nil: undefined }),
          ...
          .filter(
            (input) =>
              !(
                input.reviewResult === "fail" &&
                input.reviewFixAttempts >= input.maxReviewFixAttempts
              ),
          ),
        (input) => {
          const result = determineNextSkill(input);
  ...
  it("refactor-apply with incomplete tasks stays in refactor-apply", () => {
    fc.assert(
      fc.property(
        fc
          .record({
            currentPhase: fc.constant("refactor-apply" as string),
            tier: fc.option(tierArb, { nil: undefined }),
            planStatus: fc.option(planStatusArb, { nil: undefined }),
            hasIncompleteTasks: fc.constant(true),
            reviewResult: fc.option(reviewResultArb, { nil: undefined }),
  ...
  it("fix-analyze transitions to fix-apply", () => {
    fc.assert(
      fc.property(
        fc
          .record({
            currentPhase: fc.constant("fix-analyze" as string),
            tier: fc.option(tierArb, { nil: undefined }),
            planStatus: fc.option(planStatusArb, { nil: undefined }),
            hasIncompleteTasks: fc.option(fc.boolean(), { nil: undefined }),
            reviewResult: fc.option(reviewResultArb, { nil: undefined }),
  ...
  it("fix-apply with incomplete tasks stays in fix-apply", () => {
    fc.assert(
      fc.property(
        fc
          .record({
            currentPhase: fc.constant("fix-apply" as string),
            tier: fc.option(tierArb, { nil: undefined }),
            planStatus: fc.option(planStatusArb, { nil: undefined }),
            hasIncompleteTasks: fc.constant(true),
            reviewResult: fc.option(reviewResultArb, { nil: undefined }),
...
describe("Feature: loop-skills-fusion, Property 5: SkillScheduler total function property", () => {
  ...
  const unknownPhaseArb: fc.Arbitrary<string> = fc
    ...
    .filter((s) => !(VALID_SKILL_PHASES as string[]).includes(s));

  /** Arbitrary SchedulerInput with an unknown currentPhase value. */
  const unknownPhaseInputArb: fc.Arbitrary<SchedulerInput> = fc.record({
  ...
  const terminalPhaseArb: fc.Arbitrary<SkillPhase> = fc.constantFrom(
    "completed" as SkillPhase,
    "aborted" as SkillPhase,
  );

  /** Arbitrary SchedulerInput with a terminal currentPhase and no circuit breaker trigger. */
  const terminalInputArb: fc.Arbitrary<SchedulerInput> = fc.record({
    currentPhase: terminalPhaseArb,
    tier: fc.option(tierArb, { nil: undefined }),
    planStatus: fc.option(planStatusArb, { nil: undefined }),
    hasIncompleteTasks: fc.option(fc.boolean(), { nil: undefined }),
    reviewResult: fc.option(
      fc.constantFrom("pass", "fail").filter((r) => r !== "fail"),
      { nil: undefined },
    ),
    testPassed: fc.option(fc.boolean(), { nil: undefined }),
   ...
   */
  const anySchedulerInputArb: fc.Arbitrary<SchedulerInput> = fc.record({
    currentPhase: fc.option(fc.oneof(skillPhaseArb as fc.Arbitrary<string>, unknownPhaseArb), {
      nil: undefined,
    }),
  ...
  it("never throws for any SchedulerInput (including unknown currentPhase values)", () => {
    fc.assert(
      fc.property(anySchedulerInputArb, (input) => {
  ...
  it("unknown currentPhase values fall back to router (when circuit breaker is not triggered)", () => {
    ...
    const unknownNonBreakerArb = unknownPhaseInputArb.filter(
      (input) =>
        !(input.reviewResult === "fail" && input.reviewFixAttempts >= input.maxReviewFixAttempts),
    );

    fc.assert(
      fc.property(unknownNonBreakerArb, (input) => {
        const result = determineNextSkill(input);
  ...
  it("terminal states (completed, aborted) return themselves (idempotent)", () => {
    fc.assert(
      fc.property(terminalInputArb, (input) => {
        const result = determineNextSkill(input);
...
describe("Feature: loop-skills-fusion, Property 7: SkillScheduler convergence", () => {
   ...
   */
  const NON_TERMINAL_PHASES: SkillPhase[] = [
    "router",
    "plan",
    "build",
    "review",
  ...
  const nonTerminalPhaseArb: fc.Arbitrary<SkillPhase> = fc.constantFrom(...NON_TERMINAL_PHASES);

  /** Arbitrary tier value (includes "full" which adds the learn phase). */
  const convergenceTierArb: fc.Arbitrary<string> = fc.constantFrom("light", "standard", "full");
  ...
  function simulateConvergence(
    startPhase: SkillPhase,
    tier: string,
    maxSteps: number,
  ): { converged: boolean; finalPhase: SkillPhase; steps: number } {
    let currentPhase: SkillPhase = startPhase;
    let steps = 0;
  ...
  it("converges to completed or aborted within 20 steps from any non-terminal phase", () => {
    fc.assert(
      fc.property(nonTerminalPhaseArb, convergenceTierArb, (startPhase, tier) => {
...
describe("Feature: loop-skills-fusion, Property 8: shouldCommitForPhase commit strategy correctness", () => {
  ...
  const commitablePhasesArb: fc.Arbitrary<string> = fc.constantFrom(
    "build",
    "plan",
    "fix",
    "refactor-apply",
  ...
  const nonCommitablePhasesArb: fc.Arbitrary<string> = fc.constantFrom(
    "review",
    "test",
    "ship",
    "router",
  ...
  it("commitable phases with success=true return true", () => {
    fc.assert(
      fc.property(commitablePhasesArb, (phase) => {
        expect(shouldCommitForPhase(phase, true)).toBe(true);
      }),
      { numRuns: 200 },
    );
  ...
  it("non-commitable phases return false regardless of success", () => {
    fc.assert(
      fc.property(nonCommitablePhasesArb, fc.boolean(), (phase, success) => {
  ...
  it("any phase with success=false returns false", () => {
    fc.assert(
      fc.property(fc.oneof(commitablePhasesArb, nonCommitablePhasesArb), (phase) => {
        expect(shouldCommitForPhase(phase, false)).toBe(false);
      }),
      { numRuns: 200 },
    );
  ...
  it("unknown phase strings return false regardless of success", () => {
    fc.assert(
      fc.property(unknownPhaseArb, fc.boolean(), (phase, success) => {
        expect(shouldCommitForPhase(phase, success)).toBe(false);
      }),
      { numRuns: 200 },
    );
...
describe("Feature: loop-skills-fusion, Property 9: getCommandSequence safe default", () => {
   ...
   */
  const unknownTierArb: fc.Arbitrary<string> = fc
  ...
  it("known tiers return their specific (non-default) or matching sequences", () => {
    fc.assert(
      fc.property(fc.constantFrom(...KNOWN_TIERS), (tier) => {
        const sequence = getCommandSequence(tier);
