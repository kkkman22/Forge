...
import {
  ...
  renderTasksMarkdown,
} from "../src/spec-bundle-io.js";
import {
  renderDesignMarkdown as ssotRenderDesign,
  renderRequirementsMarkdown as ssotRenderRequirements,
  renderTasksMarkdown as ssotRenderTasks,
...
describe("spec-bundle-io delegates to spec-render (SSOT)", () => {
  it("renderRequirementsMarkdown output is identical to spec-render", () => {
    ...
    expect(renderRequirementsMarkdown(req as never)).toEqual(ssotRenderRequirements(req as never));
  });
  ...
  it("renderDesignMarkdown output is identical to spec-render (fixes open-questions numbering)", () => {
    ...
    const out = renderDesignMarkdown(design as never);
    ...
    expect(out).toEqual(ssotRenderDesign(design as never));
  });
  ...
  it("renderTasksMarkdown output is identical to spec-render", () => {
    ...
    expect(renderTasksMarkdown(tasks as never)).toEqual(ssotRenderTasks(tasks as never));
  });
});
