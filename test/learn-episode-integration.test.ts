...
import { type Episode, renderEpisode } from "../src/episode.js";
import {
...
import type { Pattern } from "../src/pattern-stats.js";
...
function phaseHistory(overrides: Partial<SessionPhaseHistory> = {}): SessionPhaseHistory {
  return {
    phases: [
...
function pattern(overrides: Partial<Pattern> & { name: string }): Pattern {
  return {
...
function episode(overrides: Partial<Episode> & { id: string; date: string }): Episode {
  return {
    schema_version: 2,
    skill: "forge-build",
    tier: "standard",
...
describe("buildEpisodeFromSession — Requirement 7.9", () => {
  ...
  it("infers outcome=partial when ship was blocked", () => {
    const ep = buildEpisodeFromSession(
      META,
      phaseHistory({ shipResult: "blocked", reviewResult: "pass", testResult: "pass" }),
      "s",
      "l",
      2,
    );
  ...
  it("infers outcome=partial when tests fail (even without a ship signal)", () => {
    const ep = buildEpisodeFromSession(
      META,
      phaseHistory({ shipResult: undefined, testResult: "fail" }),
      "s",
      "l",
      3,
    );
...
describe("archivePatternByName — Requirements 7.10, 7.14", () => {
  ...
  it("does not lose entries: active + archived covers every input pattern", () => {
    ...
    const byName = (p: Pattern) => p.name;
    expect([...result.active, ...result.archived].map(byName).sort()).toEqual(
      patterns.map(byName).sort(),
    );
  });
...
describe("buildPatternUpgradeDrafts — Requirement 7.11", () => {
  ...
  it("assigns deterministic pattern ids based on the suggestion order", () => {
    const mk = (skill: string, rootCause: string, id: string, date: string): Episode =>
      episode({ id, date, skill, root_cause: rootCause, lesson: "l" });
