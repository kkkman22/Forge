...
describe("failure-sink — buildFailureEpisode", () => {
  ...
  it("only sets root_cause and body line when rootCause is supplied", () => {
    const withCause = buildFailureEpisode(
      makeContext({ rootCause: "任务拆得太粗导致探针失灵" }),
      FIXED_NOW,
      3,
    );
  ...
  it("produces different lesson text per trigger", () => {
    ...
    const newPattern = buildFailureEpisode(
      makeContext({ trigger: "new_review_pattern", skill: "forge-review" }),
      FIXED_NOW,
      2,
    );
    const shipGate = buildFailureEpisode(
      makeContext({ trigger: "ship_gate_blocked", skill: "forge-ship" }),
      FIXED_NOW,
      3,
    );
