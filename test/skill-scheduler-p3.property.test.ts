...
const BASELINE_SEQUENCES: Record<string, string[]> = {
...
function makeSkillPromptParams(phase: string): SkillPromptParams {
  return {
    base: {
      iteration: 1,
      runId: "test-run",
...
describe("Property 1: Light tier command sequence updated, all others unchanged", () => {
  ...
  it("other known tiers return unchanged baseline sequences", () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom(...otherTiers), (tier) => {
  ...
  it("unknown tiers default to standard sequence", () => {
    fc.assert(
      fc.property(
        fc.string().filter((s) => !KNOWN_TIERS.includes(s as (typeof KNOWN_TIERS)[number])),
        (unknownTier) => {
