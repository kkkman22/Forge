...
import type { TaskGraph } from "../src/task-graph.js";

function graph(ids: string[], dependsOn: Record<string, string[]> = {}): TaskGraph {
  return {
    tasks: ids.map((id) => ({
      id,
      title: id,
...
describe("generateExecutionPackages", () => {
  it("generates bounded packages for ten or more atomic tasks", () => {
    const result = generateExecutionPackages(
      graph(Array.from({ length: 10 }, (_, i) => `T-${i + 1}`)),
    );
  ...
  it("generates packages for six to nine tasks", () => {
    const result = generateExecutionPackages(
      graph(Array.from({ length: 6 }, (_, i) => `T-${i + 1}`)),
    );
  ...
  it("keeps five small tasks in one package", () => {
    const result = generateExecutionPackages(
      graph(Array.from({ length: 5 }, (_, i) => `T-${i + 1}`)),
    );
