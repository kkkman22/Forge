...
const MODULES = ["grill", "error-recovery", "review", "decide"] as const;
...
describe("long-term evolution: domain kernel / adapter boundaries", () => {
  it("legacy top-level modules stay thin compatibility adapters", () => {
    for (const mod of MODULES) {
      ...
      expect(content, `${mod}.ts should re-export from submodule index`).toContain(
        `./${mod}/index.js`,
      );
      expect(content, `${mod}.ts should not declare local business functions`).not.toMatch(
        /export\s+function\s+(?!.*from)/,
      );
    }
  });
  ...
  it("each extracted module has explicit kernel files and an index adapter", () => {
    for (const mod of MODULES) {
      ...
      expect(
        files.filter((f) => f !== "index.ts").length,
        `${mod} needs kernel modules`,
