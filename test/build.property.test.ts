...
import {
  ...
  type FixAttemptSequence,
  type PlanStatus,
  type SpecStatus,
  shouldEscalateToDebug,
} from "../src/build.js";
...
const fixAttemptResultArb: fc.Arbitrary<FixAttemptResult> = fc.constantFrom("success", "failure");

/** A sequence of fix attempts (arbitrary length). */
const fixAttemptSequenceArb: fc.Arbitrary<FixAttemptSequence> = fc
 ...
 */
const sequenceWithEscalationArb: fc.Arbitrary<FixAttemptSequence> = fc
  .tuple(
    ...
    fc.constant(["failure", "failure", "failure"] as FixAttemptResult[]),
  ...
  .map(([prefix, block, suffix]) => {
    ...
    if (safePrefix.length > 0) {
      ...
      for (let i = safePrefix.length - 1; i >= 0; i--) {
        if (safePrefix[i] === "failure") trailingFailures++;
        else break;
      }
 ...
 */
const sequenceWithoutEscalationArb: fc.Arbitrary<FixAttemptSequence> = fc
  .array(
    fc.tuple(
      // 0, 1, or 2 consecutive failures
      fc.array(fc.constant("failure" as FixAttemptResult), { minLength: 0, maxLength: 2 }),
      // Followed by a success
      fc.constant("success" as FixAttemptResult),
    ),
 ...
 */
const pureFailureSequenceArb = (n: number): fc.Arbitrary<FixAttemptSequence> =>
  fc.constant({
    attempts: Array.from({ length: n }, () => "failure" as FixAttemptResult),
  });
...
describe("Property 10: 连续失败升级", () => {
  ...
  it("success resets the consecutive failure counter (Req 10.6)", () => {
    fc.assert(
      fc.property(
        fc.array(
          fc.tuple(
            fc.array(fc.constant("failure" as FixAttemptResult), { minLength: 1, maxLength: 2 }),
            fc.constant("success" as FixAttemptResult),
          ),
        ...
        (chunks) => {
          ...
          const sequence: FixAttemptSequence = { attempts };
  ...
  it("exactly 3 consecutive failures triggers escalation", () => {
    fc.assert(
      fc.property(pureFailureSequenceArb(3), (sequence) => {
        const result = analyzeFixAttempts(sequence);
  ...
  it("exactly 2 consecutive failures does NOT trigger escalation", () => {
    fc.assert(
      fc.property(pureFailureSequenceArb(2), (sequence) => {
        const result = analyzeFixAttempts(sequence);
  ...
  it("all successes → no escalation", () => {
    fc.assert(
      fc.property(
        fc.array(fc.constant("success" as FixAttemptResult), { minLength: 1, maxLength: 20 }),
        (attempts) => {
          const result = analyzeFixAttempts({ attempts });
  ...
  it("shouldEscalateToDebug convenience function matches analyzeFixAttempts", () => {
    fc.assert(
      fc.property(fixAttemptSequenceArb, (sequence) => {
