...
const uncommittedResult = (
  overrides?: Partial<UncommittedChangeResult>,
): fc.Arbitrary<UncommittedChangeResult> =>
  fc
    .record({
...
const gitScanResult = (): fc.Arbitrary<GitScanResult> =>
  fc.record({
    commits: fc.constant([]),
    matches: fc.constant([]),
...
const phaseInconsistency = (): fc.Arbitrary<PhaseInconsistency | null> =>
  fc.oneof(
    fc.constant(null),
    fc.record({
      ...
      >,
      direction: fc.constantFrom("behind", "ahead") as fc.Arbitrary<
        PhaseInconsistency["direction"]
      >,
...
const VALID_CATEGORIES: InterruptionCategory[] = [
  ...
  it("returns exactly one category whose evidence conditions are satisfied", () => {
    fc.assert(
      fc.property(
        ...
        phaseInconsistency(),
        verificationPassed(),
        (uncommitted, gitScan, progInc, phaseInc, verif) => {
...
describe("Feature: error-recovery-strategy, Property 11: classification priority", () => {
  it("returns highest-priority category when multiple conditions are true", () => {
    ...
    const result = classifyInterruption(
      uncommitted,
      { commits: [], matches: [], noNewCommits: true },
      progInc,
      null,
      null,
    );
  ...
  it("returns (b) when (a) is false but (b) is true", () => {
    ...
    const result = classifyInterruption(
      uncommitted,
      { commits: [], matches: [], noNewCommits: false },
      progInc,
      null,
      null,
    );
    ...
    fc.assert(
      fc.property(
        ...
        ({ testFile, implFile, hasImpl, verificationPassed }) => {
          const changes: FileChange[] = [{ filePath: testFile, status: "added" }];
          if (hasImpl) {
...
describe("Feature: error-recovery-strategy, Property 13: test file identification", () => {
  it("returns true iff path matches a test file pattern", () => {
    fc.assert(
      fc.property(fc.string({ minLength: 1, maxLength: 60 }), (path) => {
