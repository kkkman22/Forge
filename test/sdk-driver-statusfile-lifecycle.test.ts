/**
 * Unit tests for StatusFile lifecycle management in SdkDriver (skill-aware mode).
 *
 * Verifies that the SdkDriver correctly manages Loop-related fields in the
 * StatusFile across the full lifecycle:
 * - Startup writes all Loop fields (mode, loop_run_id, loop_iteration, skill_sequence)
 * - Normal completion clears all Loop fields
 * - Abnormal exit preserves phase field (and skill_sequence for resume)
 * - Residual state from previous run is detected and cleaned before writing new fields
 *
...
describe("iteration updates loop_iteration and phase", () => {
  it("updates loop_iteration and phase after each iteration", async () => {
    ...
    const iterationUpdates = writtenContents.filter(
      (content) => content.includes("loop_iteration:") && content.includes("phase:"),
    );
