...
function shouldRegister(skill: Skill, enabledFlags: Set<string>): boolean {
  if (!skill.requiredFlag) return true;
...
describe("Property 4: Pack Conditional Registration", () => {
  it("skills without requiredFlag always register", () => {
    fc.assert(
      fc.property(
        fc.record({ name: fc.string({ minLength: 1 }), requiredFlag: fc.constant(null) }),
        fc.uniqueArray(flagArb, { minLength: 0, maxLength: 5 }),
        (skill, flags) => {
  ...
  it("skills with requiredFlag register iff flag is present", () => {
    fc.assert(
      fc.property(
        fc.record({ name: fc.string({ minLength: 1 }), requiredFlag: flagArb }),
        fc.uniqueArray(flagArb, { minLength: 0, maxLength: 5 }),
        (skill, flags) => {
  ...
  it("registration is monotonic: adding flags never unregisters", () => {
    fc.assert(
      fc.property(
        fc.record({ name: fc.string({ minLength: 1 }), requiredFlag: flagArb }),
        fc.uniqueArray(flagArb, { minLength: 0, maxLength: 3 }),
        flagArb,
        (skill, baseFlags, extraFlag) => {
          ...
          if (baseResult) expect(extResult).toBe(true);
        },
      ),
      { numRuns: 200 },
    );
  ...
  it("forge-mutate: only registers with mutation_critical_modules", () => {
    ...
    expect(
      shouldRegister(
        mutate,
        new Set(["core_subdomains", "mutation_critical_modules", "forced_acceptance_contexts"]),
      ),
    ).toBe(true);
  });
});
