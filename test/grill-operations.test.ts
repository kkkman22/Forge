...
const GLOSSARY_WITH_REFS: Glossary = {
  schema_version: 1,
  updated: "2026-05-05",
  terms: [
 ...
 */
function collectPendingIds(tree: DecisionTree): Set<string> {
  const out = new Set<string>();
  const visit = (node: DecisionTreeNode): void => {
    if (node.status === "pending") out.add(node.id);
...
describe("selectNextQuestion", () => {
  ...
  it("does not descend into children of an unresolved parent", () => {
    const tree = generateDecisionTree(
      "Lock the spec for the 档位 router.",
      GLOSSARY_WITH_REFS,
      FIXED_NOW,
    );
  ...
  it("descends into pending children once the parent is resolved", () => {
    ...
    const resolvedRoots = ["functionality-1", "boundary-1", "dependency-1"].reduce(
      (acc, id) => applyAnswer(acc, id, "answered", FIXED_NOW),
      tree,
    );
...
describe("isComplete", () => {
  ...
  it("returns true once every node is resolved", () => {
    ...
    const resolved = collectAllIds(tree).reduce(
      (acc, id) => applyAnswer(acc, id, `answer for ${id}`, FIXED_NOW),
      tree,
    );
...
describe("Feature: skills-cross-pollination R4, applyAnswer introduces no new pending nodes", () => {
  it("pending set shrinks by exactly the resolved id (or stays identical on miss)", () => {
    fc.assert(
      fc.property(descriptionArb, answerArb, fc.string(), (description, answer, rawNodeId) => {
...
describe("Feature: skills-cross-pollination R4, applyAnswer replay is deterministic", () => {
  it("same (nodeId, answer) sequence on equal trees produces identical final trees", () => {
    fc.assert(
      fc.property(
        descriptionArb,
        fc.array(fc.tuple(fc.string(), answerArb), { maxLength: 15 }),
        (description, rawSequence) => {
          ...
          const sequence: Array<[string, string]> = rawSequence.map(([raw, answer], index) => {
            const useRealId = index % 2 === 0 && ids.length > 0;
            const nodeId = useRealId ? (ids[index % ids.length] as string) : raw;
            return [nodeId, answer];
          });

          const replay = (tree: DecisionTree): DecisionTree =>
            sequence.reduce(
              (acc, [nodeId, answer]) => applyAnswer(acc, nodeId, answer, LATER_NOW),
              tree,
            );
