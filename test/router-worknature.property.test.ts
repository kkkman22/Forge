...
const refactorDescArb: fc.Arbitrary<string> = fc
  .constantFrom(
    "优化",
    "重构",
    "重写",
  ...
  .chain((keyword) =>
    fc.string({ minLength: 0, maxLength: 20 }).map((prefix) => `${prefix} ${keyword} ${prefix}`),
  );

/** Generates a description string that contains at least one bugfix keyword. */
const bugfixDescArb: fc.Arbitrary<string> = fc
  .constantFrom(
    "bug",
    "报错",
    "异常",
...
const neutralDescArb: fc.Arbitrary<string> = fc
  ...
  .filter((s) => {
    ...
    return (
      !refactorKws.some((kw) => lower.includes(kw.toLowerCase())) &&
...
describe("Property 33: detectWorkNature keyword detection", () => {
  it("returns 'refactor' for descriptions with refactor keywords (no bugfix keywords)", () => {
    fc.assert(
      fc.property(refactorDescArb, (desc) => {
  ...
  it("returns 'bugfix' for descriptions with bugfix keywords (no refactor keywords)", () => {
    fc.assert(
      fc.property(bugfixDescArb, (desc) => {
  ...
  it("returns 'feature' when both refactor and bugfix keywords are present (ambiguous)", () => {
    fc.assert(
      fc.property(refactorDescArb, bugfixDescArb, (refactorDesc, bugfixDesc) => {
...
describe("Property 35: classifyTask work_nature field", () => {
  it("classifyTask returns work_nature field", () => {
    fc.assert(
      fc.property(taskSignalsArb, workNatureArb, (signals, nature) => {
  ...
  it("classifyTask commandSequence equals getRouterSequence(tier) (routerPhases are workNature-agnostic by design)", () => {
    fc.assert(
      fc.property(tierArb, workNatureArb, (tier, nature) => {
