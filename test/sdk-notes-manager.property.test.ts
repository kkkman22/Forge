...
const agentOutputArb: fc.Arbitrary<AgentOutput> = fc.record({
  success: fc.boolean(),
  ...
  gate_result: fc.option(
    fc.constantFrom("passed" as const, "blocked" as const, "skipped" as const),
    { nil: undefined },
  ),
});
...
describe("Feature: sdk-driver-decomposition, Property 5: buildIterationEntry field mapping", () => {
  ...
  it("maps all fields correctly for any iteration number, success flag, and AgentOutput", () => {
    fc.assert(
      fc.property(fc.nat({ max: 10_000 }), fc.boolean(), agentOutputArb, (n, s, o) => {
