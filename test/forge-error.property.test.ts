 ...
 */
const forgeErrorArb: fc.Arbitrary<ForgeError> = fc
  .tuple(fc.boolean(), messageArb, nonEmptyCodeArb)
  .map(([useFirst, message, code]) =>
    useFirst ? new TestForgeError(message, code) : new AnotherForgeError(message, code),
  );
...
describe("Property 1: ForgeError hierarchy — instanceof chain", () => {
  ...
  it("TestForgeError.name is 'TestForgeError' and AnotherForgeError.name is 'AnotherForgeError'", () => {
    fc.assert(
      fc.property(messageArb, nonEmptyCodeArb, (message, code) => {
