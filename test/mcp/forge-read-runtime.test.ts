...
describe("execReadScript runtime sandbox", () => {
  ...
  it("allows scripts to read only files listed in FORGE_FILES through readFile()", async () => {
    ...
    const result = await execReadScript(
      "console.log(readFile(FORGE_FILES[0]).trim())",
      "javascript",
      ["allowed.txt"],
      5000,
      { cwd: root },
  ...
  it("rejects attempts to read project files not listed in FORGE_FILES", async () => {
    ...
    const result = await execReadScript(
      "console.log(readFile('secret.txt'))",
      "javascript",
      ["allowed.txt"],
      5000,
      { cwd: root },
  ...
  it("blocks Function-constructor escape attempts inside the VM", async () => {
    ...
    const result = await execReadScript(
      "try { globalThis.constructor.constructor('return process')(); } catch (err) { console.log(err.name); }",
      "javascript",
      ["allowed.txt"],
      5000,
      { cwd: root },
