...
import {
  handleReadCached,
  registerForgeReadCached,
} from "../../src/mcp/tools/forge-read-cached.js";
...
// ---------------------------------------------------------------------------
// P6 FIX: session ID must be derived from root (not hardcoded "mcp-default"),
// and concurrent reads must not lose cache entries via read-modify-write races.
// ---------------------------------------------------------------------------

type CachedHandler = (input: {
  path: string;
  start_line?: number;
  end_line?: number;
}) => Promise<{ content: Array<{ text: string }> }>;

function collectReadCachedHandler(root?: { path: string }): CachedHandler {
  let handler: CachedHandler | null = null;
  const fakeServer = {
    tool: (_name: string, _desc: unknown, _schema: unknown, h: CachedHandler) => {
      handler = h;
    },
  };
  registerForgeReadCached(fakeServer as never, root as never);
  if (!handler) throw new Error("forge_read_cached handler not registered");
  return handler;
}
...
describe("forge_read_cached session isolation (P6)", () => {
  ...
  it("derives a per-project session id from root (not the global 'mcp-default')", async () => {
    ...
    const before = new Set(
      (await readdir(tmpdir())).filter((f) => f.startsWith("forge-read-cache-")),
    );
