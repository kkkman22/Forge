...
const forgePathArb: fc.Arbitrary<string> = fc.oneof(
...
const holderArb: fc.Arbitrary<string> = fc
...
const lockInfoArb: fc.Arbitrary<LockInfo> = fc
...
describe("Property 25: Lock 过期检测", () => {
  ...
  it("staleness is monotonic: if stale at time T, stale at T+1", () => {
    fc.assert(
      fc.property(lockInfoArb, fc.integer({ min: 0, max: 120_000 }), (info, elapsedMs) => {
        const acquiredMs = new Date(info.acquiredAt).getTime();
...
describe("Property 25: Lock 获取逻辑", () => {
  ...
  it("same holder → always acquired (re-entrant)", () => {
    fc.assert(
      fc.property(forgePathArb, holderArb, isoDateArb, (path, holder, acquiredAt) => {
        const existing: LockInfo = {
          holder,
          acquiredAt,
          targetFile: path,
        };
  ...
  it("stale lock by another holder → acquired", () => {
    fc.assert(
      fc.property(forgePathArb, holderArb, holderArb, (path, holder, otherHolder) => {
...
describe("Property 25: LockInfo 序列化往返", () => {
  ...
  it("createLockInfo produces valid lock info", () => {
    fc.assert(
      fc.property(forgePathArb, holderArb, isoDateArb, (path, holder, nowIso) => {
