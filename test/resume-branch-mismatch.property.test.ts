 ...
 */
function setupTwoRunDirectories(firstRunId: string, secondRunId: string): void {
  ...
  (existsSync as Mock).mockImplementation((p: string) => {
    ...
    if (p === `${CWD}/.forge/runs/${firstRunId}/notes.md`) return true;
    if (p === `${CWD}/.forge/runs/${secondRunId}/notes.md`) return true;
    return false;
  });
...
describe("Bug Condition: resumeRun ignores branchName parameter", () => {
  ...
  it("creates a new run when no existing run matches the requested branchName", () => {
    ...
    (readFileSync as Mock).mockReturnValue(buildNotesContent(existingRunId, "forge/other-branch"));
