 ...
 */
const patternArb: fc.Arbitrary<Pattern> = fc
  .record({
...
const patternsArb: fc.Arbitrary<Pattern[]> = fc
...
describe("pattern-stats — updatePatternStats invariants", () => {
  ...
  it("keeps confidence in [0,1] and counters coherent across any sequence", () => {
    const outcomeArb = fc.constantFrom("success" as const, "failure" as const);
    const startingPattern: Pattern = {
    ...
    fc.assert(
      fc.property(fc.array(outcomeArb, { minLength: 1, maxLength: 50 }), (outcomes) => {
...
describe("pattern-stats — findStaleOrDecayedPatterns", () => {
  ...
  it("returns an order-preserving subset of the input patterns", () => {
    fc.assert(
      fc.property(
        ...
        (patterns, nowIso, maxAgeDays) => {
          const stale = findStaleOrDecayedPatterns(patterns, new Date(nowIso), maxAgeDays);
          for (const p of stale) expect(patterns).toContain(p);
          let lastIndex = -1;
  ...
  it("marks patterns with enough samples and low confidence as decayed", () => {
    ...
    const result = findStaleOrDecayedPatterns(
      [decayed, strong],
...
describe("pattern-stats — findUpgradableEpisodes", () => {
  ...
  it("does not suggest when fewer than minOccurrences", () => {
    ...
    const suggestions = findUpgradableEpisodes(
      episodes,
      [],
