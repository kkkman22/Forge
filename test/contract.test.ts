...
describe("Contract: hook lifecycle events are registered", () => {
  ...
  it("ConfigChange points to scripts/config-changed-hook.mjs using args", () => {
    const groups = srcHooks.hooks.ConfigChange as Array<{
      hooks: Array<{ args?: string[]; command?: string }>;
    }>;
    const hasCorrectScript = groups.some((group) =>
      group.hooks.some(
        (h) =>
          (h.args?.includes("scripts/config-changed-hook.mjs") ?? false) ||
  ...
  it("PermissionDenied points to scripts/permission-denied-hook.mjs using args", () => {
    const groups = srcHooks.hooks.PermissionDenied as Array<{
      hooks: Array<{ args?: string[]; command?: string }>;
    }>;
    const hasCorrectScript = groups.some((group) =>
      group.hooks.some(
        (h) =>
          (h.args?.includes("scripts/permission-denied-hook.mjs") ?? false) ||
  ...
  it("WorktreeRemove points to scripts/worktree-remove-hook.mjs using args", () => {
    const groups = srcHooks.hooks.WorktreeRemove as Array<{
      hooks: Array<{ args?: string[]; command?: string }>;
    }>;
    const hasCorrectScript = groups.some((group) =>
      group.hooks.some(
        (h) =>
          (h.args?.includes("scripts/worktree-remove-hook.mjs") ?? false) ||
          (h.command?.includes("scripts/worktree-remove-hook.mjs") ?? false),
      ),
    );
    expect(hasCorrectScript, "WorktreeRemove must reference scripts/worktree-remove-hook.mjs").toBe(
      true,
    );
  });
  ...
  it("new lifecycle hooks use args array (not command string)", () => {
    ...
    for (const event of newEvents) {
      const groups = srcHooks.hooks[event] as Array<{
        hooks: Array<{ args?: string[]; command?: string }>;
      }>;
      for (const group of groups) {
        for (const handler of group.hooks) {
          expect(
            handler.args,
            `${event} hook should use args array, not command string`,
          ).toBeDefined();
