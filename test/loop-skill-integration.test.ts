...
import { determineNextSkill, type SchedulerInput } from "../src/skill-scheduler.js";
import {
  clearLoopFields,
  extractLoopFields,
...
describe("Full sequence simulation: router → plan → build → review → test → ship", () => {
  ...
  it("walks through the complete standard-tier phase sequence", () => {
    ...
    const step4 = determineNextSkill(
      makeInput({ currentPhase: "build", hasIncompleteTasks: true }),
    );
    expect(step4.nextPhase).toBe("build");

    // Phase 5: Build with all tasks complete → go to "review"
    const step5 = determineNextSkill(
      makeInput({ currentPhase: "build", hasIncompleteTasks: false }),
    );
...
describe("Fix loop simulation: review blocked → build fix → review pass", () => {
  ...
  it("enters fix loop on review failure and exits on review pass", () => {
    ...
    const fixStep = determineNextSkill(
      makeInput({
        currentPhase: "review",
        reviewResult: "fail",
        reviewFixAttempts: 0,
    ...
    const reReviewStep = determineNextSkill(
      makeInput({
        currentPhase: "build",
    ...
    const testStep = determineNextSkill(
      makeInput({
        currentPhase: "review",
        reviewResult: "pass",
        reviewFixAttempts: 1,
  ...
  it("handles multiple fix loop iterations before passing", () => {
    ...
    const passStep = determineNextSkill(
      makeInput({
        currentPhase: "review",
        reviewResult: "pass",
        reviewFixAttempts: 2,
...
describe("Circuit breaker simulation: consecutive review failures exceed max", () => {
  ...
  it("aborts when reviewFixAttempts reaches maxReviewFixAttempts", () => {
    ...
    const abortResult = determineNextSkill(
      makeInput({
        currentPhase: "review",
        reviewResult: "fail",
...
describe("Skill-aware prompt contains correct phase at each step", () => {
   ...
   */
  const phases = ["router", "plan", "build", "review", "test", "ship"] as const;
