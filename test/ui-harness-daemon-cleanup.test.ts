...
import type { AgentBrowserClient, Snapshot } from "../src/agent-browser-client.js";
import { runUiHarness } from "../src/ui-harness.js";

/** Tracks open/snapshot/close calls; snapshot throws to exercise the error path. */
function makeLeakyClient(): { client: AgentBrowserClient; calls: string[] } {
  const calls: string[] = [];
  const client: AgentBrowserClient = {
    async open(_url: string, _sid: string) {
      calls.push("open");
    },
    async snapshot(_sid: string): Promise<Snapshot> {
      calls.push("snapshot");
