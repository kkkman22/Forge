...
function makeSpecDocument(overrides?: Partial<SpecDocument>): SpecDocument {
  return {
    frontmatter: {
...
function makeEarsClause(): EarsClause {
  return {
    line: 1,
    when: "用户提交表单",
    shall: "系统返回成功提示",
...
function makeRequirementsDocument(): RequirementsDocument {
  return {
...
function makeTaskSeed(): TaskSeed {
  return {
    id: "T-01",
    title: "Test task",
    goal: "Implement X",
...
function makeTasksSeedDocument(): TasksSeedDocument {
  return {
    frontmatter: makeSpecFileFrontmatter(),
    tasks: [makeTaskSeed()],
  };
...
describe("T-01 Data Contract: type existence", () => {
  ...
  it("SpecStatus allows draft and locked", () => {
    const statuses: SpecStatus[] = ["draft", "locked"];
    expect(statuses).toHaveLength(2);
  });

  it("SpecKind allows feature and bugfix", () => {
    const kinds: SpecKind[] = ["feature", "bugfix"];
    expect(kinds).toHaveLength(2);
  });

  it("SpecFileFrontmatter has required fields", () => {
    const fm: SpecFileFrontmatter = makeSpecFileFrontmatter();
...
describe("TasksSeedDocument structure", () => {
  ...
  it("has optional waves filled by plan stage", () => {
    const wave: Wave = { wave: 1, tasks: ["T-01"] };
...
describe("specDocumentToBundle adapter", () => {
  ...
  it("maps SpecDocument requirements to EarsClause format", () => {
    const spec = makeSpecDocument();
    const bundle = specDocumentToBundle(spec);
    const reqDoc = bundle.primary as RequirementsDocument;
