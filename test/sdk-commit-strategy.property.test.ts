...
import {
  applySkillAwareCommitStrategy,
  buildCommitMessageForPhase,
} from "../src/sdk-commit-strategy.js";
...
const commitEffectArb: fc.Arbitrary<OrchestratorEffect> = fc
  .string({ minLength: 1, maxLength: 80 })
  .map((msg) => ({ type: "commit" as const, message: msg }));

/** Arbitrary for non-commit effects (all other OrchestratorEffect variants). */
const nonCommitEffectArb: fc.Arbitrary<OrchestratorEffect> = fc.oneof(
  fc.record({
  ...
  fc.record({
    type: fc.constant("start_backoff" as const),
 ...
 */
const effectsWithAtLeastOneCommitArb: fc.Arbitrary<OrchestratorEffect[]> = fc
  .tuple(
    fc.array(commitEffectArb, { minLength: 1, maxLength: 5 }),
    fc.array(nonCommitEffectArb, { minLength: 0, maxLength: 5 }),
  )
...
const nonCommitablePhaseArb: fc.Arbitrary<string> = fc.constantFrom(...NON_COMMITABLE_PHASES);
...
describe("Feature: sdk-driver-decomposition, Property 3: Commit effect filtering for non-commitable phases", () => {
  ...
  it("removes all commit effects and returns stateAdjustment for non-commitable phases with success=true", () => {
    fc.assert(
      fc.property(
        effectsWithAtLeastOneCommitArb,
        nonCommitablePhaseArb,
        fc.nat({ max: 1000 }),
        fc.string({ maxLength: 200 }),
        fc.string({ maxLength: 200 }),
        fc.nat({ max: 100 }),
        (effects, phase, iterationNumber, summary, objective, currentCommitCount) => {
          const result = applySkillAwareCommitStrategy(
            effects,
            phase,
            true, // success=true
            iterationNumber,
...
describe("Feature: sdk-driver-decomposition, Property 4: Commit message format correctness", () => {
  ...
  const commitablePhaseArb: fc.Arbitrary<string> = fc.constantFrom(...COMMITABLE_PHASES);
  ...
  it("returns a message matching forge(<label>): <content> for any commitable phase", () => {
    ...
    fc.assert(
      fc.property(
        ...
        fc.string({ minLength: 1, maxLength: 200 }),
        (phase, iterationNumber, summary, objective) => {
