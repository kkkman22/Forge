...
import type { LoopStatusFields } from "../src/status-file-ext.js";
...
const executionModeArb = fc.constantFrom("interactive" as const, "autonomous" as const);
 ...
 */
const loopStatusFieldsArb: fc.Arbitrary<LoopStatusFields> = fc.record({
...
describe("Feature: loop-skills-fusion, Property 9: StatusFile Loop 字段 round-trip", () => {
  ...
  it("writeLoopFields then extractLoopFields returns semantically equivalent fields", () => {
    fc.assert(
      fc.property(statusFileArb, loopStatusFieldsArb, (content, fields) => {
...
describe("Feature: loop-skills-fusion, Property 10: StatusFile Loop 字段清除完整性", () => {
  ...
  it("clearLoopFields then extractLoopFields returns all fields as undefined", () => {
    fc.assert(
      fc.property(statusFileWithLoopFieldsArb, (contentWithFields) => {
...
describe("Feature: loop-skills-fusion, Property 4: writeLoopFields preserves non-Loop fields", () => {
  ...
  it("non-Loop fields preserved with partial LoopStatusFields writes", () => {
    ...
    const partialFieldsArb: fc.Arbitrary<LoopStatusFields> = fc.record(
      {
        mode: executionModeArb,
        loopRunId: loopRunIdArb,
        loopIteration: loopIterationArb,
    ...
    fc.assert(
      fc.property(statusFileArb, partialFieldsArb, (content, fields) => {
        const written = writeLoopFields(content, fields);
...
const skillPhaseArb = fc.constantFrom(
  "router",
  "plan",
  "build",
  "review",
 ...
 */
const loopStatusFieldsWithUuidArb: fc.Arbitrary<LoopStatusFields> = fc.record({
  mode: executionModeArb,
  loopRunId: fc.uuid(),
 ...
 */
const partialLoopStatusFieldsArb: fc.Arbitrary<LoopStatusFields> = fc.record(
  {
    mode: executionModeArb,
    loopRunId: fc.uuid(),
    loopIteration: fc.nat({ max: 10000 }),
...
describe("Feature: loop-skills-fusion, Property 3: LoopStatusFields round-trip consistency", () => {
  ...
  it("extractLoopFields(writeLoopFields(content, fields)) returns equivalent field values", () => {
    fc.assert(
      fc.property(statusFileArb, loopStatusFieldsWithUuidArb, (content, fields) => {
        const written = writeLoopFields(content, fields);
        const extracted = extractLoopFields(written);
  ...
  it("round-trip preserves only defined fields in partial LoopStatusFields", () => {
    fc.assert(
      fc.property(statusFileArb, partialLoopStatusFieldsArb, (content, fields) => {
        const written = writeLoopFields(content, fields);
        const extracted = extractLoopFields(written);
 ...
 */
function extractPhaseFromFrontmatter(content: string): string | undefined {
...
describe("Feature: loop-skills-fusion, Property 11: updateIterationStatus field update", () => {
  ...
  it("updateIterationStatus then extracting phase and loop_iteration returns written values", () => {
    fc.assert(
      fc.property(statusFileArb, phaseArb, iterationArb, (content, phase, iteration) => {
  ...
  it("updateIterationStatus creates frontmatter when none exists", () => {
    fc.assert(
      fc.property(
        bodyContentArb.filter((b) => !b.trimStart().startsWith("---")),
        phaseArb,
        iterationArb,
        (body, phase, iteration) => {
  ...
  it("updateIterationStatus overwrites previous values", () => {
    fc.assert(
      fc.property(
        ...
        phaseArb,
        iterationArb,
        (content, phase1, iter1, phase2, iter2) => {
