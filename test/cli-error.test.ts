...
describe("CLI precondition failures throw CliError", () => {
   ...
   */
  function checkGitRepo(isGitRepo: boolean): void {
    if (!isGitRepo) {
   ...
   */
  function checkCleanWorkingTree(status: string, useWorktree: boolean): void {
   ...
   */
  function checkWorktreeSource(currentBranch: string, useWorktree: boolean): void {
   ...
   */
  function checkForgeDir(hasForgeDir: boolean, hasSkillOptions: boolean): void {
    if (!hasForgeDir && hasSkillOptions) {
      throw new CliError(
        "Error: --tier, --type, and --phase require a .forge/ directory. Run `forge init` first.",
      );
    }
  }
  ...
  it("dirty-tree error has descriptive message", () => {
    try {
      ...
      expect(err).toBeInstanceOf(CliError);
      expect((err as CliError).message).toContain("Working tree is not clean");
      expect((err as CliError).exitCode).toBe(1);
    }
  });
  ...
  it("invalid-branch error has descriptive message", () => {
    try {
      ...
      expect(err).toBeInstanceOf(CliError);
      expect((err as CliError).message).toContain("Cannot create a worktree from a forge/ branch");
      expect((err as CliError).exitCode).toBe(1);
    }
  });
  ...
  it("missing-forge-dir error has descriptive message", () => {
    try {
      ...
      expect(err).toBeInstanceOf(CliError);
      expect((err as CliError).message).toContain("require a .forge/ directory");
      expect((err as CliError).exitCode).toBe(1);
    }
  });
...
describe("main().catch() handler logic", () => {
   ...
   */
  function simulateCatchHandler(err: unknown): { message: string; exitCode: number } {
