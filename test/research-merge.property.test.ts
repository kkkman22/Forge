...
const researchResultArb: fc.Arbitrary<SubagentResult> = fc.record({
  agentType: fc.string({ minLength: 1, maxLength: 20 }),
  status: fc.constantFrom("success", "failure", "timeout") as fc.Arbitrary<
...
describe("Feature: agent-team-migration, Property 6: research findings merge completeness", () => {
  it("merged document contains all findings from every successful subagent with none lost", () => {
    fc.assert(
      fc.property(fc.array(researchResultArb, { minLength: 1, maxLength: 8 }), (results) => {
  ...
  it("buildResearchSubagents creates one invocation per topic", () => {
    fc.assert(
      fc.property(fc.array(researchTopicArb, { minLength: 1, maxLength: 5 }), (topics) => {
    ...
    fc.assert(
      fc.property(
        fc.array(
          fc.record({
            agentType: fc.string({ minLength: 1, maxLength: 20 }),
            status: fc.constantFrom("failure", "timeout") as fc.Arbitrary<"failure" | "timeout">,
