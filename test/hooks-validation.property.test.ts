...
function setupHooksDir(tmpDir: string, content: string): void {
...
describe("Feature: audit-remediation-v221, Property 1: Hooks validation correctly classifies JSON structures", () => {
  ...
  it("returns valid: true for any JSON with hooks.PreToolUse array", () => {
    fc.assert(
      fc.property(validHooksJsonArb, (jsonObj) => {
  ...
  it("returns valid: false with non-empty reason for any JSON without hooks.PreToolUse array", () => {
    fc.assert(
      fc.property(arbitraryJsonValueArb, (jsonVal) => {
        ...
        const hasValidStructure =
          ...
          "hooks" in jsonVal &&
          (jsonVal as Record<string, unknown>).hooks !== null &&
          typeof (jsonVal as Record<string, unknown>).hooks === "object" &&
          !Array.isArray((jsonVal as Record<string, unknown>).hooks) &&
          "PreToolUse" in ((jsonVal as Record<string, unknown>).hooks as Record<string, unknown>) &&
          Array.isArray(
            ((jsonVal as Record<string, unknown>).hooks as Record<string, unknown>).PreToolUse,
          );
  ...
  it("returns valid: false for objects where hooks.PreToolUse is not an array", () => {
    ...
    fc.assert(
      fc.property(nonArrayPreToolUseArb, (jsonObj) => {
        setupHooksDir(tmpDir, JSON.stringify(jsonObj));
        const result = validateHooksPresence(tmpDir);
        expect(result.valid).toBe(false);
        expect(result.reason).toBeDefined();
