...
import type { AgentInterface } from "../src/loop-types.js";
...
const factoryArb: fc.Arbitrary<(config: unknown) => AgentInterface> = fc.constant(
  () =>
    ({
      name: "mock-agent",
      async run() {
...
describe("Feature: multi-platform-support, Property 1: registration idempotency", () => {
  it("re-registering overwrites without error", () => {
    fc.assert(
      fc.property(agentNameArb, factoryArb, factoryArb, (name, f1, f2) => {
        const registry = createAgentRegistry();
        registry.register(name, f1 as never);
        expect(registry.has(name)).toBe(true);
        expect(registry.listAgents()).toContain(name);

        // Re-register should not throw.
        registry.register(name, f2 as never);
        expect(registry.has(name)).toBe(true);
        expect(registry.listAgents()).toContain(name);
...
describe("Feature: multi-platform-support, Property 2: unregistered name throws", () => {
  it("throws a descriptive error listing available agents", () => {
    fc.assert(
      fc.property(
        fc.array(fc.tuple(agentNameArb, factoryArb), { minLength: 0, maxLength: 10 }),
        agentNameArb,
        (entries, unknownName) => {
          const registry = createAgentRegistry();
          for (const [name, factory] of entries) {
            registry.register(name, factory as never);
          }

          // If the unknown name happens to be registered, skip.
          if (registry.has(unknownName)) return;

          expect(() => registry.resolve(unknownName, { cwd: "/tmp" })).toThrow(/is not registered/);
          expect(() => registry.resolve(unknownName, { cwd: "/tmp" })).toThrow(
            new RegExp(`Available agents: ${registry.listAgents().join(", ")}`),
          );
        },
      ),
      { numRuns: 200 },
