 ...
 */
const translationDataArb: fc.Arbitrary<TranslationData> = fc.letrec((tie) => ({
  leaf: leafValueArb,
  node: fc.dictionary(
    keySegmentArb,
    fc.oneof({ depthIdentifier: "td", withCrossShrink: true }, tie("leaf"), tie("tree")),
    { minKeys: 1, maxKeys: 4 },
  ),
  tree: fc.dictionary(
    keySegmentArb,
    fc.oneof(
      { depthIdentifier: "td", withCrossShrink: true, depthSize: "small" },
      tie("leaf"),
      tie("node"),
    ),
    { minKeys: 1, maxKeys: 4 },
  ),
})).tree as fc.Arbitrary<TranslationData>;

/**
 * Collect all dot-separated key paths that lead to string leaves
 * in a TranslationData object.
 */
function collectLeafPaths(data: TranslationData, prefix = ""): string[] {
  const paths: string[] = [];
 ...
 * in a TranslationData object.
 */
function collectBranchPaths(data: TranslationData, prefix = ""): string[] {
  const paths: string[] = [];
  for (const key of Object.keys(data)) {
    const fullPath = prefix ? `${prefix}.${key}` : key;
    const value = data[key];
...
describe("Feature: i18n-support, Property 1: 翻译数据 JSON 往返一致性", () => {
  ...
  it("JSON.parse(JSON.stringify(data)) deeply equals original for any valid TranslationData", () => {
    fc.assert(
      fc.property(translationDataArb, (data) => {
    ...
    fc.assert(
      fc.property(translationDataArb, (data) => {
        ...
        for (const path of leafPaths) {
          ...
          let expected: string | TranslationData = data;
          for (const seg of segments) {
            expected = (expected as TranslationData)[seg];
          }
