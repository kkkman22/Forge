...
import type { ThreatSeverity, ThreatType } from "../src/prompt-defense.js";
import { PATTERNS, type ThreatPattern } from "../src/prompt-defense-patterns.js";

const VALID_TYPES: ReadonlySet<ThreatType> = new Set<ThreatType>([
...
const VALID_SEVERITIES: ReadonlySet<ThreatSeverity> = new Set<ThreatSeverity>([
  "critical",
  "high",
  "medium",
  "low",
...
const MIN_PER_CATEGORY: Record<ThreatType, number> = {
...
function countByType(): Record<ThreatType, number> {
...
describe("prompt-defense PATTERNS — structural contract", () => {
  ...
  it("meets the minimum pattern count for every category", () => {
    ...
    for (const [type, min] of Object.entries(MIN_PER_CATEGORY) as Array<[ThreatType, number]>) {
      expect(
        counts[type],
        `category ${type} has ${counts[type]} patterns, expected >= ${min}`,
  ...
  it("uses category-matching id prefixes", () => {
    const prefixByType: Record<ThreatType, string> = {
    ...
    for (const p of PATTERNS) {
      expect(
        p.id.startsWith(prefixByType[p.type]),
        `${p.id} should start with "${prefixByType[p.type]}" for type ${p.type}`,
      ).toBe(true);
    }
  });
});
...
describe("prompt-defense PATTERNS — description PII hygiene", () => {
  ...
  const PII_SHAPES: ReadonlyArray<{ name: string; re: RegExp }> = [
  ...
  it.each(
    PATTERNS.map((p: ThreatPattern) => [p.id, p.description]),
  )("%s description does not contain a concrete PII / secret sample", (_id, description) => {
    for (const { name, re } of PII_SHAPES) {
      expect(
        re.test(description),
        `description should not embed a ${name}; got "${description}"`,
      ).toBe(false);
    }
  });
});
