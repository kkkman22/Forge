...
describe("Feature: agent-team-migration, Property 2: parallel execution fault tolerance", () => {
  it("preserves all successful results and reports all failures for any mix of outcomes", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.array(subagentResultArb, { minLength: 1, maxLength: 10 }),
        async (results) => {
          const invocations: SubagentInvocation[] = results.map((r, i) => ({
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (successResults) => {
          ...
          const executor = async (): Promise<SubagentResult> => successResults[0];
          const outcome = await runSubagentsInParallel(invocations, executor);
  ...
  it("every built invocation has non-empty prompt, valid permissionMode, positive maxTurns, and valid agentType", () => {
    fc.assert(
      fc.property(
        ...
        (context) => {
          const invocations = buildSubagentInvocations(
            [context.agentType],
