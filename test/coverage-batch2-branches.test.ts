...
import { evaluateAdrCriteria } from "../src/adr-criteria.js";
import {
  appendToBacklog,
  type BacklogEntry,
...
describe("backlog (branch coverage)", () => {
  const entry = (overrides: Partial<BacklogEntry> = {}): BacklogEntry => ({
    id: "hash-001",
    severity: "P2",
    filePath: "src/a.ts",
    lineNumber: 10,
  ...
  it("findOverlappingEntries matches by filePath", () => {
    const matches = findOverlappingEntries(
      [entry({ filePath: "src/a.ts" })],
      ["src/a.ts", "src/b.ts"],
    );
