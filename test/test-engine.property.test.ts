...
import {
  ...
  CHECKLIST_LABELS,
  type ChecklistState,
  validatePreCompletionChecklist,
...
const allPassChecklistArb: fc.Arbitrary<ChecklistState> = fc.constant({
  testsJustRan: true,
  allTestsPass: true,
  typeCheckPass: true,
  lintPass: true,
...
const someFailChecklistArb: fc.Arbitrary<ChecklistState> = fc
  .record({
...
const anyChecklistArb: fc.Arbitrary<ChecklistState> = fc.record({
  testsJustRan: fc.boolean(),
  allTestsPass: fc.boolean(),
  typeCheckPass: fc.boolean(),
  lintPass: fc.boolean(),
 ...
 */
const singleFailChecklistArb: fc.Arbitrary<{
  state: ChecklistState;
  failedKey: keyof ChecklistState;
}> = fc.constantFrom(...CHECKLIST_KEYS).map((failedKey) => {
...
describe("Property 12: 完成前验证清单完整性", () => {
  it("all 7 items true → verification passes (Req 7.3)", () => {
    fc.assert(
      fc.property(allPassChecklistArb, (state) => {
  ...
  it("any item false → verification fails (Req 7.3)", () => {
    fc.assert(
      fc.property(someFailChecklistArb, (state) => {
        const result = validatePreCompletionChecklist(state);
  ...
  it("exactly one item false → verification fails with that item listed", () => {
    fc.assert(
      fc.property(singleFailChecklistArb, ({ state, failedKey }) => {
        const result = validatePreCompletionChecklist(state);
  ...
  it("for any checklist state, passed ↔ all items true (biconditional)", () => {
    fc.assert(
      fc.property(anyChecklistArb, (state) => {
        const result = validatePreCompletionChecklist(state);
