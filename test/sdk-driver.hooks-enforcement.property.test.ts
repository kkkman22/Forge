...
import type { SdkDriverConfig } from "../src/sdk-driver-types.js";

function buildMinimalConfig(overrides: Partial<SdkDriverConfig> = {}): SdkDriverConfig {
  return {
...
function createTrackingAgent(): AgentInterface & { invocationCount: number } {
  const obj: AgentInterface & { invocationCount: number } = {
    invocationCount: 0,
    name: "test-agent",
    async run() {
      obj.invocationCount++;
