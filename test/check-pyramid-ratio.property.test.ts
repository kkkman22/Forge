...
import {
  evaluatePyramidRatio,
  isCriticalAc,
  type PyramidRatioInput,
  parseSpecForRatio,
} from "../scripts/check-pyramid-ratio.js";
import type { PyramidConfig } from "../src/accept-driver.js";

const cfg = (over: Partial<PyramidConfig> = {}): PyramidConfig => ({
...
function run(criteria: PyramidRatioInput["criteria"], config: PyramidConfig = cfg()) {
...
describe("evaluatePyramidRatio — degradation (Req7 AC3)", () => {
  it("strict_pyramid: false → warn-only (skip, not heavy)", () => {
    const r = run(
      [ac("1", "e2e"), ac("2", "e2e"), ac("3", "e2e"), ac("4", "e2e")],
      cfg({ strictPyramid: false }),
    );
    expect(r.skip).toBe(true);
...
describe("parseSpecForRatio — spec → gate input", () => {
  function specWithAcs(acs: { id: string; vb: string; critical?: boolean }[]): string {
    const blocks = acs
      .map(
        (a) =>
          `${a.id}. WHEN x THEN y SHALL z${a.critical ? " <!-- @critical -->" : ""}
   **Verify-By**: ${a.vb}
