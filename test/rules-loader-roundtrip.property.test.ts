...
import { type AtomicRule, loadAllRules, renderSuggestionSuffix } from "../src/rules-loader.js";

// ---------------------------------------------------------------------------
// Generators
// ---------------------------------------------------------------------------

const lintBindingArb = fc.oneof(
  fc.constant(null),
  fc.string({ minLength: 1, maxLength: 30 }).map((s) => `biome/${s}`),
  fc.record({
...
describe("rules-loader round-trip property [R13.5]", () => {
  it("parse → serialize → parse produces equivalent frontmatter (200 iterations)", () => {
    fc.assert(
      fc.property(ruleFrontmatterArb, (fm) => {
  ...
  it("renderSuggestionSuffix returns non-empty string for rules with lint_binding", () => {
    const ruleWithStringBinding: AtomicRule = {
      name: "test",
      alwaysApply: true,
    ...
    const ruleWithDualBinding: AtomicRule = {
      name: "test",
      alwaysApply: true,
...
function renderRuleMd(fm: { name: string; alwaysApply: boolean; lint_binding: unknown }): string {
  let yaml = "---\n";
  yaml += `name: "${fm.name}"\n`;
  ...
  if (fm.lint_binding === null) {
    ...
    const obj = fm.lint_binding as { biome: string; eslint: string };
    yaml += "lint_binding:\n";
...
function parseFrontmatterFromContent(content: string): { name: string; alwaysApply: boolean } {
