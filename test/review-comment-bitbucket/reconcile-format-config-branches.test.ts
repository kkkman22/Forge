...
describe("reconcile (branch coverage)", () => {
  ...
  it("reconciles when matching comment exists", () => {
    ...
    const plan = reconcile({
      currentFindings: [f],
      existingTasks: [],
      existingComments: [{ hash, resolved: false, marker: `forge-review:hash=${hash}` } as never],
      autoReconcileResolved: true,
      autoReopenRegressed: true,
    });
...
describe("formatFinding (branch coverage)", () => {
  ...
  it("formats a P2 finding", () => {
    expect(
      formatFinding(finding({ priority: "P2" }), "run-1", "forge-review").comment_text,
    ).toContain("P2");
  });
  it("formats a P3 finding", () => {
    expect(
      formatFinding(finding({ priority: "P3" }), "run-1", "forge-review").comment_text,
    ).toContain("P3");
  });
});
...
describe("parseCommentChannelConfig (branch coverage)", () => {
  ...
  it("accepts rate_limit", () => {
    expect(parseCommentChannelConfig({ rate_limit_interval_ms: 1000 }).rate_limit_interval_ms).toBe(
      1000,
    );
  });
});
