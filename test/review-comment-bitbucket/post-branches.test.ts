...
describe("postReviewToBitbucket — set_review_status error branch", () => {
  ...
  it("does not call set_review_status when request_changes_on_p0_p1=false", async () => {
    ...
    await postReviewToBitbucket(
      "review.md",
      "pr-1",
      { ...DEFAULT_CONFIG, request_changes_on_p0_p1: false },
      CTX,
      bb as never,
      [P0],
      { baseDir: undefined },
  ...
  it("does not call set_review_status when no P0/P1 findings", async () => {
    const bb = mockClient();
    const p2: Finding = { ...P0, priority: "P2" };
    await postReviewToBitbucket("review.md", "pr-1", DEFAULT_CONFIG, CTX, bb as never, [p2], {
      baseDir: undefined,
    });
    expect(bb.set_review_status).not.toHaveBeenCalled();
  });
...
describe("postReviewToBitbucket — P3 filtering branch", () => {
  it("P3 findings are filtered out before reconciliation", async () => {
    const bb = mockClient();
    const p3: Finding = { ...P0, priority: "P3" };
    await postReviewToBitbucket("review.md", "pr-1", DEFAULT_CONFIG, CTX, bb as never, [p3], {
      baseDir: undefined,
    });
