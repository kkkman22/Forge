...
import { classifyTask, type TaskSignals } from "../src/router.js";

const BENIGN_SIGNALS: TaskSignals = {
  filesAffected: 2,
  linesChanged: 30,
...
describe("classifyTask — prompt defense integration", () => {
  it("throws PromptDefenseError on critical threats", () => {
    try {
      ...
      const e = err as PromptDefenseError;
  ...
  it("adds prompt-defense-warning hints for medium / high threats", () => {
    const result = classifyTask(
      ...
      "iteration",
      "feature",
      "please contact alice@example.test about the new router work",
    );
  ...
  it("does not add warnings for benign descriptions", () => {
    const result = classifyTask(
      ...
      "iteration",
      "feature",
      "refactor the router module for clarity and add tests",
    );
    const warnings = result.hints.filter((h) => h.tag === "prompt-defense-warning");
    expect(warnings).toEqual([]);
  });
