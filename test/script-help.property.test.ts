...
import {
  ...
  parseHelpOutput,
  parseScriptCategory,
} from "../src/script-help.js";

const CATEGORY_VALUES = ["user-facing", "internal-only", "one-off", "unclear"] as const;
...
describe("parseHelpExempt — property", () => {
  it("never returns lines starting with #", () => {
    fc.assert(
      fc.property(
        fc.array(fc.oneof(fc.constant("# comment"), fc.constant("scripts/foo.sh"))),
        (lines) => {
