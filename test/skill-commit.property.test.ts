...
const COMMITABLE_PHASES = ["build", "plan", "fix"] as const;

/** Phases that do NOT produce committable changes. */
const NON_COMMITABLE_PHASES = ["review", "test", "ship", "router", "learn"] as const;
...
const anyKnownPhaseArb: fc.Arbitrary<string> = fc.constantFrom(
  ...COMMITABLE_PHASES,
  ...NON_COMMITABLE_PHASES,
);
...
describe("Feature: loop-skills-fusion, Property 12: SKILL 阶段 commit 策略一致性", () => {
  ...
  it("deterministic — same inputs always produce the same output", () => {
    fc.assert(
      fc.property(anyKnownPhaseArb, fc.boolean(), (phase, success) => {
