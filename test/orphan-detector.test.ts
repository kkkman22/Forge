			...
			fc.assert(
				fc.property(
					fc.oneof(
						...
						fc.uint8Array({ minLength: 1, maxLength: 20 }).map((arr) =>
				...
				fc.asyncProperty(
					fc.array(
						fc.record({
							...
							command: fc.oneof(
								fc.constant("node forge-loop"),
								fc.constant("vitest run"),
								fc.constant("caffeinate -i"),
								fc.constant("python unrelated"),
					...
					async (entries) => {
						...
						for (const orphan of result) {
							...
							const matchesPattern = ["forge", "vitest", "caffeinate"].some((p) =>
								entry!.command.includes(p),
							);
				...
				fc.property(
					fc.array(
						fc.record({
							...
							source: fc.constant("ppid-detection" as const),
						}),
						{ maxLength: 10 },
					),
					(orphanProcesses) => {
						...
						const expectedWarned = orphanProcesses.filter(
							(o) => o.elapsedSeconds <= threshold,
						);
	...
	describe("cleanupStaleSessions", () => {
		it("cleans up stale session PID files", async () => {
			...
			const killSpy = vi.spyOn(process, "kill").mockImplementation((pid: number) => {
				if (pid === 99999) throw new Error("ESRCH"); // Session dead
