...
const BASE_LOCALES = ["zh", "en", "ja", "ko", "fr", "de", "es", "pt", "ru", "it"] as const;
...
const rawLocaleArb = fc
  .tuple(
    baseLocaleArb,
    fc.option(fc.constantFrom(...regionSuffixes), { nil: undefined }),
    fc.option(fc.constantFrom(...encodingSuffixes), { nil: undefined }),
    fc.option(fc.constantFrom(...variantTags), { nil: undefined }),
    fc.boolean(), // whether to uppercase the whole thing
  )
  .map(([base, region, encoding, variant, upper]) => {
    let raw = base;
    if (region) raw += region;
...
const supportedSetArb = fc
  ...
  .map((arr) => new Set(arr) as SupportedLocales);
 ...
 */
function optionalLocaleArb(locales: readonly string[]): fc.Arbitrary<string | undefined> {
...
describe("Feature: i18n-support, Property 5: 语言优先级解析正确性", () => {
  ...
  it("highest-priority supported source wins", () => {
    const supported: SupportedLocales = new Set(["zh", "en", "ja", "ko"]);
    const supportedArr = ["zh", "en", "ja", "ko"];
    const defaultLocale = "en";

    fc.assert(
      fc.property(
        optionalLocaleArb(supportedArr),
        optionalLocaleArb(supportedArr),
        optionalLocaleArb(supportedArr),
        optionalLocaleArb(supportedArr),
        (cliLang, configLang, envLang, systemLocale) => {
          const sources: LocaleSources = { cliLang, configLang, envLang, systemLocale };
          ...
          let expectedSource: string = "default";
  ...
  it("cliLang always takes precedence when supported", () => {
    const supported: SupportedLocales = new Set(["zh", "en", "ja"]);
    const supportedArr = ["zh", "en", "ja"];

    fc.assert(
      fc.property(
        fc.constantFrom(...supportedArr),
        optionalLocaleArb(supportedArr),
        optionalLocaleArb(supportedArr),
        optionalLocaleArb(supportedArr),
        (cliLang, configLang, envLang, systemLocale) => {
  ...
  it("returns default when all sources are absent", () => {
    fc.assert(
      fc.property(supportedSetArb, baseLocaleArb, (supported, defaultLocale) => {
...
describe("Feature: i18n-support, Property 7: 不支持的语言回退", () => {
  ...
  it("unsupported locale falls back to default with warning", () => {
    const supported: SupportedLocales = new Set(["zh", "en"]);

    fc.assert(
      fc.property(unsupportedLocaleArb, (unsupportedLocale) => {
        // Ensure the locale is truly unsupported
        fc.pre(!supported.has(normalizeLocale(unsupportedLocale)));

        const sources: LocaleSources = { cliLang: unsupportedLocale };
        const result = detectLocale(sources, supported, "en");
  ...
  it("skips unsupported sources and finds supported lower-priority source", () => {
    ...
    fc.assert(
      fc.property(
        ...
        (unsupportedCli, supportedConfig) => {
          const sources: LocaleSources = {
