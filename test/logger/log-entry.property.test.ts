...
const metadataWithNewTypesArb = fc.oneof(
  subagentTimingMetadataArb.map((st) => st as Record<string, unknown>),
  degradationResultMetadataArb.map((dr) => dr as Record<string, unknown>),
...
describe("Feature: observability-enhancements, Property 1: LogEntry JSON round-trip with new metadata types", () => {
    ...
    fc.assert(
      fc.property(
        fc.record({
          ...
          event: fc.string({ minLength: 1, maxLength: 50 }),
          message: fc.string({ minLength: 1, maxLength: 200 }),
          metadata: subagentTimingMetadataArb.map((st) => st as Record<string, unknown>),
        }),
        (entry) => {
          const json = formatAsJson(entry);
          const parsed = JSON.parse(json);
    ...
    fc.assert(
      fc.property(
        fc.record({
          ...
          event: fc.string({ minLength: 1, maxLength: 50 }),
          message: fc.string({ minLength: 1, maxLength: 200 }),
          metadata: degradationResultMetadataArb.map((dr) => dr as Record<string, unknown>),
        }),
        (entry) => {
          const json = formatAsJson(entry);
          const parsed = JSON.parse(json);
  ...
  it("should produce deeply equal object after JSON round-trip for LogEntry with mixed metadata types", async () => {
    ...
    fc.assert(
      fc.property(logEntryWithNewMetadataArb, (entry) => {
        const json = formatAsJson(entry);
        const parsed = JSON.parse(json);
        expect(parsed).toEqual(JSON.parse(JSON.stringify(entry)));
      }),
