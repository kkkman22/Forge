  ...
  for (const entry of SKILL_FUNCTION_REGISTRY) {
    it(`${entry.functionName} is exported from src/${entry.module}`, async () => {
      ...
      expect(
        ...
        `${entry.functionName} not found as exported function in src/${entry.module}`,
      ).toBe(true);
    });
    ...
    it(`${entry.functionName} has expected parameters: [${entry.parameterNames.join(", ")}]`, () => {
      ...
      const sigPattern = new RegExp(
        `export\\s+function\\s+${entry.functionName}\\s*\\(([^)]*(?:\\([^)]*\\)[^)]*)*)\\)`,
      );
  ...
  for (const dir of skillDirs) {
    ...
    for (const funcName of refs) {
      it(`${dir}/SKILL.md → ${funcName} has a registry entry`, () => {
        ...
        expect(
          entry,
          `"${funcName}" referenced in skills/${dir}/SKILL.md but missing from SKILL_FUNCTION_REGISTRY. ` +
  ...
  for (const entry of SKILL_FUNCTION_REGISTRY) {
    for (const skill of entry.skills) {
      it(`skills/${skill} references ${entry.functionName}`, () => {
        ...
        const content = readFileSync(skillPath, "utf-8");
        expect(
          content.includes(entry.functionName),
          `${entry.functionName} declared in registry for skills/${skill} but not found in file content`,
        ).toBe(true);
      });
    }
  }
