...
const validFrontmatterArb: fc.Arbitrary<KnowledgeFrontmatter> = fc
...
const validDocumentArb: fc.Arbitrary<KnowledgeDocument> = fc
...
const validPatternArb: fc.Arbitrary<InstinctPattern> = fc
  .tuple(
...
const lowConfidencePatternArb: fc.Arbitrary<InstinctPattern> = fc
  .tuple(
    fc.string({ minLength: 1, maxLength: 30 }).filter((s) => s.trim().length > 0),
    fc.double({ min: 0.0, max: 0.29, noNaN: true }).map((v) => Math.round(v * 100) / 100),
    tagsArb,
    fc.array(fc.string({ minLength: 1, maxLength: 20 }), { minLength: 1, maxLength: 3 }),
    fc.string({ minLength: 1, maxLength: 50 }),
  )
...
const withinLimitsStateArb: fc.Arbitrary<KnowledgeBaseState> = fc
  .tuple(
    fc.array(validDocumentArb, { minLength: 0, maxLength: 20 }),
    fc.array(validPatternArb, { minLength: 0, maxLength: 10 }),
    fc.constant(DEFAULT_KNOWLEDGE_LIMIT),
  )
...
const overLimitStateArb: fc.Arbitrary<KnowledgeBaseState> = fc
  .tuple(fc.integer({ min: 1, max: 10 }), fc.array(validPatternArb, { minLength: 0, maxLength: 5 }))
  .chain(([limit, instinctPatterns]) =>
    fc
...
const stateWithLowConfidencePatternsArb: fc.Arbitrary<KnowledgeBaseState> = fc
  .tuple(
    fc.array(validDocumentArb, { minLength: 0, maxLength: 10 }),
    fc.array(validPatternArb, { minLength: 0, maxLength: 5 }),
    fc.array(lowConfidencePatternArb, { minLength: 1, maxLength: 5 }),
    fc.constant(DEFAULT_KNOWLEDGE_LIMIT),
  )
...
const anyKnowledgeBaseStateArb: fc.Arbitrary<KnowledgeBaseState> = fc
  .tuple(
    fc.array(validDocumentArb, { minLength: 0, maxLength: 30 }),
    fc.oneof(
      fc.array(validPatternArb, { minLength: 0, maxLength: 5 }),
      fc
        .tuple(
          fc.array(validPatternArb, { minLength: 0, maxLength: 3 }),
          fc.array(lowConfidencePatternArb, { minLength: 0, maxLength: 3 }),
        )
        .map(([v, l]) => [...v, ...l]),
    ),
...
describe("Property 13: 知识文档格式有效性", () => {
  ...
  it("generated documents always have valid frontmatter (Req 9.2, 9.3)", () => {
    fc.assert(
      fc.property(
        ...
        bodyArb,
        (title, tags, date, rawConfidence, body) => {
  ...
  it("frontmatter contains all required fields (Req 9.2)", () => {
    fc.assert(
      fc.property(validDocumentArb, (doc) => {
...
describe("Property 14: 知识库维护不变量", () => {
  it("after maintenance, doc count ≤ limit (Req 9.4)", () => {
    fc.assert(
      fc.property(anyKnowledgeBaseStateArb, (state) => {
  ...
  it("over-limit state is trimmed to exactly the limit (Req 9.4)", () => {
    fc.assert(
      fc.property(overLimitStateArb, (state) => {
        const result = maintainKnowledgeBase(state);
  ...
  it("low-confidence patterns are removed during maintenance (Req 9.5)", () => {
    fc.assert(
      fc.property(stateWithLowConfidencePatternsArb, (state) => {
        const lowCount = state.instinctPatterns.filter(
          (p) => p.confidenceScore < MIN_CONFIDENCE,
        ).length;
  ...
  it("within-limits state is unchanged after maintenance (Req 9.4, 9.5)", () => {
    fc.assert(
      fc.property(withinLimitsStateArb, (state) => {
        const result = maintainKnowledgeBase(state);
    ...
    fc.assert(
      fc.property(overLimitStateArb, (state) => {
        ...
        if (result.removedDocuments.length > 0 && result.documents.length > 0) {
          const maxRemovedConfidence = Math.max(
            ...result.removedDocuments.map((d) => d.frontmatter.confidence),
          );
          const minKeptConfidence = Math.min(
            ...result.documents.map((d) => d.frontmatter.confidence),
          );
