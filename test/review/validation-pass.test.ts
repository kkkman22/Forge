...
import type { MergedFinding } from "../../src/review/types.js";
import {
  applyValidationBatch,
  applyValidationResult,
  serializeValidationRecord,
  type ValidationResult,
} from "../../src/review/validation-pass.js";
...
const confirmed: ValidationResult = { confirmed: true, reason: "holds", adjusted_confidence: 100 };
...
describe("serializeValidationRecord (R5.8 — jsonl)", () => {
  it("produces a single-line JSON record with before/after severity", () => {
    const v = applyValidationResult(
      merged({ severity: "P0", filePath: "x.ts", lineNumber: 5 }),
      rejected,
    );
