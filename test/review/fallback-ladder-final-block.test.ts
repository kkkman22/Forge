...
function inv(agentType: string): SubagentInvocation {
  return { agentType, prompt: "...", permissionMode: "default", maxTurns: 10 };
}

function preamble(_agentType: string): string {
...
function validReport(layer: number, reviewer: string): string {
...
describe("runReviewFallbackLadder × final-report contract", () => {
  ...
  it("treats SDK-success-but-no-final-block as L0 failure and falls through to L1", async () => {
    ...
    mockedRunner.mockImplementation(
      (invs: SubagentInvocation[], _exec: unknown, _concurrency: number) => {
        call++;
        if (call === 1) {
    ...
    const executor = async (i: SubagentInvocation): Promise<SubagentResult> => ({
      agentType: i.agentType,
      status: "success",
