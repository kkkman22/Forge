...
import { scanInput, type ThreatType } from "../src/prompt-defense.js";

// ---------------------------------------------------------------------------
// Benign sample corpus (≥ 100)
// ---------------------------------------------------------------------------

const BENIGN_SAMPLES: ReadonlyArray<string> = [
...
// ---------------------------------------------------------------------------
// Malicious sample corpus (≥ 50) with expected category
// ---------------------------------------------------------------------------

interface MaliciousSample {
  text: string;
  expectedType: ThreatType;
}

const MALICIOUS_SAMPLES: ReadonlyArray<MaliciousSample> = [
...
describe("scanInput — benign corpus", () => {
  ...
  it("marks every benign sample as safe", () => {
    const offenders: Array<{ sample: string; ids: string[] }> = [];
...
describe("scanInput — malicious corpus", () => {
  ...
  it("flags every malicious sample with the expected type", () => {
    const offenders: Array<{ sample: string; expected: ThreatType; detected: string[] }> = [];
    ...
    if (offenders.length > 0) {
      const preview = offenders
        ...
        .map(
          (o) =>
            `  expected=${o.expected} detected=[${o.detected.join(", ")}] sample="${o.sample.slice(0, 70)}"`,
        )
        .join("\n");
...
describe("scanInput — fuzzing", () => {
  ...
  it("never throws on arbitrary Unicode inputs", () => {
    fc.assert(
      fc.property(fc.stringMatching(/^[\s\S]{0,2000}$/), (input) => {
        expect(() => scanInput(input)).not.toThrow();
      }),
      { numRuns: 50 },
    );
...
describe("scanInput — PII echo protection", () => {
  ...
  const PII_TEMPLATES: ReadonlyArray<(sentinel: string) => string> = [
  ...
  it("never echoes the sentinel token in the scan result", () => {
    fc.assert(
      fc.property(
        fc.integer({ min: 0, max: PII_TEMPLATES.length - 1 }),
        fc.stringMatching(/^[a-z0-9]{8,16}$/),
        (templateIdx, sentinel) => {
