...
const agentOutputBaseArb: fc.Arbitrary<AgentOutput> = fc.record({
  success: fc.boolean(),
  summary: fc.string(),
...
const agentOutputArb: fc.Arbitrary<AgentOutput> = fc.oneof(
  agentOutputBaseArb,
  fc.record({
    success: fc.boolean(),
    summary: fc.string(),
...
const invalidAgentOutputArb: fc.Arbitrary<unknown> = fc.oneof(
...
describe("Feature: gnhf-inspired-enhancements, Property 9: AgentOutput 验证正确性", () => {
  ...
  it("valid AgentOutput objects pass validation (valid: true)", () => {
    fc.assert(
      fc.property(agentOutputArb, (output) => {
  ...
  it("objects missing required fields return validation errors (valid: false)", () => {
    fc.assert(
      fc.property(invalidAgentOutputArb, (data) => {
