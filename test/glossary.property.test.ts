...
const termArb: fc.Arbitrary<GlossaryTerm> = fc
  .record({
    term: termNameArb,
  ...
  .map((t) => {
    ...
    if (t.aliases !== undefined) out.aliases = t.aliases;
    if (t.source_session !== undefined) out.source_session = t.source_session;
    return out;
  });
 ...
 */
const glossaryArb: fc.Arbitrary<Glossary> = fc
  .record({
  ...
  .map((g) => {
    ...
    const out: Glossary = { schema_version: g.schema_version, updated: g.updated, terms };
 ...
 */
function buildGlossary(terms: GlossaryTerm[]): Glossary {
...
describe("glossary — findTerm", () => {
  ...
  it("resolves both canonical name and any alias case-insensitively", () => {
     ...
     */
    const matchesQuery = (term: GlossaryTerm, query: string): boolean => {
      const needle = query.toLowerCase().trim();
      if (term.term.toLowerCase().trim() === needle) return true;
        ...
        for (const term of g.terms) {
          ...
          if (term.aliases !== undefined) {
            for (const alias of term.aliases) {
              ...
              if (hit !== null) expect(matchesQuery(hit, alias)).toBe(true);
            }
          }
        }
      }),
...
describe("glossary — detectConflict", () => {
  ...
  it("flags same name + different definition as a conflict", () => {
    ...
    fc.assert(
      fc.property(glossaryArb, candidateArb, definitionArb, (g, candidate, altDefinition) => {
        ...
        if (existing.definition.trim() === candidate.definition.trim()) return;
        if (
          g.terms.some((t) => t.term.toLowerCase().trim() === candidate.term.toLowerCase().trim())
        ) {
          return;
        }

        const withExisting: Glossary = { ...g, terms: [existing, ...g.terms] };
        const result: ConflictResult = detectConflict(withExisting, candidate);
...
describe("glossary — mergeTerm", () => {
  ...
  it("is idempotent under append, replace, and add_alias", () => {
    ...
    fc.assert(
      fc.property(glossaryArb, termArb, strategyArb, (g, candidate, strategy) => {
...
describe("glossary — findStaleterms", () => {
  ...
  it("returns a subset of input terms in input order", () => {
    fc.assert(
      fc.property(glossaryArb, dateArb, fc.integer({ min: 0, max: 365 }), (g, nowIso, maxAge) => {
