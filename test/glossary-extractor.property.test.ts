...
import {
  ...
  filterCandidates,
  type TermCandidate,
} from "../src/glossary-extractor.js";
 ...
 */
const candidateArb: fc.Arbitrary<TermCandidate> = fc.record({
  term: fc
...
describe("glossary-extractor — extractCandidates", () => {
  ...
  it("never throws for any string input", () => {
    fc.assert(
      fc.property(fc.string(), fc.array(fc.string(), { maxLength: 10 }), (text, existing) => {
...
describe("glossary-extractor — filterCandidates", () => {
  ...
  it("stricter rules produce a subset of looser rules' output", () => {
    ...
    fc.assert(
      fc.property(
        ...
        fc.integer({ min: 1, max: 3 }),
        fc.integer({ min: 1, max: 3 }),
        fc.array(extraPatternArb, { maxLength: 2 }),
        (candidates, minFreqLoose, minFreqDelta, minLenLoose, minLenDelta, extraPatterns) => {
          const cap = candidates.length + 10; // non-binding on both sides

          const loose: ExtractionRules = {
          ...
          const looseKey = (c: TermCandidate) => `${c.term.toLowerCase()}\u0000${c.frequency}`;
  ...
  it("caps the result at maxCandidatesPerSession", () => {
    const cs: TermCandidate[] = Array.from({ length: 15 }, (_, i) => ({
...
describe("glossary-extractor — DEFAULT_EXTRACTION_RULES", () => {
  it("encodes the documented defaults", () => {
    ...
    const matchesAny = (s: string) =>
      DEFAULT_EXTRACTION_RULES.excludePatterns.some((p) => p.test(s));
