...
const GLOSSARY_WITH_SPEC: Glossary = {
  schema_version: 1,
  updated: "2026-05-05",
  terms: [
 ...
 * filtered out.
 */
function buildResolvedTree(glossary: Glossary): DecisionTree {
  const base = generateDecisionTree(
    "Lock the spec for a new Event Sourcing pipeline.",
    glossary,
    FIXED_NOW,
  );
  const withFunctionality = applyAnswer(
    base,
    "functionality-1",
    "We use Event Sourcing to persist audit trails. Event Sourcing keeps history.",
    FIXED_NOW,
  );
  const withBoundary = applyAnswer(
    withFunctionality,
    "boundary-1",
    "Out of scope: EventSourcing replay tooling and EventSourcing projections.",
    FIXED_NOW,
  );
  return withBoundary;
}
...
function collectAllIds(tree: DecisionTree): string[] {
  const out: string[] = [];
...
describe("extractNewGlossaryCandidates", () => {
  ...
  it("skips glossary aliases too", () => {
    const treeText = generateDecisionTree(
      "We need 档位 routing for 档位 rules.",
      GLOSSARY_WITH_SPEC,
      FIXED_NOW,
    );
...
describe("renderGrillFindings", () => {
  ...
  it("uses the first non-empty line of rootDescription as the H1 title", () => {
    ...
    expect(
      output.startsWith("# Grill Findings: Lock the spec for a new Event Sourcing pipeline."),
    ).toBe(true);
  });
  ...
  it("Q&A Pairs section shows only resolved nodes", () => {
    ...
    expect(qaSection).not.toContain(
      "- Q: Which existing modules or external services must this coordinate with?",
    );
  });
  ...
  it("emits `none` under New Glossary Candidates when the list is empty", () => {
    ...
    const resolved = collectAllIds(tree).reduce(
      (acc, id) => applyAnswer(acc, id, "lowercase answer only", FIXED_NOW),
      tree,
    );
 ...
 * adjacent sections.
 */
function extractSection(output: string, start: string, end: string): string {
