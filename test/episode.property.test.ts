...
const tierArb: fc.Arbitrary<EpisodeTier> = fc.constantFrom("light", "standard", "full");
const outcomeArb: fc.Arbitrary<EpisodeOutcome> = fc.constantFrom("success", "partial", "failure");
...
const episodeV2Arb: fc.Arbitrary<Episode> = fc
  .record({
  ...
  .map((e) => {
    ...
    if (e.root_cause !== undefined) out.root_cause = e.root_cause;
    if (e.solution !== undefined) out.solution = e.solution;
    if (e.user_rating !== undefined) out.user_rating = e.user_rating;
    if (e.related_pattern !== undefined) out.related_pattern = e.related_pattern;
...
describe("episode — generateEpisodeId", () => {
  ...
  it("is idempotent for any (date, seq) pair", () => {
    fc.assert(
      fc.property(dateArb, seqArb, (date, seq) => {
