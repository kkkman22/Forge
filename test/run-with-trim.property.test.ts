 ...
 */
function runWithTrim(exitCode: number, lines: string[]): { exitCode: number; stdout: string } {
...
`;
  ...
  try {
    ...
    const execError = error as { stdout?: Buffer; status?: number };
...
describe("Property 1: Exit code preservation", () => {
  it("preserves the original exit code for any command (Req 3.1, 3.5)", () => {
    fc.assert(
      fc.property(exitCodeArb, linesArb, (exitCode, lines) => {
...
describe("Property 2: Success output truncation", () => {
  it("truncates output when >30 lines and exit=0 (Req 3.2, 3.3)", () => {
    fc.assert(
      fc.property(fc.array(lineArb, { minLength: 31, maxLength: 100 }), (lines) => {
  ...
  it("passes through full output unchanged on non-zero exit (Req 3.4)", () => {
    fc.assert(
      fc.property(
        ...
        fc.array(lineArb, { minLength: 1, maxLength: 100 }),
        (exitCode, lines) => {
          const result = runWithTrim(exitCode, lines);
