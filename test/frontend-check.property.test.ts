...
import { detectTierAvailability, type TierAvailability } from "../src/frontend-check.js";

const VALID_B_VALUES = ["preferred", "degraded", "unavailable"] as const;
const VALID_C_VALUES = ["available", "unavailable"] as const;
...
describe("detectTierAvailability — unit", () => {
  it("all available → preferred B + available C", () => {
    ...
    expect(result).toEqual<TierAvailability>({
      a: true,
      b: "preferred",
      c: "available",
      reasons: {
