...
function specWith(body: string, frontmatter = ""): string {
  const fm = frontmatter ? `---\n${frontmatter}\n---\n` : "";
  return `${fm}# Spec\n\n## Acceptance Criteria\n\n${body}\n`;
}

function ac(id: string, verifyBy: string, evidence = "src/foo.ts"): string {
...
describe("validateVerifyBy — field-level pure function", () => {
  it.each([
    ...
    "forge_exec:e2e",
    "manual",
  ] as const)("accepts legal value '%s'", (value) => {
...
describe("validateContract — Evidence existence enforcement (AC7)", () => {
  ...
  it("blocks when Evidence file does not exist on disk (AC7)", () => {
    ...
    expect(result.valid).toBe(false);
    expect(result.errors.some((e) => /Evidence.*not found/i.test(e) && e.includes("1.1"))).toBe(
      true,
    );
  });
...
const _layerCheck = null as unknown as Layer;
void _layerCheck;
