  ...
  describe("getDescendants", () => {
    ...
    it("finds direct children via pgrep", async () => {
      mockExecFileSync.mockImplementation((cmd: string, args: string[]) => {
        ...
        if (cmd === "pgrep" && args[1] === "101") throw new Error("no children");
        if (cmd === "pgrep" && args[1] === "102") throw new Error("no children");
        if (cmd === "ps" && args[1] === "101") return "node";
        if (cmd === "ps" && args[1] === "102") return "vitest";
        return "";
      });
