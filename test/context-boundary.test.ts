...
describe("resolveFileContext", () => {
  ...
  it("matches file path by directory prefix in ownership map", () => {
    expect(
      resolveFileContext("src/domain/reservation/services/booking-service.ts", ownershipMap, null),
    ).toBe("reservations");
  });

  it("matches another context correctly", () => {
    expect(resolveFileContext("src/domain/guest/models/guest.ts", ownershipMap, null)).toBe(
      "guest-management",
    );
  });
  ...
  it("JSDoc context overrides ownership map", () => {
    ...
    expect(
      resolveFileContext("src/domain/guest/handlers/checkin.ts", ownershipMap, "reservations"),
    ).toBe("reservations");
  });
