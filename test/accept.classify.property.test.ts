...
import { classifyScenarioTypeWithMeta, type Scenario, type ScenarioType } from "../src/accept.js";

function scenario(over: Partial<Scenario> & { verifyBy?: string }): Scenario {
  return {
    id: "s",
    given: "",
    when: "",
...
describe("classifyScenarioType — property (always returns valid type)", () => {
  ...
  function scenarioArb(): fc.Arbitrary<Scenario & { verifyBy?: string }> {
    return fc.record({
  ...
  it("Verify-By scenarios never produce 'mixed'", () => {
    fc.assert(
      fc.property(scenarioArb(), (s) => {
        if (
          s.verifyBy &&
          ["vitest:unit", "vitest:component", "bash:contract", "forge_exec:e2e"].includes(
            s.verifyBy,
          )
        ) {
          expect(classifyScenarioTypeWithMeta(s).type).not.toBe("mixed");
