...
let _mockChild: MockChildProcess | null = null;
...
function makeMockChild(): {
  ...
  child.on = vi.fn((event: string, cb: (code?: number) => void) => {
  ...
  return {
    child,
    emitExit: (code: number) => child.emit("exit", code),
  };
}

describe("CliSubprocessDriver stuck timer + signal_chain.jsonl", () => {
  let runDir: string;
  let mockSpawnResult: ReturnType<typeof makeMockChild>;
  ...
  it("AC 1.6: CliDriverConfig accepts stuckTimeoutMs optional field", () => {
    ...
    const configWithField: CliDriverConfig = {
      cwd: "/tmp",
      runId: "r1",
      runDir: "/tmp/r1",
      permissionMode: "bypassPermissions",
    ...
    const configWithoutField: CliDriverConfig = {
      cwd: "/tmp",
      runId: "r2",
      runDir: "/tmp/r2",
      permissionMode: "bypassPermissions",
