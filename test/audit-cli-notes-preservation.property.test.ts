 ...
 */
const emptyEntriesDocArb: fc.Arbitrary<NotesDocument> = runIdArb.map((runId) => ({
  runId,
  entries: [],
}));
 ...
 */
const notesDocumentWithBranchArb: fc.Arbitrary<NotesDocument> = fc
  .tuple(
    runIdArb,
    fc.option(cleanLineArb, { nil: undefined }),
    fc.array(roundTripEntryArb, { minLength: 0, maxLength: 8 }),
  )
  .map(([runId, branchName, entries]) =>
    branchName !== undefined ? { runId, branchName, entries } : { runId, entries },
  );
...
describe("Preservation Property 2a: Empty entries persist as empty string", () => {
  ...
  it("persist expression returns empty string for all documents with zero entries", () => {
    fc.assert(
      fc.property(emptyEntriesDocArb, (doc) => {
...
describe("Preservation Property 2b: formatNotesDocument round-trip", () => {
  ...
  it("parseNotesDocument(formatNotesDocument(doc)) round-trips branchName", () => {
    fc.assert(
      fc.property(notesDocumentWithBranchArb, (doc) => {
        const markdown = formatNotesDocument(doc);
        const parsed = parseNotesDocument(markdown);
