...
function parseAgentFile(relPath: string): AgentFrontmatter {
...
describe("Contract: review subagent Turn Budget Discipline (Stage 1 — spec-check)", () => {
  ...
  it("spec-check.md preserves Step 0 forge_git IRON-LAW (regression guard)", () => {
    ...
    expect(
      body,
      "Step 0 forge_git IRON-LAW must remain after Turn Budget Discipline insertion",
    ).toContain('forge_git(subcommand="diff-content")');
  });
...
describe("Contract: codex toml integrity (Stage 2)", () => {
  ...
  for (const tomlPath of TOML_FILES) {
    it(`${tomlPath} developer_instructions contains Turn Budget Discipline IRON-LAW segment`, () => {
      ...
      expect(
        content,
        "Codex toml must include the same Turn Budget Discipline IRON-LAW as the .claude/agents/*.md counterpart",
      ).toContain("## Turn Budget Discipline");
    ...
    it(`${tomlPath} developer_instructions contains layer-specific Final Report header`, () => {
      ...
      expect(
        content,
        `Codex toml must reference ${expectedLayer} in the Final-Report Block contract`,
      ).toContain(expectedLayer);
    });
  }
  ...
  it(".codex/agents/spec-check.toml is a known Out-of-Scope absence (followup spec codex-review-parity)", () => {
    ...
    expect(
      exists,
      "If .codex/agents/spec-check.toml is added, extend agent-prompt-discipline.test.ts to cover it.",
    ).toBe(false);
  });
});
