...
- **<!-- ssot:begin topic=commands render=count -->37<!-- ssot:end topic=commands --> 个命令**覆盖从需求分析到代码交付的完整周期
...
| 命令速查 | [docs/reference-commands.md](docs/reference-commands.md) | 查看全部 <!-- ssot:begin topic=commands render=count -->37<!-- ssot:end topic=commands --> 个命令和路由详解 |
...
**技术栈**：TypeScript 5.9（strict）、301 个 TypeScript 模块、Vitest 4.1、fast-check 4.7（属性测试）、Biome 2.4（lint + format）。运行时依赖：`@anthropic-ai/claude-agent-sdk`、`@modelcontextprotocol/sdk`、`commander`、`minimatch`、`yaml`、`zod`。

**测试策略**：8101 个测试（663 个测试文件）验证不变量。覆盖率 ~87% statements。
