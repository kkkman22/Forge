...
├── src/                         # 核心逻辑（134 个 TypeScript 模块，含纯函数模块及有状态/运行时模块：CLI、SDK 适配器、副作用执行器、运行管理器等）<!--exact: 134 个 TypeScript 模块-->
...
**测试策略**：4691 个测试（267 个测试文件，其中 132 个为 fast-check 属性测试文件）验证不变量（invariant），而非特定输入输出。覆盖率 ~89% statements。<!--exact: 测试数、文件数、属性测试数; approximate: 覆盖率-->
