---
description: "Use when user says zoom out, gets lost in details during skill execution, or runs `/forge zoom-out`"
context: fork
...
- Must restore the original skill phase after zoom-out completes (user says `continue`/`继续`, or next `/forge` command auto-resumes).
...
| User never says continue | Next `/forge` command auto-resumes before dispatch |
