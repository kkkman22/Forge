...
`/forge learn`（默认提取模式）每次开发后**只增不减**——knowledge 文件会持续增长腐化。`--deep` 是收敛动作：定期跑（建议每 7 天，或 R5 cron 自动触发），对账真实轨迹、去重、压缩、验证、prune。
...
[可选] Workflow candidates: 如发现重复工作流，建议跑 /forge distill（本 spec 范围外）
```
