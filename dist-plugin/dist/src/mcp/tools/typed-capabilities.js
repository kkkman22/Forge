...
import { queryEvidenceArtifacts } from "../../evidence-artifact.js";
...
export function validateTypedCapabilityOutput(toolName, value) {
    return TypedCapabilityOutputSchemas[toolName].safeParse(value);
}
export function preferredTypedCapabilitiesForConsumer(consumer) {
    return [...ConsumerTypedCapabilityPreferences[consumer]];
}
export function registerTypedCapabilityTools(server, root) {
    const projectRoot = root?.path ?? process.cwd();
    const registerTool = (name, config, handler) => server.registerTool.call(server, name, config, handler);
    registerTool("forge_check_command", {
    ...
    }, async ({ profile }) => jsonToolResult("forge_check_command", await runCheckProfile(projectRoot, String(profile))));
    registerTool("forge_diff_summary", {
        description: "Return a structured git diff summary.",
        inputSchema: {},
    }, async () => jsonToolResult("forge_diff_summary", await diffSummary(projectRoot)));
    registerTool("forge_dist_sync", {
    ...
    }, async (input) => jsonToolResult("forge_artifact_query", {
        schema_version: 1,
...
async function runCheckProfile(projectRoot, profile) {
    const command = profile === "typecheck"
        ? "npm run typecheck"
        : profile === "test"
            ? "npm test"
            : profile === "docs"
...
async function diffSummary(projectRoot) {
...
async function distSyncStatus(projectRoot) {
    ...
    return {
        schema_version: 1,
        status: summary.modified.count + summary.staged.count + summary.untracked.count === 0
            ? "pass"
            : "fail",
        summary,
    };
}
async function docsDriftStatus(projectRoot) {
...
function jsonToolResult(toolName, value) {
...
//# sourceMappingURL=typed-capabilities.js.map