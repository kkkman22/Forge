...
async function gracefulShutdown(signal) {
    ...
    try {
        ...
        if (registry.size() > 0) {
            ...
            console.error(`[forge-context] Process registry cleanup: terminated=${cleanupResult.terminated} forcedKill=${cleanupResult.forcedKill} alreadyExited=${cleanupResult.alreadyExited} errors=${cleanupResult.errors.length}`);
        }
        await server.close();
