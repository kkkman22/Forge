...
**根因**:R3.AC2 要求"指针由 plan approve 或 build 启动时设置,阶段切换时更新 phase"。`tryReadActivePlanPointer` 读取器实现了且实质有效,但**整个仓库没有任何写入者**。`.tinkerman/state/` 在生产 worktree 为空。生产中每次运行都走 legacy mtime 回退,指针机制永不触发。validation.md 和 VAL-R3-002 却标记 PASS。
