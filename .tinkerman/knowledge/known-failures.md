...
retention：>100 条触发自动归档到 `.tinkerman/archive/known-failures-<date>.md`，保留最新 80 条。
...
- pattern_id: security-control-stub-returns-ok-true
  ...
  source_review: ".tinkerman/reviews/forge-single-entry-skills-collapse.md (P1-S1, P1-S2)"
...
- pattern_id: mock-content-used-in-production-path
  ...
  source_review: ".tinkerman/reviews/forge-single-entry-skills-collapse.md (P1-S1)"
...
- pattern_id: spec-skill-doc-skeleton-incomplete
  ...
  source_review: ".tinkerman/reviews/missions-inspired-rigor-audit-fixes.md"
...
- pattern_id: build-handoff-not-consumed-after-impl
  ...
  first_seen_commit: 448916f39b4af374d9c8ffa165c0113cb66faffa
  last_seen_commit: 448916f39b4af374d9c8ffa165c0113cb66faffa
  signature: "实现 schema/parser 完整且单元测试 100% 绿，但 SKILL 部署后 .tinkerman/progress/*.md 持续 0 条真实 handoff block"
...
- pattern_id: ci-check-command-frontmatter-drift
  ...
  signature: "templates/config.md frontmatter 已声明 ci_check_command 字段，但仓库 .tinkerman/config.md 未补齐该字段；forge-test SKILL Layer 3 因此走逐项回退分支，AI 自拼 typecheck/lint/test 三件套，漏掉 dist-sync、check-doc-structure、validate-skill-* 等十余条校验，推送后 GitHub CI 才暴露失败"
  fix_required: "1) 验证命令 grep '^ci_check_command:' .tinkerman/config.md 必须有输出且值与 package.json scripts.check 一致；2) 缺失时 forge-test SKILL 应通过 detectCiCommandDrift 主动检测并降级到 npm run check（Spec local-ci-parity Req 2 已实现）；3) forge init 在检测到 package.json scripts.check 时应主动建议默认值（Req 4 已实现）。"
  ...
  verification_command: "grep '^ci_check_command:' .tinkerman/config.md"
...
- pattern_id: skill-function-defined-without-production-caller
  ...
  source_review: ".tinkerman/reviews/forge-kiro-style-spec-workflow-round2.md, forge-kiro-style-spec-workflow-round3.md"
...
- pattern_id: ears-rewrite-identity-wrap-defeats-anl-01
  ...
  source_review: ".tinkerman/reviews/forge-kiro-style-spec-workflow-round2.md (P0-4)"
...
- pattern_id: type-broken-fixture-passes-tests-fails-typecheck
  ...
  source_review: ".tinkerman/reviews/forge-kiro-style-spec-workflow-round3.md (§4.1)"
...
- pattern_id: double-implementation-only-rename
  ...
  source_review: ".tinkerman/reviews/forge-kiro-style-spec-workflow-round3.md (P0-11)"
...
- pattern_id: rollback-uses-derived-path-not-recorded-path
  ...
  source_review: ".tinkerman/reviews/forge-kiro-style-spec-workflow-round4.md (P2-B)"
...
- pattern_id: dangling-exports-subpath-after-source-deletion
  ...
  source_review: ".tinkerman/reviews/code-slim-0612.md (P0-S1)"
...
- pattern_id: concurrent-session-git-collision
  ...
  source_review: ".tinkerman/progress/code-slim-0612.md T6 R2 handoff (session-collision note)"
