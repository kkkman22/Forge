...
supersedes: []
related_specs:
  - .tinkerman/specs/agentic-acceptance/  (to be created)
---
...
- **`/forge accept` 命令已存在**（`skills/forge/lib/accept/instructions.md`），定位为「行为验收、跑运行时、可阻断 ship」，输出到 `.tinkerman/acceptance/<topic>/`。与 `/forge verify`（三态证据判定）、`/forge test`（单测）、`/forge review`（质量审计）已明确区分。**本任务是增强现有命令，非新建。**
...
- **P0-1 凭据不进 git**：spec 用占位符 `{{FORGE_E2E_PASSWORD}}`，运行时从 `.tinkerman/secrets.env`(0600, 已 gitignore) 注入。新增证据目录 `.tinkerman/acceptance/` 显式加入 `.gitignore`。
...
- **P1-3 供应链 pin**：agent-browser 版本 SHA256 写入 `.tinkerman/config.md`，禁止自动升级。
...
## Next
→ `/forge spec`：将 D1-D6 落为功能规格 `.tinkerman/specs/agentic-acceptance/`。
