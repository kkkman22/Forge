...
The three new runners do **not** start a browser or hit a real API. They are thin shells that call `forge_exec` against the **project's own** test command (the one named in `.tinkerman/config.md` `ci_check_command`, or `npm run test:unit` / `test:component` / `test:contract` by convention), scoped to the AC's `Evidence` file. They return `{ok, reason, stdout_tail}` and let `aggregateVerdicts` do the rest.
