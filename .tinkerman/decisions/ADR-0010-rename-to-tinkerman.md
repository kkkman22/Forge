...
| 状态目录 | `.tinkerman/` | `.tinkerman/` | **高**（所有 spec/progress/decisions/knowledge 路径 + scripts 硬编码） |
...
1. **`.tinkerman/` → `.tinkerman/` 是最大风险点**。建议：迁移脚本（读旧路径写新路径 + 校验）+ **兼容期**（双路径读取，旧 `.tinkerman/` 软链或 fallback），N 个版本后移除兼容。`init.sh` / 所有读 `.tinkerman/` 的 scripts 须同步。
...
5. **分批执行**：包名 + plugin 元数据 → 命令 + scripts → 目录迁移（`.tinkerman/`）→ 文档全文 → marketplace / repo。每批 `pre-push-ci-check.sh` + `check-dist-sync` 验证。
...
- `.tinkerman/` → `.tinkerman/` 若处理不当，破坏既有 spec / progress / decisions / knowledge 的连续性（组织记忆丢失 = 违反 ADR-0009 保留项 #4）
