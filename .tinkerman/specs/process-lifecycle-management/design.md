...
sequenceDiagram
    participant CLI as forge-loop-cli
    participant PIDFiles as .tinkerman/.pids/
...
**设计决策：**
- PID 文件存储在 `.tinkerman/.pids/` 目录，文件名格式 `session-<sessionId>.pid`
...
| 场景 | 处理策略 |
|------|----------|
| PID 文件目录不存在 | 自动创建 `.tinkerman/.pids/` 目录 |
