...
created: 2026-06-29
tier: full
spec_ref: ".tinkerman/specs/domain-knowledge-threading/requirements.md REQ-6 (deferred to slice C)"
---
...
`.tinkerman/glossary.md`: the pack's domain terms (e.g. PMS's "Reservation"/"Booking")
...
Source: slice B spec `.tinkerman/specs/domain-knowledge-threading/requirements.md` REQ-6.

## Glossary

- **Flat glossary**: `.tinkerman/glossary.md` — the single authoritative, writable
...
2. THE loader SHALL compose: the flat-glossary load (`ensureGlossaryExists`
   *semantics* — read `.tinkerman/glossary.md`; seed the 12 core terms if absent) →
...
3. THE wiring SHALL preserve the **flat write-sovereignty**: pack terms are
   read-only supplements; the flat `.tinkerman/glossary.md` remains the only file
...
- **INV-4 (Backward compat)**: flat `.tinkerman/glossary.md` stays the write-sovereignty
...
- **INV-5 (Flat sovereignty)**: pack terms are NEVER written back to
  `.tinkerman/glossary.md`; on name/alias collision, the flat definition wins (pack does
  not override). `packTermCount` counts only genuinely-appended (non-covered) terms.

## Out of Scope

- Migrating the flat `.tinkerman/glossary.md` to the layered model (flat stays primary).
