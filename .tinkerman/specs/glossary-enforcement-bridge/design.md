...
`.tinkerman/glossary.md` only). This slice reunifies them: enforcement now sees pack
...
```
              ...
              (flat, authoritative)              ← src/glossary/merge.ts (NEW)
                           │                     │
                  .tinkerman/glossary.md      loadGlossary(enabledPacks, fs)
                                                  │
...
- `skills/forge/lib/{spec,decide,plan,grill,build,review}/references/function-contracts.md`:
  change "`glossary` — Parsed `.tinkerman/glossary.md` structure" → "`glossary` —
  `loadEnforcementGlossary(rootDir, fs)` result (flat `.tinkerman/glossary.md` + enabled
...
- Add a one-line note: pack terms are read-only supplements; the flat file remains
  the only write target (`/forge learn` writes back to `.tinkerman/glossary.md`, never to
  packs).
