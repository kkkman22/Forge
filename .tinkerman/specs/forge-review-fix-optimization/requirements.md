...
- **Backlog_File**: A persistent file (`.tinkerman/backlog.md`) that captures P2/P3 findings not addressed in the current release cycle.
- **Knowledge_Base**: The collection of files under `.tinkerman/knowledge/` that store accumulated project experience, including `instincts.md`, `known-failures.md`, `evolved-rules.md`, and `metrics.md`.
- **Status_File**: The file `.tinkerman/status.md` that tracks the current task, tier, and phase of a Forge workflow execution.
...
- **CI_Check_Command**: The project-defined command(s) in `.tinkerman/config.md` that run the full CI validation suite.
...
1. WHEN a review subagent completes its analysis, THE Review_Summarizer SHALL write the full review report to `.tinkerman/reviews/<topic>-<reviewer>.md`.
...
1. WHEN `/forge ship` executes and unfixed P2 or P3 findings exist in the review reports, THE Backlog_Manager SHALL append those findings to `.tinkerman/backlog.md` with source review reference, severity, file path, and one-line description.
2. THE Backlog_Manager SHALL not create duplicate entries for findings already present in `.tinkerman/backlog.md`.
...
6. IF `.tinkerman/backlog.md` does not exist when a capture is triggered, THEN THE Backlog_Manager SHALL create the file with a standard header.
...
2. THE Knowledge_Extractor SHALL write extracted knowledge to `.tinkerman/knowledge/solutions/<topic>.md` with structured fields for each dimension.
3. WHEN a knowledge entry's pattern has been observed 3 or more times, THE Knowledge_Extractor SHALL promote the pattern to `.tinkerman/knowledge/instincts.md` with a confidence score between 0.3 and 0.9.
...
5. THE Knowledge_Extractor SHALL update `.tinkerman/knowledge/metrics.md` with session statistics including command usage counts, routing accuracy, and execution quality metrics.
6. THE Knowledge_Extractor SHALL update `.tinkerman/knowledge/known-failures.md` when a debug session identifies a recurring failure pattern.
...
6. THE Status_Manager SHALL maintain backward compatibility with the existing single-task `.tinkerman/status.md` format by migrating to the multi-task format on first multi-task use.
...
1. WHEN `/forge review` completes, THE P1_Fix_Checklist SHALL be created at `.tinkerman/reviews/<topic>-checklist.md` containing every P0 and P1 finding with status "unfixed".
...
**User Story:** As a developer using Forge, I want SKILL documents to direct the AI to read CI commands from `.tinkerman/config.md` rather than assembling commands independently, so that the full CI validation suite is always executed.

#### Acceptance Criteria

1. THE forge-build SKILL_Document SHALL instruct the AI to read the `ci_check_command` field from `.tinkerman/config.md` and execute those commands for final validation.
2. THE forge-test SKILL_Document SHALL instruct the AI to read the `ci_check_command` field from `.tinkerman/config.md` and execute those commands for the test verification checklist.
3. THE SKILL_Documents SHALL instruct the AI to fail the validation step if `.tinkerman/config.md` does not contain a `ci_check_command` field, rather than falling back to self-assembled commands.
4. WHEN `.tinkerman/config.md` specifies multiple CI commands, THE SKILL_Documents SHALL instruct the AI to execute all commands in the specified order and report the first failure.
5. THE forge-build SKILL_Document and forge-test SKILL_Document SHALL reference `.tinkerman/config.md` as the single source of truth for validation commands, with no hardcoded command alternatives.
...
2. THE Context_Budget_Manager SHALL write the budget report to `.tinkerman/knowledge/sessions/<date>-<topic>-budget.md`.
