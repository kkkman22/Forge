...
| 写入 `.tinkerman/` 文件 | 攻击者通过 issue 提供的"复现代码"包含注入 | 用户粘贴的代码片段中含 `<system>` 标签 |
...
**类型**：PreToolUse Hook
**触发**：Write / Edit 工具目标为 `.tinkerman/` 目录下的文件
...
**排除路径**（不扫描）：
- `.tinkerman/reviews/` — review 产物本身需要引用安全模式
- `.tinkerman/knowledge/` — 知识库内容
- Hook 源码文件自身
