...
## REQ-01: 测试隔离修复 — fallback-ladder 使用真实 .tinkerman/reviews/（P1）

**当** `test/review/fallback-ladder.test.ts` 的 `tempDir` 使用 `process.cwd()/.tinkerman/reviews/` **则** 真实评审文件在 `afterEach` 的 `rmSync` 中被删除。

**当** 修复后运行 `npx vitest run test/review/fallback-ladder.test.ts` **则** 该测试使用 `tmpdir()` 创建隔离目录，不触碰真实 `.tinkerman/reviews/`。

**当** 修复后运行 `npm test` **则** 已有的 `.tinkerman/reviews/*.md` 文件保持不变。

**Verify-By**: vitest
**Evidence**: 运行测试前后 `.tinkerman/reviews/` 目录内容不变；`grep -n "process.cwd()" test/review/fallback-ladder.test.ts` 无匹配

**Current State**: `test/review/fallback-ladder.test.ts:17` 使用 `join(process.cwd(), ".tinkerman", "reviews")`；`afterEach` (line 61) 执行 `rmSync(tempDir, { recursive: true, force: true })`。项目其他测试均正确使用 `tmpdir()`。
...
**当** 审计文件被篡改（内容被清空）**则** 下次 ship 可通过 `.tinkerman/findings/force-skip-review-*.md` 的存在性检测发现异常。
...
**当** review 后代码发生非 `.tinkerman/` 文件变更 **则** `checkShipGateWithFreshness()` 返回 `allowed: false`。

**当** review 后仅 `.tinkerman/` 目录内文件变更 **则** 视为 fresh，不阻断 ship。
...
**Verify-By**: vitest
**Evidence**: 测试覆盖 4 种 freshness 场景（无 commit → fresh，commit 一致 → fresh，仅 .tinkerman/ 变更 → fresh，非 .tinkerman/ 变更 → blocked）
...
- JSDoc 更新为 "Blocks ship when review is stale due to non-.tinkerman/ code changes"
- 保留 warning 信息在 `result.reasons` 中（含变更文件列表）

**明确不改变**: freshness 检测逻辑（`checkReviewFreshness()` 函数不变）、`.tinkerman/` 文件排除规则
