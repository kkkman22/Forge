...
| `.tinkerman/` 路径 | 103 个 ts 文件硬编码字面量 | 状态契约脆弱根源 |
...
| `.tinkerman/specs/` | 457 .md，无 status 一致性巡检 | status 漂移不可见 |
...
### REQ-03 `.tinkerman/` 路径常量化（降级版：局部常量，不建集中模块）

> **decide 重判**：不新建 `forge-paths.ts` + `resolveForgePath()`，服从 ADR-0008 #3"不新增 adapter 层"。

**方案**：在含 `path.join(..., ".tinkerman", ...)` 的模块顶部加**局部常量** `const FORGE_DIR = ".tinkerman"`，39 处 path.join 场景渐进替换。
**白名单（不替换）**：6+ 处 `.includes(".tinkerman/...")` / regex（子字符串语义，替换破坏行为）。
**sandbox deny 特例**：`sandbox-policy.ts` 的 `.tinkerman/sandbox.json` / `.sandbox-active.json` 若替换，**须加值等价单测**（断言字符串完全一致），防 deny 漂移→绕过。建议 sandbox-policy 这几处**不替换**（风险 > 收益），仅加注释。
...
- 扫描 `.tinkerman/specs/**/{requirements,design,tasks}.md` + 单文件 spec 的 frontmatter。
