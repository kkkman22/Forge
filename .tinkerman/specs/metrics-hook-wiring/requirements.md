...
- `.tinkerman/.metrics/` 目录**不存在**(亲自 `ls` 验证)——证实从未运行过采集。
...
- 让 `metrics-recorder.mjs` 在每次用户 prompt 提交时被调用,采集使用率数据到 `.tinkerman/.metrics/`。
...
- 不把 metrics 上报到外部服务——纯本地 `.tinkerman/.metrics/`。
...
### Requirement 2: .tinkerman/.metrics/ 目录与数据落地

**User Story:** As a maintainer, I want metrics data to land in a known location, so that aggregate-metrics can read it。

#### Acceptance Criteria

1. THE `metrics-recorder.mjs` SHALL 将采集结果写入 `.tinkerman/.metrics/`(目录不存在时自动创建)。
2. THE 数据文件命名/格式 SHALL 与 `aggregate-metrics.mjs` 的读取约定一致(查看 aggregate 脚本的输入路径假设,保持兼容)。
3. THE `.gitignore` SHALL 评估是否忽略 `.tinkerman/.metrics/`(建议忽略,避免把个人使用数据提交;若团队需要聚合统计,在 design.md 决策)。
...
2. THE hook SHALL 在 `.tinkerman/` 目录不存在(非 forge 项目)时静默跳过,不报错——避免污染非 forge 项目。
...
- [ ] 在 forge 项目中提交一次 `/forge status` 后,`.tinkerman/.metrics/` 出现数据文件
