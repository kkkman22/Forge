...
在 `.claude-plugin/plugin.json` 注册 `UserPromptSubmit` hook 调用已存在的 `scripts/metrics-recorder.mjs`,让使用率数据落到 `.tinkerman/.metrics/`。零行为变更,解锁 ROADMAP 所有 pending 的"基于使用率"评估。不修改采集逻辑,只接线。
...
### D4: .tinkerman/.metrics/ 忽略入 git

- **问题**:.tinkerman/.metrics/ 是否提交?
- **选择**:**加入 .gitignore**
...
- `.tinkerman/.metrics/<date>-commands.jsonl`(append-only,与 runs/ 事件流格式一致)
- 每行:`{ts, sub?, kind: "slash"|"natural"}`

## 数据模型

无 schema 变更。新增 `.tinkerman/.metrics/` 目录(运行时创建)。
...
| 非 forge 项目误触发 | metrics-recorder 检查 `.tinkerman/` 不存在则立即 exit 0 |
