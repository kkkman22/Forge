...
- [ ] 3. 验证 metrics-recorder.mjs 输出路径与 aggregate 兼容
  ...
  - metrics-recorder 写入 `.tinkerman/.metrics/<date>-commands.jsonl` 格式与 aggregate 读取一致
  - 只采 `/forge <sub>` 命令名,不存 prompt 全文(隐私)
  - _Requirements: 2.2, D3_

- [ ] 4. .gitignore 忽略 .tinkerman/.metrics/
  - `.gitignore` 新增 `.tinkerman/.metrics/`
  - _Requirements: 2.3_

- [ ] 5. metrics-recorder 幂等与性能保障
  - 顶层 try/catch,任何异常 → exit 0 + stderr 静默
  - `.tinkerman/` 不存在时立即 exit 0(非 forge 项目跳过)
  - append-only O_APPEND 写入
  - 实测执行 < 100ms
  - _Requirements: 3.1, 3.2, 3.3_

- [ ] 6. 端到端验证
  - forge 项目提交 `/forge status` 后 `.tinkerman/.metrics/` 出现数据
