...
```
        ...
        │   - write .tinkerman/decisions/<date>-*.md   │
        └─────────────────────┬───────────────────┘
                              │
           ┌──────┬───────────┼───────────┬──────┐
           ▼      ▼           ▼           ▼      ▼
...
**Observability**：每个 teammate 的 tmux pane 对用户实时可见。`TaskCompleted` hook 在每个 teammate 完成时触发，写入 `.tinkerman/runs/`。
      ...
      (existing impl)                  (new skill)
           ...
           └──────────────┬───────────────┘
                ...
                metrics JSON merged
                          │
                          ▼
            .tinkerman/runs/<ts>-decide-poc.md
...
2. 解析 topic 参数
3. 写 `.tinkerman/runs/<ts>-decide-teams-run.md` 的 `started_at` 段
4. `Agent(subagent_type="forge-decide-lead", prompt=<topic + metadata>)`
5. 等待 Agent 返回（带 20 分钟超时提示）
6. 追加 `.tinkerman/runs/<ts>-decide-teams-run.md` 的 `finished_at` 段
...
- Step 4：用固定模板合成 ADR
- Step 5：写入 `.tinkerman/decisions/<date>-<topic-slug>.md`
...
**文件**：`.tinkerman/decisions/<date>-agent-teams-poc.md`
...
**文件**：`.tinkerman/specs/forge-decide-agent-teams/poc-topics.md`
...
## B: 重构 config 系统
**Complexity**: medium
**Topic**: 把 `.tinkerman/config.md` 的 YAML frontmatter 拆成多个独立文件，保留向后兼容。

## C: 引入 plugin 系统
**Complexity**: complex
**Topic**: 为 Forge 设计 plugin 机制，允许第三方扩展 skill/agent/hook，同时保持 `.tinkerman/` 状态一致性。
```
...
| 所有 teammate 失败 | exit 1 | "全部 teammate 失败，请检查 `.tinkerman/runs/`" |
...
5. **归档条件**：PoC_Report committed 且至少 1 个 reviewer approved 后，spec 移入 `.tinkerman/archive/`。
