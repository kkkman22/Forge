...
templates/CLAUDE.md (stable)          templates/evolved-rules.md (new)
   ...
   forge init                              forge init
        │                                       │
        ▼                                       ▼
   CLAUDE.md                     .tinkerman/knowledge/evolved-rules.md
...
The template file that `forge init` copies to `.tinkerman/knowledge/evolved-rules.md`.
...
At session start, read `.tinkerman/knowledge/evolved-rules.md` and treat its rules as project-specific error-prevention directives. These rules are distilled from accumulated project knowledge and represent patterns where Claude would make mistakes without explicit guidance.
...
4. **Log** — Record the change in `.tinkerman/knowledge/rule-changelog.md`
...
```markdown
- `.tinkerman/knowledge/evolved-rules.md`（only updatable by `/forge learn` rule distillation, not deletable outside maintenance）
- `.tinkerman/knowledge/rule-changelog.md`（append-only — only new entries, no deletion of history）
```
...
{
  "type": "command",
  "command": "if [ -f .tinkerman/knowledge/evolved-rules.md ]; then echo '=== Evolved Rules ==='; cat .tinkerman/knowledge/evolved-rules.md; fi",
  "timeout": 5
}
```
...
{
  "type": "command",
  "command": "if [ -f .tinkerman/knowledge/evolved-rules.md ] && grep -q 'PENDING' .tinkerman/knowledge/evolved-rules.md 2>/dev/null; then count=$(grep -c 'PENDING' .tinkerman/knowledge/evolved-rules.md 2>/dev/null || echo 0); echo \"⚠️ 有 $count 条待审核的规则提案。运行 /forge learn 查看并审批。\"; fi"
}
```
...
- Session count is determined by counting entries in `.tinkerman/knowledge/sessions/` directory
