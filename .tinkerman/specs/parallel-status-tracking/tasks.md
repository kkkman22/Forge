...
layout: tasks
created: 2026-04-29
spec_ref: ".tinkerman/specs/parallel-status-tracking/requirements.md"
---
...
- [x] 1.2 实现 `resolveStatusPath(ctx: ResolverContext): ResolvedStatus`
  - 检测 `.tinkerman/status/` 目录是否存在来判断 single/multi 模式
  - single 模式返回 `.tinkerman/status.md`，multi 模式返回 `.tinkerman/status/<task-id>.md`
...
- [x] 2.2 实现 `readTaskStatus(io, forgeRoot, taskName): string`
  - 优先读取 `.tinkerman/status/<task-id>.md`，不存在则回退到 `.tinkerman/status.md`
  - 读取失败时返回空字符串（graceful degradation）
- [x] 2.3 实现 `writeTaskStatus(io, forgeRoot, taskName, content): void`
  - 多任务模式写入 `.tinkerman/status/<task-id>.md`（自动创建目录）
  - 单任务模式写入 `.tinkerman/status.md`
...
- [x] 3.1 实现 `migrateToMultiTask(io, forgeRoot): void`
  - 从 `.tinkerman/status.md` 读取 current_task → slugify → 创建 `.tinkerman/status/` 目录
  - 将原内容写入 `.tinkerman/status/<task-id>.md`
  - 清空 `.tinkerman/status.md`（保留空 frontmatter 标记）
...
- [x] 5.4 修改 `skills/forge-status/SKILL.md`
  - § 2 数据来源：添加 `.tinkerman/status/*.md` 扫描
  - § 3 输出格式：多任务时显示汇总表
...
- [x] 6.1 修改 `UserPromptSubmit` hook
  - 添加 `.tinkerman/status/` 目录检测，多任务模式下读取最近更新的任务文件
  - 单任务模式保持原有行为
- [x] 6.2 修改 `PostToolUse` hook
  - 条件改为 `if [ -d .tinkerman/status ] || [ -f .tinkerman/status.md ]`
...
- [x] 7.2 验证向后兼容：无 `.tinkerman/status/` 目录时所有行为与修改前一致
