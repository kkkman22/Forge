...
创建 SessionStart hook 脚本，在每次新会话启动时自动检测 `.tinkerman/` 目录中的活跃状态，向用户提醒未完成的工作。新增 1 个脚本文件 + hook 注册。
...
# - 知识库膨胀警告（.tinkerman/knowledge/ 文件数接近上限）
...
- 手动测试：创建 `.tinkerman/status.md` 含 `phase: build`，启动新会话确认提醒出现
