...
Runtime loader turning `.tinkerman/config.md` into a validated `EnabledPacks`.
...
/**
 * Read .tinkerman/config.md, discover packs, and return validated EnabledPacks.
...
| Case | Handling |
|------|----------|
| `.tinkerman/config.md` missing | `loadEnabledPacks` returns warning + empty enabled |
