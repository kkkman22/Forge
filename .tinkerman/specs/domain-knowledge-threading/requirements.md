...
created: 2026-06-29
tier: full
spec_ref: ".tinkerman/decisions/2026-06-27-domain-example-reference-impl.md §7 (slice B)"
---
...
is defined and unit-tested, but **no runtime path reads `.tinkerman/config.md`'s
`packs:` field and no decide/plan/build/review phase skill consumes any of it.**
The glossary check still reads the flat `.tinkerman/glossary.md`; `src/index.ts`
...
Source decision: `.tinkerman/decisions/2026-06-27-domain-example-reference-impl.md`
...
- **Enabled_Packs**: the resolved, validated set of packs a project has turned
  on, read from `.tinkerman/config.md` frontmatter `packs:` field. Represented by
...
- **Zero-Pack-Zero-Impact**: invariant — when no pack is enabled, Forge's
  ...
  flat `.tinkerman/glossary.md` remains the sole glossary source when `packs:` is
...
`.tinkerman/config.md` from disk, discovers packs, and returns a validated
...
2. `loadEnabledPacks` SHALL compose the existing pure functions:
   ...
   It SHALL read `.tinkerman/config.md` via the injected `FileSystem`.
3. WHEN `.tinkerman/config.md` has no `packs:` field, `loadEnabledPacks` SHALL
   return `enabled.order = []` with empty errors (Zero-Pack path).
4. WHEN `.tinkerman/config.md` is absent entirely, `loadEnabledPacks` SHALL return
...
6. `customLayerRoot` SHALL resolve to `<rootDir>/.tinkerman/custom`.
...
1. THIS slice SHALL NOT change `runGlossaryCheck`'s current input (the flat
   `.tinkerman/glossary.md` via `glossary-driver.ts`). Bridging the layered
...
- Bridging layered `loadGlossary` into `runGlossaryCheck` enforcement
  (slice C — REQ-6).
- Migrating `.tinkerman/glossary.md` to the layered model (flat file stays the
  enforcement source).
