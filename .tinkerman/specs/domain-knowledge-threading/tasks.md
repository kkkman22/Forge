...
layout: tasks
created: 2026-06-29
spec_ref: ".tinkerman/specs/domain-knowledge-threading/requirements.md"
slice: "B（领域知识全栈接线）"
...
- **Target**: `loadEnabledPacks(rootDir, fs)` 组合 `loadPackRegistry` + `parseEnabledPacks`，读 `.tinkerman/config.md`，返回 `{ enabled, errors, warnings }`。
...
- **GREEN**：按 design §Component 1 实现 `loadEnabledPacks`。customLayerRoot=`<rootDir>/.tinkerman/custom`。无 node:fs 直接导入（INV-3）。
...
- **Depends On**: [5, 6, 7]
- **Files**: `npm run check`（全绿）+ `npx tsc -p tsconfig.build.json`（dist emit 含新模块无错）+ typedoc（新模块 TSDoc 无 warning）+ 写 `.tinkerman/progress/domain-knowledge-threading.md`
