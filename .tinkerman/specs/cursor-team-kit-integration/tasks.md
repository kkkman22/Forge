...
layout: tasks
created: 2026-05-08
spec_ref: ".tinkerman/specs/cursor-team-kit-integration/requirements.md"
---
...
- [x] 4. verify.ts orchestrator + artifact invariant
  ...
  - [x] 4.2 GREEN：实现 `src/verify.ts` 的 `runVerify()`
    - 步骤：(1) acquireFileLock `.tinkerman/.locks/<topic>.lock` (2) 写 claim.md (3) resolveBaseline (4) 捕获 artifacts (5) 写 diff (6) 构造 Evidence_Chain (7) 写 verdict.md (8) releaseFileLock
...
- [x] 11. canvas-renderer.ts
  ...
  - [x] 11.3 GREEN：实现 `src/canvas-renderer.ts`
    ...
    - Bitbucket 失败时写 `.tinkerman/findings/<topic>/canvas-errors.log` + footer notice [R14.2]
    - 引用：Design §4.4
...
- [x] 14. forge-control-cli SKILL + 适配器
  ...
  - [x] 14.5 实现 `src/cli-harness.ts` orchestrator
    ...
    - 写产物到 `.tinkerman/findings/<topic>/cli-harness/` [R5.3]
...
- [x] 15. forge-control-ui SKILL + 适配器
  ...
  - [x] 15.5 实现 `src/ui-harness.ts` orchestrator
    ...
    - 不一致写 `.tinkerman/findings/<topic>/ui-harness/mismatches.md`（Severity/File/Issue/Suggestion schema）[R6.6]
    - 全部 tier 失败 → INCONCLUSIVE [R6.8]
    - 引用：Design §4.6
  - [x] 15.6 修改 `.claude/agents/quality-check.md`
    - session start 额外读 `.tinkerman/findings/<topic>/ui-harness/mismatches.md` 并作为行加入 Layer 2 输出 [R6.6]
    - 引用：Design §4.2、§4.6
...
- [x] 16. forge-test --cli flag
  - [x] 16.1 修改 `skills/forge-test/SKILL.md`
    - 新增 `--cli` flag 触发条件：`package.json bin` 非空 OR `--cli` 显式 flag OR `.tinkerman/config.md` `cli_harness: true` [R5.1]
    - 引用：[R5.1]，Design §2.3
...
- [x] 20. forge-fix-conflicts SKILL
  ...
  - [x] 20.2 实现冻结区三选项流程 [R7.3, R7.4, R7.5]
    ...
    - "unlock then merge"：改 status 为 `draft` + 写 `.tinkerman/debug/unlock-<ts>.md` + 三路合并
    - "abort merge"：调 `git merge --abort` 或 `git rebase --abort`
    - 引用：Design §4.7
  - [x] 20.3 实现 validation gate + Three-Strike 计数
    - 合并后跑 `npm run check`（fallback 到 `.tinkerman/config.md` `ci_check_command`）[R7.11, R14.11, R14.12]
...
- [x] 21. forge-ship Post_Push_Verify（≤ 50 行扩展）
  - [x] 21.1 修改 `src/ship.ts`，追加 `executePostPushVerify(topic, prCreated)` 函数
    ...
    - 失败 → 写 `.tinkerman/ship/<topic>-post-push-verify.md`（schema 按 Design §3.2.4）[R8.2]
  ...
  - [x] 21.4 修改 `.tinkerman/config.md`（当前仓库）+ `templates/config.md`
    - 新增 `.tinkerman/ship/` 为 Open_Zone [R8.7]
...
- [x] 25. 横切关注点收尾
  - [x] 25.1 `.tinkerman/config.md` 新增 `findings_retention_days`（默认 30）+ `templates/config.md` 同步 [R12.12]
  - [x] 25.2 `scripts/prune-event-logs.sh` 扩展到 `.tinkerman/findings/`
...
- [x] 27. Acceptance Criteria 追溯矩阵
  - [x] 27.1 创建 `.tinkerman/specs/cursor-team-kit-integration/acceptance-matrix.md`
