...
│                         ▼                                                 │
│   State & Hooks (不变)                                                   │
│   ├── .tinkerman/ (三区保护 + 新增 /findings/<topic>/, /ship/, /recap/)     │
...
|---|---|---|---|
| `forge-verify` | `skills/forge-verify/SKILL.md` | `/forge verify <topic>` 或 bugfix tier 自动触发 [R1.1, R1.7] | `.tinkerman/findings/<topic>/verify-this/*` |
| `forge-recap` | `skills/forge-recap/SKILL.md` | `/forge recap --since <window>` [R9.1] | stdout + 可选 `.tinkerman/findings/recap/<ts>.md` |
| `forge-control-cli` | `skills/forge-control-cli/SKILL.md` | `forge-test --cli` / `package.json bin` 非空 / `cli_harness: true` [R5.1] | `.tinkerman/findings/<topic>/cli-harness/*` |
| `forge-control-ui` | `skills/forge-control-ui/SKILL.md` | 项目含 UI 框架依赖 或 designer 产出 UI spec [R6.1] | `.tinkerman/findings/<topic>/ui-harness/*` |
| `forge-fix-conflicts` | `skills/forge-fix-conflicts/SKILL.md` | 检测到 `git diff --name-only --diff-filter=U` 非空 | 原位合并 + `.tinkerman/debug/unlock-<ts>.md` |
...
- 新增在 session start 读取 `rules/*.md` 和 `.tinkerman/findings/<topic>/ui-harness/mismatches.md` 的步骤 [R3.6, R6.6]

### 2.3 现有 SKILL 新 flag（3 处）

| SKILL | 新增 flag | 行为 | 关联 |
|---|---|---|---|
| `forge-review` | `--canvas <topic>` | 在既有 `.tinkerman/reviews/<topic>.md` 基础上渲染 HTML 画布 | R4 |
...
```yaml
---
unlocked_file: ".tinkerman/specs/user-api/spec.md"
...
### 3.3 `.tinkerman/config.md` 新增可选字段 [R12.8]
...
**向后兼容保证**：全部字段 optional，已有项目无需 edit `.tinkerman/config.md` 即可继续正常运行 [R12.8]。
...
**Evolution_Marker 触发** [R2.5]：同一模式在本次 review 的多个文件中出现 ≥ 2 次时，仅发射**一条** marker 指向 `.tinkerman/knowledge/known-failures.md`，含 pattern 名 + 出现次数。
...
export interface CanvasOptions {
  topic: string;
  dataSources?: ("reviews" | "diff" | "log" | "bitbucket")[]; // 默认读 .tinkerman/config.md
}
...
export interface UiHarnessOptions {
  ...
  designerSpecPath?: string;         // .tinkerman/specs/<feature>/spec.md designer section
}
 ...
 *   - 执行断言
 *   - 不一致写入 .tinkerman/findings/<topic>/ui-harness/mismatches.md
 ...
 *   1. frozen: .tinkerman/specs/*\/spec.md (若 status=locked)
 *              .tinkerman/plans/*.md (若 status=approved)
 *              .tinkerman/config.md
 *   2. guarded: .tinkerman/progress/**
 *               .tinkerman/reviews/**
 *               .tinkerman/knowledge/instincts.md
 *               .tinkerman/knowledge/known-failures.md
 *               .tinkerman/knowledge/solutions/**
 *               .tinkerman/decisions/ADR-*.md
 *   3. open: .tinkerman/**（其他）
 *   4. source: 非 .tinkerman/ 路径
 */
...
/**
 * 推送成功后调用 [R8.1]:
 * - 运行 npm run check（或 fallback 到 .tinkerman/config.md 的 ci_check_command）[R14.11]
 * - 600 秒超时 [R8.1]
 * - 失败写 .tinkerman/ship/<topic>-post-push-verify.md [R8.2]
...
复用 `src/run-manager.ts` 已有的 `acquireFileLock()`。新增 SKILL 在写 `.tinkerman/findings/<topic>/` 前必须取得 `.tinkerman/.locks/<topic>.lock`。

```typescript
// 调用模板（适用于所有新 SKILL）
import { acquireFileLock, releaseFileLock } from "./run-manager.js";

const lockHandle = await acquireFileLock(`.tinkerman/.locks/${topic}.lock`, { timeoutMs: 0 });
if (!lockHandle) {
  throw new CliError(
    `Another <skill-name> run for topic "${topic}" is in progress. ` +
    `Wait for it to finish or remove .tinkerman/.locks/${topic}.lock if it is stale.`
  );
}
try {
  // ...skill logic...
...
新增字段：`.tinkerman/config.md` 的 `findings_retention_days`（默认 30）。

```bash
# scripts/prune-event-logs.sh 扩展
RETENTION_DAYS=$(grep '^findings_retention_days:' .tinkerman/config.md | awk '{print $2}' || echo 30)
find .tinkerman/findings -mindepth 1 -maxdepth 2 -type d -mtime +"$RETENTION_DAYS" -exec rm -rf {} +
```
...
| `.tinkerman/config.md`（当前项目） | 自我演进：补齐新字段，但保持 optional | R12.8 |
| `scripts/prune-event-logs.sh` | 扩展到 `.tinkerman/findings/` 的保留期剪枝 | R12.12 |
...
quality-check    rules-loader    ui-harness output    review flow
 ...
 session start        │                  │                  │
      ...
      │                                  │                  │
      │ read .tinkerman/findings/<topic>/ui-harness/mismatches.md
      ├─────────────────────────────────▶│                  │
...
| 5.5 `.tinkerman/config.md` 新增可选字段 + `templates/config.md` 同步 | R12.8 | 2 文件 patch |
...
| 5 | **Three_Zone_Model 被新 SKILL 绕过** | High：安全纪律失效 | Low | [R12.9] 明确不改 PreToolUse hook；新 SKILL 所有写路径在 `.tinkerman/findings/**`、`.tinkerman/ship/**`、`.tinkerman/debug/**`（均 Open_Zone）；`fix-conflicts` 的"unlock then merge"路径要求显式用户选项并留日志 [R7.4] |
...
| forge-plan | 否 | 无 |
| forge-build | 否 | 无 |
| forge-review | 是（新 `--canvas` flag） | **外部契约不变**；`.tinkerman/reviews/<topic>.md` 仍按原逻辑产出；canvas 是"额外产物"而非替代 |
...
- `.tinkerman/config.md` 无新增必填字段，已有项目无需 edit [R12.8]
...
- Canvas 的主要产物（`.tinkerman/reviews/<topic>.md` 转 HTML）完全可以用本地数据构建。
