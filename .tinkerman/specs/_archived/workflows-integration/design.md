...
│  │  (git / frozen-zone / .tinkerman/ writes)    │                        │
│  └─────────────────────────────────────────┘                        │
│                                                                      │
│  ┌─────────────────────────────────────────┐                        │
...
| **WorkflowAuditWriter** | `src/workflow-audit-writer.ts` (新建) | 双写产物到 `.tinkerman/` 审计区、frozen-zone 校验、mkdir-p | R4, R11 |
...
export interface DispatchContext {
  ...
  forgeRoot: string;        // .tinkerman/ 目录绝对路径
...
tryL0(ctx) →
  try {
    ...
    isolatePartialFindings(ctx)      // → .tinkerman/runs/<runId>/l0-partial/
...
export interface AdapterConfig {
  runDir: string;           // .tinkerman/runs/<runId>/
...
对于 `.tinkerman/reviews/`、`.tinkerman/knowledge/sessions/` 中的任何文件，WorkflowAuditWriter 写入后旧内容的字符级 prefix 不变：`assert(new_content.startsWith(old_content))`。
...
| 目标路径 | Zone | 允许操作 | 违规处理 |
|----------|------|---------|---------|
| `.tinkerman/reviews/*.md` | Guarded | append-only | 覆盖 → FrozenZoneViolation |
| `.tinkerman/decisions/<date>-*.md` | Open | create new file | — |
| `.tinkerman/knowledge/sessions/*.md` | Open | create new file | — |
| `.tinkerman/specs/*/spec.md` (locked) | Frozen | **禁止** | FrozenZoneViolation |
| `.tinkerman/plans/*.md` (approved) | Frozen | **禁止** | FrozenZoneViolation |
| `.tinkerman/config.md` | Frozen | **禁止** | FrozenZoneViolation |
| `.tinkerman/runs/<runId>/*` | Open | free write | — |
| `.tinkerman/knowledge/tool-health.md` | Open | append-only | — |
...
```
forge-loop-cli.ts
  → 读 .tinkerman/config.md: max_parallel_agents=6, review.subagent_concurrency=3
...
用户输入 "/forge review"
  ...
  → WorkflowAuditWriter.write({ subcommand: 'review', payload: result })
    → append to .tinkerman/reviews/<topic>.md
...
forge-loop --objective "..." --max-iterations 10
  ...
  → main loop iteration N:
    → CliSubprocessDriver.run(prompt, cwd)
      ...
      → stderr → .tinkerman/runs/<runId>/stderr.log
