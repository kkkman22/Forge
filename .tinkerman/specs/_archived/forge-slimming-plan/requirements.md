...
### Requirement 3: 归档已交付的 .tinkerman/plans/ 与 .tinkerman/specs/

**User Story:** As a developer navigating `.tinkerman/`, I want plans and specs whose features have already shipped moved into `.tinkerman/archive/`, so that the active planning surface only shows work in flight.

#### Acceptance Criteria

1. THE cleanup task SHALL audit `.tinkerman/plans/` 与 `.tinkerman/specs/` 下的每个条目，基于 ROADMAP、CHANGELOG 与 progress 文件判断其是否已交付。
2. WHEN a plan or spec is determined to correspond to a shipped feature, THE cleanup task SHALL move the corresponding directory into `.tinkerman/archive/<ISO-date>-<slug>/`, preserving its internal structure.
3. IF a spec or plan is ambiguous (无法从 ROADMAP / CHANGELOG / progress 明确判定已交付), THEN THE cleanup task SHALL leave it in place and record the ambiguity in `.tinkerman/archive/.audit-pending.md`.
4. THE cleanup task SHALL update cross-references in `docs/`, `README.md`, and `.tinkerman/features/` to point at the new archived location when the referenced file has been moved.
5. THE audit log SHALL be written to `.tinkerman/archive/.audit-YYYY-MM-DD.md` with columns: `path`, `status (shipped/active/ambiguous)`, `evidence`, `action taken`.
...
1. THE cleanup task SHALL scan `docs/`, `README.md`, `CHANGELOG.md`, and `.tinkerman/features/` for mentions of v2.3 observability items (结构化 JSON 日志、命令执行耗时统计、性能基线)。
...
4. WHERE a future audit script scans for "orphaned" skills, THE Forge_Main_Package SHALL expose an allowlist file（例如 `.tinkerman/audit-keep.md`）that covers these two items。
...
1. WHEN the user invokes `/forge resume` without `--from-pr`, THE Forge_Command `/forge resume` SHALL first invoke the Native_Command `/resume` (or trigger Checkpointing restore) to restore the Claude Code session, then layer Forge's Five_Question_Recovery prompt based on `.tinkerman/status.md` and `.tinkerman/progress/*.md`。
...
**User Story:** As a developer aborting a Forge task, I want `/forge abort` to only archive `.tinkerman/status.md` and reset Forge-local state, while delegating any session-level abort concerns to the platform, so that Forge does not reimplement session cancellation.

#### Acceptance Criteria

1. THE Forge_Command `/forge abort` SHALL retain only two responsibilities: (a) archive `.tinkerman/status.md` 到 `.tinkerman/archive/<ISO-date>-abort/`, (b) reset Forge-local working state (progress、status、findings 当前指针)。
...
1. THE T3 task SHALL produce an Integration_Evaluation_Report saved to `.tinkerman/decisions/<ISO-date>-forge-maintenance-evaluation.md`。
...
2. THE T3 task SHALL produce a usage metrics report saved to `.tinkerman/decisions/<ISO-date>-grill-zoomout-usage.md` containing: total invocations of each skill, invocation source (manual vs. loop-triggered), and per-day distribution。
...
2. THE "approximately 20" target SHALL be interpreted as a range of 18 to 22 inclusive; landing outside this range SHALL be justified in `.tinkerman/decisions/<ISO-date>-skill-count-deviation.md`。
...
5. THE slimming changes SHALL NOT silently change the semantic of any existing command output format consumed by downstream tooling (e.g. `.tinkerman/reviews/*.md`, `.tinkerman/runs/*.md`)。
...
3. IF a T1/T2/T3 change would incidentally require a Forge Loop core-engine modification, THEN THE change SHALL be deferred and recorded as a follow-up in `.tinkerman/features/` rather than proceeding。
...
3. WHERE a T2 or T3 change's output writes to `.tinkerman/`, THE change SHALL respect the existing Frozen_Zone 规则; attempts to write to locked files SHALL be blocked by the existing hooks。
