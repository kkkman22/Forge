...
spec-check 在 review 时执行 R6（Claimed New File Existence on main branch）+ R7（Pack/Loader Integration Evidence）检查，需要枚举 `.tinkerman/plans/` 下的文件做存在性验证。当 fixture 含 ≥ N 个 plan 文件时，spec-check 把所有 turn 都消耗在 Read/Grep plans 文件上，从未进入文本生成阶段。

```
C(X) :≡  reviewer = "spec-check"
           ∧ |.tinkerman/plans/*.md| ≥ N_plans (实测 N_plans ≥ 49 必触发)
...
- `.tinkerman/findings/subagent-result-truncation-stage1.md` — Stage 1 数据
- `.tinkerman/findings/subagent-result-truncation-stage2.md` — Stage 2 数据
- `.tinkerman/findings/subagent-result-truncation-stage3.md` § Closure Note — 三次失败重排决策
...
1. **Plans-enumeration scoping**：spec-check 的 R6/R7 检查按 review topic 限定到 `.tinkerman/plans/<topic>.md` 而不是枚举整个目录。需要识别 review topic 的来源。
...
1.1 WHEN `/forge review` 调用 `spec-check` subagent AND fixture 含 ≥ N_plans active plan 文件（实测 N_plans = 49 必触发）THEN the system 在 spec-check 内部触发 R6/R7 检查路径，spec-check 通过 Glob/Read `.tinkerman/plans/*.md` 逐个验证 main-branch 存在性。
...
2.2 WHEN spec-check 执行 R6 检查（Claimed New File Existence on main branch）THEN the system SHALL 把检查 scope 限定到与 review topic 相关的文件路径，而不是无条件枚举 `.tinkerman/plans/`。具体 scope 限定方式由 design 阶段从候选 1/2/3/4 中选定。
...
- **Fixture（与前序 spec 相同，已就位）**：
  - `.tinkerman/plans/` ≥ 5 个 ≥ 4 KB active plan（实测 49 个，必触发 Bug Condition）
  - `.tinkerman/knowledge/evolved-rules.md` ≥ 8 KB（实测 9580 字节）
- **触发**：在 Claude Code 主 agent 会话执行 `/forge review`（不能从 Kiro 触发）。
- **观测**：
  1. spec-check 的最终 result 字段是否含 `## Layer 1 — Spec Alignment` + severity 表格。
  2. spec-check 的 tool_uses 数量与具体 tool 类型（Glob/Read/Grep 集中在 `.tinkerman/plans/*` 路径上 = 命中 Bug Condition）。
  3. `.tinkerman/reviews/<topic>.md` 是否落盘（如果 design 选择候选 4 拆分 agent）。
- **counterexample 起点**：
  - `.tinkerman/findings/subagent-result-truncation-stage2.md` § Stage 2 Real Smoke 中 spec-check 的 6 tool_uses preamble 输出
  - `.tinkerman/findings/subagent-result-truncation-stage3.md` § Closure Note 中 background 模式仍截断的零文本输出
