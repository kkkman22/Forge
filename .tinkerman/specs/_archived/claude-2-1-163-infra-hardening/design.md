...
- `.claude-plugin/bin/forge-doctor` 已能检查 `.tinkerman/`、config、settings、hook script、MCP source、git branch。
...
- quoted string 内路径
- 相对路径和绝对路径
- 简单 subshell/backtick 中的路径字面量，例如 `$(echo ~/.tinkerman/config.md)` 不执行，只提取字面量信号
...
- 对包含 `.tinkerman/config.md`、`.tinkerman/specs/`、`.tinkerman/plans/` 等高风险片段但无法解析的表达 fail closed。
