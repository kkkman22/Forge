...
┌─────────────────────────────────────────────────────────┐
│                    用户入口                               │
│  forge-loop CLI (--lang 选项)  │  .tinkerman/config.md (持久化)│
├─────────────────────────────────────────────────────────┤
│                   适配层（I/O 边界）                      │
...
interface LocaleSources {
  /** CLI --lang 参数值 */
  cliLang?: string;
  /** .tinkerman/config.md 中的 lang 字段 */
  configLang?: string;
  /** FORGE_LANG 环境变量 */
  envLang?: string;
...
**职责**：读写 `.tinkerman/config.md` 中的语言偏好。复用 `frontmatter.ts` 的解析能力。
...
```json
{
  "cli": {
    "error": {
      ...
      "noForgeDir": "Error: --tier, --type, --phase, and --nature require a .tinkerman/ directory. Run `forge init` first.",
...
| Config 文件不存在 | `.tinkerman/config.md` 缺失 | 创建默认配置文件 |
