...
- **Config_Store**：配置持久化模块，负责读写 `.tinkerman/config.md` 中的用户偏好设置
...
1. THE Locale_Detector SHALL 按以下优先级确定当前语言（从高到低）：CLI `--lang` 参数 > `.tinkerman/config.md` 中的 `lang` 字段 > 环境变量 `FORGE_LANG` > 系统 locale（`LANG` / `LC_ALL`）> 默认值 `en`
...
1. THE Config_Store SHALL 在 `.tinkerman/config.md` 的 frontmatter 中读写 `lang` 字段
2. WHEN 用户通过 CLI 设置语言偏好时，THE Config_Store SHALL 将新值写入 `.tinkerman/config.md`
3. WHEN `.tinkerman/config.md` 不存在时，THE Config_Store SHALL 创建该文件并写入默认配置
4. THE Config_Store SHALL 保留 `.tinkerman/config.md` 中的其他已有字段不被覆盖
...
3. WHEN `.tinkerman/config.md` 中不包含 `lang` 字段时，THE Locale_Detector SHALL 正常工作并回退到环境变量或默认值
