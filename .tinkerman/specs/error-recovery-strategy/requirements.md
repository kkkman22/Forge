...
根据 dogfooding 观察（Finding 8），当前 `/forge resume` 仅从 `.tinkerman/status.md` 和 `.tinkerman/progress/<topic>.md` 读取状态，不检查 git log，无法从 commit 推断任务完成状态。这导致五种中断场景下的状态不一致无法被自动检测和修复：
...
- **Progress_Reconciler**：进度协调器，负责比对 git log 中的 commit 记录与 `.tinkerman/progress/<topic>.md` 中的任务状态，识别并修复不一致。
- **Phase_Reconciler**：阶段协调器，负责比对 `.tinkerman/progress/<topic>.md` 中的任务完成状态与 `.tinkerman/status.md` 中的 phase 字段，识别并修复阶段推进不一致。
...
- **Plan_Document**：位于 `.tinkerman/plans/<topic>.md` 的计划文档，包含任务列表、commit message 模式和依赖关系。
- **Progress_Document**：位于 `.tinkerman/progress/<topic>.md` 的进度文档，记录每个任务的完成状态。
- **Status_Document**：位于 `.tinkerman/status.md` 的全局状态文档，记录当前任务、档位、阶段和更新时间。
- **Interim_Log**：位于 `.tinkerman/knowledge/sessions/<date>-<topic>-interim.md` 的中间会话日志，记录 Restatement Checkpoint 的快照信息。
...
3. WHEN uncommitted changes are relevant to the current task, THE Uncommitted_Change_Detector SHALL run the verification commands defined in `.tinkerman/config.md` (ci_check_command or verify_commands) against the changed files to assess validity.
...
1. THE Recovery_Engine SHALL serialize the Recovery_Report to a structured markdown format that can be written to `.tinkerman/debug/last-recovery.md`.
