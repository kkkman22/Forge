...
**核心问题**：当前 `/forge resume` 仅从 `.tinkerman/status.md` 和 `.tinkerman/progress/<topic>.md` 读取状态，不检查 git log，无法从 commit 推断任务完成状态。这导致五种中断场景下的状态不一致无法被自动检测和修复。
...
| 场景 | 处理方式 |
|------|---------|
| `serializeRecoveryReport` 抛出异常 | 回退到纯文本格式输出，记录警告到 `.tinkerman/debug/` |
