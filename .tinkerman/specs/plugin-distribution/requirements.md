...
- **Forge_State_Dir**：`.tinkerman/` 项目状态目录，是"项目级资产"，与 plugin（全局或用户级资产）有本质区别。
...
1. THE project SHALL produce a feasibility report at `.tinkerman/specs/plugin-distribution/feasibility.md` covering: current file layout vs plugin-required layout diff, required refactors, migration risk, rollback plan, and a go/no-go recommendation.
...
3. THE feasibility report SHALL document how Forge_State_Dir (`.tinkerman/`) interacts with plugin semantics: the plugin ships skills/agents/hooks but `.tinkerman/` is always project-local and **not** part of the plugin distribution.
...
**User Story:** As a user running `claude plugin update forge`, I want updates to be incremental and non-destructive to my project-level Forge state, so that upgrades don't wipe my `.tinkerman/` or break pinned skill customizations.

#### Acceptance Criteria

1. THE Forge_Plugin_Package SHALL clearly distinguish "plugin-owned" files (skills, agents, hooks, templates) from "project-owned" files (`.tinkerman/`, `.claude/settings.json` user-added entries); updates never touch project-owned paths.
...
5. THE `.tinkerman/decisions/` SHALL include an ADR documenting the decision to adopt plugin distribution, Phase A outcome, and the timeline for deprecating Current_Dist_Script.
