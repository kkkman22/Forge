...
- 新增 ADR `.tinkerman/decisions/<date>-plugin-distribution.md`

**关键不变**：`.tinkerman/` 目录结构、现有 skill/agent/hook 文件、Forge Loop、18 个命令的行为。
...
└── .tinkerman/                   # project state (NEVER in plugin distribution)
```
...
CC: register /forge command, make skills discoverable
  │
  ▼
User: /forge init              # writes to project's .tinkerman/
```

**关键分界**：plugin 安装只放 CC 能发现的"指令集"。每个项目的 `.tinkerman/` 仍由 `/forge init` 在项目根创建，完全不受 plugin update 影响。
