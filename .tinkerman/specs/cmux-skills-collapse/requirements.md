...
- **Cmux_Integration_Plan**：`.tinkerman/plans/cmux-integration.md` 与对应的进度文件 `.tinkerman/progress/cmux-integration.md`。
...
4. THE Forge_Dispatcher SHALL 在「未安装 cmux」环境下满足以下 4 条可量化的 Zero_Impact_Invariant 属性：
    ...
    b. 调用任意非 `Cmux_Gated_Subs` sub 时，不创建任何位于 `.tinkerman/`、`.claude/` 或仓库其它位置的额外临时文件、缓存文件或 dedupe 文件；
    c. 调用任意非 `Cmux_Gated_Subs` sub 时，不修改 `.tinkerman/`、`.claude/`、`hooks/` 下任何已有配置文件（mtime 与 sha256 与迁移前一致）；
...
3. WHEN 维护者在仓库内全文搜索字符串 `cmux-skills/install.sh`，THE Forge_Repository SHALL 不返回除归档目录（`.tinkerman/archive/`）和已归档 spec 之外的任何匹配。
...
6. IF 旧引用必须在归档目录中保留以供历史溯源，THEN THE Forge_Repository SHALL 仅在 `.tinkerman/archive/` 或带「archived」/「superseded」标记的 spec 文档中保留这些字符串。
...
4. WHEN 维护者读取 `.tinkerman/progress/cmux-integration.md`，THE 进度文件 SHALL 在 Task 27 或 Task 30 行追加链接或文字指向 `.tinkerman/specs/cmux-skills-collapse/`。
