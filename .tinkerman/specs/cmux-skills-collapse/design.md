...
> 主入口 ADR：[ADR-0004](../../../.tinkerman/decisions/ADR-0004-skills-collapse-and-dispatcher.md)。
...
| **plan / progress** | `.tinkerman/plans/cmux-integration.md` + `.tinkerman/progress/cmux-integration.md` | MODIFY | Task 27 / Task 30 状态与 Notes 同步（R9） |
...
- `allowed_tools` 取 `Read` + `Bash`（满足读 `.tinkerman/.cmux-snapshot.json` 与调 cmux CLI 的最小集）。
...
| **R3.4.b** 不创建额外临时文件 | snapshot `.tinkerman/`、`.claude/`、临时目录的文件列表 → 跑 `/forge build` → 再 snapshot → 二者集合相等 |
| **R3.4.c** mtime + sha256 不变 | 跑前后对 `.tinkerman/`、`.claude/`、`hooks/` 下所有文件取 `(mtime, sha256)`，跑后比对 |
