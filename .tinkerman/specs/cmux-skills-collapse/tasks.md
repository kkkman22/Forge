...
layout: tasks
created: 2026-05-24
spec_ref: ".tinkerman/specs/cmux-skills-collapse/requirements.md"
---

# Implementation Plan — cmux-skills-collapse

> 基于 `.tinkerman/specs/cmux-skills-collapse/{requirements.md, design.md}`。
...
- [x] 8. 删除 cmux-skills/ 根目录及 install.sh
  - **实施目标**：执行 `git rm cmux-skills/install.sh`、`rmdir cmux-skills/forge-{sidebar-sync,browser-qa,loop-signals}`、`rmdir cmux-skills/`。验证仓库内除 `.tinkerman/archive/` 与本 spec 文档外不再含字符串 `cmux-skills/install.sh`。
  - **涉及文件**：
    - `cmux-skills/install.sh`（DELETE）
    - `cmux-skills/`（DELETE，目录）
  - **验证命令**：`! test -d cmux-skills && rg -n 'cmux-skills/install.sh' --glob '!.tinkerman/archive/**' --glob '!.tinkerman/specs/cmux-skills-collapse/**' --glob '!docs/reference-advanced.md' && echo OK || echo "still referenced"`（期望输出 `OK`，rg 返回非零即视为通过）
...
- [x] 12. Zero-Impact 测试：未装 cmux 时 /forge build 字节级一致
  - **实施目标**：实现 design §8.3 R3.4.a-d 的 4 条属性测试。R3.4.a：捕获迁移前 commit + 迁移后 commit 在「未装 cmux」环境下跑 dispatcher 单测的 stdout/stderr，`diff --binary` 必须空。R3.4.b：snapshot `.tinkerman/`、`.claude/` 文件列表。R3.4.c：跑前后 `(mtime, sha256)` 比对。R3.4.d：build-dist 后比对 31 个非 cmux sub 的 instructions.md sha256 集合。
  - **涉及文件**：
...
- [x] 15. 更新 .tinkerman/plans/cmux-integration.md（File Mapping + Task 27/30 Notes）
  - **实施目标**：在 plan 的 File Mapping 表中：把 `cmux-skills/forge-sidebar-sync/SKILL.md` 等 4 行改为 `skills/forge/lib/forge-cmux-{sidebar-sync,browser-qa,loop-signals}/instructions.md` + Notes 列注明「moved by spec cmux-skills-collapse」；删除 `cmux-skills/install.sh` 行（或改为 DELETE 操作 + Notes）。Sprint 5 Task 27 状态保持 done 但 Notes 加入「superseded by .tinkerman/specs/cmux-skills-collapse/」；Sprint 6 Task 30 Notes 改为「reference-advanced.md 已按条件分发方式重写（cmux-skills-collapse R6）」。Objective、Design Reference Index、Sprint 1–4 不动（R9.5）。
  - **涉及文件**：
    - `.tinkerman/plans/cmux-integration.md`（MODIFY）
  - **验证命令**：`grep -c 'cmux-skills-collapse' .tinkerman/plans/cmux-integration.md`（期望 ≥ 2）
  - **关联 Requirements**：R9.1, R9.2, R9.3, R9.5

- [x] 16. 更新 .tinkerman/progress/cmux-integration.md（Task 27/30 链接）
  - **实施目标**：在 Sprint 5 Task 27 行 Notes 列末尾追加 `→ superseded by .tinkerman/specs/cmux-skills-collapse/`；在 Sprint 6 Task 30 行 Notes 列追加 `→ rewritten per .tinkerman/specs/cmux-skills-collapse/ R6`。其它 Task 状态不动。
  - **涉及文件**：
    - `.tinkerman/progress/cmux-integration.md`（MODIFY）
  - **验证命令**：`grep -c 'cmux-skills-collapse' .tinkerman/progress/cmux-integration.md`（期望 ≥ 2）
...
本 tasks.md 取代 `.tinkerman/plans/cmux-integration.md` 中 Sprint 5 Task 27（cmux-skills/ 创建）的后续维护责任，并触发 Sprint 6 Task 30（reference-advanced.md）的重写。Sprint 1–4 与 Sprint 5 的 Task 24–26、28 不受影响（已 done，状态保持不变）。
