...
- **选择理由**：选 B。AST 分析复杂、慢、易错，且 Forge 不该承担静态分析框架的维护成本。路径 glob（`*.vue`/`*.tsx`/`src/**/route*`）+ diff≥100 行是论文/社区实践的朴素启发式，足够抓住绝大多数行为性变更，误报成本（多跑一次验证）远低于漏报成本（漏掉跑不起来的 bug）。规则放 `.tinkerman/config.md` 可配，用户可调阈值。
...
- **候选方案**：
  ...
  - B. 每个 build commit 时落盘到 `.tinkerman/runs/<run_id>/commit-narrative.md`，Mission Summary 摘录。
...
`.tinkerman/runs/<run_id>/commit-narrative.md`（每个 build commit 追加一节）：
...
| 产物 | 路径 | 性质 |
|---|---|---|
| triage inbox | `.tinkerman/triage-inbox.md` | 新增，markdown，append-only |
| triage 时间戳 | `.tinkerman/state/triage-state.json`（`last_triage_at`） | 新增，轻量 JSON |
| commit 叙事 | `.tinkerman/runs/<run_id>/commit-narrative.md` | 新增，markdown，append-only |
| config 块 | `.tinkerman/config.md` 的 `triage:` 块 | 新增配置 |
...
| 行为验证证据（截图）占用磁盘 | 截图存 `.tinkerman/runs/<run_id>/` 随 run 归档清理，不进 git |
