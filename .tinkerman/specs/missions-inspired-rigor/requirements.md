...
- **Atomic Task Handoff（条款 2）**：`.tinkerman/progress/<topic>.md` 当前由 build agent 自由发挥写状态，每个项目 schema 不一致；Missions 强制 5 字段 schema（completed / not_completed / commands_executed / issues_found / procedure_compliance），让"下一个 worker 拿到的就是机器可读状态"。
- **Validator-time Knowledge Capture（条款 3）**：Forge 的 `/forge learn` 在收尾才跑，KB 是事后追忆；Missions 让 validator 在抓到 issue 时直接写 `.tinkerman/knowledge/known-failures.md`，下一次 review 启动时 KB 自带"上次类似 diff 出过这个问题"。
...
- **Atomic Task Handoff**：build agent 完成单个原子任务后写入 `.tinkerman/progress/<topic>.md` 的结构化记录，包含 5 个固定字段。
- **Validator-time Knowledge Capture**：`spec-check` / `quality-check` / `security-check` 在 review 阶段输出 P0/P1 时，强制 append 一行到 `.tinkerman/knowledge/known-failures.md` 的累积模式。
...
1. WHEN build agent 完成一个原子任务并准备 commit THEN forge-build SKILL SHALL 要求 build agent 在 `.tinkerman/progress/<topic>.md` 对应任务条目下追加一份 5 字段 handoff block，字段为 `completed`、`not_completed`、`commands_executed`、`issues_found`、`procedure_compliance`。
2. WHEN handoff block 写入 `.tinkerman/progress/<topic>.md` THEN 该 block SHALL 使用 fenced code block 包裹（语言标记为 `yaml` 或 `handoff`），便于机器解析。
...
5. WHEN 下一个原子任务启动（同 topic）THEN build agent SHALL 先读取 `.tinkerman/progress/<topic>.md` 中上一个任务的 handoff block 作为接续输入；**禁止**仅依赖会话内对话历史推断状态。
...
**User Story:** 作为 review subagent，我希望在抓到 P0/P1 issue 时立即把它登记到 `.tinkerman/knowledge/known-failures.md`，这样下一次 review 启动时 spec-check / quality-check / security-check 可以自动检索"过去类似 diff 是否出过同样问题"。
...
2. WHEN forge-review SKILL 收到三层评审报告 THEN forge-review SKILL SHALL 把所有 `known-failures append-block` 合并去重后追加到 `.tinkerman/knowledge/known-failures.md`。
...
4. WHEN spec-check / quality-check / security-check 任一 agent 启动 Step 0 之后 THEN 该 agent SHALL 读取 `.tinkerman/knowledge/known-failures.md` 作为辅助上下文，并在输出报告头部列出"本次 diff 命中的历史失败模式（按 pattern_id 列表）"。
...
2. WHEN forge-loop 在迭代之间交接状态 THEN 交接 SHALL 仅通过以下文件完成：`.tinkerman/status.md`、`.tinkerman/specs/<topic>/spec.md`、`.tinkerman/plans/<topic>.md`、`.tinkerman/progress/<topic>.md`、`.tinkerman/findings/<topic>.md`、`.tinkerman/knowledge/known-failures.md`、`.tinkerman/runs/<run-id>/events.ndjson`。
3. WHEN forge-loop SKILL 在某次 SKILL 调用前 THEN forge-loop SKILL SHALL 在 `.tinkerman/runs/<run-id>/events.ndjson` 中写入一条 `phase_start` 事件，包含 `phase`、`iteration`、`wall_clock_elapsed_seconds`、`token_budget_used` 字段。
...
6. WHEN forge-loop 触发熔断器（三振出局 / token 预算超限 / 用户中断）THEN forge-loop SHALL 在 `.tinkerman/status.md` 写入完整 phase + skill_sequence + 最近一次 events.ndjson cursor，使 `/forge resume` 能在新会话内从断点继续。
7. WHEN 用户运行 `/forge resume <run-id>` THEN forge-resume SKILL SHALL 读取 `.tinkerman/runs/<run-id>/events.ndjson` 的最新 cursor + `.tinkerman/status.md`，在新会话中重启 forge-loop，状态完全恢复。
...
**Verify-By**: `manual`
**Evidence**: 录屏或 `.tinkerman/progress/missions-inspired-rigor.md` 中可见 build agent 在第 N+1 任务的开头明确引用第 N 任务的 handoff block 内容
...
**Verify-By**: `manual`
**Evidence**: 在 `forge-loop` 一次完整运行后，`.tinkerman/runs/<run-id>/events.ndjson` 中可见多条 `phase_start` / `phase_end` 事件，且每条事件附带不同的 `session_id`
...
- 依赖 `.tinkerman/knowledge/known-failures.md` 已存在并属于受保护区（已在 `config.md` 声明）
- 依赖 `forge-context` MCP 已捆绑（由 `forge-context-mcp-bundling` spec 在 v2.4.0 提供）
- 依赖 `.tinkerman/runs/<run-id>/events.ndjson` 事件流基础设施已就位（由 `structured-observability` spec 提供）
...
- **R4 fresh-context 调用可能因 cache miss 增加 token 消耗**：依赖 Forge 已有的 prefix-cache 友好的状态文件分区（冻结 / 受保护 / 开放），实测 cache 命中率应 ≥80%；监控指标进入 `.tinkerman/knowledge/metrics.md`。
