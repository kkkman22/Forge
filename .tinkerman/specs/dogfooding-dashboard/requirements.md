...
Forge 把自己定位为"用工程纪律约束 AI 编码得到更好结果"的框架，并在 `.tinkerman/` 目录积累了完整的自举运行数据（specs / findings / decisions / review / episodes）。但目前没有一个对外可见的数字回答"用了 Forge 之后结果真的更好吗"。这是 Forge 最大的可信度盲区——对一个"纪律框架"而言，自举数据是最强的证据，远胜任何 README 描述。

本特性从 `.tinkerman/` 现有数据中聚合出行为 KPI，生成一个静态仪表盘，把"纪律有效性"从口号变成可查证的数字。
...
### Requirement 1: 从 .tinkerman/ 聚合三类核心行为 KPI

仪表盘必须从现有 `.tinkerman/` 文件中聚合出反映纪律执行率的三类指标，**不要求新增任何埋点**。
...
- 当 同一 .tinkerman/ 快照下连续两次生成时 系统应当 产出字节一致的结果（确定性）
- 当 .tinkerman/ 缺失某类数据时 系统应当 在对应 KPI 处显示"无数据"而非崩溃
...
- `.tinkerman/dashboards/dogfooding.md`：生成的仪表盘产物（gitignore 或纳入版本由后续决定）

### Modified
- 无（只读 `.tinkerman/`，不改任何现有状态文件）

### Unchanged
- `.tinkerman/` 下所有现有文件的内容与格式
