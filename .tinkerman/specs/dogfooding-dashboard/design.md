...
一个只读聚合器：扫描 `.tinkerman/` 目录，统计纪律执行率，产出静态 Markdown 仪表盘。数据全在，工程量集中在"口径定义"与"聚合脚本"。
...
| KPI | 数据源 | 聚合方式 |
|-----|--------|----------|
| spec→ship 完整链路率 | `.tinkerman/specs/*/tasks.md` + git log | 统计同时有 locked spec + ship 痕迹的 feature 数 / 总 feature 数 |
| 三层评审拦截数 | `.tinkerman/findings/*` + `.tinkerman/specs/*/` review 记录 | 按 `[layer][severity]` 解析（复用 `guarded-merger.ts` 的 `parseReviewFindings` 格式）计数 P0/P1/P2 |
| replay 证据链占比 | `.tinkerman/episodes/` 或 replay 痕迹 | 统计有证据链的 episode / 总 episode |
...
scripts/build-dogfooding-dashboard.mjs
  ...
  ├─ renderMarkdown()     → 产出 .tinkerman/dashboards/dogfooding.md
  └─ main()               → 编排 + 写文件
```
...
| 级别 | 数量 | 抽样源 |
|------|------|--------|
| P0 | 3 | .tinkerman/findings/audit-remediate-p0p1.md |
| P1 | 11 | ... |
| P2 | 27 | ... |
```
...
- `.tinkerman/` 目录缺失 → 提示"非 Forge 项目"并退出码 1
...
测试 fixture 用一个最小化的 `.tinkerman/` 影子目录（`test/__fixtures__/dogfooding-sample/`），不依赖真实仓库数据。
...
|--------|------|------------------|
| specs 目录 | `.tinkerman/specs/*/` | 170+ spec 目录，含 tasks.md / design.md |
| findings | `.tinkerman/findings/*` | review 发现记录，`[layer][severity]` 格式 |
| episodes | `.tinkerman/episodes/` | schema_version 1\|2 已可解析 |
| 现有解析器 | `src/guarded-merger.ts:272` `parseReviewFindings` | 可直接复用解析评审格式 |

## Proposed Change

**要改变的**：新增一个只读聚合脚本，把分散在 `.tinkerman/` 的运行痕迹汇总成一份可信度仪表盘。

**明确不改变的**：
- `.tinkerman/` 下任何现有文件的内容与格式
- 不新增任何埋点 / hook / 运行时采集
...
- 删除生成的 `.tinkerman/dashboards/`

**Mount Points**：纯新增脚本，零 mount point。若产物纳入 gitignore 则需加一行 `.tinkerman/dashboards/`。
