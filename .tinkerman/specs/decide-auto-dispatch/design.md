...
在 `/forge decide` 的执行流程中新增 `auto` 模式，根据 tier（从 `.tinkerman/status.md` 读取）自动选择 inline 或 Agent Teams 执行模式。

**变更范围**：
- 修改 `skills/forge/lib/decide/instructions.md`（添加 auto 分发逻辑）
- 修改 `skills/forge/lib/router/instructions.md`（确保 tier 传递）
- 修改 `.tinkerman/config.md`（`decide_dispatch_mode: inline` → `auto`）
...
1. 读取 `.tinkerman/config.md` 的 `decide_dispatch_mode` 字段
2. 如果为 `inline` → 使用 inline 模式
3. 如果为 `agents` → 使用 Agent Teams 模式
4. 如果为 `auto`：
   a. 读取 `.tinkerman/status.md` 的 `tier` 字段
...
| Tier 来源 | `.tinkerman/status.md` | router 参数传递 | status.md 是跨阶段的单一事实源 |
...
| 场景 | 行为 |
|------|------|
| `.tinkerman/config.md` 无 `decide_dispatch_mode` | 默认 `auto` |
| `.tinkerman/status.md` 无 `tier` 字段 | 默认 `standard` |
