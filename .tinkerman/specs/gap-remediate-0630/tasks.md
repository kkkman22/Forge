...
- **TDD Steps**: 非 TDD(裁决记录)。
  - 在 `.tinkerman/specs/arch-review-remediate-0626/tasks.md` 的 T-01 标题下加裁决块:
...
- **Verify Command**: `grep -A3 "裁决(2026-06-30):撤销" .tinkerman/specs/arch-review-remediate-0626/tasks.md`(命中)且 `test -d src/state-machine`(目录仍在)
...
- **Verify Command**: `grep -c "重评(2026-06-30)" .tinkerman/specs/arch-review-remediate-0626/tasks.md`(≥2)
...
- **TDD Steps**:
  ...
  - GREEN:
    ...
    3. `.tinkerman/knowledge/evolved-rules.md` R3 的 Infra_Ref 追加 `scripts/check-unused-module.mjs`(不占 R15 名额,只更新)。
...
- **REQ**: REQ-04
- **TDD Steps**:
  - RED: 先 PoC——`tsx scripts/check-evidence-verdicts.mjs`(新建)扫描 `.tinkerman/artifacts/` 调用 `validateArtifactBackedVerdict`,记录误报数 + 耗时。无断言,只采集。
  - **Decision Point**:
...
- **TDD Steps**: 非 TDD(调查 + 文档)。
  ...
  - 文档:更新 `.tinkerman/specs/spec-lifecycle-management/requirements.md §2`,明示"status 仅写在 requirements.md;design.md/tasks.md 不得重复定义 status 字段"。
- **Verify Command**: `grep -n "status 仅" .tinkerman/specs/spec-lifecycle-management/requirements.md`(命中)+ spot-check 清单留存
