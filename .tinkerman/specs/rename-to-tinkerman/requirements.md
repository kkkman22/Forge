...
4. 状态目录 `.tinkerman/` → `.tinkerman/`（迁移脚本 + 双路径读取兼容期）
...
10. **行为等价**：CLI 输出语义 / hook 行为 / 状态目录语义（`.tinkerman/` 与原 `.tinkerman/` 等价）完全不变

## Constraints
- **行为等价**（ADR-0008 原则）：改名不改逻辑，公开契约（CLI / MCP / frozen marker）语义不变
- **组织记忆连续性**（ADR-0009 保留项 #4）：`.tinkerman/` → `.tinkerman/` 迁移不得丢失 specs / progress / decisions / knowledge——迁移脚本必须校验文件数 + 内容哈希
