...
layout: tasks
created: 2026-06-04
spec_ref: ".tinkerman/specs/project-charter/requirements.md"
---
...
- [ ] 1.1 创建 charter 模板
  - [ ] 在 `templates/` 或 `.tinkerman/templates/` 创建 `charter-template.md`
...
- [ ] 1.4 手动验证：创建一个 charter
  ...
  - [ ] 确认交互式问答流程
  - [ ] 确认 `.tinkerman/charter.md` 生成且格式正确
  - [ ] 确认 frontmatter 字段完整
  - [ ] 运行 `/forge charter show` 确认输出
  **Verify-By**: `.tinkerman/charter.md` 存在且 `grep "INV-" .tinkerman/charter.md` 有输出
  **关联需求**: R1, R2
...
- [ ] 2.1 修改 decide skill/agent 增加 charter grounding
  - [ ] 在 decide 执行前检查 `.tinkerman/charter.md` 是否存在且 `status: active`
