...
- **Verify**:`bash scripts/check-spec-contract.sh .tinkerman/specs/layered-test-pyramid/requirements.md && npx vitest run test/check-spec-contract.property.test.ts`
...
- **Verify**:`bash scripts/check-pyramid-ratio.sh .tinkerman/specs/layered-test-pyramid/requirements.md && npx vitest run test/check-pyramid-ratio.property.test.ts`
...
- **TDD Steps**:
  ...
  - GREEN:更新 `skills/forge/lib/{spec,test,accept,init}/instructions.md`、`AGENTS.md` 测试章节、`.tinkerman/config.md` 模板。
  - REFACTOR:无。
