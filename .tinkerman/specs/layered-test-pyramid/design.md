...
| **init** | `skills/forge/lib/init` + `scripts/init.sh` | `.tinkerman/` 初始化 | **改:加--recipe透传** |
...
| **config** | `.tinkerman/config.md` | 项目配置 | **扩:加test_commands/ratio配置** |
...
9. `.tinkerman/config.md`(模板):加 test_commands/e2e_ratio_threshold/strict_pyramid。
