...
**决策**:`redactSecrets()` 调用从 read-time 移到 write-time,wrap `fs.writeFileSync` target `.tinkerman/**/*.md`。

**为什么**:
- 任何下游消费者读 `.tinkerman/*.md` 都已脱敏(无需每处单独调)
- 未来新 `.tinkerman/**` 产物自动脱敏(无需 per-call wiring)
- 配合 `.gitignore` `.tinkerman/reviews/` 关闭 exfil pipeline
...
| .gitignore | 修改 (+ .tinkerman/reviews/) |
| (已有的 .tinkerman/reviews/*.diff-context.md) | git rm --cached |
...
| .tinkerman/decisions/ADR-0009-audit-remediation-scope.md | 新增 |
