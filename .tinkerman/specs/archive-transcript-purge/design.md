...
- 新增 `.tinkerman/decisions/<date>-cc-purge-integration.md` ADR
- 新增 `test/archive-purge.test.sh` 或等价测试

**不涉及**：`.tinkerman/archive/` 目录结构、归档的文件移动逻辑、spec 完成判定规则。
...
└──────────────────────────┬─────────────────────────────────┘
          ...
          │  - move .tinkerman/specs/<slug>       │
          │  - move .tinkerman/plans/<slug>.md    │
          │  - move .tinkerman/progress/<slug>.md │
          │  → .tinkerman/archive/<date>-<slug>/  │
          └────────────────┬──────────────────┘
                           │
          ┌────────────────▼──────────────────┐
...
**文件**：`.tinkerman/archive/<YYYY-MM-DD>-<slug>/purge-manifest.json`
