...
`hooks/hooks.json`）各自 `cat .tinkerman/knowledge/evolved-rules.md` 全量内容，
`.claude/settings.json` 还残留一段 `head -50 .tinkerman/plans/*.md + tail -20
.tinkerman/progress/*.md`。所有 hook 命令对"调用方是主 agent 还是 subagent"完全不感知，
...
subagent 上下文中：(a) `.tinkerman/knowledge/evolved-rules.md` 的无界全文，
(b) `.tinkerman/plans/*.md` 的 head -50、`.tinkerman/progress/*.md` 的 tail -20 全文叠加，
...
- **Example 1 — SessionStart 全量 cat**：spec-check subagent 启动时，
  ...
  `cat .tinkerman/knowledge/evolved-rules.md` 把全文（>4KB 时不止 4KB）注入到 subagent
...
- **Example 2 — UserPromptSubmit head/tail**：spec-check 在评审循环中第二次
  ...
  `.tinkerman/plans/*.md`（10+ 历史文件）的 head -50 与 `.tinkerman/progress/*.md` 的 tail -20
...
- **Example 3 — inject-plan-context.mjs**：quality-check subagent UserPromptSubmit
  触发 `node inject-plan-context.mjs`，脚本读取 `.tinkerman/plans/*.md` 输出 8KB plan
...
1. **三份独立的 hook 配置都存在 SessionStart 全量 cat**：`.claude/settings.json`、
   ...
   `cat .tinkerman/knowledge/evolved-rules.md`，无字节上限。每个 subagent 是独立
...
`inject-evolved-rules.mjs`）SHALL 在不读 `.tinkerman/knowledge/`、`.tinkerman/plans/`、
`.tinkerman/progress/` 任何文件的前提下立即 `process.exit(0)` 且向 stdout 写入零字节。
...
配置文件层面 SHALL NOT 引入任何匹配模式 `head|tail|cat .tinkerman/(plans|progress)/.*`
但缺少字节上限的命令。
...
该性质来自现有 `inject-plan-context.mjs` 的纯函数语义（仅依赖 `.tinkerman/plans/*.md`
...
```
 ...
 (.claude/settings.json:14-22)      (.claude/settings.json:25-32)
   cat .tinkerman/knowledge/             head -50 .tinkerman/plans/*.md
       evolved-rules.md              tail -20 .tinkerman/progress/*.md
 ┃                                   ┃
 ┃ 修改点 ①                          ┃ 修改点 ②（DELETE 整段）
 ▼                                   ▼
 ...
 (.claude-plugin/plugin.json)        (.claude-plugin/plugin.json + hooks/hooks.json)
   cat .tinkerman/knowledge/               node inject-plan-context.mjs
       evolved-rules.md
 ┃                                   ┃
 ...
 (hooks/hooks.json)                  (.claude-plugin/plugin.json + hooks/hooks.json)
   cat .tinkerman/knowledge/               node cmux-mirror/sync-once.mjs .forge
       evolved-rules.md
 ┃                                   ┃
 ┃ 修改点 ④                          ┃ 修改点 ⑥ (脚本入口加 router 短路)
 ▼                                   ▼
...
- `scripts/inject-evolved-rules.mjs` — SessionStart 注入器（取代三份配置中的内联
  ...
  `.tinkerman/knowledge/evolved-rules.md`。
...
- 任何异常路径（stdin 关闭、partial read、`JSON.parse` 抛错、字段缺失但 `agent_id`
  ...
  一次，也不让 subagent 被注入。
- 模块本身无副作用，不写文件、不调用任何 hook 子系统、不依赖 `.tinkerman/`。

### Component 2: `scripts/inject-evolved-rules.mjs` (NEW)

替代三份配置文件中内联的 `cat .tinkerman/knowledge/evolved-rules.md`。
...
**Specific Changes**:
1. **DELETE UserPromptSubmit 整段** (修改点 ①)：移除 `head -50 .tinkerman/plans/*.md
   2>/dev/null; ... tail -20 .tinkerman/progress/*.md ...` 的整个 entry。该段不再
   存在，主 agent 的 plan 注入仍由 plugin.json + hooks.json 的
   `inject-plan-context.mjs` 提供。
2. **改写 SessionStart 第二段** (修改点 ②)：把内联 `cat .tinkerman/knowledge/
...
.tinkerman/knowledge/evolved-rules.md` 字符串，因此需要在 Step 3 同步更新 baseline，
...
4. **Settings.json head/tail Counterexample (config-level)**：构造一个 fixture
   `.tinkerman/plans/` 含 5 个大 plan 文件、`.tinkerman/progress/` 含 3 个，断言
...
FOR ALL stdinPayload WHERE isBugCondition(stdinPayload) DO
  ...
  ASSERT expectedBehavior(result):
      ...
      AND no .tinkerman/* file was read
END FOR
```
...
- 它能在配置文件层面（plugin.json / hooks.json / settings.json）扫描所有 hook
  命令，断言不引入未受预算约束的 `head|tail|cat .tinkerman/(plans|progress)/.*` 模式。
...
3. **Settings.json Re-injection Prevention**：扫描 plugin.json + hooks.json +
   ...
   .tinkerman/(plans|progress)/.*` 但缺少字节上限 (`-c 数字` / `node .*max=数字`) 的
   命令字符串（Property 4）。
...
- **`test/hooks-config-integrity.property.test.ts`** (NEW)：用 `fast-check`
  ...
  不引入匹配 `head|tail|cat .tinkerman/(plans|progress)/.*` 但缺少字节上限的命令；
...
- **End-to-end Subagent Injection Drill**：在 `.tinkerman/plans/` 放 5 个 ≥ 4KB plan
  文件、`.tinkerman/knowledge/evolved-rules.md` 放 ≥ 8KB 内容，手工构造 stdin JSON
...
| `inject-evolved-rules.mjs` 找不到 `.tinkerman/knowledge/evolved-rules.md` | `try { statSync } catch → process.exit(0)` 不输出（与现有内联 `if [ -f ... ]` 等价） | `test/inject-evolved-rules.test.ts` 场景 1 |
