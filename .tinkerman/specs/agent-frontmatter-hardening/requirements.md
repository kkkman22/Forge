...
1. THE 以下 agent SHALL 在 frontmatter 中添加 `initialPrompt`：
   - `forge-build`：`initialPrompt: "读取 .tinkerman/plans/ 中的当前 plan，从 TaskList 获取下一个未完成 task，开始 RED→GREEN→REFACTOR 循环。"`
   - `forge-plan`：`initialPrompt: "读取 .tinkerman/specs/ 中的当前 spec，分析代码库，生成原子化 TDD-ready task 列表。"`
