...
4. WHEN the hashes differ, THE check SHALL compute the diff and categorize it:
   - Diff only involves `.tinkerman/` files → pass (state updates don't affect code quality)
...
4. WHEN `reviewedCommit !== currentHead` AND all `changedFiles` start with `.tinkerman/`, THE function SHALL return `{ fresh: true, reason: "changes only in .tinkerman/ state files" }`.
5. WHEN `reviewedCommit !== currentHead` AND any `changedFiles` does NOT start with `.tinkerman/`, THE function SHALL return `{ fresh: false, reason: "project code changed since review", changedFiles: <list of non-.forge files> }`.
