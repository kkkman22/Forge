...
forge-review 完成时
  ...
  → 写入 .tinkerman/reviews/<topic>.md frontmatter: reviewed_at_commit

forge-ship 门禁检查时
  → 读取 reviewed_at_commit
  → 读取当前 HEAD
  → 比较：相同 → pass；不同 → 检查 diff
  → diff 仅 .tinkerman/ → pass；涉及项目代码 → ⚠️ 警告
```
...
### Property 3: 仅 .tinkerman/ 变更通过

*For any* different commits and file list where ALL files start with `.tinkerman/`, `checkReviewFreshness(a, b, files)` SHALL return `{ fresh: true }`.

### Property 4: 项目代码变更触发警告

*For any* different commits and file list where ANY file does NOT start with `.tinkerman/`, `checkReviewFreshness(a, b, files)` SHALL return `{ fresh: false }`.
...
- 生成器：随机 commit hash（hex string）、随机文件路径列表（含/不含 `.tinkerman/` 前缀）
