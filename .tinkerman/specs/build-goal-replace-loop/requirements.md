...
1. THE `persistent-loop.sh` SHALL 保留 phase transition 逻辑：检测 `.tinkerman/status.md` 的当前 phase，当 phase 未完成时自动触发下一阶段。
...
1. THE `.tinkerman/config.md` SHALL 新增 `build.use_goal: true` 配置项（默认值 `true`）。
...
2. WHEN `.tinkerman/config.md` 无 `build.use_goal` 字段，THE 默认值 SHALL 为 `true`（新行为）。
