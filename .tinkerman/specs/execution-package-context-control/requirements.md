...
- **Package Boundary**: A deterministic recovery point after a package completes or fails. State is stored in `.tinkerman/`, not in conversation history.
...
3. THE decide phase SHALL assess whether the PM spec is sufficient, identify product, architecture, and security decisions, and write those decisions to `.tinkerman/decisions/`.
...
1. THE build phase SHALL accept a package selector such as `--package <id>` or infer the next incomplete package from `.tinkerman/status.md` and `.tinkerman/progress/`.
...
6. After package success, build SHALL update `.tinkerman/status.md` with `current_package`, `completed_packages`, and `next_package`.
...
6. Complete raw logs, diffs, and detailed reports SHALL be written to `.tinkerman/runs/` or `.tinkerman/progress/` and referenced by `report_path`.
...
5. The resume flow SHALL use `.tinkerman/status.md` and package handoff files rather than conversation history.
...
4. On resume, THE loop SHALL read `.tinkerman/loop-state.json`, `.tinkerman/status.md`, and package handoff files to determine the next package.
...
1. THE plan context injection hook SHALL detect the current phase from `.tinkerman/status.md`.
...
5. WHEN compaction occurs mid-package, resume SHALL recover to the current package and current task using `.tinkerman/status.md`, `.tinkerman/progress/`, and interim package state.
...
10. The PreToolUse plan-context hook SHALL derive phase/package from `.tinkerman/status.md` when invoked without explicit `--phase`, because current settings call `scripts/inject-plan-context.mjs` without a phase argument.
