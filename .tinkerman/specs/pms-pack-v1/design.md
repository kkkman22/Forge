...
### 3.5 Micro_Review Output Entry (in `.tinkerman/progress/<topic>.md`)
...
export interface AcceptGateInput {
  ...
  acceptanceArtifactPath: string | null;         // .tinkerman/acceptance/<topic>/report.md
}
...
7. 原子写 `.tinkerman/mutation/<timestamp>.md`（frontmatter + body 汇总）
...
4. 写 .tinkerman/config.md frontmatter：
   packs: [pms, ...]
5. 若 pms 启用，交互提示：
   - business_day_cutoff_hour (default 4)
   - business_day_timezone (default Asia/Shanghai)
   写入 .tinkerman/config.md frontmatter
6. 创建 .tinkerman/custom/ 空目录
7. 打印欢迎消息：
