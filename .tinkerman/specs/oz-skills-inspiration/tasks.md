...
layout: tasks
created: 2026-05-08
spec_ref: ".tinkerman/specs/oz-skills-inspiration/requirements.md"
---
...
- [x] 2. Phase 1.2 — SKILL.md 章节骨架统一（需求 2）
  ...
  - [x] 2.5 纳入 `npm run check`
    - 修改 `.tinkerman/config.md` 的 `ci_check_command` 或 `package.json` 的 `check` 脚本
...
- [x] 3. Phase 1.3 — Skill Style Guide + Template（需求 3）
  - [x] 3.1 创建风格指南主文档（`.tinkerman/knowledge/skill-style-guide.md`）
  ...
  - [x] 3.3 编写版本演进策略（章节 9）
    ...
    - 变更记录落地到 `.tinkerman/knowledge/skill-style-guide-changelog.md`
  ...
  - [x] 3.8 在 CONTRIBUTING.md 引用风格指南
    ...
    - 链接 `.tinkerman/knowledge/skill-style-guide.md`
    - 说明 PR 自检清单在指南末尾
    - _Requirements: 3.10_
...
- [x] 4. Phase 2.1 — Scripts as Black Box 纪律（需求 4，可与 Phase 1 并行）
  ...
  - [x] 4.4 首版脚本审计
    ...
    - 产出 `.tinkerman/findings/scripts-help-audit.md`（见 design §4.5 格式）
  ...
  - [x] 4.9 更新风格指南 scripts/ 章节
    - 修改 `.tinkerman/knowledge/skill-style-guide.md` 的 scripts/ 用途边界章节
...
- [x] 5. Phase 3.1 — Frontend-Check Review Agent（需求 5）
  ...
  - [x] 5.12 集成到 forge-review（`src/review.ts` 扩展）
    ...
    - 结果写入 `.tinkerman/reviews/<topic>.md` 的 `## Layer 4: Frontend Check` 段落
  ...
  - [x] 5.16 .tinkerman/reviews/assets 纳入 retention
    - 修改 `scripts/prune-event-logs.sh`（或新建 `prune-review-assets.sh`）
    - 扫描 `.tinkerman/reviews/assets/*` mtime > 30d 的文件自动归档到 `.tinkerman/archive/reviews/`
    - 在 `.tinkerman/config.md` 声明 retention 策略
    - _Requirements: 5.11_
...
- [x] 6. Phase 4.1 — Acceptance Scenario Eval（需求 6，MVP）
  ...
  - [x] 6.8 实现 API runner（`src/accept-driver.ts`）
    ...
    - 按 `Then` 断言判定 verdict
    - 产出 artifact 写入 `.tinkerman/acceptance/<topic>/<scenario-id>/response.json` + `output.log`
  ...
  - [x] 6.13 实现聚合与报告（`src/accept.ts` 扩展）
    ...
    - 输出写入 `.tinkerman/reviews/<topic>-acceptance.md`（按 design §6.8 格式）
  ...
  - [x] 6.19 Retention 策略
    ...
    - `.tinkerman/acceptance/` 默认保留 30 天，超期归档到 `.tinkerman/archive/acceptance/`
    - 在 `.tinkerman/config.md` 声明
    - _Requirements: 6.13_
