...
- **Skill_Style_Guide**：`.tinkerman/knowledge/skill-style-guide.md`（或 `templates/SKILL-TEMPLATE.md`），项目级 SKILL.md 作者规范，涵盖 frontmatter 字段、章节骨架、命名、引用语法
...
- **Login_State_Cache**：cmux browser state save 产出的登录态 JSON 缓存，路径 `.tinkerman/cache/login-state-<project>.json`，必须在 `.gitignore` 中排除
...
- **Scenario_Artifact**：Acceptance_Scenario_Eval 的产出目录 `.tinkerman/acceptance/<topic>/`，包含每个 scenario 的执行脚本、截图、响应 JSON、verdict
...
**价值理由**：Oz AGENTS.md 里的 "Skill Metadata Style Guide (Canonical)" + "Authoring Template (Copy/Paste)" 是极高 ROI 的单点投资——把散落在 19 个已有 skill 里的隐式约定显式化。Forge 有 `.tinkerman/config.md` 讲文件保护分区，有 CLAUDE.md 讲宪法，但**缺失"怎么写一个合格的 SKILL.md"的单一文档**。新人（含 AI agent）每次新建 skill 都在重新发明轮子。
...
1. THE 新增文档 `.tinkerman/knowledge/skill-style-guide.md` SHALL 作为 SKILL.md 作者规范的单一事实来源
2. THE 文档 SHALL 位于开放区（Open Zone），允许自由修改
3. THE 文档 frontmatter SHALL 包含 `style_guide_version: X.Y`（语义化版本），规则重大变更时版本升级并追加到 `.tinkerman/decisions/` 走 ADR 流程
...
4. THE 首版审计 SHALL 产出 `.tinkerman/findings/scripts-help-audit.md`，列出每个脚本的当前状态：`user_facing_with_help` / `user_facing_missing_help` / `internal_only` / `one_off` / `unclear`；分类依据 SHALL 附带 evidence（何处调用、调用方）
...
7. THE Tier B 交互扫描工作流 SHALL 遵循以下步骤：
   ...
   - Step 2：`cmux browser surface:N state load .tinkerman/cache/login-state-<project>.json`（若存在且未过期）
   ...
   - Step 4：遍历 spec 定义的关键页面（或用户指定 URL 列表）：
     ...
     - `cmux browser surface:N eval "JSON.stringify(await axe.run())"` → 保存 `.tinkerman/reviews/assets/axe-<page>.json`
     - `cmux browser surface:N screenshot --out .tinkerman/reviews/assets/<page>.png`
     - `cmux browser surface:N console list > .tinkerman/reviews/assets/console-<page>.log`
     - `cmux browser surface:N errors list > .tinkerman/reviews/assets/errors-<page>.log`
...
9. THE 输出 SHALL 追加到 `.tinkerman/reviews/<topic>.md` 的 Layer 4 段落，格式遵循需求 2 的 Deliverable 规范，字段：
...
11. THE 所有 `.tinkerman/reviews/assets/` 下的文件 SHALL 纳入 `.tinkerman/archive/` 的 retention 策略（与 event-log 一致），保留 30 天后自动归档
12. THE 登录态处理策略 SHALL 明确：
    - 缓存路径 `.tinkerman/cache/login-state-<project>.json`（开放区文件）
    - `.gitignore` SHALL 包含 `.tinkerman/cache/` 条目以避免凭证入库——本 spec tasks 阶段负责补齐 `.gitignore`
...
**现状事实**（2026-05-08 审计）：
- Forge 现有 spec 模板（`.tinkerman/specs/` 下的 requirements.md）使用 **"User Story + Acceptance Criteria"** 格式，不是严格 BDD Scenario 格式
...
6. THE 每条 scenario 执行 SHALL 产生一个 Scenario_Artifact：
   - 目录 `.tinkerman/acceptance/<topic>/<scenario-id>/`
...
11. THE acceptance 报告 SHALL 汇总写入 `.tinkerman/reviews/<topic>-acceptance.md`，包含总 PASS/FAIL/SKIP/WARN 数、每条 scenario 的简要判定、详细 evidence 路径、来源分布统计
...
13. THE `.tinkerman/acceptance/` 目录 SHALL 纳入 retention 策略，默认保留 30 天
...
- **不触及** `.tinkerman/` 三区保护模型、三维路由、TDD 铁律等宪法级约定
...
| 登录态缓存凭证入库 | 中 | 高 | 事故恢复可能需数小时 | `.gitignore` 必含 `.tinkerman/cache/` + tasks 阶段强制校验 + skill 文档警示 |
