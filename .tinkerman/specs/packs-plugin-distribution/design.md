...
created: 2026-06-27
brownfield: true
related_requirements: ".tinkerman/specs/packs-plugin-distribution/requirements.md"
---
...
init.sh:1254  pack 定位逻辑不变（FORGE_ROOT 解析已正确）
  ...
  └─ 【新增】--pack 成功时追加埋点到 .tinkerman/knowledge/tool-health.md
...
复用 `.tinkerman/knowledge/tool-health.md` 现有条目格式。现有格式（实测）：
```
...
```
`source` 取值：`plugin`（FORGE_ROOT=CLAUDE_PLUGIN_ROOT）/ `clone`（其他）。写入逻辑：init.sh 在 pack 成功定位 + manifest 校验通过后，`echo` 追加到 `${PROJECT_ROOT}/.tinkerman/knowledge/tool-health.md`（若 .tinkerman/ 不存在或不可写则跳过）。
...
packs/pms/  ──build-dist.sh──> CC_BUNDLE/packs/pms/  ──plugin install──> ~/.claude/plugins/.../packs/pms/
                              ...
                              CC_BUNDLE/packs/README.md                  + README.md
                                              ...
                                              init.sh: 埋点 → .tinkerman/knowledge/tool-health.md
```
...
| 埋点写入失败（.tinkerman/ 不可写） | 静默跳过，不阻断 init |
