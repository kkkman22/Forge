...
`/forge review` 编排层（`src/review.ts` 中的 `buildReviewSubagents()`）生成的 subagent prompt 与 `skills/forge/lib/review/instructions.md` §2.0 契约不一致：管线确实写出了 `.tinkerman/reviews/.diff-context.md`，但生成的 prompt 仅向 subagent 传递了一份变更文件清单（如 `Changed files: src/file1.ts, ..., src/file20.ts`），从未在 prompt 内引用 diff-context 文件。
...
FUNCTION isBugCondition(invocation)
  ...
  // invocation.prompt 不包含 '.tinkerman/reviews/.diff-context.md'
  // 字面量路径时，触发 bug。
  RETURN invocation.kind ∈ {spec-check, quality-check, security-check}
     AND NOT contains(invocation.prompt, '.tinkerman/reviews/.diff-context.md')
END FUNCTION
```
...
FOR ALL invocation WHERE isBugCondition(invocation) DO
  ...
  ASSERT contains(result.prompt, '.tinkerman/reviews/.diff-context.md')
...
- 不修改 review 管线写出 `.tinkerman/reviews/.diff-context.md` 的现有逻辑
...
1.1 WHEN `buildReviewSubagents()` 为 spec-check / quality-check / security-check 生成 invocation THEN 系统生成的 prompt 仅含变更文件清单（如 `Changed files: src/file1.ts, src/file2.ts, ..., src/file20.ts`），不引用 `.tinkerman/reviews/.diff-context.md`
...
2.1 WHEN `buildReviewSubagents()` 为 spec-check / quality-check / security-check 生成 invocation THEN 系统 SHALL 在 prompt 中引用 `.tinkerman/reviews/.diff-context.md` 字面量路径，作为 subagent 读取统一 diff 的入口
...
3.2 WHEN review 管线运行 THEN 系统 SHALL CONTINUE TO 在子进程启动前写出 `.tinkerman/reviews/.diff-context.md` 文件（管线写文件的现有逻辑不变）
