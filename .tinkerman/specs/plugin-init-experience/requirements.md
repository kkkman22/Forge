...
2. Init 仍是脚本：`scripts/init.sh` 创建 `.tinkerman/` 目录、复制 7 个 agent 角色、生成 `CLAUDE.md` 项目宪法、按交互收集项目名/技术栈/安全级别/CI 检查命令。
3. **缺口**：plugin 用户运行 `/forge plan` 时，SKILL 检测到 `.tinkerman/` 不存在 → 提示"请先运行 forge init" → 用户找不到该命令。
...
- **方案 C**：SessionStart 钩子检测"plugin 已激活但 `.tinkerman/` 不存在"的状态，**非阻断**地输出引导文本，提示用户运行 `/forge init`。
...
- **Bootstrap 提示**：SessionStart 钩子在 `.tinkerman/` 不存在时输出的非阻断引导文本，建议用户运行 `/forge init`。
- **冷启动**：用户从未在当前项目运行过 init，`.tinkerman/` 目录不存在。
...
6. WHERE `init.sh` 已经初始化过项目（`.tinkerman/` 已存在），THE 现有"是否覆盖"交互提示 SHALL 保持不变，与 plugin / clone 模式无关。
...
2. WHEN bootstrap-check 启动，THE 脚本 SHALL 检测当前工作目录的 `.tinkerman/config.md` 是否存在。
3. WHEN `.tinkerman/config.md` 不存在 AND `.tinkerman/.bootstrap-dismissed` 标记不存在 AND `${CLAUDE_PLUGIN_ROOT}` 已设置（说明 plugin 已激活），THE 脚本 SHALL 输出引导文本到 stdout：
...
4. WHEN `.tinkerman/config.md` 已存在，THE 脚本 SHALL 静默退出码 0，不输出任何文本。
5. WHEN `.tinkerman/.bootstrap-dismissed` 文件存在，THE 脚本 SHALL 静默退出码 0。
...
8. THE bootstrap-check SHALL 不读取或写入 `.tinkerman/` 内部任何受保护文件；仅检测 `.tinkerman/config.md` 存在性与 `.tinkerman/.bootstrap-dismissed` 存在性。
...
1. WHEN 本 spec ship 完成，THE `.tinkerman/knowledge/known-failures.md` SHALL 追加一条模式："新分发渠道引入但 SKILL 提示文案与 Command 入口未同步"，置信度 ≥ 0.7。
