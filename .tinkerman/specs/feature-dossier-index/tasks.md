...
layout: tasks
created: 2026-05-11
spec_ref: ".tinkerman/specs/feature-dossier-index/requirements.md"
---
...
- `src/conflict-classifier.ts`（`.tinkerman/features/**` 白名单到 open zone）
- `README.md`（`.tinkerman/ 目录结构` 章节补一行说明）
- `src/learn.ts`（R9，可选）
...
- [x] 4. 实现 `scanStagesForTopic`
  - [x] 4.1 RED — 测试在 `test/fixtures/feature-dossier/` 下准备最小 `.tinkerman/` 树
...
- [x] 6. 实现 `discoverTopics`
  ...
  - [x] 6.3 性能验证
    - 对当前 Forge 仓库 `.tinkerman/` 根跑 `discoverTopics`
    - 断言耗时 ≤ 5 秒
    - _Requirements: 5.5_
...
- [x] 7. 创建 `scripts/rebuild-feature-dossier.mjs` 骨架
  - [x] 7.1 基础 CLI 参数解析
    ...
    - `.tinkerman/` 不存在时 exit 2 + 提示 `forge init`
    - _Requirements: 3.1, 3.5, 7.1_
  - [x] 7.2 单 topic 模式
    - 调用 `scanStagesForTopic` + `buildDossier`
    - 在 frontmatter 注入 `generated_at: new Date().toISOString()`
    - 写入 `.tinkerman/features/<topic>.md`，目录不存在时 `mkdirSync({recursive: true})`
    - 成功输出 `dossier: wrote .tinkerman/features/<topic>.md (N stages, M files)` + exit 0
...
- [x] 10. Hook 烟测
  - [x] 10.1 新增 `test/feature-dossier.hook.test.ts`
    - 模拟 `TOOL_INPUT_FILE=.tinkerman/plans/foo.md`（配合 fixture），`child_process.execSync` 调用 CLI `--from-path` 模式
    - 断言 `features/foo.md` 被生成
    - 断言 stdout 为空（Hook 静默契约）
    - _Requirements: 4.4, 4.7_
  - [x] 10.2 循环防护测试
    - `TOOL_INPUT_FILE=.tinkerman/features/foo.md` 作为输入
    - 断言 exit 0 且 `features/foo.md` 的 mtime 未变化（CLI 识别为 dossier 自身，静默退出）
    - _Requirements: 4.4_
  - [x] 10.3 未知路径测试
    - `TOOL_INPUT_FILE=.tinkerman/decisions/ADR-TEMPLATE.md` 作为输入
...
- [x] 11. `src/conflict-classifier.ts` 白名单更新
  - [x] 11.1 RED — 扩展 `test/conflict-classifier.test.ts`（若存在）或新建测试
    - 断言 `.tinkerman/features/foo.md` 被分类为 `open` zone
    - 断言该路径不在 frozen / guarded 的 pattern 列表中
    - _Requirements: 6.1, 6.2, 6.3_
  - [x] 11.2 GREEN — 在 classifier 的 priority chain 中加入判定
    - 在 open zone 的匹配列表中显式列出 `.tinkerman/features/**`
    - _Requirements: 6.1_
  - [x] 11.3 核对 `scripts/check-frozen.sh` 和 `src/check-frozen.ts`
    - 这两个模块本就只处理 `.tinkerman/specs/`、`.tinkerman/plans/`、`.tinkerman/config.md`，`.tinkerman/features/` 不在其中
    - 仅人工核对一次，无需修改
    - _Requirements: 6.2_

- [x] 12. README 文档更新
  - [x] 12.1 在 `.tinkerman/ 目录结构` 章节加一行
    - 示例结构里加一行 `├── features/` 带注释说明"自动生成的功能索引（PostToolUse Hook 维护）"
    - _Requirements: 8.8_
  - [x] 12.2 在 `.gitignore 建议` 章节补充说明
    - 加一条注释："`.tinkerman/features/` 可选纳入或排除：纳入则团队共享功能回顾索引；排除则作为本地派生状态"
    - _Requirements: 7.5_
...
- [x] 13. 一次性跑通当前仓库
  - [x] 13.1 `npm run dossier:rebuild:all`
    ...
    - 手动抽查 3-5 个 dossier 内容正确性：
      ...
      - `sdk-driver-decomposition`（只存在于 `.tinkerman/specs/`，预期不会出现在 Forge dossier 中——注意是 scope 边界）
  - [x] 13.2 `git status .tinkerman/features/` 确认
    - 确认只新增了 `.tinkerman/features/*.md`，无其它文件改动
...
- [x] 14. R9 归档 dossier 到 archive
  - [x] 14.1 定位 `src/learn.ts` 中的 archive 逻辑
    - 找到把 topic 相关文件 copy 到 `.tinkerman/archive/<date>-<topic>/` 的代码位
    - _Requirements: 9.1, 9.4_
  - [x] 14.2 在 copy 之前调用 `buildDossier`
    - 立即重建最新 dossier（捕获 archive 时刻的快照）
    - 把结果写入 `.tinkerman/archive/<date>-<topic>/dossier.md`
...
- [x] 16. 可选：添加 ADR
  - [x] 16.1 在 `.tinkerman/decisions/` 下新增 `ADR-XXXX-feature-dossier-index.md`
    - 记录"派生视图 vs 物理重组"的架构选择
    - 记录"Hook 驱动而非 Skill 驱动"的理由
    - 更新 `.tinkerman/knowledge/adr-index.md`
