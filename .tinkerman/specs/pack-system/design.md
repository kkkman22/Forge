...
| R2 项目级启用 | `.tinkerman/config.md` YAML frontmatter `packs:` 字段 + `src/pack/config.ts` |
...
| R5 Bounded Context 引擎 | `src/context/registry.ts`（纯函数） + `packs/<name>/contexts/` + `.tinkerman/custom/contexts/` |
...
| R11 Custom Override | `src/pack/resolver.ts`（纯函数） + `.tinkerman/custom/` 目录约定 |
...
│  └─────────────────────────────────────────────────────────────────────────┘ │
│                                                                              │
│                ↓ 从 packs/<name>/ 与 .tinkerman/custom/ 读取数据                 │
└──────────────────────────────────────────────────────────────────────────────┘
                        │
                        ▼
┌──────────────────────────────────────────────────────────────────────────────┐
...
### 3.3 `.tinkerman/config.md` frontmatter 扩展
...
interface EnabledPacks {
  ...
  customLayerRoot: string;           // 绝对路径 to .tinkerman/custom/
}
```
...
**职责**：从 `.tinkerman/config.md` 解析 `packs:` frontmatter 字段，关联 `Pack_Registry` 生成 `EnabledPacks`。
...
实现要点：
- 遍历 `.tinkerman/custom/contexts/*.md` 和每个 enabled pack 的 `contexts/*.md`
...
实现要点：
- 遍历 `.tinkerman/custom/glossary/*.md` 和每个 enabled pack 的 `glossary/*.md`
- 文件名即 context 名（`folio.md` → context `folio`，特殊值 `_shared.md`）
- 解析每个文件的多个 `## <Term>` 段
- 每条入口含 `context`、`term`、`definition`、`aliases`、`updated`、`source`
- **向后兼容**（R6.5）：若 `enabledPacks` 为空且不存在任何 `.tinkerman/custom/glossary/`，降级到加载 `.tinkerman/glossary.md` 为 `_shared` context
...
2. 读取 .tinkerman/config.md frontmatter.packs
...
- **路径穿越防护**：`pack/resolver` 使用 `path.resolve` + 前缀校验，确保 resolved 路径位于 `packs/<name>/` 或 `.tinkerman/custom/` 下，不越界
- **YAML 安全解析**：使用 `yaml` 库的 safe parse 模式（默认），不允许任意 JS 对象实例化

## 8. Migration Path

### 8.1 既有 `.tinkerman/glossary.md` 向后兼容

R6.5 要求：`enabledPacks = []` 且无 `.tinkerman/custom/glossary/` 时，自动降级读 `.tinkerman/glossary.md`，视作 `_shared` context 下的术语表。用户不需要迁移。

### 8.2 既有 plan 文件的 Expected Output

R10.6 要求：legacy plan 没有 Expected 字段时，self-check 只 warn 不 error；用户可渐进式补齐。

### 8.3 `.tinkerman/specs/` 已有 specs
