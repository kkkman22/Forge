...
layout: tasks
created: 2026-04-29
spec_ref: ".tinkerman/specs/skill-function-integration-audit/requirements.md"
---
...
- [x] 1. 审计：生成 SKILL-纯函数对接状态报告
  ...
  - 对照 `skills/*/SKILL.md` 中的引用，标记每个函数的对接状态：
    ...
    - ❌ 未对接：SKILL 中完全未引用
  - 将审计报告输出到 `.tinkerman/findings/skill-function-audit.md`
...
- [x] 4. 修复 forge-ship/SKILL.md 的对接缺失
  - 在 §2 门禁检查处添加 `checkShipGate()` 函数调用说明：
    - 参数来源：`review` 从 `.tinkerman/reviews/<topic>.md` frontmatter 解析，`test` 从 Layer 1 + Layer 3 结果构造，`progress` 从 `.tinkerman/progress/<topic>.md` 解析
...
- [x] 5. 修复 forge-learn/SKILL.md 的对接缺失
  - 在 §2 执行质量分析处添加：
    - `analyzeSkillFeedback(entries)` — 参数：从 `.tinkerman/knowledge/skill-feedback.md` 解析的反馈条目；返回：`{ commandStats, alertCommands, totalEntries }`；用途：识别高失败率命令
  ...
  - 在 §9 执行流程中添加新步骤（在步骤 12 "会话层清理"之前）：
    ...
    - 将报告追加到 `.tinkerman/knowledge/sessions/<date>-<topic>.md`
  - 验证：所有函数调用说明包含三要素
