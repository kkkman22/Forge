...
- **Frozen Zone**：Forge 的 spec/plan 锁定保护机制，独立于沙箱，保护 `.tinkerman/specs/`、`.tinkerman/plans/` 等文件
...
6. WHEN `--sandbox` 未指定且无 `.tinkerman/sandbox.json` 时，THE 系统 SHALL 保持现有 `bypassPermissions` 行为不变（向后兼容）
...
1. THE `.tinkerman/sandbox.json` 中的 `fileSystem.allow` SHALL 映射到 SDK `sandbox.filesystem.allowWrite`
2. THE `.tinkerman/sandbox.json` 中的 `fileSystem.deny` SHALL 映射到 SDK `sandbox.filesystem.denyWrite`
3. THE 配置 SHALL 支持新增 `fileSystem.denyRead` 字段，映射到 SDK `sandbox.filesystem.denyRead`
4. THE 默认策略 SHALL 允许写入项目根目录（cwd），禁止写入 `.tinkerman/sandbox.json` 和 `.tinkerman/.sandbox-active.json`
...
1. THE `.tinkerman/sandbox.json` 中的 `network.allow` SHALL 映射到 SDK `sandbox.network.allowedDomains`
...
1. THE `.tinkerman/sandbox.json` SHALL 支持 `version: 2` 格式，包含 `profiles` 字段定义多个命名 profile
...
4. THE `.tinkerman/.sandbox-active.json` 运行时文件 SHALL 不再需要（SDK 内部管理沙箱状态）
...
1. WHEN 无 `--sandbox` 选项且无 `.tinkerman/sandbox.json` 时，THE 系统行为 SHALL 与升级前完全一致
