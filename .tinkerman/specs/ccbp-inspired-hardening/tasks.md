...
layout: tasks
created: 2026-05-12
spec_ref: ".tinkerman/specs/ccbp-inspired-hardening/requirements.md"
---
...
- [x] 4.4 Checkpoint：验证 `.claude/rules/spec-editing.md` 的 YAML frontmatter 能被解析（用 `yq '.paths' .claude/rules/spec-editing.md` 或等价命令检查；返回合法 list）
  - 可选：人工构造一个 `.tinkerman/specs/test.md` 触发会话，验证规则被注入到 context（通过 InstructionsLoaded hook 或 debug 日志确认）
  - _Requirements: 10.5_
...
- [x] 5.6 Checkpoint：实际启动一次 Claude Code 会话（或人工触发 SessionStart / UserPromptSubmit 的测试脚本），验证 dispatcher 正确输出原本的上下文（`.tinkerman/knowledge/evolved-rules.md` + `.tinkerman/plans/*.md` 头 + `.tinkerman/progress/*.md` 尾）
...
- [x] 7.2 识别迁出候选段落（若需要瘦身）——按 design.md §7 的分类：
  ...
  - 跨项目长文档 → `.tinkerman/docs/living/<topic>.md`
  - 全局核心 → 保留在 CLAUDE.md
...
- [x] 7.5 若 7.1 显示超过 200 行，执行实际瘦身：
  - 把 7.2 识别的"跨项目长文档"条款迁到 `.tinkerman/docs/living/<topic>.md`，在 CLAUDE.md 留 `@.tinkerman/docs/living/<topic>.md` 引用
...
- [x] 10.1 创建 `.tinkerman/docs/living/ccbp-patterns-p2.md`
...
- [x] 13.4 清理临时备份文件（若 Task 7 创建了 CLAUDE.md 备份，本 Task 删除；若需保留，迁到 `.tinkerman/archive/`）
  - _Requirements: 10.4_
