...
- instruction 必须读取 `.tinkerman/status.md` 或 `.tinkerman/status/`。
- instruction 必须读取 `.tinkerman/progress/`。
...
更新 `.tinkerman/docs/partial-spec-satisfaction.md`，把旧报告中的过期项显式降级：
