...
created: 2026-06-07
tier: standard
source: ".tinkerman/docs/partial-spec-satisfaction.md"
---
...
修复 `.tinkerman/docs/partial-spec-satisfaction.md` 复核后仍值得做的确定性缺口：补齐当前 hook manifest 注册、给 cleanup git 操作增加超时、补 resume 阶段回归测试，并把已被当前架构替代的旧 spec 状态文档化，避免后续按过期审计误修。
...
**User Story:** 作为 Forge 用户，我希望修改 `.tinkerman/config.md` 后已有的 `config-changed-hook.mjs` 能通过当前 hook manifest 触发，而不是只停留在脚本和测试中。
...
4. THE test SHALL fail if `skills/forge/lib/resume/instructions.md` no longer mentions reading `.tinkerman/status.md` and `.tinkerman/progress/`.
...
1. `.tinkerman/docs/partial-spec-satisfaction.md` SHALL list `structured-observability`, `ship-delivery-unification`, `cmux-integration` loop producer, and `pms-pack-v1` as superseded/过期 where applicable.
...
**Verify-By**: `rg "Superseded|不恢复|pms-pack-v1" .tinkerman/docs/partial-spec-satisfaction.md`
