...
- 该步骤插入在现有 Step 2（创建 `.tinkerman/`）之后、Step 3 之前——因为 Stop hook 注入的 status.md 依赖 `.tinkerman/` 已存在。
...
- `hooks.events.Stop` 注册一条 `type: command` hook，命令把 `.tinkerman/status.md` 摘要作为 additionalContext 注入（compact 补偿，对齐 v2 §6.4/§9.5）。
...
- 脚本模拟 SessionStart hook stdin（含 subagent caller 信号为 false 的正常路径），分两种场景：
  1. 预置 `.tinkerman/knowledge/evolved-rules.md`（含至少一条 `### R1:` + `**Content**:` 规则）→ 断言 stdout additionalContext 非空且含 Content 摘要。
...
| Stop hook 注入 status.md 在无 `.tinkerman/` 项目报错 | Stop 命令加 `2>/dev/null \|\| true` 容错（与现有 hooks.json 风格一致）|
