...
1. **debug→build 没有正式状态转移**：`skill-scheduler.ts:97` 的 `determineNextSkill` 状态机里**没有 debug 分支**。debug 是 fork 旁路，跑完后只写 `.tinkerman/debug/<slug>.md` 的 `status: "resolved"`，控制权靠主 agent 隐式上下文恢复回到 build，硬跑剩余 task——没有代码/调度规则保障交接，也不读 debug 的 root_cause。
...
**decide 决策**（`.tinkerman/decisions/2026-06-17-dynamic-replan-loop.md`）：**做，Standard tier。** 作为独立 spec（不并入 PR #98 loop-engineering-adoption），避免 scope 膨胀。明确 veto：全量重新 plan（回滚成本高）、引入 LangGraph/Temporal（分发摩擦致命）。
...
- **incremental replan**：只重写 `.tinkerman/plans/<topic>.md` 中 `status !== "completed"` 的 task，受 plan phase Spec Lock 门禁约束。
...
1. THE `.tinkerman/debug/<slug>.md` 的 frontmatter SHALL 新增 `failure_class` 字段，取值 `fixable_bug | assumption_invalidated | environmental`，默认 `fixable_bug`。
...
4. THE 状态转移 SHALL 读 `.tinkerman/debug/<slug>.md` 的 `status` 和 `failure_class` 作为决策依据（结束隐式上下文恢复）。
...
5. THE 修订后的剩余计划 SHALL 写回 `.tinkerman/plans/<topic>.md`，并**显著标注为 replan 版本**（带 `replan_of: <original>` + `invalidated_assumptions`），供用户 review。
...
2. THE replan 事件 SHALL 复用 loop-engineering-adoption R3 的 commit-narrative 机制：把"为什么改方向（哪个假设失效）+ 改了什么（剩余 task 修订摘要）"写进 `.tinkerman/runs/<id>/commit-narrative.md`。
...
| 验收点 | 验证方式 |
|---|---|
| failure_class 字段写入 | debug 跑完，`.tinkerman/debug/<slug>.md` 含 `failure_class` |
...
| 增量 replan 只改剩余 | replan 后 `.tinkerman/plans/<topic>.md` 的已完成 task 不变，未完成 task 被修订 |
