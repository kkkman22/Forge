...
- **候选方案**：
  - A. 新建独立状态文件（如 `.tinkerman/replan/<topic>.json`）。
  - B. 加在 `.tinkerman/debug/<slug>.md` frontmatter。
- **选择理由**：选 B。debug 文件是失败诊断的权威载体，root_cause / resolution 已在那里。failure_class 是 root_cause 的分类，天然属于同一文件。新建独立文件增加文件数、制造两个真相源（debug 文件说 root_cause，replan 文件说 class）。frontmatter 加字段是 schema 兼容扩展（`.tinkerman/debug/*.md` 解析已是宽松的）。
...
- **问题描述**：replan 的触发状态存哪？
- **候选方案**：
  - A. 新建 `.tinkerman/replan-state.json`。
...
if (currentPhase === "debug") {
  const debugFile = readDebugFile(topic);  // 读 .tinkerman/debug/<slug>.md frontmatter
...
WHEN status.md 的 replan_pending === "true":
  ...
  4. 写回 .tinkerman/plans/<topic>.md，标 replan 版本（replan_of + invalidated_assumptions）
...
| 文件 | 字段 | 性质 |
|---|---|---|
| `.tinkerman/debug/<slug>.md` | `failure_class` | 新增 frontmatter |
| `.tinkerman/debug/<slug>.md` | `invalidated_assumptions` | 新增 frontmatter（条件） |
| `.tinkerman/status.md` | `replan_pending` | 新增（瞬时，passthrough） |
| `.tinkerman/status.md` | `invalidated_assumptions` | 新增（瞬时，passthrough） |
| `.tinkerman/plans/<topic>.md` | `replan_of` / `invalidated_assumptions` | 新增 frontmatter（replan 版本标记） |
