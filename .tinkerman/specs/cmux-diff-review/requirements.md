...
问题陈述：当前 `/forge review` 的三层（spec/quality/security）发现以 `.tinkerman/reviews/<topic>.md` markdown + 桌面通知呈现，评审者需在"评审报告"与"代码 diff"两个视图间来回对照，行级定位成本高；而人类评审者对 agent 改动的行级意见又只能以自由文本 prompt 传达，缺少与 diff 行绑定的结构化通道。cmux 0.64.11 的 `cmux diff`（流式 CodeView diff viewer）+ 0.64.15 的 review comments + TextBox 直送，正好补这条行级双向通道。
...
4. 零新运行时依赖：纯 `cmux diff` CLI + 既有 `.tinkerman/reviews/` schema。
...
3. THE Zero_Impact_Invariant SHALL 成立：diff 通道不可用时，`.tinkerman/reviews/<topic>.md` 产物、frontmatter（`cmux-integration` R15 的 layers_status/completed_at）、桌面通知与 `cmux-integration` R5 逐字节一致。
...
4. WHEN agent 接收该载荷，THE `/forge build` 或 `/forge review` SHALL 把它视为与 `.tinkerman/reviews/<topic>.md` 同等的评审输入（结构化发现），按既有 P0/P1 阻断规则处理（宪法 §3.3）。
...
3. THE Forge 三层发现（`.tinkerman/reviews/<topic>.md`）SHALL 经 Finding_To_Comment_Map 映射后，在 diff viewer 以行级评论形式呈现（方向 B 的"回灌"）；cmux 侧写入评论的 API 为 Implementation_Gate (G1)，不可用时降级为"仅打开只读 diff + 评审报告 markdown"。
4. WHEN diff 为空（无变更），THE skill SHALL 不打开空 diff viewer（避免 0.64.13 已修的 "no diff beep"），仅在 sidebar log 记 `no changes to review`。
5. THE 打开 diff 与回灌评论 SHALL NOT 修改 `.tinkerman/reviews/<topic>.md` 内容（它是 source of truth；diff viewer 是呈现层，对齐 `cmux-integration` Out of Scope #7）。
...
2. **替换 `.tinkerman/reviews/<topic>.md`** — 它仍是 source of truth（对齐 `cmux-integration` Out of Scope #7）；diff viewer 是呈现与输入层。
