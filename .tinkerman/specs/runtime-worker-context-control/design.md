...
phase workers write reports, logs, and summaries to `.tinkerman/`, while the main
...
/forge
  ...
  -> phase worker request
       ...
       artifact_path: .tinkerman/runs/<run>/workers/<phase>.md
       summary_path: .tinkerman/runs/<run>/workers/<phase>.json
...
```json
{
  ...
  "artifact_path": ".tinkerman/runs/run-1/workers/review.md",
  "commands": [
    {
      "cmd": "npx vitest run test/review.test.ts",
      "result": "pass",
      "evidence_path": ".tinkerman/runs/run-1/workers/review-vitest.log"
    }
  ],
  "findings": {
    "p0": 0,
    ...
    "items": [
      {
        ...
        "evidence_path": ".tinkerman/reviews/quality-check.md"
      }
    ]
  },
  "next_action": "test"
