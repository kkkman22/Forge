...
| forge-plan | 读取 .tinkerman/ 文件 | checkFilesystemPolicy read |
...
| `strict` | 仅允许 src/ 和 .tinkerman/ 的读写，禁止网络 |
| `readonly` | 仅允许读取，禁止任何写入 |
| `ci` | CI 环境 profile，禁止写入 .tinkerman/ 进度文件 |

Profile 定义方式：`.tinkerman/sandbox.json` 中可定义多个 profile，通过 `--sandbox <profile>` 选择。
...
```
--sandbox            使用 .tinkerman/sandbox.json 中的 default profile
