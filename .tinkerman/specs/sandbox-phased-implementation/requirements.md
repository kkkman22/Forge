...
**目标**：让项目通过 `.tinkerman/sandbox.json` 声明文件系统访问权限，即使没有 OS 级隔离，也能在 SKILL 层面做基本的权限检查。
...
```json
{
  ...
  "profile": "default",
  "filesystem": {
    "read": ["src/**", ".tinkerman/**", "package.json", "tsconfig.json"],
    "write": ["src/**", ".tinkerman/progress/**", ".tinkerman/reviews/**"],
...
- `forge init` 生成 `.tinkerman/sandbox.json` 模板（全部允许）
- 模板中包含注释说明各字段用途

#### 1.4 `--sandbox` CLI 增强

- `--sandbox` 选项接受 profile 名称（如 `--sandbox strict`）
- `--sandbox` 无参数时使用 `.tinkerman/sandbox.json` 中的 `default` profile
- Profile 不存在时报错退出
...
- [ ] `.tinkerman/sandbox.json` schema 定义完成
