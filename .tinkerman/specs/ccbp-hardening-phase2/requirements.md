...
同时，对 Claude Code CLI 近 6 个月更新的调研（详见 `.tinkerman/decisions/`）识别出三条被 `ccbp-inspired-hardening` 未包含但高 ROI 的原生能力：**Hooks 的 `if:` 条件过滤（v2.1.85）**、**PreCompact / PostCompact hook（v2.1.105/76）**、**Agent `isolation: "worktree"` + `hooks:` + `initialPrompt` frontmatter（v2.1.0/50/83）**。

本 spec 是 `ccbp-inspired-hardening` 的 Phase 2，专注收尾工作：

- **完成遗留迁移**：rules 候选规则全量落地、hooks dispatcher 剩余 4 类事件迁移
- **引入官方新能力**：Hooks `if:` 条件过滤替代内联 bash 判断、PreCompact 保护 `.tinkerman/status.md`、agent worktree 隔离
...
- **Phase1_Spec**：`.tinkerman/specs/ccbp-inspired-hardening/`，本 spec 的前置依赖。
- **Hook_If_Filter**：Claude Code v2.1.85 引入的 hook 定义 `if:` 字段，使用 permission-rule 语法（如 `"Bash(git *)"`、`"Write(.tinkerman/**)"`），仅匹配时才 spawn hook 命令，减少无谓的 bash 调用。
- **PreCompact_Hook**：Claude Code 的 hook 事件之一，在会话 compaction 开始前触发；hook 可通过 `exit 2` 阻塞 compaction，或通过 stdout 注入自定义 compact 指令。
- **PostCompact_Hook**：compaction 完成后触发；此时历史被压缩到 summary，hook 可重新注入关键上下文（例如 `.tinkerman/status.md` 当前指针）。
...
- **Compaction_Snapshot**：本 spec 定义的 `.tinkerman/.compact-snapshot.md` 临时文件，由 PreCompact_Hook 写入，由 PostCompact_Hook 消费后删除。
...
1. THE project SHALL audit every entry in `hooks/hooks.json` and identify hooks that currently rely on inline `if [ -f ... ]; then ...; fi` or `if [ -d ... ]` patterns in the `command` field; each such case SHALL be listed in `.tinkerman/docs/living/hooks-if-migration.md` with its current pattern and proposed `if:` expression.
2. WHEN a hook's inline condition can be expressed as a permission-rule pattern (e.g. `Bash(git *)`, `Write(.tinkerman/**)`, `Edit(src/**)`), THE hook SHALL be migrated to use the `if:` field, with the inline guard removed from the `command`.
3. IF an inline condition checks project state beyond tool input (e.g. checking `.tinkerman/.sandbox-active.json` existence), THEN THE condition SHALL remain in the command but be wrapped in the fastest possible check (avoid `jq` / `find`; use `[ -f ... ]`).
4. THE migration SHALL produce a measurable reduction in hook spawn count on a baseline session; baseline measurement (bash-spawn-counter running for 10 minutes of typical `/forge build` workflow) SHALL be captured before and after, with the delta documented in `.tinkerman/runs/<date>-if-migration-baseline.md`.
...
1. THE project SHALL register a PreCompact_Hook at `scripts/hook-precompact.sh` that, before compaction begins, writes a Compaction_Snapshot file at `.tinkerman/.compact-snapshot.md` containing: (a) current slug from `.tinkerman/status.md` frontmatter, (b) current phase, (c) last-3-line tail of the active `.tinkerman/progress/<slug>.md`, (d) active PR number (if `--from-pr` was used), (e) ISO 8601 timestamp.
...
3. IF the PreCompact_Hook cannot read `.tinkerman/status.md` (missing or unparseable), THEN THE hook SHALL exit 0 without writing a snapshot and without blocking compaction; a one-line warning is written to `.tinkerman/runs/<date>-compact-events.jsonl`.
...
7. THE compaction events (snapshot written, snapshot restored, snapshot missing) SHALL be logged to `.tinkerman/runs/<date>-compact-events.jsonl` in the same format as the frozen-events jsonl (if that spec has landed), or in a minimal `{timestamp, event, slug}` schema otherwise.
...
1. THE `forge-plan` agent definition SHALL declare an `initialPrompt` frontmatter field with a concise kickoff prompt (≤500 characters) instructing the agent to: (a) read `.tinkerman/specs/<slug>/spec.md` if a slug is provided, (b) summarize the understood scope in ≤5 bullet points, (c) ask clarifying questions via `AskUserQuestion` before drafting the plan.
...
4. THE `.tinkerman/` directory SHALL remain project-local and unaffected by the worktree (verified by test: spawning forge-build agent with worktree isolation, writing to `.tinkerman/progress/<slug>.md`, then verifying the file lands in the original repo not the worktree).
...
6. THE `.tinkerman/config.md` SHALL document the new worktree behavior in the "AI 可自由修改" section, noting that `forge-build` runs in an isolated worktree by default.
...
3. THE content of each rule SHALL be extracted from existing documentation (CLAUDE.md, `.tinkerman/features/`, or existing SKILL.md files) WITHOUT modifying the rule text; the migration is "move + reference", not "rewrite".
...
5. THE `forge-src.md` rule SHALL contain TypeScript/JavaScript coding conventions extracted from existing Forge docs: strict null checks, `.tinkerman/config.md` config reading patterns, import ordering, test co-location rules.
...
1. THE project SHALL re-measure `wc -l CLAUDE.md` after Req 7 migrations complete; the measurement SHALL be recorded in `.tinkerman/runs/<date>-claude-md-baseline.md`.
2. IF the line count exceeds 200, THEN additional content SHALL be moved to `.claude/rules/` (new rule files beyond Req 7 list) or `.tinkerman/docs/living/` (long-form docs), until the count ≤ 200.
...
4. THE `.tinkerman/decisions/` SHALL include an ADR recording this phase2 work, with references to specific Claude Code versions for each native capability used, and a rollback plan should any single change prove problematic.
