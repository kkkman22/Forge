...
- 不迁移 `.tinkerman/features/` 里的规则到 `.claude/rules/`（留给后续）
- 不引入 plugin / agent teams / channels 等 Claude Code 新能力（各自已有独立 spec）
- 不对 `.tinkerman/config.md` 做破坏性改动（只追加对 worktree 行为的说明）
...
CC → 触发 PreCompact hook
      → scripts/hook-precompact.sh 读 .tinkerman/status.md
      → 写 .tinkerman/.compact-snapshot.md:
           slug=my-feature
           phase=build
...
hooks:
  Stop:
    - type: command
      command: |
        bash -c '
          ci_cmd=$(yq ".ci_check_command // \"npm run check\"" .tinkerman/config.md 2>/dev/null)
...
forge router → 调用 Agent(subagent_type="forge-plan", prompt="
  请读取 .tinkerman/specs/my-feature/spec.md，
...
name: forge-plan
initialPrompt: |
  If a slug was provided by the caller, read `.tinkerman/specs/<slug>/spec.md` now.
...
- `.tinkerman/` 目录仍然在主 repo 里（用户能看到 progress 实时更新）——因为 `.tinkerman/` 不在 worktree 里
...
```json
{
  "hooks": {
    "SessionStart":     [{"hooks": [{"command": "bash .claude/hooks/scripts/dispatcher.sh"}]}],
    "UserPromptSubmit": [{"hooks": [{"command": "bash .claude/hooks/scripts/dispatcher.sh"}]}],
    "PreToolUse":       [{"if": "Bash(git *)|Write(.tinkerman/**)|...",
                          "hooks": [{"command": "bash .claude/hooks/scripts/dispatcher.sh"}]}],
    "PostToolUse":      [{"if": "Write(.tinkerman/**)|Edit(.tinkerman/**)",
                          "hooks": [{"command": "bash .claude/hooks/scripts/dispatcher.sh"}]}],
...
**pre-change**：CLAUDE.md 里有一节"TypeScript 约定"（strict null、import 顺序等），无论用户是在写 `.md` 文档还是在调 `.tinkerman/specs/`，这节都占 context。
...
- 编辑 `.tinkerman/specs/foo.md` 时：`spec-editing.md` + `branch-protection.md` 进 context
...
| `.tinkerman/runs/<date>-if-migration-baseline.md` | 执行时生成 | Req 1 的 before/after 测量 |
| `.tinkerman/runs/<date>-claude-md-baseline.md` | 执行时生成 | Req 8 的行数测量 |
| `.tinkerman/runs/<date>-compact-events.jsonl` | 运行时滚动 | compaction hook 事件日志 |
| `.tinkerman/decisions/<date>-ccbp-hardening-phase2.md` | ADR | 本 spec 的决策记录 |
...
| `.gitignore` | 加 `.tinkerman/.compact-snapshot.md` |
...
**降险**：
1. Req 5.6 要求 `.tinkerman/config.md` 文档化新行为
...
```
CC                   PreCompact hook      .tinkerman/status.md   snapshot file   PostCompact hook
 |                        |                     |                |                 |
 |-- approach limit ----->|                     |                |                 |
 |                        |-- read slug/phase ->|                |                 |
 |                        |<-- data ------------|                |                 |
...
**关键点**：
- `.tinkerman/progress/` 写入用**绝对路径**（或相对于项目根），不是相对于 cwd——这是 Forge 已有的约定，不需要改
...
PreToolUse(Write src/x.ts)       |                          |                     |
     ...
     |<-- skipped -------------- |                          |                     |
     |                                                      |                     |
PreToolUse(Write .tinkerman/progress/foo.md)                    |                     |
     |                           |                          |                     |
     |-- dispatch -------------->|                          |                     |
     |                           |-- match -> invoke ------>|                     |
     |                           |                          |-- EVENT=PreToolUse  |
...
| Req | 验证手段 | 观察点 / 断言 |
|---|---|---|
| 1.1 | 静态审计 | `.tinkerman/docs/living/hooks-if-migration.md` 包含当前 hooks.json 每条内联 if 的迁移计划 |
| 1.2–1.3 | diff 检查 | `hooks/hooks.json` 中内联 `if [` / `if [ -f` 数量 `<=` 迁移后预期值 |
| 1.4 | 脚本基线 | `scripts/bash-spawn-counter.sh` 测量，before/after 落地到 `.tinkerman/runs/` |
...
| 2.5 | 静态 | `.gitignore` 包含 `.tinkerman/.compact-snapshot.md` |
| 2.6 | 代码审计 | `hook-precompact.sh` 无 `exit 2` / `return 2` 语句 |
| 2.7 | 运行时 | `.tinkerman/runs/<date>-compact-events.jsonl` 格式符合 schema |
...
| 5.4 | 手动 e2e | 测试 `.tinkerman/progress/` 落在主 repo，不是 worktree |
| 5.5 | contract test | 其他 3 个 agent 均无 `isolation` 字段 |
| 5.6 | 文档 | `.tinkerman/config.md` 的"开放区"补充 worktree 说明 |
...
### D1 — 为什么不把 PreCompact snapshot 写进 `.tinkerman/status.md` 本身？

**选择**：独立文件 `.tinkerman/.compact-snapshot.md`（git-ignored）

**理由**：
- `.tinkerman/status.md` 是"受保护区"的开放子集，任何写入都会在 git diff 中出现
...
**理由**：
- `forge-plan`：产出 plan 文件到 `.tinkerman/plans/`，不需要隔离代码状态
...
| `.tinkerman/features/` 规则迁到 `.claude/rules/` | 独立的"rules 全量迁移"spec |
