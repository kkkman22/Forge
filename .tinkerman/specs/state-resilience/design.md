...
/**
 * Infer current workflow phase from git history and .tinkerman/ file presence.
 * Pure function — no side effects, no file writes.
 */
export function reconstructStateFromGit(
  forgeFiles: string[],  // 列出 .tinkerman/ 下所有文件
): ReconstructedState {
  ...
  if (hasReviews) {
    evidence.push("found .tinkerman/reviews/ files → at least review phase");
    return { inferredPhase: "review", confidence: "high", evidence };
  }
  if (hasProgress) {
    evidence.push("found .tinkerman/progress/ files → at least build phase");
    return { inferredPhase: "build", confidence: "high", evidence };
  }
  if (hasPlans) {
    evidence.push("found .tinkerman/plans/ files → at least plan phase");
    return { inferredPhase: "plan", confidence: "medium", evidence };
  }

  evidence.push("no .tinkerman/ state files found → starting from router");
...
*For any* set of .tinkerman/ files, `reconstructStateFromGit` SHALL return a phase that is consistent with the most advanced state file present. If reviews/ exists, the inferred phase SHALL be at least "review".
...
| Property 3 | `test/status-resolver.property.test.ts` | 随机 .tinkerman/ 文件列表 |
