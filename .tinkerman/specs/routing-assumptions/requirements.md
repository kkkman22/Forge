...
created: 2026-05-01
tier: standard
status_note: "Req1 (router generates 3–5 cited assumptions, src/router.ts:843 generateAssumptions) + Req3 (assumptions field in ForgeRoutingResult type) delivered. Req2 (persisted to .tinkerman/status.md) delivered: the router SKILL §5 documents assumptions as a status field to write, state.ts:144 reads it back, and it is optional (Req2.2). The audit's initial 'router never writes assumptions' finding was a layer confusion — router.ts is intentionally a pure function; the write happens at the SKILL layer per §5 instructions, which is the project's established pattern."
---
# Requirements Document

## Introduction

Forge 路由器在分析任务时会扫描项目代码结构、读取 `package.json`、检查 `.tinkerman/config.md` 等文件，基于这些信息做出隐式判断（如技术栈、影响范围、分页模式等）。当前这些判断是隐式的——Agent 默默假设然后往下走，用户没有机会在第一步纠正错误假设。
...
- **假设来源（Assumption Source）**：Agent 做出判断的依据，如 package.json、项目路由扫描、现有代码模式、.tinkerman/config.md 等。
...
3. THE assumptions SHALL be derived from actual project scanning (package.json, code structure, .tinkerman/config.md, git history, existing specs), NOT from generic templates.
4. EACH assumption SHALL cite its source (e.g., "基于 package.json devDependencies", "基于项目中已有的 GET /api/tasks 分页模式", "基于 .tinkerman/config.md 技术栈").
...
1. THE forge-router SKILL.md §5 status update SHALL write the assumptions list to `.tinkerman/status.md` in a new `assumptions` field (YAML string array).
