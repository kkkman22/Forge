...
**来源研究**：`.tinkerman/findings/ruflo-comparison-research.md`（后续沉淀）。所有决策均避开 Ruflo 的规模派特性（swarm/federation/HNSW/Rust WASM/100+ agent 库），聚焦治理层。
...
2. THE ADR 文档文件名 SHALL 形如 `.tinkerman/decisions/ADR-0042-spec-lock-mechanism.md`
...
5. THE `.tinkerman/knowledge/adr-index.md` SHALL 作为所有 ADR 的聚合索引，列出每条 ADR 的 id / title / status / date / 所在文件路径
...
9. THE ADR 文档 SHALL 位于受保护区（Guarded zone），可追加但不得删除或覆盖，与现有 `.tinkerman/decisions/` 保护策略一致
10. THE `.tinkerman/decisions/` 目录下新增一个 `ADR-TEMPLATE.md` 文件，作为新建 ADR 的起始模板
...
3. THE `.tinkerman/status.md` 的 frontmatter 契约 SHALL 由 `src/schemas/status-file.ts` 的 Zod schema 定义
4. THE `.tinkerman/config.md` 的 frontmatter 契约 SHALL 由 `src/schemas/config-file.ts` 的 Zod schema 定义
...
1. THE forge-loop 每次运行 SHALL 在 `.tinkerman/runs/<runId>/events.jsonl` 写入事件流（JSON Lines 格式，每行一个完整 JSON 对象）
...
10. THE 事件流的保留策略 SHALL 由 `.tinkerman/config.md` 的 `event_log_retention_days` 字段控制，默认 30 天，过期由 `/forge learn` 或单独的 `scripts/prune-event-logs.sh` 归档/删除
...
7. THE baseline 基线 SHALL 存储在 `.tinkerman/knowledge/metrics.md` 的 `performance_baselines` 段中，由 CI 在 main 分支合并后自动更新
