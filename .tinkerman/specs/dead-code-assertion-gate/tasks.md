...
- **TDD Steps**: 非 TDD（文档）。
  ...
  - `.tinkerman/knowledge/evolved-rules.md` R3 的 Infra_Ref 追加 `scripts/check-unused-module.mjs`（评估是否需微调 R3 Content 提及"删除前验证"；不新增 R15）。
- **Verify Command**: `grep check-unused-module skills/forge/lib/build/instructions.md`（命中）+ `grep check-unused-module .tinkerman/knowledge/evolved-rules.md`（命中）
