...
[forge-grill（可选前置）] ← 需求 4：苏格拉底式追问
   │   └─> 产出 .tinkerman/findings/grill-<topic>.md
...
## Spec
**定义**：需求锁定的产物，位于 `.tinkerman/specs/<feature>/spec.md`，一旦 locked 即进入冻结区。
**更新**：2026-05-05
**来源**：初始预置
```
...
| `.tinkerman/findings/*.md` | ✅ |
| `skills/**/SKILL.md` | ❌ 冻结区 |
| `.tinkerman/config.md` | ❌ 冻结区 |
| 锁定的 spec | ❌ 冻结区 |
...
```
1. 扫描 .tinkerman/reviews/**, .tinkerman/progress/**, .tinkerman/findings/** 的 Evolution 标记
2. aggregateEvolutionMarkers → EvolutionReport
3. 生成 .tinkerman/knowledge/evolution-report.md（开放区，每次 learn 覆盖）
...
- 文件 `.tinkerman/reviews/xxx.md:42` 的 target `forge-nonexistent` 不存在
```
...
- Integration test: 触发 zoom-out 后对话显示三段式，`.tinkerman/` 无副作用
