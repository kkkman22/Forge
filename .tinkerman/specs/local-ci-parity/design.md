...
1. **配置层**：补齐 `.tinkerman/config.md` frontmatter 的 `ci_check_command` 字段（解锁冻结区一次性修复）。
...
```
┌─────────────────────────────────────────────────────────┐
│ Layer 1：.tinkerman/config.md frontmatter（一次性修复）        │
...
**位置**：`.tinkerman/config.md`（**冻结区**，需用户解锁）
 ...
 *
 * @param frontmatter - 已解析的 .tinkerman/config.md frontmatter（任意来源）
...
- 新增"漂移检测"子段落：调用 `detectCiCommandDrift(frontmatter, packageJsonRaw)`，按 `kind` 分支：
  ...
  - `drift_with_npm_check` → 输出 `warning` 文本（不阻断）→ 使用 `forge_exec` 执行 `npm run check` → 在结果末尾追加 `[ci-drift-detected]` 标记到 `.tinkerman/findings/<topic>-ci-drift.md`（一次性记录，重复触发时若 finding 已存在则跳过追加）
...
### `.tinkerman/findings/<topic>-ci-drift.md`（漂移记录，仅在 SKILL 检测到 `drift_with_npm_check` 时产出）
...
recommendation: "add ci_check_command to .tinkerman/config.md frontmatter"
---
...
### `.tinkerman/knowledge/known-failures.md` 追加条目（Requirement 5）

```markdown
### 模式：frontmatter 字段在仓库 config.md 缺失但模板存在
- **置信度**：0.8
- **检测信号**：GH CI 失败列表里包含本地从未运行的命令（如 dist-sync、check-doc-structure）
- **根因**：模板 `templates/config.md` 有字段，但 `forge init` 未补齐到本仓库 `.tinkerman/config.md` frontmatter
- **验证命令**：`grep ci_check_command .tinkerman/config.md` —— 无输出 = 漂移
- **修复**：补齐 frontmatter；若 SKILL 已升级，自动检测 + 警告
- **Source**：`.tinkerman/specs/local-ci-parity/`
```
...
- 在 tmp 目录构造 `.tinkerman/config.md`（无 `ci_check_command`）+ `package.json`（含 `scripts.check`）。
- mock `forge_exec` 调用，断言 SKILL 调度逻辑选择 `npm run check` 而非三件套。
- 断言 `.tinkerman/findings/<topic>-ci-drift.md` 被创建一次，重复 build 不重复创建。
...
- 断言 `.tinkerman/config.md` frontmatter 解析后含 `ci_check_command` 字段（一旦本仓库补齐，contract 守住不再回退）。
...
| 解锁冻结区改 `.tinkerman/config.md` 引发其他副作用 | 状态文件保护被绕过 | 仅修改 frontmatter 一行；diff 在 PR 中可视；通过 `npm run check` 本身验证一致性 |
