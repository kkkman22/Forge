...
layout: tasks
created: 2026-05-16
spec_ref: ".tinkerman/specs/local-ci-parity/requirements.md"
---
...
- [x] 1. 解锁并补齐 .tinkerman/config.md frontmatter（用户授权后）
  - 1.1 用户解锁后，编辑 `.tinkerman/config.md` 的 frontmatter，在 `post_push_verify_enabled: true` 之后新增一行 `ci_check_command: "npm run check"`
  - 1.2 验证：`grep '^ci_check_command:' .tinkerman/config.md` 返回 `ci_check_command: "npm run check"`
...
- [x] 4. forge-test SKILL Layer 3 集成漂移检测
  ...
  - 4.2 描述 `drift_with_npm_check` 时一次性写入 `.tinkerman/findings/<topic>-ci-drift.md` 的去重逻辑
...
- [x] 7. /forge learn 沉淀知识
  - 7.1 运行 `/forge learn`，确认 `.tinkerman/knowledge/known-failures.md` 新增 "frontmatter 字段在仓库 config.md 缺失" 模式条目，置信度 ≥ 0.7
...
**用户授权门**：Task 1 涉及修改 `.tinkerman/config.md`（冻结区），按宪法 §2.2 的 `HARD-GATE: frozen-zone-protection`，build 阶段开始前必须由用户明确确认解锁。

**验证清单**（全部任务完成后核对）：

- `grep '^ci_check_command:' .tinkerman/config.md` 返回非空
...
- `.tinkerman/knowledge/known-failures.md` 含新条目
