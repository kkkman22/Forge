...
Forge 有 `/forge learn` 和 `.tinkerman/knowledge/`，但 build 中做的"先这样简化、以后再升级"的延迟决策**没有显式代码标记**，散落在 commit message 或进度文件里，learn 阶段无法系统回收。
...
**跳过记录格式**（写进 `.tinkerman/progress/<topic>.md`）：
```
...
- [ ] 新增跳过/替代记录格式说明，明确写进 `.tinkerman/progress/<topic>.md`。
...
**4b. learn 阶段回收**：`/forge learn` 扫描本次 build 产生的所有 `forge:defer` 注释，汇总进 `.tinkerman/knowledge/deferred.md`（新建），格式：
...
- [ ] `.tinkerman/knowledge/deferred.md` 创建（初始模板，空表 + 表头说明）。
- [ ] `/forge learn` 的 instructions（`skills/forge/lib/learn/instructions.md` 或等效）新增一步：grep `forge:defer`，汇总写进 `deferred.md`。
- [ ] `deferred.md` 纳入 `.tinkerman/knowledge/` 的 20 文档上限管理（见 AGENTS.md §4.2，由 learn 既有逻辑处理，不需新机制）。
...
- [ ] `.tinkerman/knowledge/deferred.md` 创建，learn instructions 含回收步骤。
